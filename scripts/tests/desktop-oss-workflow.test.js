...
# 'missing' models a 404, which curl does not retry; 'flaky' models a transient
# 5xx, which --retry-all-errors retries until the attempt budget is spent.
attempt=0
while :; do
  attempt=$(( attempt + 1 ))
...
describe('Desktop OSS mirror workflow', () => {
  it('short-circuits only after the stable GitHub and OSS publication is complete', () => {
    ...
    // The probe binds steps.version.outputs, so it must stay wired after
    // 'Resolve version': moved above it, RELEASE_TAG and RELEASE_VERSION are
    // both '', the feed comparison can never match, and the skip becomes a
    // permanent no-op while every replay row below stays green.
    ...
    // clobber is the only force switch. An Electron bridge dispatch over a
    // completed release short-circuits too, because 'Create GitHub release'
    // refuses to upload to an existing release without clobber=true.
