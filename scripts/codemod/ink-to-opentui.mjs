  ...
  for (;;) {
    if (!skipInsignificant()) {
      return {
        name,
        nameEnd,
        error: i >= n ? 'unterminated opening tag' : 'unexpected string in opening tag',
      };
    }
    const c = src[i];
    if (c === '>') return { name, nameEnd, attrs, selfClosing: false, end: i + 1 };
    if (c === '/' && src[i + 1] === '>') {
    ...
    if (c === '{') {
      ...
      if (closeIdx === -1) {
        return { name, nameEnd, error: 'unbalanced braces in spread attribute' };
      }
    ...
    if (src[q] === '=') {
      ...
      if (qc === '"' || qc === "'") {
        ...
        if (r >= n) return { name, nameEnd, error: `unterminated value for attribute ${aname}` };
        attrs.push({
          kind: 'string',
          name: aname,
          start: ns,
        ...
        if (closeIdx === -1) {
          return { name, nameEnd, error: `unbalanced braces in attribute ${aname}` };
        }
        attrs.push({
          kind: 'expr',
          name: aname,
        ...
        return { name, nameEnd, error: `malformed value for attribute ${aname}` };
      }
    } else {
...
function analyzeStyleObject(src, code, comment, start, end) {
  ...
  for (;;) {
    ...
    if (!code[i]) {
      ...
      if (r >= end) return { error: 'unterminated string inside existing style object' };
    ...
    while (i < end) {
      if (!code[i]) {
        if (comment[i]) return { error: 'comment inside existing style object' };
        saw = true;
        i++;
        continue;
      }
      ...
      if (c === '{' || c === '[' || c === '(') depth++;
        ...
        if (depth === 0) return { error: 'unbalanced value inside existing style object' };
        depth--;
      } else if (c === ',' && depth === 0) {
        break;
      }
...
function planStyleCollection(src, code, comment, open, notes) {
  ...
  if (styleIdxs.length === 1) {
    ...
    if (s.kind !== 'expr') return manual('existing style attribute is not an object expression');
    ...
    const analysis = analyzeStyleObject(src, code, comment, objOpen + 1, objClose);
    if (analysis.error) return manual(`existing style object: ${analysis.error}`);
    ...
    merged = base === '' ? entries.join(', ') : `${base}, ${entries.join(', ')}`;
  } else {
  ...
  return { slotIdx, remove, styleText: `style={{ ${merged} }}`, count: collectedIdx.length };
}
...
export function transformSource(src) {
  ...
  const notes = [];
  const zeroStats = () => ({ box: 0, text: 0, propsCollected: 0, styleTags: 0 });
  const bail = (msg, at) => ({
    output: src,
    changed: false,
  ...
  for (let k = 0; k < n; k++) {
    ...
    if (parsed.error) {
      return bail(`could not parse <${nm}> (${parsed.error}) — file left unchanged`, k);
    }
  ...
  for (const ev of events) {
    ...
    if (!top || top.name !== ev.name) {
      return bail(`mismatched closing tag </${ev.name}> — file left unchanged`, ev.start);
    }
  ...
  for (const el of elements) {
    ...
    if (plan) {
      edits.push({ start: el.open.start, end: el.open.end, text: renderOpenTag(src, el.open, plan) });
...
function main() {
  ...
  for (const f of files) {
    ...
    for (const note of res.notes) console.log(`  MANUAL (line ${note.line}): ${note.msg}`);
