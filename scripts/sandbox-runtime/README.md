...
1. Run `node scripts/sandbox-runtime/build.mjs /absolute/empty/installation`. The directory must be empty. The builder resolves repository paths from its own location and bundles the actual CLI and core sources into one production headless launcher. The manifest records the source inputs, all test scripts, and output hashes.
...
3. On Linux, run `node /absolute/installation/verify.mjs /absolute/installation /tmp/runtime-shell-report.json`. Store output on a writable Linux filesystem. Lima may mount the host repository read-only; copy the report back afterward.
