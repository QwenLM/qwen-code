// Runner-routing regression guards for ci.yml, serve-ab.yml, e2e.yml,
// security-checks.yml, and the qwen-autofix.yml scan lane.
//
...
describe('security-checks.yml runner routing', () => {
  ...
  for (const jobName of JOBS) {
    ...
    it(`${jobName} heals the shared workspace before its checkout`, () => {
      ...
      // ci.yml sweeps `.qwen` and its `.qwen.root-orig` recovery twin in
      // one loop, so the rename-aside pins on the loop variable.
      assert.match(steps[clean].run, /"\$GITHUB_WORKSPACE\/\.qwen"/);
      assert.match(steps[clean].run, /mv -- "\$stale_qwen"/);
      assert.deepEqual(
        sansIf(steps[heal]),
  ...
  it('dependency-cve keeps setup-node off the pool', () => {
    // The action's post step uploads the npm cache to GitHub; on the
    // pool's slow egress that save ran 14+ minutes and timed out the
    // job's first pool run (2026-08-26, ecs-qwen-runner-sg-1). Hosted
    // keeps the action; the pool reuses the machine's Node and its
    // persistent npm cache, exactly as ci.yml does.
