...
function handleRequest({ body }) {
  if (body.messages.some(m => m.role === 'tool')) {
    return { kind: 'message', content: 'Fixture tool execution complete.' };
  }
  const tool = body.tools?.find(t => t.function?.name?.includes('render_amplitude_chart'));
  if (!tool) return { kind: 'message', tool_calls: [toolCall('mcp__amplitude-fixture__render_amplitude_chart', {})] };
  return { kind: 'message', tool_calls: [toolCall(tool.function.name, {})] };
}
