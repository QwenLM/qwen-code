...
describe('qwen serve — transcript paging route', () => {
  ...
  it('reads a both-states transcript from active and maps unavailable snapshots to 409', async () => {
    ...
    const conflictPage = await conflict.json();
    expect(conflictPage).toMatchObject({
      sessionId: conflictId,
      hasMore: false,
    });
    expect(JSON.stringify(conflictPage)).toContain(
      'active conflicting transcript',
    );
    expect(JSON.stringify(conflictPage)).not.toContain(
      'archived conflicting transcript',
    );
