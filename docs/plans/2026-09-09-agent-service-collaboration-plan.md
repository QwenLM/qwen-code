...
以下按段记录，未通过项写未通过，不用百分比。四个审计脚本当时全绿（`scripts/audit/` 后已整体移除，不再可复跑）：
