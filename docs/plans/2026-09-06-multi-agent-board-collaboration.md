...
**Verified in isolated runtime patches.** A two-segment headless execution
reproduced `USAGE_METADATA.round` as `[1, 1]`; the cumulative-round patch changes
it to `[1, 2]` (#11200). Structured external input now carries `deliveryId`
through the consumed event and transcript (#11202), and resident continuation
returns an actionable result instead of a boolean (#11204). Source and runtime
...
4. **Hidden host session, keepalive, and programmatic launcher** — create the
   ...
   produces `capacity_wait`. Runtime preparation work is isolated in #11204.
...
Targeted lint and core typecheck also pass on the split branch. They establish
...
| Consume the correlated external-input runtime contract from #11202                                         | `core/src/agents/mesh/`                |
