...
> Status: Revised after source-backed review. A small, uncommitted correction to
> the pre-existing admission layer is parked in the review working tree (§5.1);
> the workspace transaction, durability contract, and execution path remain
> unbuilt.
...
**Verified locally in an uncommitted review patch.** Targeted tests plus build,
typecheck, and lint found and checked concrete defects that source review
predicted: `blocked` was absent from store validation, an unknown `@name` fell
back to the assignee, child budget fallback failed open, running work counted
against the pending queue, and a human reply on one child reset a sibling's turn
gate. Those checks validate only that small rules/storage patch, not this
architecture. The targeted test command and count are kept in §5.3.
...
| 10  | An agent is **serial across threads**. Pending work is selected globally by `(queuedAt, runId)` FIFO and the waiting thread names the active thread.                                                                                                                                                                                                                   | One slow thread blocks other work, but file enumeration order cannot starve it.                                                                                                                                            |
...
| 17  | A child **inherits the parent's current turn count** and charges tokens to the root.                                                                                                                                                                           | Creating a child does not mint immediate unattended turns; later human input resets only the child being supervised.                                                                                                |
...
| 19  | A stuck run, and any run still `running` after a **daemon restart**, is reconciled and revived once. A second execution failure is terminal. A launch failure is typed and terminal unless classified transient.                                               | Recovery replays only work not committed by the delivery watermark; queued launch failures cannot poison the backlog indefinitely.                                                                                  |
...
ThreadRun           id, agentId, sessionId, status, triggerMessageIds[],
                    ...
                    finalMessageId, usageTokens, chargedUsageRounds[],
                    failureStage,
...
root token charges, parent dependency reports, and channel notifications.
Consumers acknowledge event ids, so a restart may duplicate an effect but
cannot silently lose it. The parent post stores the event id as its idempotency
key.
...
share one atomic thread-file replacement. Cross-file effects use durable,
idempotent outbox events. In particular, every runtime `USAGE_METADATA` event
records `token_charge(runId, attempt, round, usageTokens)` beside the run; the
root applies it once using `appliedTokenChargeIds`. The transcript's round usage
reconstructs an event if the process dies before the event-file write. Admission
drains pending charge events for the root before checking the token gate, and
finish reconciles rather than creating the first charge. Parent reports and
notifications use the same write-source-first, apply-idempotently,
acknowledge-last protocol.
Thread deletion refuses non-terminal runs, descendants, or unacknowledged
outbox events; otherwise it could erase work or a charge that another file has
not yet observed.
...
dispatcher (daemon)
  ...
  choose each agent's oldest queued run by (queuedAt, runId)
    ...
    idle → continue resident, revive, or launch
...
turn completes → flush transcript → finishRun
  ...
  reconcile per-round token usage, unconsumed ids, and cross-file outbox
  select next FIFO run
        │
sweeper: run with no activity for N minutes, or `running` at daemon start
        → reconcile and revive once; second failure → terminal failed
```
...
and selects the minimum `(queuedAt, runId)` so leaving work queued cannot create
file-order starvation. Registry capacity is the same kind of momentary fact and
...
| human feedback that successfully books/delivers                                                 | `blocked`, `in_review` | acknowledge current blockers/review candidates, set `in_progress`, reset only this thread's turn count        |
...
| turn/token/queue/unavailable gate leaves no runnable target                                     | any non-`done`         | persist outcome, set `blocked`, enqueue one deduplicated gate notification                                    |
...
close yields `in_review` and emits the parent report. Successful human feedback
records its message sequence as the acknowledgement boundary for earlier close
records. A successfully booked parent dependency event acknowledges the matching
wait. This defines the cases where one agent waits, blocks, or reviews while
another is still working without inventing a separate blocker object.
...
turn. A human post resets only that thread's turn count. Tokens are charged from
idempotent per-round usage events to the root and never reset.
...
### 5.1 Local review correction — verified but uncommitted
...
1. **Local admission foundation** — review and either retain or discard the
   parked correction in §5.1 when implementation begins. It is not committed.
2. **Capability boundary** — built-in read-only shell allowlist intersected
   with the agent definition. Prove disallowed commands cannot reach execution.
3. **Versioned storage protocol** — add `schemaVersion`, the workspace mutation
   lock, atomic same-thread booking, cross-file outbox/token-charge replay, and
   fail-closed migration before any new process writes the expanded model.
...
5. **Run envelope and tools** — add the §3 delivery/provenance fields, prompt
   ...
   mesh context, incremental usage charging, and minimal `thread_post`,
...
6. **Minimal live vertical slice** — assigned parent → launch → assigned child →
   ...
   work together.
7. **Dispatcher reliability** — global FIFO, direct running delivery,
   acceptance recording, completion reconciliation, launch failure, done/
   cancellation, restart and stall recovery, parent dependency triggers, and outbox
   replay.
8. **REST routes and Web Shell** — roster, thread list/view, busy reason, gates,
   failures, cancellation, and transcript slices; absorb #11140's entry.
9. **Channel notifications** for blocker raised, aggregate in_review, gate
   tripped, and terminal failure. Last because it consumes state transitions
   proven by steps 6-8.

Steps 1-5 are unit-testable. Step 6 is the early integration gate; step 7 adds
failure injection and restart tests; step 8 adds daemon/browser tests.

### 5.3 Acceptance

Evidence for the parked local correction only:
...
Build, package lint, and core typecheck also passed when that code patch was
made. They establish compile/style health only; they do not validate the design
or the unbuilt execution path. Update the test count above if the patch changes.
...
per-thread turns plus idempotent per-round root token charges; all dispatcher
...
neither loses nor double-charges an event; runtime saturation waits without
...
| Correlated mesh external-input/consumed event                                                               | `core/src/agents/runtime/`             |
...
gets both a runtime binding and a self-contained prompt envelope:

```
MESH RUN (trusted runtime envelope)
...
- **Trust comes from message role, not a heading.** The runtime envelope and
  tool policy are a system/developer message; title, body, and posts are a
  separately encoded user-data message with attribution. Provenance does not
  make their instructions safe, but it stops them redefining the trusted frame
  and lets policy/audit trace the originating run. The current thread remains
  the only mutation scope even if memory from another thread says otherwise.
...
Token accounting consumes each run's per-round usage events and charges the
root; the terminal registry stats delta is only a reconciliation check.
...
5. **Concurrent token charging.** The workspace lock makes completed charges
   ...
   booking) or an accounting limit with bounded overshoot.
...
7. **Semantic duplicates after replay.** Exact retries of one tool call are
   ...
   system can preserve provenance and show that it was a retry; whether the UI
   should offer automatic duplicate collapse is undecided. Silent loss remains
...
**规则修正**：turn gate 改为每线程，token gate 保持根树维度；子线程继承父线程当前 turn 计数；running coalesce 也计 turn；未知 @ 不再误唤醒 assignee；无目标、agent unavailable、capacity wait、launch failure、done/cancel、assignment trigger 都有明确语义；跨线程 queued run 按 `(queuedAt, runId)` 全局 FIFO。全局锁只处理并发，跨文件崩溃一致性由可重放 outbox 保证；`blocked/in_review` 按所有 agent 的 run 聚合，不再由最后一个 agent 覆盖。

**验证边界**：当前规则/存储层已有定向测试、构建、类型与 lint 证据；launcher、线程工具、dispatcher、delivery watermark、恢复、REST、Web Shell、通知都还没端到端跑通。§5.2 把 live vertical slice 提前，§9 记录 7 个仍需产品或存储取舍的问题。
