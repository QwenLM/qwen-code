...
**Verified in integrated runtime changes.** A two-segment headless execution
...
result instead of a boolean. Source and runtime
...
4. **Hidden host session, keepalive, and programmatic launcher** — create the
   ...
   produces `capacity_wait`. The typed continuation preparation is integrated
   in this PR.
...
Targeted lint and core typecheck also pass on the current branch. They establish
