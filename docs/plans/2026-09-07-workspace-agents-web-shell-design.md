...
Empty: **No agents yet. An agent is a persistent identity with its own instructions; each task keeps its own conversation.**
...
| Agent identity bar                      | `WorkspaceAgent.color`, falling back to `--accent-red/orange/yellow/green` |
...
**这份文档做什么**：在第 9 步动工前定下 workspace agents 在 Web Shell 的设计方向，并且对着 Multica 已上线的实现（`issue-detail`、`execution-log-section`、`comment-trigger-chips`、`thread-nav-panel`、`blocked-trigger-copy`）来定，而不是凭想象。
