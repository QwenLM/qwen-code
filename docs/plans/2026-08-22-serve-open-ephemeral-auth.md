...
- Return without changing the options unless all conditions are true:
  `--open`; loopback hostname; `serveWebShell !== false`; Web Shell assets
  resolve; `shouldLaunchBrowser()` is true; and the currently selected token is
  empty after trim.
...
- Do not change `RunHandle`, `ServeOptions`, `CredentialStore`, bearer
  ...
  separation, WebSocket authentication, or the Web Shell token reader.
...
- Document process lifetime, per-tab `sessionStorage`, restart rotation,
  tab-close loss, browser-launch command visibility, and the existing
  secret-bearing manual-URL fallback on launch failure.
...
| `localhost`, `127.0.0.1`, `::1`, and `[::1]`             | Eligible loopback forms                                               |
...
4. Send an invalid empty body to strict `POST /workspace/memory` in a temporary
   trusted workspace. Verify the unauthenticated request returns
   `token_required`, while the authenticated request reaches body validation
   and returns `invalid_scope` without changing workspace data.
...
- Treat the no-token local-client change as the primary compatibility risk and
  put the explicit shared-token migration in the PR body.
