...
- The missing §5.1 patch is now committed in #11072; its three named files pass 37 tests, plus targeted lint and core typecheck.
- C1 reproduced as usage rounds `[1, 1]` across two execute segments and is fixed to `[1, 2]` in the current PR.
...
- I5's boolean ambiguity is fixed in the current PR. The design now inserts a minimal dispatcher before the live slice and gives `paused` its actual `resumeBackgroundAgent` path.
...
9. I9: fixed by pushing the patch into #11072 and updating §0.2.
...
3. The reproduced two-segment usage-round sequence was `[1, 1]`; after the integrated fix it is `[1, 2]`.
