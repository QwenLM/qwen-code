...
- The missing §5.1 patch is now committed on the split mesh branch; its three named files pass 37 tests, plus targeted lint and core typecheck.
- C1 reproduced as usage rounds `[1, 1]` across two execute segments and is fixed to `[1, 2]` in #11200.
- C2 contained a false premise: current `AgentHeadless.executeTurn` already emits `EXTERNAL_MESSAGE` for resident `task_prompt` continuations, and cold revival seeds `initialUserPrompt`. What was actually missing was durable correlation. #11202 adds `deliveryId` to structured external input, the consumed event, resident structured continuation, and the transcript record.
- I5's boolean ambiguity is fixed independently in #11204. The design now inserts a minimal dispatcher before the live slice and gives `paused` its actual `resumeBackgroundAgent` path.
...
9. I9: fixed by pushing the patch on the split mesh branch and updating §0.2.
...
3. The reproduced two-segment usage-round sequence was `[1, 1]`; after #11200 it is `[1, 2]`.
