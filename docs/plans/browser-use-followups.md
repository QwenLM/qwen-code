...
| Trusted client boundary | The model cell is untrusted. A hash-allowlisted `browser-client` receives privileged Node REPL capabilities such as the native pipe, telemetry, approvals, response metadata, and after-code hooks. | The Browser SDK is loaded from bundled resources without an equivalent trusted-module boundary. | Reference   |
...
| Windows support      | Package and register a Windows-compatible Native Messaging host with idempotent install, status, and uninstall behavior.                                                      | Packaged Qwen Code completes the managed Chrome preflight on Windows using its bundled Browser Use runtime.     |
