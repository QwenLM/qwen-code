...
运行通道默认使用短期 mTLS workload identity；外部 broker 再按操作签发 sender-constrained capability JWT。令牌协议上限为 5 分钟，但 `exp` 不得超过默认 60 秒 SCM 授权租约，并包含 `iss/aud/typ`、作为 opaque principal 的 `sub`、`tenant_id/workspace_id/repository_id/jti/iat/exp`、revocation epoch、固定能力集、证书确认 `cnf` 和 `req_hmac`，不包含可陈旧的管理员角色。agent 在 event ID 已原子落盘并序列化 exact request bytes 后，把 body digest 交给 broker；broker 在签发前以 purpose-separated broker/Gateway key 对 method、route、event/idempotency key 和 body digest 计算 `req_hmac`。Gateway 重新计算并验证该绑定，不能依赖“第一次请求先到先绑定”，从而避免被窃取的未绑定 `jti` 抢占。

每个 workspace binding 保存 `last_verified_at/authz_expires_at`；SCM webhook 或轮询确认撤权后立即禁用 binding、递增 epoch、停止签发并 drain daemon。SCM 不可用且租约过期时 Gateway 拒绝记忆请求，Qwen 无记忆继续。Gateway 使用固定算法白名单和可信 `kid`，严格验证 issuer、audience、显式 `typ`、mTLS `cnf`、时间、workspace/repository binding、capability、`req_hmac`、授权租约与 revocation epoch，并拒绝请求体中的任何身份覆盖。`jti` replay binding 保留到 `exp + clock-skew allowance`；完全相同的短期重试返回既有/进行中结果，其他复用一律拒绝。较长的事件幂等和防复活由下文 raw event receipt 负责，不能把 token replay TTL 与业务幂等 TTL 混为一谈。任何结果记录都不含 raw body 或渲染后的 memory context，只保存 content-free outcome、resource ID 和 recalled canonical ID；返回内容时重新执行当前授权、epoch 和 `active+live` gate 后再渲染。
...
agent 为每个 Hook/API 逻辑操作在第一次网络请求前生成独立随机 UUID event ID 并原子落盘；内部重试必须复用，但不能在两个操作、turn 或 session 间复用。`(tenant_id,event_id)` 唯一键在不暴露低熵 prompt digest 的前提下保证这些重试幂等。该设计不宣称 daemon 在本地协调状态落盘前崩溃时仍能 exactly-once。
...
管理 API 使用企业 OIDC access token，但授权按 scope 分离，不能把 repository maintainer 当作 personal 或 tenant administrator：
...
- 业务状态为 `candidate → active|rejected|expired`，以及 `active → superseded|expired|tombstoned`；擦除状态独立为 `live → pending_erasure → erased`。召回只允许 `active + live`。`erased` 表示含内容行已删除，管理 API 从防复活账本投影最终状态，而不是让已删除的 canonical row 继续承担状态存储。
...
- canonical 和 derived content 同样使用 record-specific deletion handle 加密。删除是可恢复但不可撤销的 saga，不能伪装成跨数据库与 key service 的事务。删除/expiry/offboarding 先向数据库备份域之外的 deletion ledger 幂等写入并确认 durable `deletion_intent`；从该确认起，recall、extractor 和 provider worker 即使仍看到旧 active row 也必须拒绝该 canonical version。随后数据库事务把业务状态置为 `tombstoned`、擦除状态置为 `pending_erasure`、递增 scope epoch 并写入高优先级 key/provider erasure job。若 ledger 成功而数据库事务失败，reconciler 必须继续完成 tombstone，管理面保持 pending，绝不能删除 intent 或恢复召回。worker 再销毁并验证 content handle、删除含内容的 canonical/evidence 行、核验 provider 擦除，并把 ledger reconciliation state 推进到 `erased`。账本只保存由独立 deletion-ledger service key 计算、tenant-scoped 的 canonical/provider/version HMAC、scope type、稳定删除原因码和 reconciliation 状态，不保留 principal/repository linkage 或自由文本。KMS/key service 故障只能留下已告警 pending 状态。主体删除覆盖 personal record 和仍可识别该主体的 evidence；共享 repository record 按企业所有权政策处理，并且不得包含个人数据。handle 销毁后，备份可以残留密文但不能保留可用密钥。
...
flowchart LR
    ...
    K --> C["Remove content-bearing canonical/evidence rows"]
