...
> 状态：方案草案 v2，未实现。v1（三个方案、中继驱动）被本版替换，替换原因见 §1。
...
1. 一个 Mac 伴侣进程：拉起本地 `node_repl`，连远端 `/acp`，中继 MCP 帧，持有 macOS 权限，提供本地刹车；
2. 一个把伴侣绑定到远端具体会话的配对入口；
3. skill 读参考文档的方式要改成不经过 `node_repl`。

## 1. 为什么 v1 的三个方案都不对

v1 把"驱动守护进程 `qwen-cua-driver serve`"当成 computer use 的核心，围绕它设计了三个方案：驱动 HTTP MCP 走 SSH 隧道（A）、SDK 经转发的 socket 连守护进程（B）、伴侣中继 `qwen-cua-driver mcp`（C）。复查代码后发现前提就错了：
...
skill 看到 node_repl 可用，跳过 bootstrap          └─ 菜单栏状态 + 一键停止
```
...
5. **权限宿主**：TCC 授权记在拉起 `node_repl` 的进程身份上（§1）。原型阶段从终端启动，授权记在终端名下，能用；正式版应该是签名的 `.app`，有稳定的 bundle ID，并引导用户授予辅助功能和屏幕录制权限。
6. **本地刹车**：远端的审批发生在开发机的 Web Shell 上；那个会话如果开了 YOLO，就不会再询问。所以 Mac 这边必须有自己的否决权：菜单栏显示"远程会话 X 正在控制本机"；一键停止：发 `mcp_unregister` 并结束 `node_repl` 子进程。
...
- **bootstrap 不需要改**：skill 只在 `node_repl` 不可用时才安装；远端会话里已经有伴侣注册的 `node_repl`，就不会在 Linux 上执行 `qwen mcp add` 和 `npm install`。要在原型里确认模型确实这样做。
...
#11548 让独立 Web Shell 能连到远程 daemon，是这个入口最自然的位置：远程开发时用户面前就是 Mac 上的浏览器 Web Shell，页面知道 daemon 地址和当前 `sessionId`。但 #11548 本身没有按会话绑定或一次性凭据的逻辑，它用的是 daemon 的长期 bearer token，伴侣的凭据不应复用它。daemon 的 LAN listener 已经要求升级请求携带配对凭据、不接受运行时 token，可以参考那套机制。具体形态待定。
...
| Qwen Live Host           | `packages/live-host`                                                                | 签名公证的 macOS 应用、TCC 授权引导、自动安装                            | daemon 发现只认回环地址；协议是 Live 专用      |
...
1. **模型行为**：远端会话里已有伴侣注册的 `node_repl` 时，模型是否确实跳过 bootstrap、不在 Linux 上执行 `qwen mcp add` 和 `npm install`。
...
5. **配对凭据**的形态和生命周期（§3.3）。
6. **伴侣放在哪个宿主里**：`qwen` CLI 子命令、并入 live-host、并入 desktop-shell，还是独立应用。做成 CLI 子命令时，对已装 qwen 的开发者没有新增安装。

## 7. 实施切片

**片1：Node 命令行原型伴侣**

形如 `qwen-mac-bridge --daemon <url> --token <t> --session <id>`：复用 `bridge-client.ts` 的连接、初始化和注册重试逻辑，拉起 `node_repl` 做 stdio 中继；同时把 skill 读参考文档的方式改成 `read_file`。

验收：远端会话里出现 `node_repl` 工具，且只在该会话可见；skill 不在 Linux 上跑 bootstrap；`getPlatform()` 返回 `macos`；一个真实任务跑通；记录截图体积和单步耗时；关掉伴侣后工具消失。步骤见 `docs/verification/remote-computer-use/README.md`。

**片2：配对入口**

Web Shell 会话里加"连接我的 Mac"入口，凭据只对单个会话有效。

**片3：签名宿主和本地刹车**

并入选定的宿主（§6 第 6 点），获得稳定的 TCC 身份、菜单栏状态和一键停止。
