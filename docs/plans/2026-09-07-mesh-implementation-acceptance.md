...
**Runtime wiring landed for the demo path.** Every launch, resident continuation, resume, and revive persists the next run binding, and both real background-turn seams establish it inside the turn body. Mesh agents see the six thread tools while ordinary subagents do not. Resident continuation, paused resume, and cold revival carry the run id in structured external input; the correlated consumed event advances that run's accepted/consumed ids and watermark, usage events upsert cumulative rounds, and body completion terminalizes the mesh run. Only a new launch consumes its initial prompt synchronously; recovered deliveries are consumed through `EXTERNAL_MESSAGE` and their initial input is cleared before a later resident turn. This correction was deliberately not expanded with new test code or a local CI/build pass; step 7's live model run is the next evidence gate.
