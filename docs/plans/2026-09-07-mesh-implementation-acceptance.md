...
**Step 6 landed (core half).** `dispatcher.ts` selects each idle agent's oldest queued run by the lock-issued `queueSequence` — never `queuedAt`, never file order — assembles the turn envelope, and starts the body through a port whose four entry points map onto the four real runtime operations (`launch`, `continue_resident`, `resume`, `revive`). `capacity_wait` and a runtime that reports the agent busy both leave the run queued with its attempt unspent, because both are observations about this instant. A failed or unavailable start is terminal with a typed `failureStage` and releases the queue slot, so a broken definition cannot look like an agent that is merely slow. `startRun` now records `contextThroughSequence` and moves the agent's delivery watermark, since the initial prompt is consumed when the turn starts. Parent reports drain through the outbox with the event id as the idempotency key; a second pass posts nothing more, and a notification no consumer claims stays pending rather than being acknowledged into silence.
...
**Turn-seam binding and tool registration landed.** `AgentMeta.meshRun` carries which thread the body's next turn is for. The dispatcher writes it immediately before a non-launch start; on the launch path it travels with the launch options into the first record, because a turn that began before a patch would have run with no frame at all. Both real seams — `runBackgroundTurn` in the resume path and in the launch path — re-read it once, synchronously, and open the AsyncLocalStorage frame around the turn body. Re-reading per turn is what lets one body work thread A then thread B; a frame opened around the lifetime would have pinned it to A.
The six thread tools are registered process-wide so one place knows a tool exists, and hidden from every agent that is not on a thread: a refusal the model can only discover by calling is a wasted turn and a misleading schema. Two tests pin both directions.
Observed after this wiring: `src/agents/runtime/agent-core.test.ts`, `src/tools/agent/agent.test.ts`, `src/agents/mesh/`, `src/tools/mesh-thread.test.ts` → 16 files, 476 tests passed; `agent-core.test.ts` alone 48; targeted ESLint clean.
...
The routes are registered in `server.ts`, and `ThreadsRoute` is the self-contained container that loads the surface: it debounces the routing preview and stamps each response with the draft it answered, so a slow reply to an older draft can never overwrite the answer to what is on screen — the whole value of the preview is that it describes the post about to be sent. A daemon that cannot be reached says so and names the fix.
Not done in step 9: the container is not mounted into `App.tsx`'s panel switch, because the way to _open_ it is #11140's absorbed sidebar entry and doing both here would collide; no transcript-slice panel; no Playwright visuals, which are this step's real evidence and need a browser this machine does not have.
