...
| `resume`                 | `string`                                                     | -                | Session ID to resume. Loads the prior session's conversation history so the agent retains full context.                                                                                                                                                                                                                                                                                                                                                                               |
| `sessionId`              | `string`                                                     | Auto-generated   | Explicit session ID for SDK-CLI alignment. Pass this value to `resume` in a subsequent query to continue the session.                                                                                                                                                                                                                                                                                                                                                                 |
| `hooks`                  | `boolean`                                                    | `false`          | Enable the CLI hook system. Auto-enabled when `hookCallbacks` is provided.                                                                                                                                                                                                                                                                                                                                                                                                            |
| `hookCallbacks`          | `Partial<Record<HookEvent, HookCallback \| HookCallback[]>>` | -                | SDK-side hook callbacks keyed by event name (`PreToolUse`, `PostToolUse`, `Stop`, `Notification`, `SubagentStop`). See [Hook Callbacks](#hook-callbacks).                                                                                                                                                                                                                                                                                                                             |
| `extensions`             | `string[]`                                                   | -                | Extension names to enable. Extensions provide skills, agents, hooks, MCP servers, and commands.                                                                                                                                                                                                                                                                                                                                                                                       |
| `includeDirs`            | `string[]`                                                   | -                | Additional directories to include in the workspace. Useful for multi-root workspaces or monorepo setups.                                                                                                                                                                                                                                                                                                                                                                              |
...
Register SDK-side callbacks for hook events. When `hookCallbacks` is provided, the hook system is auto-enabled (no need to set `hooks: true`).
...
const conversation = query({
  prompt: 'Implement the payment handler',
  options: {
    hookCallbacks: {
      PreToolUse: [auditLogger],
...
The return value from a hook callback controls execution:
...
See [examples/sdk-hooks](./examples/sdk-hooks) for more patterns.

## Task and Memory Events

The SDK defines typed interfaces for task and memory lifecycle events emitted as system messages:

```typescript
import { isTaskEvent, isMemoryEvent } from '@qwen-code/sdk';

for await (const message of conversation) {
  if (isTaskEvent(message)) {
    console.log(`Task ${message.data.action}: ${message.data.task.title}`);
  }
  if (isMemoryEvent(message)) {
    console.log(`Memory ${message.data.action}: ${message.data.memory.name}`);
  }
}
```
...
| -------------------- | -------- | ----------------------------------------------------------------- |
| `data.action`        | `string` | `'created'`, `'updated'`, `'completed'`, `'claimed'`, `'stopped'` |
| `data.task.id`       | `string` | Task identifier                                                   |
| `data.task.title`    | `string` | Task title                                                        |
| `data.task.status`   | `string` | Current status                                                    |
| `data.task.assignee` | `string` | Optional assignee                                                 |

### SDKMemoryEvent

| Field              | Type     | Description                           |
| ------------------ | -------- | ------------------------------------- |
| `data.action`      | `string` | `'saved'`, `'updated'`, `'deleted'`   |
