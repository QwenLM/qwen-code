...
| `approvalMode` | no       | Explicit mode for this restore. Privileged modes fail with `403 trust_gate` in an untrusted workspace. When omitted, the daemon best-effort replays a remembered caller-owned mode, if one exists.                                                                                                                                                                                                                       |
...
`state` mirrors ACP's `LoadSessionResponse` — `models` is a `SessionModelState`, `modes` a `SessionModeState`, `configOptions` an array of `SessionConfigOption`. Missing fields are agent-decided. Late attachers (the `attached: true` paths below) get the SAME `state` snapshot the original load caller saw — the daemon caches it on the entry; runtime mutations (e.g. `model_switched`) are delivered on the SSE stream, not on subsequent attach responses. One correction: when a load or resume applies an approval mode — an explicit `approvalMode` on the request, or a remembered mode replayed on a mode-less cold restore (see `POST /session/:id/approval-mode`) — the cached snapshot's `modes.currentModeId` is updated to the applied mode before that caller's response returns. An attach admitted while a cold replay round trip is still in flight can still be served the cold-load snapshot, so treat the `approval_mode_changed` frame on the SSE stream as authoritative.
...
The daemon also remembers the last explicitly selected mode per session id — set through this route or supplied as `approvalMode` on `session/load` or `session/resume` — and replays it best-effort when that session is cold-restored after its ACP child was torn down (idle reap or crash): a `session/load` or `session/resume` that omits `approvalMode` re-applies the remembered mode, while one that carries `approvalMode` overrides it. A plan-mode outcome is the exception: the parked half would re-enter plan mode on a mode-less restore with its execution mode lost, so a selection whose result is `plan` retires the memory instead of being remembered. This memory lasts only for the lifetime of the workspace's daemon runtime; it does not survive a daemon restart or a workspace runtime disposal or replacement, and it is never written to settings — `persist` remains the only durability switch across those boundaries. The memory is dropped when the session's mode changes from another observed origin (an in-session mode change, or a workspace-wide persisted write in either spelling — a `persist: true` write carrying `planMode` retires it too, and one landing while a remembered-mode replay is still in flight supersedes the replayed value in the live child as well), when the session is deliberately destroyed through a user-initiated delete (a killed session's persisted id stays restorable, so daemon-internal cleanup kills keep the memory, exactly like an idle reap or crash), when a restore reports the id missing, when a successful fresh spawn reuses the id, when a `POST /session` attach applies a different mode to the live session, or when the child definitively rejects the replay. A rejected replay never fails the load — the daemon logs the rejection, forgets the mode it attempted, and completes the restore on the child's cold-load mode; a replay that fails without a definitive answer (a timeout or a transport close) keeps the memory and retries it on the next cold restore. A failed fresh spawn retains the previous incarnation's memory, unless a retirement landed while that spawn was still in flight. A workspace settings reload retires the memory of the live, idle sessions whose mode it moves — such a session emits the mode-update notification that drops its park. A reload that converges on the mode a session already holds emits a no-op re-announcement the daemon deliberately does not treat as a change, so that session's memory survives the reload. A session whose child was torn down, or that was busy at reload time, is not reached at all and keeps its remembered mode until it is deliberately destroyed or superseded by a persisted write — a mode-less restore re-applies the remembered mode and keeps it parked (only a missing id or a definitive rejection drops it), while a restore carrying `approvalMode` replaces it with the requested mode. A mode supplied at session creation (`POST /session`) is not remembered because the daemon cannot distinguish a caller's deliberate create-time choice from a client's echo of the settings-derived mode.
