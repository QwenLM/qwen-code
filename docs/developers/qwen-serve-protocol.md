...
Activity-ordered cursors — the organized view and the `parentSessionId` / `sourceType` filtered lists — are not snapshot-isolated, and a trusted active list orders rows by the later of the transcript mtime and the live activity watermark. A live watermark is in-memory only, so a session's key can regress to its mtime when the live entry retires between two page fetches. The cursor compensates: it carries the identities already emitted at a live-derived key and excludes them for the rest of the pass, so one pagination pass returns a session at most once. The carried list is bounded at 64 identities; in the pathological case of more than 64 watermark-keyed rows in one pass, the excess identities degrade to an at-most-once duplicate rather than an error, so callers that accumulate pages should still key rows by `sessionId`. Rows can still move or be skipped under concurrent activity, exactly as before this field existed; a caller that needs a coherent view reloads from the first page after an activity change.
