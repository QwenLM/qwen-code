...
OPTIONS preflight requests (OPTIONS with `Access-Control-Request-Method` or `Access-Control-Request-Headers`) short-circuit with `204 No Content` plus the headers above. This is the conventional CORS pattern and is safe — the preflight only confirms which methods/headers the daemon will accept; the actual subsequent request still runs the full chain (host allowlist → bearer/listener authority → routes), so anti-DNS-rebinding and the configured deployment policy still apply before any state is read or mutated. Plain OPTIONS requests from matched origins keep flowing downstream with CORS headers attached.
...
Strict writes from a non-trusted token-less request return `401 token_required`
before control code runs. Trusted-loopback primary requests execute normally.
...
trusted-loopback primary request is authorized. A non-trusted
token-less request returns `401 { "code": "token_required" }`. With
...
Strict mutation gate: token-less trusted-loopback primary requests pass; non-trusted token-less requests receive `401 token_required`.
...
- `401 token_required` — strict gate denied a non-trusted token-less request
