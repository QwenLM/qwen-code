...
| `hosted_harness_private_v1`         | the daemon was started with `--profile hosted-harness`, so the private Hosted Harness contract used by the Java Managed Agent service is available. That profile requires a loopback bind, a bearer token and `--no-web`, and runs ordinary sessions on the managed execution engine.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
