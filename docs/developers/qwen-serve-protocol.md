...
`session_turn_status` advertises `GET /session/:id/turns/current` and `GET /session/:id/turns/:promptId`. The current route returns the running turn, queued FIFO head, latest settled outcome, or `{ "state": "idle" }`; the prompt-specific route returns that prompt's bounded live or persisted outcome, including `resultText` for the final main answer shown to the user when available. Persisted outcomes survive a normal daemon restart only when session recording successfully appended the result; the live overlay is process-local, and the endpoint is not an exactly-once result store. Rewind follows the persisted active transcript branch and clears the process-local overlay, so recording-disabled or failed sessions do not retain historical promptIds across rewind. A result exceeding the bounded response size carries `resultTruncated: true` and `resultCode: "RESULT_TEXT_TRUNCATED"`; consumers that require a complete answer must not deliver that result. States are `idle`, `queued`, `running`, `completed`, `cancelled`, or `error`. A supported prompt-specific lookup with no matching turn returns `404 prompt_not_found`; clients must pre-flight this tag to distinguish that result from an older daemon that lacks the routes.
