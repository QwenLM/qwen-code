...
| `hosted_harness_private_v1`         | the daemon was started with `--profile hosted-harness`, so the private Hosted Harness contract used by the Java Managed Agent service is available. That profile requires a loopback bind, a bearer token and `--no-web`, and runs ordinary sessions on the managed execution engine. Only its authenticated session API, `/capabilities` and `/health` are served.                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `managed_sessions`                  | the daemon was started with Managed Agents enabled (`--experimental-managed-agents`), so the `/managed/sessions` routes for Managed Session creation, prompts, listing and transcripts are registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `managed_session_cancel`            | `managed_sessions` is advertised and the Managed prompt service supports cancellation, so `POST /managed/sessions/:id/cancel` is registered.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
