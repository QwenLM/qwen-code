...
| ----------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `maxQueued` | no       | Per-subscriber live-backlog cap. Range `[16, 2048]`, default 256. Bump for cold reconnects with `Last-Event-ID: 0` against a large replay ring so the force-pushed replay frames don't immediately trip eviction. Out-of-range / non-decimal values return `400 invalid_max_queued` before the SSE handshake opens. Pre-flight `caps.features.slow_client_warning` — old daemons silently ignore the param. |
