...
| `realtime_voice`                    | the macOS WebShell daemon has Live Voice enabled and the native Host opted in with `QWEN_LIVE_NATIVE_HOST=1` (off by default; see below). `/live/status` reports readiness, but the capability is withdrawn until the feature is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Live Voice uses the Web Shell endpoint (`realtime_voice_web`, WS `/live/web`) on every platform by default. The native macOS Host is opt-in: only a macOS daemon started with `QWEN_LIVE_NATIVE_HOST=1` advertises `realtime_voice`, registers WS `/live/host` and installs the Host when Live is enabled. Without it, `/live/setup` reports `nativeHost: false` with a non-retryable `install` error, and `POST /live/setup/install` and `POST /live/setup/launch` answer `409 live_native_host_unavailable`.
