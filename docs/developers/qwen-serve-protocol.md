...
`limits.memory` is additive and reports the daemon's resolved memory figures: a required `enforced: false`, `configuredBudgetMb`, `effectiveBudgetMb` (the configured value capped at resolved cgroup/host memory), `budgetSource` (`flag` / `derived`), `availableMemoryMb`, `availableMemorySource` (`constrained` / `host`), `legacyChildCeilingMb` (a conservative model of the ceiling an ACP child receives today, which can sit below the real figure), `insufficientMemory`, and a `modeled` object holding `rootReserveMb`, `childPoolMb`, and `minChildHeapMb`. `runtime.memory` additionally reports `registeredWorkspaces` (the registration count — non-removed workspace entries, including draining, transitioning, or blocked ones; not a live-child count), `activeAcpChildren` (daemon-managed ACP children with a live channel — every runtime still holding a process, including draining or blocked entries; not channel workers, MCP descendants, or unattached spawn reservations), `childRssCoverage` (`primary_only` today), and a `modeled` object holding `recommendedShareAtRegisteredMb` (`null` when no workspace is registered) and `recommendedShareAtActiveMb` (`null` when no child is active). Each share is floored at the minimum child heap, so on a small host share × count can exceed the child pool; read a share as a per-child floor, not a partition of the pool. All of it is observation: no child spawn argument derives from these values, and no request is refused on their basis.
