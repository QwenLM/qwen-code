...
['health', 'capabilities', 'session_create', 'session_scope_override',
 ...
 'workspace_qualified_voice', 'client_mcp_over_ws', 'cdp_tunnel_over_ws',
...
`workspace_qualified_voice` advertises Voice routes selected by a trusted workspace runtime: `GET` and `POST /workspaces/:workspace/voice`, `POST /workspaces/:workspace/voice/transcribe`, and `WS /workspaces/:workspace/voice/stream`. The selector follows the same id-or-encoded-absolute-cwd rules as other plural routes. An unknown selector returns `400 { code: "workspace_mismatch" }`; an untrusted selector returns `403 { code: "untrusted_workspace" }` without falling back to primary. Legacy `/workspace/voice`, `/workspace/voice/transcribe`, and `/voice/stream` remain primary-only. Clients gate settings on `workspace_qualified_voice` plus `workspace_voice`, batch transcription on it plus `workspace_voice_transcription`, and streaming on it plus `voice_transcribe`.
...
| `workspace_qualified_voice`         | multi-workspace runtimes are active, so the workspace-qualified Voice routes can select a secondary runtime.                                                                                                                                                                                                                                                                                                                                                                                                    |
