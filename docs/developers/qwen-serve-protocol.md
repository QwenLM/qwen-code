...
`limits.memory` is additive and reports the daemon's resolved memory figures: a required `enforced: false`, `configuredBudgetMb`, `effectiveBudgetMb` (the configured value capped at resolved cgroup/host memory), `budgetSource` (`flag` / `derived`), `availableMemoryMb`, `availableMemorySource` (`constrained` / `host`), `legacyChildCeilingMb` (the ceiling an ACP child receives today), `insufficientMemory`, and a `modeled` object holding `rootReserveMb`, `childPoolMb`, and `minChildHeapMb`. `runtime.memory` additionally reports `registeredWorkspaces`, `activeAcpChildren` (daemon-managed ACP children only — not channel workers, MCP descendants, or unattached spawn reservations), `childRssCoverage` (`primary_only` today), and a `modeled` object holding `recommendedShareAtRegisteredMb` and `recommendedShareAtActiveMb` (`null` when no child is active). All of it is observation: no child spawn argument derives from these values, and no request is refused on their basis.
