...
Starts a Dynamic Workflow through the existing live session owner. Body:
`{script, args?, sourceRef:{id,revision,digest?,title?}, clientRequestId, expectedWorkspaceCwd?}`.
Returns `{sessionId,runId}` when accepted; inspect tasks for completion. Scripts and
JSON args are each limited to 256 KiB. The session's supported-commands response
advertises `workflowRunV1`; true requires workflows enabled and workspace trust.

The route observes mutable-owner and worktree-reset admission, permits an active
parent prompt, and never creates or reroutes a missing session. An optional
absolute expectedWorkspaceCwd must resolve to the session owner's canonical cwd.
The same optional query on `GET /session/:id/tasks` checks ownership before reading
workflow status. The ACP equivalents enforce the same workspace check.

Same-session request IDs deduplicate identical content, including concurrent
requests and uncertain startup failures; conflicting content returns 409
workflow_request_conflict. This guarantee lasts for the live owner instance, not
across process restarts. Other structured errors: 400 invalid_workflow_run_request,
403 workflow_disabled, 409 workflow_workspace_mismatch, 422 workflow_start_failed.
Tasks expose optional sourceRef, dispatch stepId and final result (at most 64 KiB
of complete JSON; otherwise resultOmitted=true). Provenance does not grant tool
permissions. See [the SDK contract](./daemon/13-sdk-daemon-client.md#start-a-dynamic-workflow-from-an-external-definition).
