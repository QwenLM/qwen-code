...
['health', 'capabilities', 'session_create', 'session_id_override', 'session_scope_override',
 ...
 'workspace_skills_config_runtime', 'workspace_extensions_config_runtime',
...
`workspace_extensions_config_runtime` advertises the Extension runtime catalog at `/workspace/runtime/extensions` and `/workspaces/:workspace/runtime/extensions`. Durable inventory remains under `/extensions`, and workspace activation remains under `/workspaces/:workspace/extensions`. In the split path, clients call `workspace.ensureRuntime()`, `workspace.workspaceRuntimeExtensions()`, `workspace.workspaceExtensions()`, and `client.extensionCatalog()`. The reference Web Shell page overlays runtime rows when the durable catalog generation and coordinator are available, the runtime catalog is initialized, and the capability and catalog epochs equal the coordinator epoch. Capability readiness and generation equality are freshness checks, not overlay gates: unless the capability is ready, the runtime catalog is initialized, both epochs match, and the capability and projection desired/applied generations equal the durable catalog generation, the page re-reads the catalog and projection. A missing or lagging projection does not discard matching-epoch live rows. When the feature is absent, clients keep using `/workspace/extensions` and must not infer the split contract from `extension_management_v2`.

`workspace_extension_mentions` advertises that composer Extension references are sourced from the selected workspace's live runtime and resolve in that same runtime. Web Shell clients gate both the `+` and `@` Extension menus on this feature and retain the legacy primary-workspace loader when it is absent.
...
| `GET /workspaces/:workspace/runtime/extensions`                    | the workspace selector resolves before the trust gate: an unregistered selector answers `400 workspace_mismatch`, a registered but non-active entry answers `503 workspace_runtime_unavailable` with `Retry-After: 1`, and only an active entry reaches the trust gate, where an untrusted target answers `403 untrusted_workspace`; trusted active target: `200` (`workspace_extensions_config_runtime`) runtime catalog snapshot — initialized/current-epoch only when the runtime is already live, and the read does not start or prepare a cold runtime |
...
`workspaceActivation` is `enabled`, `disabled`, or `null` for inheritance. `activationSource` is `default`, `workspace_override`, `legacy_path_rule`, or `cli_override`. `desiredGeneration` is the durable store generation. For coordinator runtimes, `appliedGeneration` is the generation certified in the current live runtime epoch, or `0` without such certification; legacy runtimes retain controller-recorded applied generations. A cold runtime can lag until its next runtime ensure, not merely until the next poll. Generation equality alone does not prove readiness: also check the live runtime's capability state and epoch. When automatic Store recovery restores a lower generation, a fresh poller read clears applied certification and reconciles it; reads overtaken by another observed mutation and late operation receipts cannot lower the desired generation. If a recovery and recommit between two reads reuse the same generation for different content, the poller detects the store's changed content hash and clears applied certification the same way.
...
Interactive global installs and updates share `POST /workspace/extensions/operations/:operationId/interactions/:interactionId` as their answer endpoint; there is no equivalent global-base alias. A `waiting_for_input` snapshot includes `interaction.id` and `interaction.kind` (`setting` or `marketplace_plugin`). Reply with `{ "value": "..." }` for settings or `{ "pluginName": "..." }` for marketplace choices. Pending input is keyed by the process-global operation ID, not isolated by route prefix.
...
`status` transitions from `queued` to `running`, optionally `waiting_for_input`, then to `succeeded`, `succeeded_with_warnings`, or `failed`. An answered interaction returns the operation to `running`, and it may re-enter `waiting_for_input` more than once, each time with a new `interaction.id`, until it reaches a terminal status. While running, `phase` is `preparing`, `committing`, or `reconciling`. Terminal success may include `result` with `status` equal to `installed`, `enabled`, `disabled`, `updated`, `uninstalled`, `checked`, or `refreshed`; reconciliation results can additionally contain `refreshed`, `failed`, and `error`, while batch activation results contain ordered `results`. Update checks return `result.states`, keyed by extension name, with values such as `checking for updates`, `update available`, `up to date`, `not updatable`, or `error`. Credentials and authorization headers are never operation fields.
...
| `workspace_extensions_config_runtime` | authoritative workspace runtime lifecycle and the split Extension config/runtime routes are wired; the Extension runtime-catalog routes enforce that workspace's trust and runtime state, while `/workspaces/:workspace/extensions` stays readable without trust and reports it via the `trusted` field in the body; `/extensions` is a workspace-independent durable catalog and carries no trust field, so clients must take the selected workspace's trust from the projection (or `/capabilities` `workspaces[].trusted`) before merging.                                                                                                                                                                                                                                               |
| `workspace_extension_mentions`        | workspace-qualified composer Extension discovery and same-runtime `@ext:` resolution are wired; each target route still enforces that workspace's trust and runtime state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
