...
`workspace_extensions_config_runtime` advertises the Extension runtime catalog at `/workspace/runtime/extensions` and `/workspaces/:workspace/runtime/extensions`. Durable inventory remains under `/extensions`, and workspace activation remains under `/workspaces/:workspace/extensions`. In the split path, clients call `workspace.ensureRuntime()`, `workspace.workspaceRuntimeExtensions()`, `workspace.workspaceExtensions()`, and `client.extensionCatalog()`. Runtime rows may override the durable catalog only when the Extension capability is `ready`, the runtime catalog is initialized, the capability and catalog epochs equal the coordinator epoch, the capability desired/applied generations equal the durable catalog generation, and the workspace activation projection's desired/applied generations also equal the durable catalog generation. The reference Web Shell page backs the projection-generation condition with a re-read of the catalog and projection, so a client that observes a mismatch can recover by re-reading rather than treating it as terminal. When the feature is absent, clients keep using `/workspace/extensions` and must not infer the split contract from `extension_management_v2`.
