...
Scaffold an empty `QWEN.md` (or whatever `getCurrentMemoryFilename()` returns under `--memory-file-name` overrides) at the daemon's primary workspace root. Mechanical only — for AI-driven content fill, follow up with `POST /session/:id/prompt`.
