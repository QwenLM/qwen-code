...
| `standalone_session_transcript_v1`  | the standalone-session runtime is installed (same condition as `standalone_sessions_v1`), so the turn-index and transcript paging routes `GET /standalone/sessions/:id/turn-index` and `GET /standalone/sessions/:id/transcript` are registered. A separate tag from `standalone_sessions_v1` because these routes postdate it: clients preflight this tag so an older daemon surfaces a loud capability error instead of a 404.                                                                                                                                                                                                                                                                                                                                                            |
