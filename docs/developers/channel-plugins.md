...
**Streaming hooks** — override `onResponseProgress(chatId, text, sessionId, segment)` for cumulative progressive display for the current output. The value restarts at each response boundary, so replace the preview rather than computing a delta, or guard a delta with a prefix check. Override `onResponseComplete(chatId, fullText, sessionId, segment)` for each complete output; `segment.requestFinal === false` marks a non-final delivery. In daemon-managed named-task mode, `segment.sourceLabel` is immutable delivery metadata for that segment. Render it once on each independently visible message or card, including a separately visible final response, but do not add it to raw buffers or model text. In `process_and_result` mode, `onOutputSegmentEnd()` fires only for the request's last segment, so clear adapter-owned segment state from both `onResponseComplete()` and `onOutputSegmentEnd()`.
