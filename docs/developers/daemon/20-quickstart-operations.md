...
# 12. Keep an idle ACP child reusable for 60s after work drains
...
| `--http-bridge`                         | boolean                        | `true`                                           | -                                        | Bridge mode: production attempts to preheat the trusted primary `qwen --acp` child; trusted secondaries start on their first runtime command or Session, and untrusted workspaces cannot start ACP. Stage 2 in-process mode is not implemented yet; `--no-http-bridge` falls back and prints to stderr.               |
