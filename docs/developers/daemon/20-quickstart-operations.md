...
| `--http-bridge`                         | boolean                        | `true`                                           | -                                        | Bridge mode: every trusted workspace starts one `qwen --acp` child on the first runtime command or Session; untrusted workspaces cannot start ACP. Stage 2 in-process mode is not implemented yet; `--no-http-bridge` falls back and prints to stderr.                                                                |
...
| `--channel-idle-timeout-ms <n>`         | positive integer               | none                                             | -                                        | Compatibility auto-reap after all Session and management work drains. Unset keeps the runtime live; `0` is rejected.                                                                                                                                                                                                  |
