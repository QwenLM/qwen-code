...
| `--max-journal-events <n>`              | positive safe integer          | `10000`                                          | -                                        | Per-session baseline cap on in-flight `liveJournal` replay entries. Adaptive growth can raise it (see `--max-journal-bytes`); pinning this flag disables growth.                                                                                                                                                                                        |
| `--max-journal-bytes <n>`               | positive safe integer          | `8388608`                                        | -                                        | Per-session baseline byte cap on the in-flight `liveJournal`. Breaching turns grow the caps on demand (toward double, limited by remaining pool headroom) within one daemon-wide pool of 5% of the effective `--memory-budget-mb` (clamped to `[32, 1024]` MB), never past a 256 MiB per-session hard cap; pinning either journal flag disables growth. |
