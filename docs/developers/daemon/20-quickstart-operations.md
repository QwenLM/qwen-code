...
| `--max-journal-bytes <n>`               | number                         | `8388608`                                        | -                                        | Per-session baseline byte cap on the in-flight `liveJournal`. Breaching turns grow the caps on demand within a per-bridge pool of 5% of the effective `--memory-budget-mb` (clamped to `[32, 1024]` MB), never past a 256 MiB per-session hard cap; pinning either journal flag disables growth.                      |
