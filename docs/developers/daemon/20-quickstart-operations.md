...
| `--workspace <dir>`                     | string                         | `process.cwd()`                              | -                                        | Bound workspace. **Must be an absolute path, must exist, and must be a directory**. Boot canonicalizes it once. `POST /session` with a mismatched `cwd` returns `400 workspace_mismatch`. |
