...
1. **Resolve and trim token** from `opts.token` or `QWEN_SERVER_TOKEN`.
...
1. **Phase 1 - bridge teardown** on first signal:
   ...
   - `bridge.shutdown()` marks channels dying, sends graceful close to each ACP child stdin, waits `KILL_HARD_DEADLINE_MS` (10s) per channel, then calls `channel.kill()` if needed.
...
- Body-parser ordering: `mutate({ strict: true })` 401s happen after `express.json()`. Worst case is `--max-connections * express.json({ limit: '10mb' })`, about 2.5 GB transient on a saturated loopback listener, intentionally accepted.
