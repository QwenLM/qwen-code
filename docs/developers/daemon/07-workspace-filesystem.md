...
- **Streamed windows require a stable file snapshot.** The open handle pins the inode but does not freeze its bytes, so a successful streamed response requires inode identity, size, and modification time to remain unchanged through the read. Mutations detected by those stability checks return `hash_mismatch`; content validation can fail earlier with `binary_file`. `sizeBytes` reports the verified open-time size.
