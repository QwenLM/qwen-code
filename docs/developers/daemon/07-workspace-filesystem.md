...
2. **Avoid unbounded full-file buffering.** The inline fallback caps a buffered read at `READ_FILE_SIZE_CAP = 100 MiB`. The injected adapter applies the stricter `WorkspaceFileSystem` contract: full snapshots stop at `MAX_READ_BYTES = 256 KiB`, and larger UTF-8 files require a finite window and are streamed from an inode-bound handle.
