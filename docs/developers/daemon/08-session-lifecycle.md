...
drain, an omitted or zero `channelIdleTimeoutMs` keeps the legacy behavior and
reaps the child immediately. A positive value keeps the idle child reusable
until the configured timeout expires. The public Workspace Runtime `ensure`
