...
For a persisted worktree session, restore is an integrity-gated extension of this lifecycle. New daemon-created sidecars identify the requested workspace root explicitly; compatible legacy sidecars must identify either that workspace root or its Git repository top-level as the worktree service root. The checkout must be canonically contained below the corresponding `.qwen/worktrees/` directory, and its marker must be a single-link regular file containing the exact restored session ID. The daemon relocates an idle restored child only after those checks and returns the canonical worktree metadata with `worktreeState: "persisted-v1"`. Invalid persisted state detaches an existing attachment or kills a cold restore with `requireZeroAttaches`; a missing sidecar yields no worktree attestation, so isolation-aware clients reject the response instead of rebinding to the shared workspace. Channel-owned restores suppress the ACP agent's best-effort worktree cleanup, so validation failure there never deletes persisted worktree state. Other daemon restore callers retain that existing best-effort cleanup before the route checks any surviving sidecar.
