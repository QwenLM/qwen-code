...
- `BridgeOptions.sessionRestoreTimeoutMs` (default 60s) — ACP `loadSession` / `unstable_resumeSession` deadline. An explicitly configured initialize timeout is the compatibility fallback when no restore timeout is set.
