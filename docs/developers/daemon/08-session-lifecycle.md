...
Each independent controller should use a distinct, stable ID. Web Shell keeps the legacy `webui_` wire prefix so persisted browser controller identities survive the renderer migration. A host and an embedded WebShell should share an ID only when they intentionally act as one logical controller; once shared, daemon logs cannot distinguish which one originated a request.
