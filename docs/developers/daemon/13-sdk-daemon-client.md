...
- `normalizeDaemonEvent(evt)` maps the 50 known daemon wire events (top-level types + `session_update` sub-types) into 42 UI-friendly `DaemonUiEventType` values; unmodeled or malformed events normalize to `debug`.
