...
`addWorkspace` accepts `displayName?: string` and returns it when set. Names are limited to 256 characters and reject C0/DEL control characters. A process-local workspace keeps its name only for the current daemon process; `persist: true` stores the name with the registration so both are restored after restart. `DaemonWorkspaceCapability.displayName` remains optional so the SDK continues to interoperate with older daemons.
