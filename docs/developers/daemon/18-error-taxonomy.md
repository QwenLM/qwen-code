...
| `BridgeChannelQuarantinedError`       | 503  | Cleanup of an abandoned restore was inconclusive, so the workspace channel refuses fresh sessions until it drains. | Keep using existing sessions, wait for the channel to recycle, then retry fresh session work.                                                                                    |
