...
The long-lived `ChannelWorkerManager` owns the committed daemon selection and workspace-grouped supervisors. An explicit `--channel` selection takes precedence and remains fail-fast. On a flagless boot, every trusted registered workspace's own `serve.channels` setting is restored; this dynamically loads the channel runtime, reserves the service pidfile, resolves workspace ownership, and starts the selected workers. The workspace that listed a name breaks an otherwise ambiguous ownership tie, and an unresolvable name a non-primary workspace contributed is dropped with a log rather than failing the whole restore. `all` stays primary-only. A workspace registered after boot runs the same restore for its own names, skipping channels this daemon has explicitly stopped and skipped entirely under an explicit `--channel` selection, which bounds hosting for the daemon's whole life. That restore runs after the registration response rather than inside it. Without either startup source, the first strict-gated `PUT /workspace/channel` performs lazy initialization. `GET /workspace/channel` reads the manager snapshot and `DELETE /workspace/channel` stops it idempotently. SDK helpers are `getChannelWorkerControl()`, `setChannelWorkerSelection()`, and `stopChannelWorker()`; the CLI entry is `qwen channel set` plus remote `status` and `stop` variants.
