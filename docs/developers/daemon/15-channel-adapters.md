...
The long-lived `ChannelWorkerManager` owns the committed daemon selection and workspace-grouped supervisors. On a flagless boot, a trusted primary workspace's `serve.channels` setting is restored; this dynamically loads the channel runtime, reserves the service pidfile, resolves workspace ownership, and starts the selected workers. An explicit `--channel` selection takes precedence. Without either startup source, the first strict-gated `PUT /workspace/channel` performs that initialization. `GET /workspace/channel` reads the manager snapshot and `DELETE /workspace/channel` stops it idempotently. SDK helpers are `getChannelWorkerControl()`, `setChannelWorkerSelection()`, and `stopChannelWorker()`; the CLI entry is `qwen channel set` plus remote `status` and `stop` variants.
