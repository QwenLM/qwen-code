...
- **Polling adapters (GitHub/GitLab/Gitea) use `sendThreadMessage` instead of `sendMessage`.** The base class routes proactive sends through `sendThreadMessage(chatId, threadId, text)`. Without a `threadId`, the adapter logs an error and the message is dropped; with one, a comment is posted. `sendMessage` throws — polling channels do not support bare chat-target sends.
