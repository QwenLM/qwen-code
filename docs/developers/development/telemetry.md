...
- `qwen-code.api_request`: Outgoing request to the LLM API.
  - **Attributes**: `model` (string), `prompt_id` (string), `request_text` (string, optional — present on native OTLP log export and in `telemetry.outfile` when `log_prompts_enabled` is true; on log-to-span bridge spans it additionally requires `includeSensitiveSpanAttributes`; when `log_prompts_enabled` is false the key is present but empty on native OTLP log export and absent from `telemetry.outfile` and log-to-span bridge spans; opaque `thoughtSignature` provider payload is exported verbatim as part of `request_text`, with the policy decision tracked in #11682), `subagent_name` (string, optional)

- `qwen-code.api_response`: Response received from LLM API.
  - **Attributes**: `response_id` (string), `model` (string), `status_code` (int/string, optional), `duration_ms` (int), `input_token_count` (int), `output_token_count` (int), `cached_content_token_count` (int), `thoughts_token_count` (int), `total_token_count` (int), `prompt_id` (string), `auth_type` (string, optional), `response_text` (string, optional — present on native OTLP log export and in `telemetry.outfile` when the response has visible text and `log_prompts_enabled` is true; on log-to-span bridge spans it additionally requires `includeSensitiveSpanAttributes`; it carries no value for internal prompt ids, for responses with no visible text such as tool-call-only or thought-only turns, and whenever `log_prompts_enabled` is false — in every one of those cases the key is present but empty on native OTLP log export and absent from `telemetry.outfile` and log-to-span bridge spans), `subagent_name` (string, optional)
