...
When a safely retryable session start/load or explicit Skills runtime preparation returns `acp_child_capacity_exhausted`, Web Shell offers a workspace chooser if the daemon advertises `workspace_runtime_stop`. Cancel preserves the initiating draft and existing work. Selecting a workspace and confirming interrupts all previewed sessions and their tools, while retaining registration, files and saved history. Workspaces with independent services or enabled scheduled tasks are disabled with a reason. No additional environment variables are needed, and automatic reclamation still only selects empty runtimes.

API clients can read `GET /workspaces/runtime-stop-options`, obtain user confirmation, and submit the exact channel/epoch/token/session set to `POST /workspaces/:workspace/runtime/stop`. Treat unknown or partial outcomes as such: refresh the selected receipt, do not assume a lost response means nothing stopped, and do not blindly retry the original prompt. Continue the original operation once only after `stopped` and `released` are true and its daemon, workspace and draft remain current. See the [runtime-stop protocol](./qwen-serve-protocol.md) for request fields, error codes and receipt semantics.
