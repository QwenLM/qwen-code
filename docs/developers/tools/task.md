...
- `fork_profile` (string, optional): Only valid with `subagent_type="fork"`. Loads `.qwen/fork-profiles/<name>.md` from the active project root and applies its required `tools` array plus optional `promptHint`. It cannot be combined with `fork_tools` or a named teammate, and it is unavailable in safe mode.
