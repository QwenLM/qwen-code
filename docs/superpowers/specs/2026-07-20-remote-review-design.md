...
- **Assisted auto-approve** (`owner`, `autoApprove: true`): a strict
  **escalate-by-default classifier** keyed on `toolCall.kind`:
  ...
  | `edit`                  | `write_file`, `edit`                    | approve **iff `autofix`**, else escalate                                                       |
...
| Prompt-injection via the reviewed diff | Steer the reviewer into mutation, network, or arbitrary shell under auto-approve | Escalate-by-default: only `read`/`search` and a metacharacter-free argv-allowlisted shell auto-approve; `edit` gated on `autofix`; `web_fetch` (exfiltration) and every `other`/unknown kind escalate; shell-metacharacter check defeats `; && | $()` smuggling. Residual: **build/test auto-approval runs the PR author's own code** (`npm test` executes whatever is in the tree) — inherent to reviewing a PR, not a new hole. The "cannot write/commit/push/post" guarantee is scoped to the **reviewer's own** privileged actions, not to code the tree already executes. |
