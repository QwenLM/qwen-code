...
When auth is configured (`--token` or `QWEN_SERVER_TOKEN`), every API route except `/health` on an ordinary loopback bind requires `Authorization: Bearer <token>`; Web Shell document and asset routes remain pre-auth. `--require-auth=true` requires a token at boot and additionally moves loopback `/health` behind the bearer gate. If you started the daemon without a token on the loopback default (the `qwen serve` zero-config path), neither call requires a header and the primary listener grants local callers full operator API authority. The templates above all configure a token, so the `Authorization` header is needed in practice. If `/capabilities` returns `401`, the unit / plist token doesn't match the env-exported token your `curl` is using.
