...
Focus the Background tasks pill in the footer (use `Down Arrow` from an empty composer) and press `Enter` to open the dialog. It lists background agents, shells, monitors, and workflow runs.
...
| `s`                       | Save the script of a completed workflow run (detail view only).                                                                                          |
