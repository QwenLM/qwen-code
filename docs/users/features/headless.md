...
For longer instructions, keep them in a file and pass the path with `--append-system-prompt-file`:

```bash
qwen -p "Review this patch" --append-system-prompt-file ./review-rules.md
```

The same file can be supplied through the `QWEN_APPEND_SYSTEM_MD` environment variable, which is useful for a wrapper or session manager that wants run-scoped instructions without touching settings files:

```bash
QWEN_APPEND_SYSTEM_MD=./review-rules.md qwen -p "Review this patch"
```
...
> - `--append-system-prompt-file` takes precedence over `QWEN_APPEND_SYSTEM_MD`. When inline `--append-system-prompt` text is also present, both are appended.
> - A missing file is an error: the run fails at startup rather than continuing without the instructions.
> - `QWEN_APPEND_SYSTEM_MD` expands a leading `~` and treats the boolean-like values `0` and `false` as "disabled", matching how `QWEN_SYSTEM_MD` reads its path.
...
| `--append-system-prompt-file` | Append the contents of a file to the main session system prompt for this run. Also settable via `QWEN_APPEND_SYSTEM_MD`.                                                                                                                                                                                                                                                                                                       | `qwen -p "query" --append-system-prompt-file ./rules.md`                 |
