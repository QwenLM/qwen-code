...
- **It needs both halves of the bridge.** A withheld tool is reviewed with `tool_search` and invoked through `tool_call`; `tool_search` alone can read a schema it cannot call. If either is unregistered — `tools.toolSearch.enabled: false` denies both; a `tool_search`/`tool_call` deny rule, an `--exclude-tools` entry, or a `tools.disabled` entry removes one — the allowlist still withholds top-level schemas and nothing can load them back through the bridge. In direct and hybrid modes a warning is logged; the tools stay registered, so a direct call by name still goes through normal approval. On the hybrid session surface, callable nested tools still carry their full schemas inside `exec`. Code Mode Only also keeps callable nested schemas in `exec`, ignores eager/visible schema deferral on the session surface, and skips the bridge warning. In both code modes, AgentCore excludes tools still hidden by `tools.eager` from nested bindings. Ordinary deferred tools fall back to being declared eagerly in that case; tools you demoted with `tools.eager` do not.
