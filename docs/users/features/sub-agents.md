...
| `mcpServers`     | array            | Per-agent MCP-server overrides. Loaded from the file but **not yet round-trippable** through `/agents` edits — the local YAML writer only handles one level of nesting, so editing the agent through the wizard drops the field. Manual edits are retained on read. |
| `hooks`          | object           | Per-agent hook overrides. Same round-trip limitation as `mcpServers` — manual edits survive reads, but `/agents` edits drop the field.                                                                                                                              |
