...
Code-hosting platforms (starting with [GitHub](./github)) and authenticated workspace accounts (starting with [DingTalk Workspace](./dws)) are also supported via polling and event adapters.
...
a channel holds at most 50 pending requests overall by default (configurable
with `pairingMaxPending`), and a sender holds at most one pending request across
user and group requests (see [Pairing Rules](#pairing-rules)).
