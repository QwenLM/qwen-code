...
Explicit stops are remembered across restarts: stopping channels (`qwen channel stop` for the standalone service, or the daemon's stop routes) records them as `stopped` in a service-owned channel state file, and a later `--channel all` start skips those channels while starting the rest. The two services keep separate state files, matching their separate scopes: the standalone service records stops in its own per-workspace state file under the global channels directory, while `qwen serve` records them in the daemon's per-workspace channel state file — a stop in one mode does not carry over to the other. Starting a channel by name always starts it and records it as `active` again. When nothing remains to start — no channels configured, or every configured channel stopped — the service keeps running with 0 channels instead of exiting.
