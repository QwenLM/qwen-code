...
| `sessionScope`      | No               | How sessions are scoped: `user` (default), `chat_thread`, or `single`. Legacy `thread` remains compatible when already configured but is not offered for new Web Shell configurations. Channel plugins may declare a different default: QQ defaults to `thread`, while GitHub and GitLab default to `chat_thread` |
