...
Two code paths decide this, and they do not apply the same conditions: the interactive CLI gates generation itself, while the daemon gates it server-side for every client attached to the session.

Both paths require all of the following:
