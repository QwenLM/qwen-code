...
十七个内置媒体策略工具,分三类:九个基于 ffmpeg/sharp 产出媒体产物、五个含模型调用产出文本、三个(表末 `sample_frames`/`get_audio`/`get_clip`)把媒体直接送进上下文而**不产出产物**。前两类的产物均携带强制披露:
...
| `sample_frames`                  | video → 多张 JPEG 进上下文 | `start`/`end`(必填,`HH:MM:SS`/`MM:SS`/纯秒)、`fps`(必填,1.0 = 每秒一帧,上限 10)、`resolution` 768(长边上限,也收 `720p`/`1280x720`);帧数上限 64 | 定位窗口内的画面,不落文件、不需要再 `read_file`                                                                                                                    |
| `get_audio`                      | video/audio → WAV 进上下文 | `start`/`end`(必填)                                                                                                                            | 听某一段;视频无音轨时在任何 ffmpeg 调用之前就拒绝                                                                                                                  |
| `get_clip`                       | video → MP4 片段进上下文   | `start`/`end`/`fps`(必填)、`resolution` 768                                                                                                    | 需要音画同步的片段(可能比抽帧贵,价值在同步而非省 token)                                                                                                            |

多产物工具(关键帧)在一次调用事务里逐帧提升为独立产物;转写/caption/OCR/汇总产物走文本通道(`【媒体转写】` 形态)而非媒体通道,memory role 分别为 `transcript`/`caption`/`ocr`/`summary`。

模型直接调用**产出产物的那些工具**时,成功结果会列出每个产物的绝对路径,模型可立即用 `read_file` 读取剪辑、音频、关键帧或转写结果。

`sample_frames`/`get_audio`/`get_clip` 走相反的路:抽取在工具内部完成,媒体经上传换成预签名 URL 后**直接成为结果的一部分**,模型既不需要也拿不到中间文件路径,因此也不会因为没先建输出目录而空跑一轮。它们没有 `outputDir` 参数、不登记产物,代价是不进媒体记忆(`omni_recall_media_memory` 看不到)、也无法被固定策略驱动 —— 只能靠 `modelAccess.enabled` 逐个打开。**同名的三个子 agent 类型跟着同一个开关**:关着时它们不出现在 `list_agents` 里、也不能被 `agent` 调起,因为父进程给子 agent 播种媒体这一步本身就是在进程内调用同名工具。多帧结果里每一帧前面紧跟一个自己的 `<MM:SS.d>` 时间码,一路保持相邻直到请求体 —— 工具结果通道会把其余文本全拼成一段,但裸时间码是例外,它跟着自己那一帧走。
