...
- Resume declines a Skill it cannot vouch for, and says why in the debug log — naming, for each, the route that actually re-arms it: one whose recorded response no longer matches what is on disk, one whose recorded body was too large for the model and was truncated before it was recorded (so it can no longer be compared against the file at all), one you have since disabled, one now hidden from the model (`disable-model-invocation: true`), and a conditional (`paths:`) Skill this session has not activated yet. The named route accounts for `user-invocable: false`: a Skill that declares it has no `/<skill-name>` to run, so the message names the `paths:` file access that re-activates it instead, and says so outright when the Skill declares both `user-invocable: false` and `disable-model-invocation`, where no route re-arms it at all. The first covers more than an edited **body**: what is compared includes the Skill's absolute directory, so a checkout that was moved or renamed, a symlinked home, or a session recorded inside the container sandbox and resumed on the host all read as a mismatch even with the file untouched. A frontmatter-only edit — a new `hooks:` block, a different `allowedTools` — is the opposite case: the recorded response is unchanged, so the Skill is restored, with the edited frontmatter in force. Two further cases are reported elsewhere or not at all: a **project** Skill whose folder is no longer trusted is refused by the trust gate, which logs its own message, and a Skill that has been deleted or renamed logs only at `debug`, since there is no frontmatter left to tell whether it declared a gate. A Skill whose invocation was summarized away by a compaction leaves no tool-call record at all and is silently not restored. In every one of these cases the Skill's instructions can still be in the replayed conversation while the hooks meant to enforce them are absent.
