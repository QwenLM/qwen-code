...
- The `command:` runs through the platform shell: `bash` on macOS and Linux, and on Windows Git Bash when it is detected (`MSYSTEM`/`TERM`), otherwise PowerShell (`pwsh` preferred, Windows PowerShell 5.1 as fallback). The example above is POSIX shell — under PowerShell a `"$QWEN_SKILL_ROOT/…"` path expands to an empty string and the gate fails open silently, so write a Windows gate as `& "$env:QWEN_SKILL_ROOT/scripts/gate.ps1"`. A `.ps1` gate needs the machine's execution policy to allow scripts, which stock Windows clients refuse, and a hook that fails to load does not block the Skill -- so gate inline or in a `.cmd`/`.bat` unless you have relaxed the policy, and invoke that script with the call operator (`& "$env:QWEN_SKILL_ROOT/scripts/gate.cmd"`) -- a bare-quoted program path under PowerShell evaluates as a string and exits 0 without running the program, which Qwen only warns about in the debug log. An inline gate blocks only when it reaches `exit 2`; under the injected `Set-StrictMode` / `$ErrorActionPreference = 'Stop'` a failing statement exits 1 instead, which does not block.
- A hook may set `shell: bash` to force bash, but that spawns the bare name `bash`, and Windows searches the application directory, the working directory and `System32` before `PATH` — reordering `PATH` does not make it Git Bash. Outside a Git Bash session, write the gate for the shell you actually have. See [Hooks](hooks.md) for the Windows migration notes.
