...
- The `command:` runs through the platform shell: `bash` on macOS and Linux, and on Windows Git Bash when it is detected (`MSYSTEM`/`TERM`), otherwise PowerShell (`pwsh` preferred, Windows PowerShell 5.1 as fallback). The example above is POSIX shell — under PowerShell a `"$QWEN_SKILL_ROOT/…"` path expands the undefined variable to empty inside the string and the path is echoed with exit 0, so the gate fails open there silently; write a Windows gate against `$env:QWEN_SKILL_ROOT`. A hook may set `shell: bash` to force bash, but that resolves to whatever `bash` is on `PATH`, so on Windows outside Git Bash write the gate for the shell you actually have. See [Hooks](hooks.md) for the Windows migration notes.
