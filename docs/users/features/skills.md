...
- The `command:` runs through the platform shell: `bash` on macOS and Linux, and on Windows Git Bash when it is detected (`MSYSTEM`/`TERM`), otherwise PowerShell (`pwsh` preferred, Windows PowerShell 5.1 as fallback). The example above is POSIX shell — under PowerShell a `"$QWEN_SKILL_ROOT/…"` path expands the undefined variable to empty inside the string and the path is echoed with exit 0, so the gate fails open there silently; write a Windows gate as `& "$env:QWEN_SKILL_ROOT/scripts/gate.ps1"`, and note hooks run under `Set-StrictMode` plus `$ErrorActionPreference = 'Stop'`, which also govern any `.ps1` the command invokes — a script needing the old tolerant behaviour resets both on its first line. A hook may set `shell: bash` to force bash, but that spawns the bare name `bash`, and Windows searches the application directory, the hook's working directory, and `System32` before consulting `PATH` — reordering `PATH` does not make it Git Bash — so on Windows outside Git Bash write the gate for the shell you actually have. See [Hooks](hooks.md) for the Windows migration notes.
