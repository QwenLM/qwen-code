...
- `priority` is optional. When present, it must be a finite number. Higher values sort earlier in `/skills`, slash-command completion, and the `/help` custom commands view. Omitted or invalid values are treated as unset, which behaves like `0`.
...
- Use `priority` sparingly for Skills that should reliably appear before the default alphabetical order. Negative priorities are allowed and sort below unset Skills.
