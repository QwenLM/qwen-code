...
- **Cancellation behaviour depends on timing.** A turn cancelled _while the message is still streaming_ fires no `is_final` — the message didn't end, it was abandoned, and a consumer that buffers until `is_final` should treat cancellation-silence as its flush/discard signal (e.g. a timeout fallback). Cancelling _after the message ended_ (during the drain wait) is different: `is_final` has already been dispatched, and the still-running hook execution may be terminated mid-flight (SIGTERM).
