...
- The `security.allowedHttpHookUrls` whitelist still applies independently. In managed environments, pair this flag with a whitelist so only the intended internal endpoints are reachable. Like this flag, the whitelist is **only honored from User, System, and SystemDefaults settings scopes**; a value set in Workspace (project) settings is ignored and logged as a warning, so a cloned repository can neither widen where hook payloads may be sent nor replace the user's whitelist.
