...
- The `security.allowedHttpHookUrls` whitelist still applies independently. In managed environments, pair this flag with a whitelist so only the intended internal endpoints are reachable. Like this flag, the whitelist is **only honored from User, System, and SystemDefaults settings scopes**; a value set in Workspace (project) settings can only _narrow_ the higher-scope whitelist and is logged as a warning: workspace entries that no higher-scope entry covers are dropped, and when no higher scope sets a whitelist the workspace value is ignored entirely (an empty whitelist means "allow all", so a repository can neither widen where hook payloads may be sent nor establish a whitelist of its own).
