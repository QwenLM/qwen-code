...
- A matcher is first compared exactly. In a `|`-separated list such as `permission_prompt|idle_prompt`, the matcher matches when any entry is `*`, `.*`, or exactly the value, unless the whole matcher starts with `^` or `(`, in which case it is only a regular expression. List entries are never read as regular expressions on their own, so an escaped pipe such as `notes\|todo\.md` keeps its whole-pattern meaning
- Otherwise the matcher is an unanchored regular expression (e.g., `^run_shell_command$`, `read_.*`, `(write_file|edit)`). Empty list entries are dropped first, so a stray `|` as in `read_(file|edit)|` is ignored and never makes the matcher match everything, and a matcher of only `|` matches nothing. Because the expression is unanchored, `read` also matches `read_file` and `edit` also matches `notebook_edit`. Add `^` and `$` to match a whole value, and anchor exclusions as well: `^(?!write_file).*$` excludes `write_file`, while unanchored `(?!write_file).*` still matches it
