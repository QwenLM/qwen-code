...
If your primary model already supports image input, the bridge is skipped and the image is sent directly to that model. If Qwen Code cannot determine the primary model's input modalities, or cannot find an image-capable model, it skips the bridge and keeps the normal unsupported-image behavior.
