...
When workspace MCP pooling is enabled, daemon sessions (including Channel sessions) share eligible MCP connections, including stdio processes. Cancelling a tool call sends a request cancellation; it does not request a server restart and the cancelled call is never replayed. A server may nevertheless exit or lose its transport while handling cancellation. Disabling the pool retains the legacy per-session connection behavior.

When a previously connected server disconnects unexpectedly, the next model send in each affected session attempts to restore that session's connection, tools and prompt commands. This includes later tool loops and scheduled model turns. Concurrent sessions share a replacement process. Each model send can request recovery; transport and session-acceptance failures impose a shared five-second cooldown. Budget refusals remain eligible when capacity becomes available and retain budget-specific advice. All acquisition paths wait for old transport cleanup. There is no background reconnect timer. The session reports success or failure, with identical diagnostics shown once per user turn.
