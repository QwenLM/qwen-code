...
### Project `.mcp.json` (shipped with the repository)

A `.mcp.json` file in the project root (the Claude Code convention) declares servers that travel with the repository. Because the file comes from the repository, its servers are untrusted until you approve them: an interactive session asks before connecting, and `qwen mcp approve <name>` (or `--all`) records the approval for non-interactive use. Approval is bound to the exact server configuration — editing the entry, or changing the value of an environment variable it references, asks again.

Reference secrets by name instead of embedding them: `$VAR` / `${VAR}` in `command`, `args`, `env`, `cwd`, `url`, `httpUrl`, `headers`, `tcp`, `oauth`, `targetAudience` and `targetServiceAccount` is expanded from this workspace's environment — the shell environment plus the `.env` files and `settings.env` Qwen Code loads for this workspace — using the same `$VAR` / `${VAR}` syntax as `settings.json`. An unset variable is left as literal text; other fields such as `description` are never expanded.
