...
Before accepting completion, the verifier receives the original action records and arguments, real user supplements, required delivered output, and verified child-agent actions. It checks later mutations and failed checks as well as the references the working model selected. A passing check followed by a relevant change is not enough by itself, and writing then reverting a file does not satisfy a requirement never to write it. Related background writers must finish before a stable result can be claimed. Missing or truncated action history is reported as unavailable instead of being replaced with a summary.

Verification has bounded request size, model context, calls and elapsed time. A service error or unusable response pauses the Goal while retaining its proposal and evidence; `/goal resume` retries a still-current proposal. If the evidence cannot fit the verifier's capacity, the pause explains the bottleneck. Repeating an unchanged request does not fix that: use a verifier with enough context capacity for the retained originals. Do not remove historical constraints to make verification fit. Actual Goal budgets remain authorization limits.
