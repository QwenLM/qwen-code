...
- Unmet conditions return feedback to the working model. An unavailable verification service or incomplete required history pauses verification and retains the proposal; it does not mark the task complete or inherently blocked. Explicit delivery formats must appear in the actual answer before the model submits its completion proposal.
