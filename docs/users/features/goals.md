...
A Goal keeps Qwen Code working across turns until a stated condition is met. Set one with `/goal <objective>`, and the session keeps going on its own. When the model proposes that the objective is complete or blocked, an independent verifier judges that proposal from the transcript alone: the closing turn's tool results, delivered output and your messages, and as much of the turns before it as fits. The session stops when the verifier accepts, or when the Goal is paused, cleared, or stopped by a limit.
...
Each turn the session takes on its own reports what the Goal has spent so far, how many turns are behind it, and — unless the Goal runs unbounded — the window it is allowed. Every such turn except the final wind-down hand-off also carries standing instructions to re-check the workspace rather than trust earlier turns' reports, to work toward the end state the objective asks for, to do something different when the previous turn changed nothing (from the second turn on, once there is a previous turn to judge), and to check every requirement against evidence in the turn's own transcript before proposing that the Goal is done.

The verifier reads the transcript, not a summary of it. It is handed the newest records first — the proposing turn's tool results, the text the model delivered, your messages — and as many earlier ones as fit a request bounded by the verifier model's context window, at most about 256 kB. A tool result travels with the call that produced it, arguments included. One record contributes at most 8 000 bytes; longer output is cut in the middle so its beginning and its end both survive, and the verifier is told where the cut was. It is also told how many earlier records were left out, and it rejects a proposal whose proof may sit in that part. So a long Goal is not judged on what it did ten turns ago: the check the objective names has to be run again in the turn that proposes completion, which is what a well-written `Done when` asks for anyway. Nothing is cited, and the model cannot choose what the verifier sees.

A verifier check is bounded to two minutes per attempt, with one retry for a transient provider error. If it does not answer in time, the provider fails, or the transcript cannot be read, nothing was judged: the Goal pauses with that reason and the proposal is dropped, and `/goal resume` gives the model a fresh turn to propose again. When the verifier model's context window cannot hold even one transcript record, the pause says so and names the remedy: a side query model with a larger window, or a shorter objective. When the proposal came from the wind-down hand-off turn, the Goal stops for its budget instead, so that resuming re-arms it. A blocker that had already been recorded on three consecutive turns keeps that standing across such a pause, so the resumed turn may propose it again at once.
...
Every pause states its reason: that you interrupted it, that you ran `/goal pause`, that the session token limit blocked the next model request, that the turn failed, that three turns in a row recorded nothing the verifier could judge and no proposal — Goal bookkeeping reads (`get_goal`, `update_goal`) do not count as progress — or that a proposal could not be verified because the check timed out or failed. When the transcript itself could not be read for verification, the pause also moves the evidence window's start to that point, so a resumed Goal is judged on the work that follows. A Goal stopped by a limit keeps the reason for that limit instead.
...
- It sees the newest part of the transcript first and only as much as fits, so a check run many turns earlier may be out of its reach. The decisive check belongs in the turn that proposes completion.
