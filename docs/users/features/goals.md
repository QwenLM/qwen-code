...
A Goal keeps Qwen Code working across turns until a stated condition is met. Set one with `/goal <objective>`, and the session keeps going on its own. When the model proposes that the objective is complete or blocked, an independent verifier judges that proposal from the visible output and tool results of the turn that made it. The session stops when the verifier accepts, or when the Goal is paused, cleared, or stopped by a limit.
...
The verifier never runs commands or reads files on its own. It only sees what is already in the transcript, and only from the turn that proposed completion (for a blocked proposal, that turn and the two before it):
