...
A verifier check is bounded to two minutes. If it does not answer in time, the provider fails, or the transcript cannot be read, nothing was judged: the Goal pauses with that reason and the proposal is dropped, and `/goal resume` gives the model a fresh turn to propose again. A blocker that had already been recorded on three consecutive turns keeps that standing across such a pause, so the resumed turn may propose it again at once.
