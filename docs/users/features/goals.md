...
Goals no longer compress recorded evidence into checkpoint claims between turns. The side-model check that used to run at the end of long turns, its token spend, and the stop after three stalled checks are gone: the runtime is created without a checkpoint verifier, a checkpoint an earlier build left pending is dropped when the session is resumed, and the stall count such a build recorded is cleared so it cannot keep an idle Goal from pausing for no progress. The `model.goalCheckpointTimeoutSeconds` setting is still accepted but has no effect, and a Goal record's checkpoint fields stay at zero.
