...
Once a Goal has billed a turn, the footer pill and every status card show what it has spent against the window it is allowed, as `1.2k/30.0m`. The figure counts the model calls in the Goal's own turns, its direct foreground subagents, and its independent verification calls, including evidence follow-ups and format repair. Nested and background subagents, unrelated side queries, cron turns, and notification turns are excluded. Observable usage is counted even when a verification reply is malformed or rejected; usage the provider does not return remains unknown. Checks performed during recovery count as new calls; restoring already recorded usage does not count it again. The window is set by [`model.goalTokenBudget`](../configuration/settings.md); resuming a Goal that has spent its window grants another one on top of what it has already spent, so the figure reads `30.0m/60.0m` rather than starting over. A Goal with no budget shows only what it has spent. A Goal that has not billed a turn yet shows no figures at all.
...
Evidence stays in the original session transcript. The model can page through the Goal's evidence directory and read an original record in bounded slices. New results and directory pagination do not invalidate earlier references in the same Goal revision. Goal evidence checkpoints no longer run automatically, and `model.goalCheckpointTimeoutSeconds` is retained only for compatibility with older settings. Ordinary conversation context compression is unchanged.

Before accepting completion, the verifier receives an action directory with tool arguments, original slices for cited proof, real user supplements, required delivered output, and verified child-agent actions. It reads additional original records and slices on demand. Large persisted tool outputs are read from host-recorded artifact paths in bounded slices; a changed or missing artifact cannot be treated as complete. It checks later mutations and failed checks as well as the references the working model selected. A passing check followed by a relevant change is not enough by itself, and writing then reverting a file does not satisfy a requirement never to write it. Related background writers must finish before a stable result can be claimed. Missing or truncated action history is reported explicitly. An unrelated historical image or incomplete output does not by itself prevent a fresh current-state check from proving completion; missing history still cannot prove a condition that applies to every past action.

Verification has bounded request size, model context, calls and elapsed time. A service error or unusable response pauses the Goal while retaining its proposal and evidence; `/goal resume` retries a still-current proposal. Incomplete required proof and inspection-capacity limits return feedback to the working model to obtain focused proof or explain the remaining limitation, instead of pausing for an unchanged retry. Checking `/goal` or changing the verifier model does not invalidate a retained proposal. A budget wind-down that only reports progress also preserves it; new user input or tool activity still requires a new proposal. Do not remove historical constraints to make verification fit. Actual Goal budgets remain authorization limits.

Older completed Goals remain completed and paused Goals stay paused until you resume them. Resume no longer clears the evidence window. Old checkpoint summaries can help locate originals but cannot substitute for them. Sessions with pending verification should be resumed using this version or newer; older versions see a conservative pause instead of safely continuing the new verification protocol.
...
Typing a message while a Goal is active does not pause it. Your message runs as the next Goal turn and invalidates any pending verification result, so use it to steer the work; use `/goal pause` or `/goal clear` to stop it.
...
The verifier cannot run commands or inspect arbitrary workspace files. It can read original transcript records and their host-recorded output artifacts through a controlled evidence reader:

- Visible assistant output proves what was delivered, tool results prove external facts, and real user messages prove user instructions or approval. The objective and hidden reasoning are not proof of actions.
...
- Unmet conditions and incomplete required proof return feedback to the working model. An unavailable verification service pauses verification and retains the proposal; it does not mark the task complete or inherently blocked. Explicit delivery formats must appear in the actual answer before the model submits its completion proposal.
