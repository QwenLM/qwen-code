...
Evidence is not compressed between turns. Every turn's visible output and tool results stay on the record as they were written, up to the catalog's bounds. A Goal whose recorded evidence outgrows those bounds stops as evidence-limited when its next completion or blocked proposal is checked, with a reason that says so; `/goal resume` starts a fresh evidence window from that point, so the closing turn should run the decisive checks again before proposing completion. The `model.goalCheckpointTimeoutSeconds` setting has no effect.
