...
| `--compacted-replay-max-bytes <n>`      | `4194304`       | Per-live-session byte cap for the bounded replay snapshot returned by `POST /session/:id/load` as `compactedReplay` plus `liveJournal`. Values must be positive integers; invalid values fail at boot, and the hard ceiling is 256 MiB. When older retained replay is dropped, the snapshot begins with `history_truncated`. This does not limit the on-disk transcript.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
