...
| `--max-total-sessions <n>`              | derived         | Optional non-negative integer daemon-wide cap on fresh session creation across all registered workspaces. It applies to new child sessions, session restore, and branch/fork-created sessions; attaching to an existing live session does not consume a slot. Set to `0` for unlimited. When omitted in multi-workspace mode, a finite per-workspace cap derives the total cap as `maxSessionsPerWorkspace * workspaces.length`; otherwise it remains unlimited.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `--workspace <path>`                    | `process.cwd()` | Absolute workspace registered by the daemon. Repeat the flag to enable multi-workspace session mode; the first path is primary. Duplicate, nested, and unregistered workspace selectors fail closed with `workspace_mismatch`. Session creation and runtime-backed routes require a trusted workspace.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
Pass `--workspace` more than once to register multiple non-overlapping workspaces in one daemon. The first path is primary, and each trusted workspace owns a separate runtime/ACP boundary while sharing the daemon listener and bearer token. Use one daemon per user or security principal; workspace trust is an execution gate, not an ACL.
...
Run separate daemon processes when workspaces need different bearer-token principals, quotas, audit boundaries, or operating-system isolation. A single daemon token authorizes every route the daemon exposes, including the allowed read-only catalog for all registered workspaces.
