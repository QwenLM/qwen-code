...
- **Session stats & export** ([#4514](https://github.com/QwenLM/qwen-code/issues/4514) T2.5+T2.6) — fetch per-session aggregate metrics (`GET /session/:id/stats` — promptCount, totalTokens, files touched, context window utilization) or render the conversation in markdown / html / json / jsonl (`GET /session/:id/export?format=md|html|json|jsonl`). Both routes reuse the same SSOT as the TUI `/stats` and `/export` slash commands, so daemon surfaces and the CLI agree for any state flushed to disk. Read-only, no mutation gate; SDK helpers `client.sessionStats(id)` / `client.sessionExport(id, format)` (or session-bound `session.stats()` / `session.export(format)`).
  - **Live-session requirement (v1):** both routes need the session to be live in the daemon. Archived sessions on disk return `400` until you re-attach via `POST /session/:id/load`. The next iteration may relax this for pure-disk reads (no Config needed for the no-context-percent subset).
