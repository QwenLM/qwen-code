...
Registration is capped: one daemon holds at most 25 workspaces by default (the primary counts toward the cap), and `POST /workspaces` answers `409` with `code: "workspace_limit_reached"` when the cap is reached. Set `QWEN_SERVE_MAX_WORKSPACES=<n>` (positive integer) in the daemon's environment to change the cap; malformed values fail the boot. `GET /capabilities` reports the effective cap as `limits.maxRegisteredWorkspaces`.
