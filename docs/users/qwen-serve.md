...
- **An IPv6 wildcard bind is dialled back on `::1`, not `127.0.0.1`** — `--hostname ::` (or `[::]`, or an empty `--hostname`) binds an IPv6 socket, and a kernel with `net.ipv6.bindv6only=1` — or a host with no IPv4 at all — will not answer `127.0.0.1` on it. Workers are therefore sent to `[::1]`, so a serving certificate for such a bind needs `::1` in its SANs (`mkcert ... ::1`); the boot trust diagnostic names the gap if it doesn't. `--hostname 0.0.0.0` is unchanged and still needs `127.0.0.1`.
