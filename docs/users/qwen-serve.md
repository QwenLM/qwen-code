...
- **Chrome extension browser automation is separate from framing.** `qwen serve --allow-origin chrome-extension://<id>` lets the extension frame the Web Shell and connect to the daemon. Console/network/screenshot/click tools require an external CDP MCP adapter command: `QWEN_CDP_MCP_COMMAND=/path/to/cdp-mcp-adapter qwen serve --allow-origin chrome-extension://<id>`. The main CLI package does not bundle a browser automation adapter; clients can check `caps.features.includes('browser_automation_mcp')` before presenting those tools as available. **Shared Chrome bridge (issue [#8737](https://github.com/QwenLM/qwen-code/issues/8737)):** while the extension is connected, the daemon's `/cdp` tunnel accepts multiple concurrent clients, and interactive `qwen` processes automatically reroute a `chrome-devtools` MCP server configured with `--autoConnect` to dial `ws://<daemon>/cdp` instead of Chrome — replacing Chrome's per-session "Allow remote debugging?" consent with the extension's single `chrome.debugger` attachment. Probe `GET /cdp/status` (`usable: true`) to see whether the bridge is live; set `QWEN_NO_SHARED_CHROME_BRIDGE=1` in a client process to opt out.
