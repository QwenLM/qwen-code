...
Repeated paging through the legacy singular route reuses the sticky Workspace Runtime and its in-process transcript index cache by default. A positive `--channel-idle-timeout-ms` opts into delayed compatibility auto-reaping and should be long enough to cover the cursor walk; `0` is rejected. The workspace-qualified persisted route never starts an ACP child and is unaffected by this timeout.
...
Each active workspace runtime keeps **one bridge boundary**. Every trusted workspace opens its channel and child on its first runtime-backed command or Session; an untrusted workspace never does. The child remains live and reusable after the last Session or management operation finishes. By default the channel idle timeout is unset, which disables automatic reaping; a positive value opts into compatibility reaping after every physical lease drains, while explicit `0` is rejected. Workspace removal, daemon shutdown, explicit restart, or a channel-level crash also tears it down without selecting another runtime.
