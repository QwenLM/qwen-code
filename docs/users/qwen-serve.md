...
- **A spawned `qwen --acp` child receives its owning runtime's effective environment.** The daemon freezes a process-env base, applies that workspace's settings/env-file overlay to a runtime-local snapshot, and never writes the overlay back to `process.env`; same-named keys in another runtime do not cross over. `QWEN_SERVER_TOKEN` is scrubbed before spawn because the agent does not need the daemon bearer. Base credentials such as `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `QWEN_*`, and `DASHSCOPE_API_KEY` otherwise pass through unless the runtime overlay changes them. **This is intentional, not a sandbox.** The agent runs as the same UID with shell-tool access, and direct `read_file` can read host text paths outside registered workspaces under the normal CLI permission rules (external paths default to confirmation; allow rules and approval modes may approve them automatically). Approved external reads use configurable CLI output limits rather than WFS's 256 KiB returned-output and full-snapshot cap or 8 MiB large-text scan cap. All shared `FileSystemService.readTextFile` consumers—including pre-reads for write, edit, notebook, sed, and artifact operations—also use the child's regular CLI filesystem service and therefore do not receive those WFS limits, read audit, symlink rejection, or read-side TOCTOU protections. HTTP filesystem routes remain workspace-scoped, agent discovery-tool behavior is unchanged, and final ACP `writeTextFile` content writes continue through WFS. Anything in `~/.bashrc`, `~/.aws/credentials`, or `~/.npmrc` is therefore reachable by an approved direct read or shell access; treat authenticated daemon clients as the same security principal and do not run `qwen serve` under an identity that has credentials you would not trust the agent with.
