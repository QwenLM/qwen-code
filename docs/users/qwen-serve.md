...
- **Rotating `--tls-cert` in place needs a daemon restart** — the daemon serves the bytes it read at boot, so until it restarts, respawned workers can load newer contents than the daemon presents. Their handshakes then fail **when the daemon cert is its own trust anchor** (self-signed, or a fullchain carrying its own root). With an operator `NODE_EXTRA_CA_CERTS` CA set — the mkcert flow above — that CA anchors both generations, so respawned workers still verify the old leaf the daemon is still serving; only the serving side lags until the restart.
