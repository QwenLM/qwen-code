...
This mode is experimental and daemon-managed. It does not replace the standalone `qwen channel start` command: without `--daemon-url`, existing `qwen channel start`, `stop`, and `status` behavior remains standalone. With `qwen serve --channel`, the explicit selection wins, the daemon reserves the channel-service lease before listening, and startup fails if the initial worker cannot become ready. On a flagless boot, the daemon restores `serve.channels` from every trusted registered workspace, so a channel a non-primary workspace is configured to start comes back with the daemon instead of needing a runtime API call. If neither source selects channels, the heavy channel runtime remains unloaded and no channel-service lease is reserved until the first runtime PUT. A workspace registered after the daemon started restores its own `serve.channels` the same way, minus any channel an operator stopped in this daemon, skipped entirely once channels have been stopped daemon-wide through `DELETE /workspace/channel`, and skipped as well when the daemon was started with an explicit `--channel`, which bounds what it hosts for its whole life. Those decisions last until something enables channels again or the daemon restarts, which reads the settings again. Registering the workspace answers right away; its channels come up shortly afterwards rather than holding the registration open. A channel name listed by a workspace that cannot host it is skipped with a log, leaving the other workspaces' channels running; `all` still applies only to the primary workspace. If a ready worker later crashes, the daemon keeps running, relaunches it under a bounded restart policy, and reports its state (including `channel_worker_exited` warnings) in `GET /daemon/status`.
