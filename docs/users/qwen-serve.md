...
- **Up to one on-demand ACP child per trusted workspace, many clients** — daemon startup only registers runtimes; the first runtime-backed request starts that workspace's child. Untrusted workspaces never start one. Under the default `sessionScope: 'single'`, clients targeting the same workspace share one ACP session and collaborate on the same conversation, file diffs, and permission prompts.
...
| `--http-bridge`                         | `true`          | Stage 1 mode: each trusted workspace starts one `qwen --acp` child on its first runtime-backed command or Session. Sessions targeting a runtime multiplex onto its child via ACP `newSession()`; untrusted workspaces cannot start ACP. Stage 2 native in-process becomes available later.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
Repeated paging through the legacy singular route can reuse the Workspace Runtime and its in-process transcript index cache when a positive `--channel-idle-timeout-ms` is configured long enough to cover the cursor walk. By default, and with explicit `0`, the child is reaped as soon as runtime work drains and may restart between page requests. The workspace-qualified persisted route never starts an ACP child and is unaffected by this timeout.
...
Each active workspace runtime keeps **one bridge boundary**. Every trusted workspace opens its channel and child on its first runtime-backed command or Session; an untrusted workspace never does. By default, and with explicit `0`, the child is reaped immediately after the last Session or management operation finishes and all physical leases drain. A positive `--channel-idle-timeout-ms` keeps it live and reusable for that many milliseconds; deployments that want management pages to reuse a runtime across requests should configure such a window. Workspace removal, daemon shutdown, explicit restart, or a channel-level crash also tears it down without selecting another runtime.
