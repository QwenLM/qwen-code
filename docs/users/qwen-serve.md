...
- **An IPv6 wildcard bind is dialled back on `::1`, not `127.0.0.1`** — `--hostname ::` (or `[::]`) binds an IPv6 socket, and an empty `--hostname` binds one too when IPv6 is available (when it is not, Node falls back to binding `0.0.0.0`). A kernel with `net.ipv6.bindv6only=1` — or a host with no IPv4 at all — will not answer `127.0.0.1` on an IPv6 wildcard socket, so workers are sent to the loopback of the family actually bound: `[::1]` for the IPv6 bind, `127.0.0.1` on the IPv4 fallback. A serving certificate for an IPv6 wildcard bind therefore needs `::1` in its SANs (`mkcert ... ::1`); the boot trust diagnostic names the gap if it doesn't. `--hostname 0.0.0.0` is unchanged and still needs `127.0.0.1`.
- **Rotating `--tls-cert` in place needs a daemon restart** — the daemon serves the bytes it read at boot, so until it restarts, respawned workers can load newer contents than the daemon presents. Their handshakes fail exactly when **the trust anchor rotates out with the certificate** — always for a self-signed cert, since the leaf is its own anchor (`DEPTH_ZERO_SELF_SIGNED_CERT`), and for a fullchain only if its root is replaced too (`SELF_SIGNED_CERT_IN_CHAIN`). Rotating a fullchain to a renewed leaf under the **same** carried root keeps verifying: that root still anchors the old leaf the daemon is still serving. With an operator `NODE_EXTRA_CA_CERTS` CA set — the mkcert flow above — that CA anchors both generations for the same reason; only the serving side lags until the restart.
