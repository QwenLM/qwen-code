...
To use a remote daemon from the Web Shell on your own machine, keep your local projects and add the remote one next to them: open the local Web Shell, choose **Add workspace → Remote**, and enter the remote daemon's origin (for example `https://dev-box.example:4170`) and its bearer token. The remote daemon must allow your local page's origin, for example `qwen serve --hostname 0.0.0.0 --allow-origin http://localhost:4170`; the terminal's WebSocket needs that entry too. The token is kept per tab and per daemon origin and is never sent to another daemon. The sidebar lists local and remote projects together, and switching between them reloads the page against the selected daemon. A link whose `?daemon=` names an origin this browser has not connected to before is not contacted until you confirm it on the connection screen. An HTTPS page cannot connect to an HTTP daemon, so serve both over HTTPS outside trusted networks.
