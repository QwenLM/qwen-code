...
| ------------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **OpenAI / DashScope** (`qwen3.8-max`, `qwen3.8-max-preview`) | Flat `reasoning_effort: <effort>` body parameter                     | The five `/effort` tiers (`low`, `medium`, `high`, `xhigh`, `max`) are passed through verbatim; DashScope applies any model-specific mapping. The normal `extra_body` > `samplingParams` > `reasoning` precedence resolves conflicts with `thinking_budget`; do not explicitly set both fields in the same layer because DashScope rejects the pair. Other Qwen models continue to map a selected effort to `enable_thinking: true`. |
