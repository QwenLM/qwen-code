...
**`requestConcurrency`:** Caps the number of in-flight requests against this provider so the upstream `429 Too many concurrent requests for this model` error never fires — excess callers (typically sub-agents firing in parallel, or `/compress` interleaving with the main turn) wait FIFO instead. Set to `0` (default) for no limit. The environment variable `QWEN_REQUEST_CONCURRENCY` is honored as a fallback when the provider-level value is unset, so the cap can be applied process-wide without editing `settings.json`.
