...
Language Server Protocol (LSP) provides code intelligence features like go-to-definition, find references, and diagnostics. LSP auto-enables when a `.lsp.json` file exists in the project root. You can also enable it explicitly with `general.lsp: true` in settings or `--lsp` on the command line.

LSP server configuration is done through `.lsp.json` files in your project root directory. See the [LSP documentation](../features/lsp) for configuration details and examples.
...
| `--lsp`                      |       | Enables [LSP (Language Server Protocol)](../features/lsp) for code intelligence (go-to-definition, find references, diagnostics, etc.). Auto-enabled when `.lsp.json` exists.           |                                        | Stable. Replaces the deprecated `--experimental-lsp` flag. Requires language servers to be installed.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
