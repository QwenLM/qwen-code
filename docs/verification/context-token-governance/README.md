...
**这两个预期值都是 `地板 + 白名单内 schema`，不含按需池**（激进版 = 2,163 + (4,080 − 375)），所以 #10410 省下的预加载量到不了这里；`≥ 0.24.2` 上真正变的是地板，而它从 2,163 涨到了 2,230（见 §2.1 的更正），**读数会比上表高约 67，这是正常的，不是配置没生效**。完整推导见 §8 第 1 条。
...
1. §2.5 的 `builtinTools` 实测值（预期 9,481 或 5,868）。**这两个预期值是 `v0.24.1` 口径的，但方向不是"≥ 0.24.2 上更低"——恰恰相反，会略高一点。** 理由：两个预期值都是 `地板 + 白名单内 schema`，**不含按需池那一块**（激进版 5,868 = 2,163 + (4,080 − 375)；保守版 9,481 = 5,868 + 3,613 的 `agent`），所以 #10410 省下的那约 4.2k 预加载量根本到不了这两个数里——它在 §2.5 的两种白名单下本来就已经被排除了。真正变的是**地板，而它涨了**：§2.1 的更正是 2,163 → 2,230（`tool_call` +148、`tool_search` 375→394、`task_stop` −102），于是激进版 ≈ `2,230 + 3,705 = 5,935`（**+67**）、保守版 ≈ `9,548`（**+67**）。唯一的减项是 #12142 把 Agent 工具 `run_in_background` 里 341 字符（≈85 token）的 teammate 说明改成按开关门控，但它 ① 只落在**保守版**（激进版白名单里没有 `agent`），② 还要求该部署本来就开着 `isAgentTeamEnabled()`（默认关；没开的话这段以前也没发，减不到）。所以**别把"读数比 9,481/5,868 高几十"当成配置没生效**，按 §2.3 排查前先对一下地板。若无变化，附上 debug 日志里的 `Unknown setting` 行与它报的 settings 路径。
