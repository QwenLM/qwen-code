...
### `v0.24.2` 对既有行为的风险（这是唯一需要担心的一条）

`v0.24.1...v0.24.2` 共 53 个提交。按"改动 `packages/core/src/{tools,permissions}/` 下的非测试文件"过滤，只有三个：**#10410**（15 个文件）、**#12096**（1 个）、**#12142**（1 个）。但"改了默认行为"是另一个更宽的集合，**不止 #10410**——这一点我第一版写错了，下面是重新推导的结果：

| 提交                  | 改了什么默认行为                                                                                                                                                                                                              | 与 token 有关吗                      |
| --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| **#10410** `70edbf47` | 延迟工具改走桥接；`tools.toolSearch.threshold` 默认 10→0                                                                                                                                                                      | 是，本节主题                         |
| **#12096** `9036855d` | **Bash 注释命令的权限判定**：`permission-manager.ts` 新增 `splitCommandForRules`（+103/−5），只按工具名与是否含换行门控，**没有任何设置开关**。同一条带注释的命令可能从 deny 翻成 ask，或在宽 `Bash(echo *)` 规则下翻成 allow | **否**，但这是本窗口里风险最高的一条 |
| **#12198** `c9839a94` | 未决工作区的信任默认：`isTrustedFolder()` 从 `?? true` 翻成 `?? !this.folderTrust`                                                                                                                                            | 否                                   |
| **#12034** `4f027777` | 新增常驻上下文告警的绝对上限 `MEMORY_CONTEXT_WARNING_MAX_TOKENS = 10_000`（此前只有 15% 比例）                                                                                                                                | 是，但只影响告警是否出现             |
| #12142 / #12271       | 门控与提示文案（teammate 指引按 team 开关、`zoom_image` 提示按可达性）                                                                                                                                                        | 是，量很小                           |
| #12346                | 纯测试                                                                                                                                                                                                                        | 否                                   |

**所以本节的结论要收窄**：与"一上来几万 token"有关的默认变更确实集中在 #10410；但**升级后观察到的异常不一定都归它**，尤其是权限类。

| 风险                                       | 代价                                                                            | 怎么尽快发现                                                                                                                |
| ------------------------------------------ | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| 首次使用某个延迟工具前多一次往返           | 延迟 + 一轮                                                                     | 会话里是否出现 `tool_search` → `tool_call` 两步                                                                             |
...
| 桥接两半缺一                               | 普通延迟工具回退为急加载（安全）；被 `tools.eager` 降级的工具**不可达**并记警告 | 检查日志里的相关 warning                                                                                                    |
| **#12096：带注释的 Bash 命令权限判定变了** | **可能放宽**：deny→ask，或在宽 allow 规则下 deny→allow                          | **拿你现有的 `Bash(...)` deny 规则，对"命令里带 `#` 注释"的形态逐条重测**。这条与延迟加载无关，下面那个一行回滚**治不了它** |
...
0. **（新，最先做）§0.1 的重新取基线读数**，填这张表——只有第一行是必须的，后面几行做到哪填到哪：
   ...
   第二行减第一行 = #10410 白送的量（我推算约 4.2k）；第三行减第二行 = 白名单真正的贡献（我推算约 11.4k）。**这两个归因分开，是这次重测唯一的目的**——合在一起算会把上游的收益记到你的配置头上。

   ⚠️ **但第一行与第二行不是同一把尺子**：同一个 `v0.24.2` 里还含 #12119（`ef16196c`），它重写了 `/context detail` 里内置工具那一行的推导方式。所以升级前后的两个读数**是两台仪器量出来的**，直接相减会把口径差混进去。**两次都请把 `/context detail` 的完整输出留下**（不只记内置工具那一个数），这样口径差可以在事后被识别出来；如果只能留一个数，那第一行就只能当参考、不能当基线。
