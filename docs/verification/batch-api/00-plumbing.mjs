...
import { client, line, writeJsonl, submit, waitFor, collect, save, cleanup, bodyOf } from './lib.mjs';
...
const path = writeJsonl(
  '00-plumbing.jsonl',
  [1, 2, 3].map((i) =>
    line(`p${i}`, { model, messages: [{ role: 'user', content: `Reply with the number ${i}.` }] }),
  ),
);
...
const verdict = {
  ...
  timestamps: { created_at: batch.created_at, in_progress_at: batch.in_progress_at, finalizing_at: batch.finalizing_at, completed_at: batch.completed_at, expires_at: batch.expires_at },
  ok_lines: ok.length,
