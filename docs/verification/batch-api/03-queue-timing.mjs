...
import { client, line, writeJsonl, submit, sleep, ts, OUT, MODEL } from './lib.mjs';
...
const readPending = () => (fs.existsSync(PENDING) ? JSON.parse(fs.readFileSync(PENDING, 'utf8')) : []);
const writePending = (p) => fs.writeFileSync(PENDING, JSON.stringify(p, null, 2));
...
async function submitOne() {
  const lines = Array.from({ length: LINES }, (_, i) =>
    line(`q${i}`, { model: MODEL, enable_thinking: false, max_tokens: 4, messages: [{ role: 'user', content: 'Reply with OK.' }] }),
  );
...
async function drain() {
  ...
  for (const p of pending) {
    ...
    const rec = {
      ...
      completed_at: b.completed_at ?? b.failed_at ?? b.expired_at ?? b.cancelled_at ?? null,
    };
    rec.queue_s = rec.in_progress_at ? rec.in_progress_at - rec.created_at : null;
    rec.run_s = rec.in_progress_at && rec.completed_at ? rec.completed_at - rec.in_progress_at : null;
    rec.total_s = rec.completed_at ? rec.completed_at - rec.created_at : null;
    fs.appendFileSync(LOG, JSON.stringify(rec) + '\n');
    console.log(ts(), 'settled', p.id, b.status, `queue=${rec.queue_s}s run=${rec.run_s}s total=${rec.total_s}s`);
    // Result content is irrelevant here; free the files.
    for (const id of [b.input_file_id, b.output_file_id, b.error_file_id]) if (id) oa.files.delete(id).catch(() => {});
  }
  writePending(still);
  return still.length;
}
...
function summarize() {
  ...
  for (const [k, rs] of Object.entries(groups)) {
    ...
    out[k] = {
      ...
      queue_s: { p50: q(pick('queue_s'), 0.5), p90: q(pick('queue_s'), 0.9), max: q(pick('queue_s'), 1) },
      run_s: { p50: q(pick('run_s'), 0.5), p90: q(pick('run_s'), 0.9), max: q(pick('run_s'), 1) },
      total_s: { p50: q(pick('total_s'), 0.5), p90: q(pick('total_s'), 0.9), max: q(pick('total_s'), 1) },
    };
  }
  console.log(JSON.stringify(out, null, 2));
  fs.writeFileSync(path.join(OUT, '03-queue-timing.summary.json'), JSON.stringify({ generated: ts(), ...out }, null, 2));
}
