...
import { client, line, writeJsonl, submit, waitFor, collect, save, cleanup, usageOf, cachedOf, MODEL } from './lib.mjs';
...
const para = 'This system prompt exists only to be long and identical across every request so that the prefix cache has something to hit. It describes a fictional internal style guide: prefer short sentences, cite file paths, never speculate, and answer in the language of the question. ';
const longSystem = Array.from({ length: 60 }, (_, i) => `[${i}] ${para}`).join('\n');

const questions = Array.from({ length: N }, (_, i) => `Question ${i + 1}: what is ${i + 1} squared? Answer with the number only.`);
const common = { model: MODEL, enable_thinking: false, temperature: 0, max_tokens: 16 };

const implicitLines = questions.map((q, i) =>
  line(`a${i}`, { ...common, messages: [{ role: 'system', content: longSystem }, { role: 'user', content: q }] }),
);
const explicitLines = questions.map((q, i) =>
  line(`b${i}`, {
    ...common,
    messages: [
      { role: 'system', content: [{ type: 'text', text: longSystem, cache_control: { type: 'ephemeral' } }] },
      { role: 'user', content: q },
    ],
  }),
);
...
for (const [i, q] of questions.entries()) {
  const r = await oa.chat.completions.create({ ...common, messages: [{ role: 'system', content: longSystem }, { role: 'user', content: q }] });
  realtime.push({ custom_id: `c${i}`, response: { body: r } });
}

const [batchA, batchB] = await Promise.all([waitFor(oa, subA.id), waitFor(oa, subB.id)]);
...
function summarize(rows, multiplier) {
  const per = rows.map((r) => {
    const u = usageOf(r) ?? {};
    return { id: r.custom_id, prompt: u.prompt_tokens ?? 0, cached: cachedOf(r), completion: u.completion_tokens ?? 0 };
  });
  const sum = (k) => per.reduce((s, x) => s + x[k], 0);
  const prompt = sum('prompt'), cached = sum('cached'), completion = sum('completion');
  // cost in realtime-input-price units
  const cost = ((prompt - cached) * 1 + cached * CACHE + completion * P_OUT) * multiplier;
  return {
    lines: per.length,
...
const verdict = {
  ...
  cost_vs_realtime: { batch_implicit: +(a.relative_cost / c.relative_cost).toFixed(2), batch_explicit: +(b.relative_cost / c.relative_cost).toFixed(2) },
  errors: { implicit: A.err.length, explicit: B.err.length },
  arms: { batch_implicit: { ...a, per: undefined }, batch_explicit: { ...b, per: undefined }, realtime: { ...c, per: undefined } },
};
save('02-cache.result.json', { verdict, arms: { a, b, c }, batches: { A: batchA, B: batchB }, errors: { A: A.err, B: B.err } });
