...
import { client, line, writeJsonl, submit, waitFor, collect, save, cleanup, bodyOf, MODEL } from './lib.mjs';

const oa = client();
const system = { role: 'system', content: 'You are a terse assistant. Use tools when they apply.' };
const tools = [
  {
    type: 'function',
    function: {
...
const lines = [
  ...
  line('L1-call', { ...common, tools, tool_choice: 'auto', messages: [system, { role: 'user', content: '北京现在几点？' }] }),
  ...
  line('L2-history', {
    ...
    messages: [
      ...
      { role: 'assistant', content: '', tool_calls: [{ id: 'call_1', type: 'function', function: { name: 'get_time', arguments: '{"city":"北京"}' } }] },
      { role: 'tool', tool_call_id: 'call_1', content: '{"time":"10:30"}' },
    ],
  }),
  // L3: control, no tools
  line('L3-plain', { ...common, messages: [system, { role: 'user', content: 'Say hello in one word.' }] }),
];
...
const l1 = m('L1-call'), l2 = m('L2-history'), l3 = m('L3-plain');
const verdict = {
  status: batch.status,
  request_counts: batch.request_counts,
  L1_finish_reason: l1?.finish_reason ?? null,
  L1_tool_name: l1?.message?.tool_calls?.[0]?.function?.name ?? null,
  L1_pass: l1?.finish_reason === 'tool_calls' && l1?.message?.tool_calls?.[0]?.function?.name === 'get_time',
  L2_content: l2?.message?.content ?? null,
  L2_pass: typeof l2?.message?.content === 'string' && l2.message.content.includes('10:30'),
  L3_content: l3?.message?.content ?? null,
  L3_pass: Boolean(l3?.message?.content),
  errors: err.map((e) => ({ custom_id: e.custom_id, error: e.error ?? e.response?.body?.error ?? null })),
};
verdict.pass = verdict.L1_pass && verdict.L2_pass && verdict.L3_pass && err.length === 0;
