...
| POST `/internal/managed-runtime/v3/context` | 选定 Runtime 与具名 Session：客户端接收 READY 的 binding 记录和该 Session，要求记录的 scope，以及 session 隔离下的隔离键，都属于该 Session。发送不可变 W0a binding、计算后的 digest 和 operation ID；逐字段对照原请求与接收方 incarnation 校验 receipt。 |
...
- 在发送前拒绝安装到另一个 placement 的 Runtime 或非 READY 的 binding，拒绝格式不良的 Session 或引用 ID；容忍并发的 schema 升级。
