...
选择器正向路径：普通 `default`/缺省/`api`、trusted、无 MCP/Hooks、已证明空扩展 → `managed`。Channel / cron / `managed-gateway` / standalone / 子会话 / worktree / 未知来源 / 未信任工作区 / 未证明空扩展（含扩展目录或 store 中的额外文件） → `legacy`。冷恢复读严格 owner；Managed 不兼容（含工作区已不再信任）则失败，不改选 legacy。已证明空扩展包含 daemon 自己初始化留下的空 `state.json`、普通基础 `lock` 文件和空 `staging`/`rollback`/`transactions`；真正持锁的 `lock.lock` 仍是 unknown。R3.1 已用普通 REST `POST /session` 证明该正向路径能落盘 managed owner 并完成 prompt/events/cancel；R3.2 已让活会话 HTTP transcript/turn-index 打 owner channel，未知会话分页仍走 legacy 控制通道。R3.3 已证明同一活会话 FIFO 队列。R3.4 已证明关掉 daemon 后再 `POST /session/:id/load` 同一普通会话仍走 verified managed，拦住 legacy spawn 后仍能 prompt。R3.5 已让冷 Managed 的普通 `GET /session/:id/transcript` 与 `GET /session/:id/turn-index` 走磁盘分页，不再 spawn legacy 控制子进程；未知/冷 legacy 仍走控制通道。R3.6 已让普通 REST 活会话按 body `promptId` 查原 turn：相同内容重试仍是一次 running、owner 仍 managed，不同内容 409，不改选 legacy。R3.7 已证明普通 REST SSE 断开不取消已接受 turn，用原 cursor 重连后仍能收到助手文本。本记录仍不是 R3 全部或 R4 完成证据。
