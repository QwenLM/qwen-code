...
0. 先执行 `qwen batch check`：验证凭证、端点与 Batch 路由（不产生计费
   请求），并展示 run 将冻结的设置。失败（例如 Qwen OAuth 没有 Batch
   路由）就在花任何准备费用之前停止。
...
| `retry <task-id>`         | 仅以新一次尝试重新提交 `failed` 条目（截断项需 `--max-output-tokens`） |
| `check`                   | 验证凭证/端点/Batch 路由并展示 run 将冻结的设置；不计费                |
| `clean <task-id>`         | 删除本地记录；不取消任何东西，批次可能未结束时拒绝（`--force` 除外）   |
| `list`                    | 列出已记录任务及进度                                                   |
...
收取是机械的事，用户不需要手动执行。交互会话首次渲染后启动一个收取器
（`batch-auto-collect.ts`，挂在 `startPostRenderPrefetches`）：扫描本地任务
记录中本项目未完成的任务，按任务独立退避（1 → 5 分钟）用 HTTP 查询批次状态，
批次结束时调用同一个 `collectTask`。启动时先跑一遍，因此会话关闭期间完成的
任务会在下次打开时收取。变化通过现有的更新通知通道以一条信息提示显示，模型
正在输出时会先排队。保证：

- **不调用模型、不自动重试。** 等待和收取只花 HTTP；失败项只列出重试命令，
  绝不自动重发。
- **与手动收取同样安全**——任务锁、幂等、不覆盖交付、held——因为是同一个函数。
- **收不了的就不打扰**：会话里没有 Batch 凭证时整体停用；绑定到其他端点或
  key 的任务留给用户处理。
- `general.batchAutoCollect`：`deliver`（默认）、`notify`（只提示可收取，
  不写文件）、`off`。非交互模式、`serve`、ACP 与 web-shell 没有不经模型的通知
  通道，不自动收取。

`run`（和 `retry`）还会等过服务端的校验窗口（仅在创建返回 `validating` 时，
最多约 30 秒）：整批被拒会当场带着作业级错误报出，`collect` 也会把这些错误
记为每个条目的失败原因，而不是笼统的"no result line"。
...
用户：/batch-api “把 docs/zh 翻译到 docs/en”
        │
        ▼
准备 skill（batch-api/SKILL.md）——只做语义工作：适用性、抽样、共享规则、计划文件。
实时运行，保留实时侧自身的缓存收益。
        │
        ▼ 计划 JSON（§4 的 schema）
...
- **账本必须能不靠猜测回答“远端可能存在哪些对象”。** 提交意图先于上传
  落账，输入文件 ID 先于创建落账。创建应答丢失——或进程在创建途中退出、
...
- **custom_id = `<itemId>#<attempt>`**，跨重试也能唯一映射回条目；未知或
  重复的结果行告警后忽略。每个条目记录拥有它的尝试，只有该尝试的结果行
  能改变它：重试进行中重放旧尝试的失败，不能把条目重新打开、再付一次钱。
...
- **没有价格就不做货币估算。** token 估算始终展示（粗略，按字符数/3）。
  ...
  对着服务商价格页过期。计划可以设 `maxCostUsd`，它是**估算门禁，不是
  账单上限**：估算超出时 `run` 和每次 `retry` 都拒绝提交，没有配置价格时
  直接拒绝。估算是粗略值，以服务商账单为准。
- **Batch 沿用用户的实时设置。** `run` 时执行器把已配置的采样参数、输出
  上限和思考模式冻结进任务（只取已核对为 chat-completions 请求体的字段），
  每次重试复用。Batch 专属的默认值会让与实时的成本/质量对比失去意义。
  没有已核对 Batch 对应字段的推理强度配置以提示说明，不做猜测。

- **记账写明缺口。** 用量从结果文件跨所有尝试汇总；任一行缺 usage 时总数
  明确标为*不完整*，绝不当作 0。设置价格后，`run` 记录价格来源
  （`QWEN_BATCH_PRICE_SOURCE`），并给出盈亏平衡的实时缓存命中率——即
  `0.5·(I+O) − 0.8·h·I` 为零处（§6 的模型，隐式缓存价按原价 20% 计）——
  而不是宣称省了多少。执行器测不到交互会话里的
  准备开销，所有估算和报告都如实注明，不暗示这是完整工作流的数字。
- **只在确定释放时接管锁。** 锁记录 pid 和主机名；只有同一主机上、pid 已不
  存在的锁才会被接管。pid 复用最多导致（安全的）拒绝，其他主机的锁从不接管；
  提示信息会给出需要手动删除的文件。
...
`kind` 是字面量——新种类使用新的 schema 版本。`enableThinking` 仅供用户
明确要求不同思考模式时使用；不设置时沿用冻结的设置，对强制思考的模型设
`false` 会被拒绝。
...
| 源路径逃逸项目根目录（含软链） | 组装阶段拒绝，任何内容都不会上传                                                                                 |
...
| 结果截断/含工具调用/为空       | 条目标 `failed` 并记录原因；失败请求的计费保持可见。截断项被 `retry` 跳过，除非用 `--max-output-tokens` 提高上限 |
...
重试语义：仅 `failed` 条目（以及唯一一次提交已确认从未成为批次的条目）、
作为新一次尝试、使用新的 `#<attempt>` custom_id，与 `run` 一样受计划
`maxCostUsd` 约束；存在未对账提交、前一批次仍在运行或已结束但未收取时
拒绝。held 条目永不重试
——它们需要用户决策，不是新请求。

## 6. 成本模型

执行器不做自动路由——用户用 `/batch-api` 显式选择了 Batch——但它不能在钱上
撒谎：
