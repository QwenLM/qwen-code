...
有测试固定这一点：设置临时 `QWEN_HOME`，在其中写入包含普通键和 `NODE_OPTIONS` 的 `.env`，断言 `getHomeEnvFallbackVars()` 确实暴露两者——使测试不可能空洞通过——然后断言两者都未被替换进 MCP 配置。启动后新加入 `.env` 文件的变量，两条重载路径都能看到，因为下文的快照在每次加载时都会重新读取文件。

下文快照包含哪些 `.env` 文件：`findEnvFiles` 从项目目录向上查找，取找到的第一个 `<dir>/.qwen/.env` 或 `<dir>/.env`——工作区文件仅在该工作区受信任时采用——再加上 home 候选：`<QWEN_HOME>/.env`、`QWEN_HOME` 重定向时的旧版 `~/.qwen/.env`，以及 `~/.env`。因此检出的仓库可以自带一个 `.env` 来提供其 `.mcp.json` 占位符解析出的值；这一步的门槛是工作区信任，而审批门控仍然作用于服务器本身。

**工作区快照。** 展开读取的是工作区自己的环境，从不直接读取 `process.env`。`loadEnvironment` 把工作区的 `.env` 写入 `process.env` 且从不覆盖，因此承载多个工作区的进程——一个子进程中的多个 ACP 会话、守护进程——会保留第一个工作区的值：受信任工作区 A 的会话留下 `TOKEN=A`，B 的 `.mcp.json` 就展开成 A（B 的 settings 命中缓存时甚至完全不会运行 `loadEnvironment`）。`buildWorkspaceEnvSnapshot(settings, cwd)`（`environment.ts`）在每次加载时构建：启动环境——`process.env` 减去 `isFileSourcedEnvKey` 标记为由 `.env` 文件或 `settings.env` 写入的全部键——加上本工作区的 `.env` 文件和 `settings.env`，经由 `buildRuntimeEnvironment`，也就是守护进程用冻结的启动环境喂给的同一个构建器。在 ACP 重载循环中，每个活动 `Config` 的快照由该 `Config` 自己的工作区 settings（`loadSettingsCached(cwd)`）构建，因为 `settings.env` 和 `excludedEnvVars` 是按工作区的；服务器映射和准入列表仍沿用引导 settings，与此前一致。加载器把快照作为 `{ expandEnv: true, env }` 的必需部分——没有它 `loadProjectMcpServers` 和 `assembleMcpServers` 无法通过编译——并以 `processEnvFallback: false` 解析，这是共用解析器上新增的选项。仅靠 `customEnv` 不够：叠加层中缺失的名字会回落到 `process.env`，而这些缺失恰恰是另一个工作区的文件键。该选项是增量式的（默认 `true`；八个既有调用方不变），选择它而非在加载器内复制一份解析器，是因为复制会重复 SESSION_ID 与内部密钥的守卫并随之漂移。shell 中导出的令牌位于启动环境中，照常解析；被扣下的只有另一个工作区的文件写入进程的内容。适用范围：这一结论对自己运行过 `loadEnvironment` 的进程成立——CLI、ACP `newSession`，以及重载循环（重载排除键除外，见下文）——因为只有那时文件写入的键才被跟踪。守护进程的 ACP 子进程则不同：守护进程构建工作区的 `effectiveEnv`（`run-qwen-serve.ts:4892`，在冻结的守护进程基础环境上运行 `buildRuntimeEnvironment`），并作为 `sourceEnv` 交给子进程的 spawn（`run-qwen-serve.ts:5252`；`acp-bridge/src/spawnChannel.ts`），因此在子进程中这些键是普通的 `process.env`，没有文件跟踪（`environment.ts:786`）；`launchBaseEnv` 会保留它们，第二个工作区的 `.env` 在那里无法覆盖。该路径不在本改动覆盖范围内；2026-09-11 的维护者评审也指出其自身未完成守护进程包装路由的检查。重载循环自身也有一处缺口：`reloadEnvironment` 让重载排除键留在 `process.env` 中（其删除逻辑中对 `isReloadExcludedKey` 的跳过），却只从本次新加载重建 `dotEnvSourcedKeys`，于是某工作区的重载排除 `.env` 键会丢失文件来源标记，被其他工作区的后续快照继承。本改动不覆盖此项。与初始 `loadEnvironment` 相比有两处不对称，在所有站点相同：`buildRuntimeEnvironment` 应用重载规则（`{ reload: true }`——`PATH`、`HOME`、`TMPDIR`、守护进程令牌及其他重载排除键不从文件读取），并按 `advanced.excludedEnvVars` 过滤 `settings.env`，而初始加载不这样做；这类键在 `.mcp.json` 中保持字面量——直到一次重载，见上文。工作区作用域的 `.qwen/settings.json` 由 `loadSettings` 针对进程范围的环境加 home `.env` 回退解析（`settings.ts`），因此仍会读到另一个工作区的值；这里不作改动。
