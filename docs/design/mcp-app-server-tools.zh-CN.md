...
Daemon 的 `/mcp-app-sandbox` 路由默认返回不缓存的重定向；不可达时的受限回退见下文。按需启动的独立静态 HTTP listener 绑定 `127.0.0.1` 并使用随机端口。每次渲染在该 listener 上获得新的 `UUID.localhost` 主机名，使 App 与 daemon 以及其他 App 分别隔离。它不提供 daemon API、代理或 WebSocket 端点。只有精确匹配注册 Host、GET 方法及资源路径的请求能取得文档，其余请求返回 404。
...
当前验证结果：独立沙箱 HTTP 探针 12/12 通过，完整 daemon 来源防护探针 14/14 通过，CLI 沙箱测试 14 项通过，Web App 测试 24 项通过。全量 build、typecheck 和 bundle 均通过。这些检查覆盖一次性来源、拒绝策略覆盖和 API/WS 路径、关闭竞态，以及真实 daemon 拒绝沙箱来源请求；它们不能证明 Tableau 图表已成功显示。还需验证 attachment 替换、模型完成后的 App 成功/失败，同时保留普通模型工具的活动状态。
...
回环 daemon URL 不代表浏览器与 daemon 位于同一主机。WebShell 先尝试独立来源；若 10 秒内未收到代理握手，则经现有 daemon 连接重试 `/mcp-app-sandbox?mode=opaque`，使单端口 SSH 隧道或容器端口映射仍可显示普通 App。此内联响应强制执行不含 `allow-same-origin` 的 HTTP CSP `sandbox allow-scripts allow-forms`，外层 iframe 也移除同源许可。App 不会获得 daemon 来源。界面说明受限模式；依赖非不透明第三方来源的 App（如 Tableau）在该模式下仍可能不兼容。本轮不新增公共沙箱域名或转发端口范围配置。
