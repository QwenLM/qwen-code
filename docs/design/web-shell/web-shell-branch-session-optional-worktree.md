...
Sub Agent 审计完成后更新本节。最多执行 9 轮；当一整轮无新增实质问题时停止。

| 轮次 | 无方向审计                                                                                                                                            | 方向审计                                                                                                                               | 方案调整                                                                                                                                                                             |
| ---- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | 发现 marker 写入过晚、clean 忽略 untracked、startup 调用点缺失、Git 可用性 gate 不真实、request type 漏字段                                           | 发现 target reservation/client ledger 未定义、historical+worktree overload 判别错误、marker symlink/覆盖风险，并确认 marker 时序问题   | journal 提前到首个副作用前；增加 target reservation、phase recovery、strict clean、hardened marker、server preflight、显式 SDK overload 和 attach/kill 状态机                        |
| 2    | 发现 dispatch 后 outcome unknown 存在晚到 commit 竞态、reservation 与外部抢先 load 矛盾、ahead branch 回滚需受保护的 force delete、sidecar 字段不完整 | 同样确认晚到 commit 和 reservation 矛盾；发现全局 exclusive marker 会破坏 startup adoption                                             | dispatch 前持久化 phase；dispatch 后 unknown 永不自动回滚；reservation 作为 load fence；补全 sidecar；身份匹配后才 force delete；marker 拆成 strict create 与 expected-owner replace |
| 3    | 发现 monorepo 子目录 workspace 的 Git cwd 仍被 containment 拒绝，以及 `createUserWorktree()` 允许的 daemon 管理元数据残留与“checkout 不变”表述冲突    | 发现复用 forceful `removeUserWorktree()` 有 clean/tip 检查后的 TOCTOU 数据丢失风险，并发现 startup adoption CAS 失败后仍可能写 sidecar | 增加 session-bound managed cwd 授权并覆盖全部 Git/PR cwd 消费者；收窄不变量；改用 non-force worktree remove + update-ref CAS；CAS 失败中止 adoption；补充竞态和 residue 测试         |
| 4    | 发现 `update-ref` CAS 会删除检查后被另一 worktree checkout 的 branch；发现普通 `/cd` 后 preflight 与取 HEAD 的 checkout 不一致                        | 发现 cleanup 缺少 worktree remove 与 branch 终态之间可跨 crash 重入的 phase                                                            | 分支清理改用 `git branch -d` 并允许安全残留；增加统一 base-checkout resolver；加入 cleanup-intent/worktree-removed/branch terminal phases 和 crash tests                             |
| 5    | clean                                                                                                                                                 | 发现 daemon 无法从现有 public summary 可信读取 live `effectiveCwd`，并发现 terminal sidecar/journal unlink 缺少目录 fsync 顺序         | 增加 daemon-internal Bridge execution snapshot；定义与 `/cd` 的线性化语义；terminal → sidecar unlink/fsync → journal unlink/fsync，并补 crash tests                                  |
| 6    | clean                                                                                                                                                 | clean                                                                                                                                  | 无新增实质问题，停止审计                                                                                                                                                             |

## 15. 实施与验证结果

实现已覆盖 capability、SDK/WebUI request contract、Web Shell dialog、Daemon
prepare/publish/activate transaction、ACP target-id plumbing、strict marker 与 durable
sidecar/journal、initial/dynamic primary recovery，以及全部 workspace-qualified Git/PR
cwd 消费者的 session-bound 授权。实现阶段的安全自审计额外修正了：

- `worktree: undefined` 被结构判别误认成 worktree request；
- 只在 dynamic primary 执行 recovery，遗漏首次启动的 primary；
- macOS `/var` 与 `/private/var` canonical identity；
- linked worktree marker 应写入 Git common-dir 的 `info/exclude`；
- marker 未创建阶段必须证明 marker 缺失，不能用 `undefined` 跳过 owner 校验；
- ACP 返回的 target id 必须等于 daemon reservation；
- monorepo legacy sidecar 的 `originalCwd` 不应被误当作 repo-top identity；
- managed worktree 必须与 workspace checkout 共享 Git common dir，同时允许 detached
  HEAD；
- target transcript 在 publication/activation 窗口需要 target archive shared lock；
- cold load 恢复 worktree 后必须同步返回 canonical `currentCwd`；
- stale session UI capability、符号链接 metadata 目录、marker symlink/hardlink、marker
  写后路径替换与 Windows directory fsync 必须 fail-closed 或采用平台安全语义。
- cleanup 必须用 root-anchored `/.qwen-session` 排除规则并执行最终
  path/marker/clean/ref 复验；否则 nested 同名 ignored 文件或检查后的 owner 变化会被
  `git worktree remove` 一并删除；
- sidecar exclusive create 失败时不能删除导致冲突的既有文件；journal 未持久确认
  `sidecarCreated` 而磁盘上已存在 sidecar 时，route/recovery 都保留全部资源并告警；
- source 在无副作用 preflight 后、真正 dispatch 前变为 busy、关闭、client identity
  失效或 Bridge channel 被隔离，都属于可类型化的明确 commit 前失败；必须清理准备
  资源，不能误报 outcome unknown 并永久保留 residue；其中 Agent 的
  `session_busy` 会先被 Bridge 映射为 `SessionBusyError`，route 必须识别映射后的
  类型；Agent 明确返回的 `session_not_found` 也属于未发布 target 的结束态。
- recovery/phase update 读取 journal 前必须验证其为路径身份未变化的单链接普通文件，
  不能跟随被替换的 symlink 或信任 hardlink 内容进入破坏性清理；exclusive create
  写后也要复验 inode identity。
- startup reattach 遇到同一 owner 时只补齐 Git common-dir `info/exclude` 中的 root
  marker 规则，不重写已经有效或只读的 marker；owner 变化时才执行安全
  compare-and-replace。
- recovery cleanup 除 clean/ignored 检查外还必须直接查询 Git index，拒绝已跟踪的
  `.qwen-session`，即使其内容恰好等于 journal owner。
- cleanup 还要重复校验 worktree 实际检出的 HEAD 和 branch，而不只检查预期 branch
  ref 的 tip；否则干净的 detached unique commit 可能随 worktree remove 丢失。
- session-bound Git/PR cwd 授权必须允许 owner worktree 内的真实子目录，同时把
  sidecar、snapshot 和 marker 身份固定在该 worktree 根目录；否则 `/cd` 到子目录后
  会误拒绝合法 Git 操作。
- terminal cleanup 在 `sidecarCreated=false` 时必须末端复验同名 sidecar 仍不存在；
  若它在初次检查后出现，就保留 sidecar 和 terminal journal，而不是删除冲突文件或
  清 journal 留下 orphan sidecar。

验证结果：

- `npm run build`、`npm run typecheck`、`npm run lint`、`npm run bundle` 通过；
- Core marker/sidecar、CLI recovery/route/startup/ACP、Bridge、SDK、WebUI、Web Shell
  受影响测试通过；Web Shell 六个相关文件共 498 个测试通过；
- bundled daemon/API E2E 创建了真实 worktree branch，验证原 workspace ownership、
  target marker/sidecar、session-bound Git status、错 owner 400、outcome-unknown 保留、
  首次 primary restart recovery warning，以及 restart load 的 `currentCwd` 恢复；
- 手工浏览器视觉检查未执行；dialog 行为由 jsdom Web Shell 测试覆盖，真实跨进程
  路径由 bundled daemon/API E2E 覆盖。详细记录位于 git-ignored 的
  `.qwen/e2e-tests/2026-08-16-branch-session-optional-worktree.md`。
