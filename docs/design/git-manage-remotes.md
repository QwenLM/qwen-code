...
refspec` at any line start vetoes the completion outright — that refusal has its
  ...
  behind for a CONFIGURED remote). Accepted residuals: a FORMERLY
  ...
  and a flat layout's SLASHED branch refs (`refs/remotes/release/1.0`)
  remain namespace refs of the never-configured prefix, sweepable as
