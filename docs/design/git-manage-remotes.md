...
- `fetchGitRemotes` reads the repository's **own editable scope** with a
  ...
  so the UI can name what removal destroys. Records are NUL-framed: under
...
- `gitRemoteAdd` / `gitRemoteRemove` run `git remote add -- <name> <url>` /
  ...
  (a deliberate shadow belongs to the terminal). Removal verifies the
...
refspec` at any line start vetoes the completion outright — that refusal has its
  ...
  can fix the refspec), which a completed "removal" would bury. When the
  section lives ONLY in an included file, `git remote remove` fails with
  "Could
  ...
  `remote_still_configured`. Removal also cleans up the upstream keys
  ...
  can write, so keys held in `config.worktree` would survive into a
  ...
  worktree-scope keys are unset (`--fixed-value --unset-all` for
  remote/pushRemote entries, `--unset-all` for merge) and re-verified: a
  ...
  FETCH upstream. A worktree-scope `remote.pushDefault` resolving to the
  removed remote is swept the same way, because `git push` would
  otherwise resolve to the gone remote. The same upstream keys held in
