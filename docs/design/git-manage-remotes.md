...
- Remotes view replaces the scrollable list area (search box and footer
  ...
  - Remote rows: name (a name whose sanitized form is empty renders a
    visible `(invisible name)` label; a name whose sanitized form merely
    DIFFERS from the raw one — including one that collides with a sibling
    row's text — appends a `(hidden characters)` marker, and its tooltip
    and aria-labels carry the raw name with the stripped characters as
    visible codepoint escapes, so two lookalike rows never present one
    identity), a badge naming what removal destroys beyond the URL
  ...
  - Inline add form at the bottom of the list, sticky and wrapping so the
    ...
    input + Add button (Enter submits from either input; inputs are
    disabled while a mutation is in flight; the client guard rejects only
  ...
  - Remote names and URLs are rendered through a display sanitizer derived
    ...
    another); the search filter matches sanitized values on both needle and
    haystack; mutation requests still use the raw name.
