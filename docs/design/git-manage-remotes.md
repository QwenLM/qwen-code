...
- Remotes view replaces the scrollable list area (search box and footer
  ...
  - Remote rows: name (a name whose DISPLAYED form — sanitized,
    ...
    out of the inked text — plus the two structural arms the class list
    cannot enumerate: a name that is not NFC (a canonical twin inks
    identically), and a Latin name mixing in another script's letters
    (the Cyrillic-`о` homoglyph shape) — appends a `(hidden characters)`
    marker, and its tooltip and aria-labels carry the raw name with the
    distinguishing characters as visible codepoint escapes (every
    non-ASCII character spelled out for the canonical/script arms), so a
