...
Deleting an ordinary conversation reaches beyond this client: the daemon closes the session and runs the same ownership-verified worktree cleanup it runs for any delete. A checkout with uncommitted work is preserved, and the worktree branch is deleted only when a non-force `git branch -d` accepts it, so commits that are not yet merged survive. A clean checkout whose branch is already merged is removed.
