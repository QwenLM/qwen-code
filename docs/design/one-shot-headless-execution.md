...
When that relaunch would add no Node or script arguments (the default, since `advanced.autoConfigureMemory` is off) and no Node boot variable such as `NODE_EXTRA_CA_CERTS` changed after startup, the replacement image would be the same process booted a second time. The CLI then sets `QWEN_CODE_NO_RELAUNCH` and continues in place instead of calling `execve`, which removes a full second module load and settings pass from every one-shot run.
