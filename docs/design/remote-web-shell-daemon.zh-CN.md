...
- 验证更换计算机会重新打开位置选择，取消会回到准确的来源页面且 URL 不含 token 或续接标记，普通 daemon 切换不会打开目录浏览器。
...
- 取消和更换计算机会按预期回到来源 shell；添加成功后留在所选 daemon；持久化目录只含连接 origin，不包含远程项目数据或 bearer token。
