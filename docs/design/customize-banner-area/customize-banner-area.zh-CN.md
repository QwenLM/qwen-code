...
1. **像 `Custom Code Agent` 这种 17 字符品牌名，在大多数终端上无法用
   一行 figlet 单字渲染** —— ANSI Shadow 每字母约 7 列，整行就要约
   120 列。要么把单词换行堆叠，要么换更窄的字体，要么直接用紧凑的单
   行装饰，例如 `▶ Custom Code Agent ◀`。
2. **当单档既要"宽屏好看"又要"窄屏不死"时，用 `{ small, large }` 宽
   度自适应形态**。下面这个例子里 `large` 是 120 列终端用的堆叠多行
   art，`small` 是 22 列的单行装饰，窄到装不下两者就直接隐藏 logo
   列。

```jsonc
{
  "ui": {
    "customBannerTitle": "Custom Code Agent",
    "customAsciiArt": {
      "small": "▶ Custom Code Agent ◀",
...
`banner-large.txt` 里放堆叠后的 ANSI Shadow 输出（约 54 列 × 18 行），
可以用下面的命令生成：

```bash
( npx figlet -f "ANSI Shadow" CUSTOM
  npx figlet -f "ANSI Shadow" CODE
