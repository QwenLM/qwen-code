...
1. **A 17-character brand like `Custom Code Agent` cannot be rendered as a
   single inline figlet line on most terminals** — at ~7 cols per ANSI
   Shadow letter that is ~120 cols of art alone. Either stack the words
   on multiple lines, pick a denser font, or use a compact single-line
   text decoration like `▶ Custom Code Agent ◀`.
2. **Use the width-aware `{ small, large }` form** when a single tier
   ...
   stacks the words for a 120-col terminal in `large` and falls through
   to a 22-col single-line decoration in `small`.
...
(~54 cols × 18 lines), e.g., generated by:
