...
6. If final delivery fails with a definite no-write response, the adapter stores the final text in `~/.qwen/channels/<channel>-github-pending-deliveries.json` with private file permissions and retries it after restart without rerunning the agent.
