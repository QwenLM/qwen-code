...
启动后才注册进来的可信非主 workspace 走同一条路径恢复自己的名单，恢复发生在注册
响应之后而不是响应之内，且只在本 daemon 第一次见到它时做一次：之后的移除重注册、
或信任重新物化都不再重复，否则会撤销操作者在这期间停掉某个频道的决定。它的名字以
一次变更并入已提交选择。频道托管已经被停掉时（`DELETE /workspace/channel`，或一次
未能保持的启动）不恢复——注册一个 workspace 不等于要求把托管重新打开，要等一次
`PUT /workspace/channel` 或下次启动重新读取设置。显式 `--channel` 选择界定该 daemon
整个生命周期内托管什么，已提交的 `all` 选择保持原样。
