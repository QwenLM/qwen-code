...
会让该 workspace 整份名单都不恢复。`DELETE /workspace/channel` 停掉托管之后，注册
...
之后注册的 workspace 扩充，已提交的 `all` 选择保持原样。注册钩子仍会在频道控制队列
上等待自己的那部分账目处理，所以一个永远就绪不了的 worker 会让其它注册最多延迟一个
频道启动预算。没有显式选择、持久化选择，或上面这种注册时，daemon 直到首次运行时
