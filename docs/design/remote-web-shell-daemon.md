...
The existing sidebar remains the workspace and session management UI. A successful remote connection records only its validated origin in a browser-local connection catalog; bearer tokens remain tab-scoped. The normal **Add workspace** action then starts with a computer choice: this computer or one of those connected remote computers. Selecting a remote computer navigates to that daemon and resumes the operation in directory-browser mode. The browser uses daemon-provided directory suggestions, supports parent-directory navigation and manual absolute paths, and registers the selected directory through the existing workspace mutation. Native folder selection remains hidden for remote daemons. Workspace rows from a cross-origin daemon use a small globe badge on the folder icon, while local workspace rows keep the plain folder icon. Session discovery, transcript loading, file references, terminal traffic, and execution require no parallel remote-specific implementations because they already use the selected SDK client.

The add operation remains a one-shot flow. The source URL is kept only in the current tab while navigation is in progress. Cancel returns to that URL, Change computer returns and reopens the computer choice, and a successful registration stays on the selected daemon and clears the continuation state. The connection catalog stores origins only; it does not cache remote workspaces or aggregate projects from multiple daemons. Ordinary daemon switches do not resume the flow.
...
- Verify that Change computer reopens the location choice, Cancel returns to the exact source page without a token or continuation marker, and an ordinary daemon switch never opens the directory browser.
...
- Cancel and Change computer return to the source shell predictably, while a completed add stays on the selected daemon; the persistent catalog contains connection origins, never remote project data or bearer tokens.
