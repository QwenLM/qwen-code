...
The existing sidebar remains the workspace and session management UI. A successful remote connection records only its validated origin in a browser-local connection catalog; bearer tokens remain tab-scoped. The normal **Add workspace** action then starts with a computer choice: this computer or one of those connected remote computers. That step is skipped when no remote computer has been connected, because a choice of one is not a choice. Selecting a computer this tab is not already connected to navigates to that daemon and resumes the operation in directory-browser mode; selecting the daemon already in use opens the browser in place, without reloading the shell. The browser uses daemon-provided directory suggestions, offers the path as clickable breadcrumbs alongside a manual absolute path, and registers the selected directory through the existing workspace mutation. Native folder selection remains hidden for remote daemons. A cross-origin daemon is named once as a host chip beside the sidebar's Project heading; individual workspace rows keep the plain folder icon, since one daemon serves all of them. Session discovery, transcript loading, file references, terminal traffic, and execution require no parallel remote-specific implementations because they already use the selected SDK client.

The add operation remains a one-shot flow. The source URL is kept only in the current tab while navigation is in progress. Cancel returns to that URL, Change location returns and reopens the location choice, and a successful registration stays on the selected daemon and clears the continuation state. The connection catalog stores origins only; it does not cache remote workspaces or aggregate projects from multiple daemons. Ordinary daemon switches do not resume the flow.
...
- Connect to a remote computer, choose the normal **Add workspace** action, select that computer, browse its directories, register one, and verify that the new workspace becomes active on that daemon.
- Verify that Change location reopens the location choice, Cancel returns to the exact source page without a token or continuation marker, and an ordinary daemon switch never opens the directory browser.
...
- The standalone sidebar exposes one Add workspace action that can select this computer or a previously verified remote connection, continue after navigation, browse daemon directories, and register the selected absolute path.
- Cancel and Change location return to the source shell predictably, while a completed add stays on the selected daemon; the persistent catalog contains connection origins, never remote project data or bearer tokens.
