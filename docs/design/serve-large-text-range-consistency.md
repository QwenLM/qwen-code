...
Serve now streams explicit text windows from files larger than
`MAX_READ_BYTES`. The workspace boundary opens the file once, reads through
that handle, and returns partial metadata without a full-file hash.
...
2. After streaming, `fstat` must retain the open-time size and modification
   time.
3. After closing, `lstat` must still identify the same non-symlink file with
   the open-time size and modification time.
4. A mismatch detected by these stability checks returns `hash_mismatch`.

Content validation can fail earlier without returning a result. Binary and
unsupported-encoding errors retain their existing precedence on those paths.
...
- Handle-bound range reads never use the full-buffer fast path and reuse their
  streaming buffer.
- Existing output, scan, encoding, binary, hash, and line-count limits remain
  unchanged.
