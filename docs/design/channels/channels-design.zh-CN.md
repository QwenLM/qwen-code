...
- **内容**：`text`，可选 `localControlText`、`imageBase64`/`imageMimeType`、`referencedText`
- **上下文**：`isGroup`、`isMentioned`、`isReplyToBot`，可选 `threadId`

插件职责：`senderId` 必须稳定且唯一；`chatId` 必须能区分私聊和群聊；布尔标志必须准确，以供门控逻辑使用；`text` 包含适配器规范化后的消息。若适配器在 `text` 中保留开头的路由 mention，可以同时提供 `localControlText`，其值为移除这一个 mention 后经裁剪的后缀，使本地控制和安全检查能够使用正文，同时不改变展示、历史记录或普通模型输入。
