...
`tool_search` currently reveals each matched deferred tool and refreshes the
...
The scheduler unwraps `tool_call` before resolving permissions, approvals,
hooks, invocation guards, concurrency, telemetry, and execution. All of those
consumers therefore receive the underlying tool name and arguments. The
function response sent back to the model retains the bridge name and original
call id so it still matches the model-emitted function call.
