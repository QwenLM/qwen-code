...
区分模型默认值和用户显式选择。保留现有选择优先级；没有选择时，外部默认值同时影响请求与界面。Default 恢复当前模型配置。仅按有效支持集合归一档位，不再进行内部名单的二次归一。保留已有原始参数覆盖、请求级关闭、强制思考规则和预算上限。默认值不能覆盖请求级关闭。
...
ACP 保留 `reasoning_effort`，使用已有元数据携带默认值及思考可用性。CLI 和 WebShell 展示相同的有效状态与 Default 操作，覆盖强制思考和仅支持开关的模型。
