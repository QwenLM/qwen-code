...
`capabilities.reasoning` 继续作为内置清单与客户端使用的模型发现和控件契约。外部 profile 放在 `generationConfig`，因为它还决定提供方请求及历史消息转换，并且需要与精确模型、端点、凭证和暂存的生成配置一起流转。若扩展面向客户端的能力对象，仍需增加传输 profile，还会让提供方序列化与模型发现载荷耦合。解析边界保持单向：显式生成配置在其能够表达的字段上优先，省略字段时可继承已有能力。生成配置形状无法表达的注册表约束（如 `canDisable: false`）保持权威。

档位使用 `low/medium/high/xhigh/max`。仅支持开关的 profile 拒绝档位字段。校验 profile 与协议匹配、档位无重复；同时提供 `supportedEfforts` 和 `defaultEffort` 时，默认值必须属于支持集合。省略 `supportedEfforts` 时，显式默认值会扩展继承的档位集合。Gemini 使用已有 low/medium/high 映射。声明至少包含一个字段，空对象无效。无效声明需指出模型与字段。
...
图片工具在提供方更新提交后刷新；失败时保留图片选择供下一次用户发送重试，不回滚已刷新的提供方。应用期间到达的新图片选择保留至下一次用户发送，即使没有新的提供方更新也会应用。
