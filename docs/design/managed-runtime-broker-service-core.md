...
Add explicit process adoption and reconciliation before enabling restart recovery, add JDBC Tool execution persistence for multi-instance dispatch convergence, and then expose this core through a private HTTP adapter. The HTTP `status` route and transport land with the execute handler extraction. Physical Runtime draining, Hosted Harness integration, and the Qwen-side Broker client remain separate reviewable slices.
