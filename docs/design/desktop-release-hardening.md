...
The Desktop release should preserve the last complete bundled runtime until a replacement is fully assembled, reuse a verified Node.js archive across local builds, and publish only installer/updater artifacts. Published prereleases must use a SemVer prerelease suffix so a later stable build has a strictly newer updater version.
