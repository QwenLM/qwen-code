...
A loopback daemon URL is not proof that the browser runs on the daemon host. WebShell first attempts the isolated origin. If its proxy handshake does not arrive within 10 seconds, WebShell retries `/mcp-app-sandbox?mode=opaque` through the existing daemon connection, so a single-port SSH tunnel or published container port can still render ordinary Apps. This inline response enforces HTTP CSP `sandbox allow-scripts allow-forms` without `allow-same-origin`; the outer iframe also removes that permission. The App cannot gain the daemon origin. The UI explains the restricted mode; Apps that require non-opaque vendor frames (such as Tableau) may remain incompatible there. No configurable public sandbox host or forwarded port range is introduced.
