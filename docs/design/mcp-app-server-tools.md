...
WebShell supplies an execution callback bound to the transcript session. The AppBridge advertises `serverTools` only with that callback. The host pins the server and resource URI from the rendered App and accepts only an exact raw tool name and arguments from the iframe. Historical pages retain the transcript's App session binding even while source and feedback controls are disabled. A standalone transcript without a bound session remains display-only.
