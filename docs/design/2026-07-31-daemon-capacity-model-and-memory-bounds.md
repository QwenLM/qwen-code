...
**Deferred to their own changes — everything else in the tree.** Aggregate child RSS across all workspace children is a second measurement with a second failure mode (a child that exits mid-poll), and channel workers report no RSS at all today, so covering them means building the reporting path first. Until both land, `runtime.memory.childRssCoverage` reads `primary_only` and says so in the response — a root reading must not be read as tree pressure.
