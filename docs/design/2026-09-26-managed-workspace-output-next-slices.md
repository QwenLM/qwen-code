...
For managed-context requests, persist a resource handle before process launch. A binding with such a handle but without an attested lease may not automatically launch again. A failed or ambiguous first attempt transitions to `RECOVERY_BLOCKED`; repeated warm requests and Broker restarts cannot allocate a replacement generation. When an attempt outlives its deadline, the call answers 409 `runtime_broker_recovery_blocked` once the block is recorded; if it cannot be recorded, the call answers the retryable timeout and the next call blocks. Even a crash between handle persistence and process launch blocks automatic retry. Legacy startup policy remains unchanged.
