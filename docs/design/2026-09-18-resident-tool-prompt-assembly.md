...
| Section                                                                                   | Rule                                                                                                                                                                                                                                                                    |
| ----------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `## Using Your Tools` bullets                                                             | Each bullet is emitted only when the tools it names are in the declared set. A bullet naming several tools keeps the ones present and drops the sentence when none are.                                                                                                 |
| `# Examples` transcripts                                                                  | Each example is emitted only when every tool it calls is declared. At least one example is always emitted; when the selected model's examples are all filtered out, the prompt keeps the shortest surviving one from the general set, and only then falls back to none. |
...
| Bare tool names in prose (`:420`, `:901`, `:1152`, `:1270`, `:1355`, `:913` and siblings) | Converted to `ToolNames` interpolation in the same change so they participate in gating; the display-name mention becomes the tool's display name constant.                                                                                                             |
...
- **`prompt-tool-examples.test.ts:99` breaks by design.** Its exact-equality assertion between example tool names and validator keys must become "the examples rendered for this declared set use exactly these validators".
...
3. **Invariant test:** scan the rendered prompt for every `ToolNames` value and assert each name found is in the declared set. This is the test that catches both over-gating and under-gating.
