...
| `# Examples` transcripts                                                                  | An `<example>` block is kept only when every tool it calls is declared. A block that calls no tool (`user: 1 + 2`) is never gated. The heading is kept while any block survives, so no empty section is left behind. The model-specific XML and JSON example formats use no `[tool_call: …]` notation, carry no detectable tool names, and are left ungated.                                                                                   |
