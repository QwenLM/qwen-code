...
6. Generate all 54 MCP schemas from the pinned binary and preserve MCP tool
   annotations. Every non-read-only tool uses the stronger confirmation
   surface in AUTO_EDIT and is blocked in PLAN mode. Conditional mutators such
   as `page`, `clipboard_read`, and `check_permissions` are classified from
   their arguments.
...
through that app so Accessibility and Screen Recording grants normally belong
to `com.qwencode.cua-driver`. If registration fails, permission attribution can
fall back to the launching terminal; the driver's permission status identifies
the active source. The stdio MCP child remains a thin proxy to the persistent
driver daemon.

The built-in client inherits the user's environment except for coordinate-mode
overrides, adds the model payload filter, and invokes `qwen-cua-driver mcp`. It
does not weaken driver authorization.
