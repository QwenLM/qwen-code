...
[English](./2026-09-23-batch-api-overview.md)
...
- 差异很大：09-23 的测试里执行只要 **2–7 秒**，等待基本全是排队；09-14 的探针里
  `in_progress` 到 `completed` 就要 10 分钟到 1 小时。
...
flowchart TD
    ...
    V --> OUT["写入计划指定的新目标文件<br/>绝不覆盖；冲突时挂起"]
...
- **只能由用户显式触发**：模型调用不了 `/batch-api`，不会自行把任务换成异步。`/batch`
  的描述允许模型*建议*用户输入 `/batch-api`。
...
收取是纯机械的事，由 qwen-code 包掉，只把结果告诉用户。原先的思路是不做常驻进程、
不让模型轮询；那条约束防的是"后台持续花模型的钱"，由程序查询和写文件不花模型的钱，
两者不冲突。
...
1. 默认开启并自动写入；写入规则本来就不覆盖已有文件、冲突就挂起；需要时切到 `notify`
   或 `off`。
2. 失败项**不**自动重试（重试会再花钱），通知里只给出失败原因和重试命令。
3. 收不了的情况只提示一次：会话没有可用的 Batch 凭证、提交对不上服务端的批次
   （可能在扣费）时各提示一次；绑定到其他端点或 key 的任务，切回后自动继续。
