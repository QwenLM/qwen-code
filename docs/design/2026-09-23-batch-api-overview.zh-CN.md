...
flowchart TD
    ...
    Q --> C["收取：qwen batch collect<br/>（目前需要用户触发，见第 5 节）"]
    C --> V["程序下载 → 逐条校验<br/>截断 / 空内容 / 工具调用 / 代码围栏"]
    V --> OUT["写入 out/ 下的新文件<br/>绝不覆盖；冲突时挂起"]
    V --> FAIL["失败项清单 → qwen batch retry<br/>只重发失败的"]
```

要点：

- **只能由用户显式触发**：模型调用不了 `/batch-api`，不会偷偷把任务换成异步。
- **等待不消耗模型**：提交后会话就交还用户；查看、收取、重试、取消、清理都是普通命令，
  会话里加 `!` 执行，不花模型轮次。
...
# 收取：目前需要用户触发（加 ! 不消耗模型；未完成时只报告进度）
!qwen batch collect <task-id>
#   或在另一个终端挂着等完成：qwen batch collect <task-id> --wait --timeout 86400
...
收取是纯机械的事，应由 qwen-code 包掉，只把结果告诉用户。设计约定 §3.3 原本不做常驻
...
- 通知复用"检查更新"的链路（`updateEventEmitter` + `setUpdateHandler`）：用户空闲时直接
  ...
  都已接好这条链路。
- 新增设置项 `general.batchAutoCollect`（写法参照 `general.enableAutoUpdate`），并提供
  "只通知、不写入"的选项。
...
1. 自动写入，还是只通知？建议自动写入：写入规则本来就不覆盖已有文件、冲突就挂起，同时
   提供"只通知"的开关。
2. 失败项是否自动重试？建议**不要**，重试会再花钱。只在通知里给出失败原因和重试命令。
3. 默认开启还是关闭？建议默认开启：用户显式选了 `/batch-api`，本来就预期结果会回来。
