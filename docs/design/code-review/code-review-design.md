...
改造前，仓库的 AI PR review 跑在 `.github/workflows/qwen-code-pr-review.yml` 上，调用上游 `QwenLM/qwen-code-action` 触发 bundled review skill（`packages/core/src/skills/bundled/review/SKILL.md`）。bundled skill 本身已经做了 9 个并行 review agent、确定性 lint/typecheck、跨文件影响分析、批量 verification、迭代 reverse audit、模式聚合等工作，单次评审质量已经足够。
...
> Phase 4 会在 Stage 1 与 Stage 2 之间插入一个独立的 Design Gate step；本 PR 不含该 step，Stage 1 通过即进入 tier-specific review 执行；其中 DEEP 只复用 bundled `/review` rubric。
...
早期方案里 review 评论作者仍是 `github-actions[bot]`。当前 PR 改成优先使用 `QWEN_CODE_BOT_TOKEN` / `CI_BOT_PAT` 发布评论，缺失时再 fallback 到 `GITHUB_TOKEN`；独立 GitHub App 仍属后续阶段。
...
早期 workflow 用的是 `qwen-code-action` 内部安装的已发布 CLI，跟仓库 source 不是同一份；改 bundled skill 必须等下一个 release 才能被 action 路径使用。
