...
| 指标                           | PR0+1 baseline (p50)                        | 目标                            | **PR-A 实测 (p50, n=30, Welch p<1e-9)** |
| ------------------------------ | ------------------------------------------- | ------------------------------- | --------------------------------------- |
| `before_render`                | 79-81 ms (跨 fixture)                       | -50~150 ms (PR-B)               | 不变（PR-A 范围之外）                   |
| `first_paint`                  | 420 ms (no-mcp 交互态)                      | -150~300 ms (PR-B)              | 410-425 ms（±3% 噪声）                  |
| `input_enabled`                | 480 / 875 / 7101 / 10483 ms（四个 fixture） | 慢 MCP 不再拖延                 | **472 / 472 / 471 / 490 ms ✅**         |
| `config_initialize_dur`        | 70 / 464 / 6688 / 10081 ms                  | 慢 MCP 下 ≤ 1s                  | **73 / 71 / 71 / 73 ms ✅**             |
| `mcp_first_tool_registered`    | 866 / 872 ms（one-fast / three-mixed）      | 持平（PR-A 不加速 MCP 自身）    | 857 / 852 ms ✅                         |
| `gemini_tools_lag`             | 6235 ms (three-mixed)                       | **≤ 16 ms**（一帧 batch flush） | **17.1 ms ✅（~365x 改善）**            |
| `mcp_all_servers_settled`      | 7077 / 10467 ms（three-mixed / flaky）      | 不退化                          | 7061 / 10498 ms ✅                      |
| `interactive_bundle_size` (gz) | 待测（PR-B 引入）                           | -10~25%                         | 待 PR-B                                 |
| `headless_bundle_size`         | 待测（PR-B 引入）                           | **不含 Ink/AppContainer**       | 待 PR-B                                 |

> **PR-A 全部 improvement 目标达成（four_fixtures × 30 runs 实测，详见 § 11.2.C）**。
...
设计文档 §§ 3、5-11 中**保留原 PR2/3/4 字样**以维持决策语境的可追溯性，但实际落地按 PR-A / PR-B 合并：

| 旧标签 | 新标签           | 说明                                                                   |
| ------ | ---------------- | ---------------------------------------------------------------------- |
| PR2    | **PR-B Phase α** | 异步并行 settings；并入 PR-B                                           |
| PR3    | **PR-B Phase β** | initializeApp 并行 + 入口动态 import；并入 PR-B                        |
| PR4    | **PR-A**         | 渐进式 MCP；单独 ship（最大用户感知收益 + 唯一行为语义变更，隔离风险） |

读文档时若看到"PR2/3/4"请按上表映射。

### 4.1 三步落地（最终顺序，已采纳激进合并方案）

经过 baseline 实测后，PR 顺序按"用户感知收益 + 风险隔离"重排：

| 阶段              | 范围                                                                      | 用户收益                                                     | 关键产物                                                                                                                   |
| ----------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| **PR0+1 (#3994)** | 设计文档 + Phase 0 观测基线实现 + benchmark 脚本 + baseline               | 无（基础设施）                                               | `docs/design/first-screen-performance-optimization/`、`startupProfiler` 扩展、新 checkpoint、benchmark 脚本、baseline 数据 |
| **PR-A**          | 渐进式 MCP 可用性（合并原 PR4）                                           | **MCP 用户启动 7-10 秒 → ~340ms，最大杀手锏**                | `config.ts`、`mcp-client-manager.ts`、`tool-registry.ts`、`client.ts`、`AppContainer.tsx`                                  |
| **PR-B**          | 启动主路径优化（合并原 PR2 loadSettingsAsync + 原 PR3 并行化 + 入口延迟） | 所有用户首屏 -100~150ms；headless/ACP/子命令 bundle 显著瘦身 | `settings.ts`、`core/initializer.ts`、`gemini.tsx`、新 `interactiveCli.ts`、`esbuild.config.js`                            |

**为什么这样排**：

- **PR-A 先 ship**：MCP 用户的 7-10 秒卡顿是最痛点，PR-A 一次解决。代码改动局限在 4 个文件且对应原语已存在（仓里已有 `discoverAllMcpToolsIncremental` / `removeMcpToolsByServer` / `skipDiscovery`），落地风险可控。
- **PR-B 后 ship**：原 PR2 在热缓存场景下实测收益缩水到 ~3ms（baseline 验证），单独 ship 价值低；与 PR3 合并共享"启动主路径优化"叙事，降低 review/迭代成本。
- **PR0+1 仍是硬前置**：PR-A/B 的所有指标对照都引用 PR0+1 入库的 baseline + benchmark harness。

> **强约束**：PR-A 与 PR-B 提交时**必须包含** 4 fixture × 30 次样本的 Welch's t-test 对照表（双 mode：非交互 + 交互），否则不予合并。

### 4.2 PR 间结构依赖

| PR    | 修改文件                                                                                   | 硬依赖 | 软依赖（验收） | 与其他 PR 的潜在冲突                                                          |
| ----- | ------------------------------------------------------------------------------------------ | ------ | -------------- | ----------------------------------------------------------------------------- |
| PR0+1 | profiler / events sink / 新 checkpoints                                                    | 无     | 无             | 无                                                                            |
| PR-A  | `config.ts`、`mcp-client-manager.ts`、`tool-registry.ts`、`client.ts`、`AppContainer.tsx`  | PR0+1  | PR0+1 baseline | 与 PR-B 在 `AppContainer.tsx` / `gemini.tsx` 上**可能合并冲突**，但无逻辑依赖 |
| PR-B  | `settings.ts`、`initializer.ts`、`gemini.tsx`、新 `interactiveCli.ts`、`esbuild.config.js` | PR0+1  | PR0+1 baseline | 同上，建议先 ship PR-A 再 ship PR-B 避免 rebase 工作量                        |

**结论**：PR-A / PR-B **无逻辑依赖**，可并行开发，但**建议串行合并**（PR-A 先）以避免 `AppContainer.tsx` 合并冲突。

### 4.3 UX / 破坏性变更影响

| PR    | UX 变化                                                                              | 破坏性                                                                                                                                                                                                                                                                                       | 兜底                                                                                           |
| ----- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| PR0+1 | 无（仅观测）                                                                         | ❌ 无                                                                                                                                                                                                                                                                                        | 单 commit revert                                                                               |
| PR-A  | **MCP 用户启动从 7-10 秒降到 ~340ms**（最大改善）；UI 首屏出现后"工具加载中"短暂状态 | ⚠️ **行为语义变更**：`Config.initialize()` 返回时 MCP **尚未就绪**。<br/>- 假设"init 完成即所有 MCP 工具可用"的代码失效<br/>- `MCPDiscoveryState.COMPLETED` 不再与 init 结束对齐<br/>- 测试若断言 `config.initialize()` 后 MCP 工具存在会失败<br/>- 模型在 setTools 刷新前的请求看不到新工具 | `QWEN_CODE_LEGACY_MCP_BLOCKING=1` env 兜底，保留至少 1 个 release；release notes 显式说明      |
| PR-B  | 启动快 ~100ms（部分用户感知）；非交互模式 bundle 显著瘦身                            | ⚠️ **轻度**：<br/>- 入口动态 import 把 UI 模块错误从 module-eval 时推迟到 await 时<br/>- 若有外部代码以 library 方式 import `dist/cli.js`，bundle 拆 chunk 需验证兼容性<br/>- 测试 harness 若依赖 `index.ts` 顶层求值顺序需调整<br/>- esbuild `splitting: true` 强制 `format: 'esm'`         | ACP / 子命令集成测试全绿前不合并；CI gate `scripts/check-bundle-leakage.mjs`；单 commit revert |

**审计清单**（PR-A 提交时必须勾选）：

- [ ] 全工程 grep `discoveryState === MCPDiscoveryState.COMPLETED` —— 列出所有用法，确认其语义可与新模型对齐
- [ ] 全工程 grep `config.initialize` 调用点 —— 列出哪些紧接着假设 MCP 工具可用
- [ ] `integration-tests/cli/mcp-*.test.ts` —— 任何断言 MCP 工具数量的测试改为"等到 `mcp_all_servers_settled` 事件再断言"
- [ ] 公开 release notes 段落："MCP servers now load progressively; tools become available as each server is ready. Set `QWEN_CODE_LEGACY_MCP_BLOCKING=1` to restore the previous synchronous behavior."

### 4.4 Bundle 大小影响

当前 `dist/cli.js` ≈ 23 MB（uncompressed，单 bundle）。

| PR    | 主 bundle (`dist/cli.js`)             | headless chunk (PR-B 新增) | 净变化                                                                                                            |
| ----- | ------------------------------------- | -------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| PR0+1 | 不变                                  | 不存在                     | +profiler/sink 代码 < 5 KB                                                                                        |
| PR-A  | +< 50 KB（增量 MCP 代码路径）         | —                          | 可忽略                                                                                                            |
| PR-B  | -100~500 KB（部分 UI 移到独立 chunk） | **预计 -30~50%**           | **非交互/headless/ACP/子命令路径不解析 Ink + AppContainer + themes + hooks**（实测以 `source-map-explorer` 为准） |

**强约束**：PR-B 必须附 `source-map-explorer` 截图证明 headless chunk 不含 `node_modules/ink/`、`AppContainer.tsx`、`themes/`、`hooks/use*.tsx`。CI gate `scripts/check-bundle-leakage.mjs`（PR-B 同步引入）自动校验。

### 4.5 PR0+1 资产保留策略

PR0+1 一次性引入了相当数量的"非生产"资产。统一保留到 main，按用途分类：

| 资产类别                                                          | 文件 / 位置                                                                                                                   | 决策      | 理由                                                                                                      |
| ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------- | --------------------------------------------------------------------------------------------------------- |
| **生产代码**（profiler 扩展、cross-package sink、新 checkpoints） | `packages/cli/src/utils/startupProfiler.ts`、`packages/core/src/utils/startupEventSink.ts`、`packages/{cli,core}/src/**` 改动 | ✅ 必留   | PR-A/B 都依赖；profiler 长期是诊断工具                                                                    |
| **单测**                                                          | `startupProfiler.test.ts`、`startupEventSink.test.ts`                                                                         | ✅ 必留   | 防止 profiler / sink 被无意破坏                                                                           |
| **Benchmark 脚本**                                                | `scripts/benchmark-startup.mjs`、`scripts/benchmark-heisenberg.mjs`                                                           | ✅ 保留   | PR-A/B reviewer 跑验收必备；后续 nightly perf CI 复用；将来 profiler 改动还要再验 Heisenberg              |
| **Fixtures**                                                      | `docs/design/first-screen-performance-optimization/fixtures/` (4 fixture + `_servers/echo-mcp.mjs`)                           | ✅ 保留   | benchmark 脚本依赖；放在 `docs/design/...` 下与生产代码隔离，不干扰 npm 发布                              |
| **Baseline 数据**                                                 | `baseline-data/<fixture>{,-interactive}.summary.json` (9 文件, 共 ~50 KB)                                                     | ✅ 保留   | PR-A/B `--baseline <path>` 必需输入；不入库则 reviewer 无法做 Welch's t-test 对照                         |
| **Heisenberg 报告**                                               | `baseline-data/heisenberg.summary.json`                                                                                       | ✅ 保留   | 文档化方法学验证结果；profiler 后续改动重跑可复用                                                         |
| **Raw / report 中间产物**                                         | `*.raw.jsonl`, `*.report.md`                                                                                                  | ❌ 不入库 | `baseline-data/.gitignore` 已排除；reviewer 本地重跑即可生成                                              |
| **node-pty devDependency**                                        | `package.json` devDependencies                                                                                                | ✅ 保留   | `--interactive` benchmark 需要；**仅 devDep，不进生产 bundle**；脚本内置 `spawn-helper` exec-bit auto-fix |
| **设计文档**                                                      | `docs/design/first-screen-performance-optimization/design.md` + `README.md` × 2                                               | ✅ 必留   | 记忆规则"设计先行"明确要求；3 轮 review + decision log 是 PR-A 行为语义变更的论证依据；未来维护者读       |

**总原则**：把"基础设施 / 验收工具 / 参考数据 / 设计论证"全部一次性 ship 到 main，让 PR-A/B 只动业务代码并对照已 committed 的 baseline。分离这类资产会增加 PR-A/B 的 review 协调成本，没有实质收益。
...
## 6. PR-B Phase α — 异步并行 Settings 加载 (原 PR2)

> 合并到 PR-B 的第一段。**实测收益已大幅缩水**（baseline `after_load_settings` p50 = 9ms，而非设计文档原估的 100-200ms），价值主要在冷启动 + 异步基础设施铺垫。
...
## 7. PR-B Phase β — initializeApp 并行化 + 入口延迟加载 (原 PR3)

> 合并到 PR-B 的第二段。本段产出 bundle 拆分 + headless 路径剔除 Ink（详见 § 4.4 bundle 大小分析）。
...
## 8. PR-A — 渐进式 MCP 可用性 (原 PR4)

> **单独 PR**，先于 PR-B 合并。**最大用户感知改善**：MCP 用户启动 7-10 秒 → ~340ms。详见 § 4.3 行为语义变更审计要求。
...
### 11.2.C PR-A 实测结果（已实施 + 真实测量）

> **数据落盘**：`baseline-data/pr-a/<fixture>-after.{summary.json,raw.jsonl,report.md}`。每 fixture 30 runs × Welch's t-test，对照基线 `baseline-data/<fixture>-interactive.summary.json`。

PR-A 实施后的实测数字（macOS arm64, Node 24.15, p50, 单位 ms）：

#### no-mcp（控制组：PR-A 应当 no-op）

所有指标 Δp50 均在 ±3% 噪声带内：`input_enabled` -7.85ms（-1.64%），`first_paint` -9.65ms（-2.30%）。**验证：PR-A 不引入回归到无 MCP 用户**。

#### one-fast-mcp（单快 MCP 服务器场景）

| 指标                    | Baseline | After  | Δp50 (ms) | Δp50 (%)   | t-test p | 结论    |
| ----------------------- | -------- | ------ | --------- | ---------- | -------- | ------- |
| `config_initialize_dur` | 464.40   | 71.03  | **-393**  | **-84.7%** | < 1e-9   | improve |
| `input_enabled`         | 875.34   | 471.50 | **-404**  | **-46.1%** | < 1e-9   | improve |
| `mcp_first_tool`        | 865.85   | 857.34 | -8.51     | -0.98%     | 0.036    | noise   |
| `mcp_all_settled`       | 865.88   | 857.38 | -8.50     | -0.98%     | 0.037    | noise   |
| `first_paint`           | 421.64   | 410.50 | -11.14    | -2.64%     | < 1e-3   | noise   |

**1 个快 MCP 用户启动可用时间从 875ms 降到 472ms（~1.9x）**。MCP 总发现时间不变（用户不感知）。

#### three-mixed-mcp（2 快 + 1 慢，PR-A 的标志靶点）

| 指标                    | Baseline | After     | Δp50 (ms) | Δp50 (%)    | t-test p | 结论                |
| ----------------------- | -------- | --------- | --------- | ----------- | -------- | ------------------- |
| `config_initialize_dur` | 6687.59  | 71.33     | **-6616** | **-98.93%** | < 1e-9   | improve             |
| `input_enabled`         | 7100.61  | 470.89    | **-6630** | **-93.37%** | < 1e-9   | improve             |
| `gemini_tools_lag`      | 6235.21  | **17.10** | **-6218** | **-99.73%** | < 1e-9   | **improve (~365x)** |
| `mcp_first_tool`        | 872.39   | 852.42    | -19.97    | -2.29%      | 0.052    | noise               |
| `mcp_all_settled`       | 7077.42  | 7061.01   | -16.41    | -0.23%      | 0.088    | noise               |
| `first_paint`           | 423.02   | 411.26    | -11.76    | -2.78%      | < 1e-2   | noise               |

**这是 PR-A 的杀手锏数据**：

- **TTI（`input_enabled`）：7.1 秒 → 471ms，~15x 改善**。慢 MCP 不再阻塞首屏。
- **`gemini_tools_lag`：6.2 秒 → 17.1ms，~365x 改善**。模型在每个 server ready 后约 17ms（一帧）就能看到新工具，确认 16ms batch flush 工作正常。
- `mcp_all_settled` 没变化（仍 ~7s）—— 慢 server 还是慢，但现在**完全在后台运行**，对用户不可见。

#### flaky-mcp（1 个挂死 MCP，30s 后超时降级）

| 指标                    | Baseline | After    | Δp50 (ms)  | Δp50 (%)    | t-test p | 结论    |
| ----------------------- | -------- | -------- | ---------- | ----------- | -------- | ------- |
| `config_initialize_dur` | 10081.20 | 73.36    | **-10008** | **-99.27%** | < 1e-9   | improve |
| `input_enabled`         | 10483.05 | 490.04   | **-9993**  | **-95.33%** | < 1e-9   | improve |
| `mcp_all_settled`       | 10467.40 | 10498.17 | +30.77     | +0.29%      | < 1e-3   | noise   |
| `first_paint`           | 412.97   | 425.12   | +12.15     | +2.94%      | < 1e-3   | noise   |

**挂死 MCP server 用户启动可用时间从 10.5 秒降到 490ms，~21x 改善**。`mcp_all_settled` 持平（broken server 仍需 ~10s 才被 cli 内部标记为失败，但在后台进行不阻塞 UX）。

### 11.2.D 综合改善总览（用户感知）

| 场景            | 当前 → PR-A 后      | 改善            |
| --------------- | ------------------- | --------------- |
| 无 MCP          | 480ms → 472ms       | ~持平           |
| 1 快 MCP        | 875ms → 472ms       | **-46%, ~1.9x** |
| 2 快 + 1 慢 MCP | **7101ms → 471ms**  | **-93%, ~15x**  |
| 1 挂死 MCP      | **10483ms → 490ms** | **-95%, ~21x**  |

`gemini_tools_lag` (three-mixed-mcp): **6235ms → 17.1ms, ~365x 改善** —— 印证 16ms batch flush 设计正确。

### 11.2.E PR-A 副作用审计（实测结果）

- ✅ **no-mcp 路径无回归**：所有指标在 ±3% 噪声带内。
- ✅ **`first_paint` 几乎不变**（±2.5% 噪声）：PR-A 不动 mount 前的代码路径，符合设计预期。
- ✅ **`mcp_all_settled` 不退化**：慢/挂死 server 总发现时间持平，PR-A 没有牺牲完整性。
- ✅ **`gemini_tools_lag` 17ms ≈ 一帧**：验证 16ms batch flush 实际工作（一次 MCP server ready 后，模型在下一帧内看到工具）。
- ⚠️ **`flaky-mcp.first_paint` +2.94%（13ms）**：在噪声带内，但 mean 偏移轻微正向。可能与后台 MCP 任务排队 React commit 有关；t-test p < 1e-3 说明这是真信号，但绝对幅度小，不阻塞合并。
