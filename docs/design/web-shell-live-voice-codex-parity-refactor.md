...
| Incremental backend return       | Agent-message deltas returned to the Realtime conversation at the Codex cadence and lifecycle boundaries                      |
...
Conversation-item return is not categorically rejected: it is Codex's actual
Realtime V2 compatibility path when the original handoff remains open and
completion closes that same function call. Sending synthetic conversation
items every 200 ms is not the same as either Codex V2 message-level updates or
V3 `handoff_append`; it requires separate compatibility evidence and approval
...
5. a separate qualification case confirms that 200 ms conversation-item
   injection is accepted and ordered, while explicitly retaining it as an
   unapproved possible Qwen adaptation rather than Codex V2 or V3 parity;
...
The result confirms that the public Qwen protocol can reproduce the Codex V2
open-handoff lifecycle. It does not reveal a public Qwen equivalent of V3
handoff_append. Although high-frequency conversation-item injection was
accepted and ordered, that transport observation does not convert synthetic
200 ms items into Codex V2 or V3 parity. The implementation must use the exact
Codex V2 message-level lifecycle unless a real V3-equivalent primitive is
later established. No high-frequency conversation-item adaptation,
immediately accepted asynchronous Work lifecycle, multi-response workaround,
or full-turn TTS replacement is authorized.
...
1. **Protocol qualification (completed 2026-07-30):** the bounded provider
   experiment confirmed the Codex V2 lifecycle and the limits recorded above.
2. **Same-session foundation:** attach Realtime to one normal projectless Live
   session and reproduce Codex startup task context.
3. **Task and tool surface:** retain normal tools and add Codex-equivalent
   Live/task operations using existing session services.
4. **Selective handoff:** allow direct Realtime answers and route only explicit
   handoffs into the same backend session.
5. **Incremental return:** stream backend deltas through the verified provider
   mechanism with separate progress, speakable, and completion semantics.
6. **Permissions and steering:** connect existing approval UI and same-session
   interruption without exposing raw protocol events to speech.
7. **Lifecycle and UI verification:** verify task identity, Live grouping,
   resume/new/stop, captions, progress, audio, and interruption in real use.
...
| Barge-in during work    | Existing response stops and the same backend turn is steered without overlap or duplication                                                           |
