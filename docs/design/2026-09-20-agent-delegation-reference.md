...
- **`agent.test.ts`**: six assertion sites anchored on text that moved were re-anchored, and a seventh was deliberately retained. All seven would have kept passing untouched, because that file's stub `Config` has no `getSkillManager`, so the route is `inline` and the description embeds the whole `SKILL.md` body — every moved anchor is still in it. The six are re-anchored on facts the description keeps in every shape (don't set `model` on a fork, forks inherit the full parent conversation by default, pass a short `name`, and "Choose a fork when the task needs substantial context"). The retained seventh is `toContain('Never delegate understanding')` (`:989`, unchanged from base `:979`), kept as this file's pin on the inline shape: its dependence on the stub is disclosed in the test's own comment (`:984-987`), and the inlined body is separately asserted verbatim at `agent-description-budget.test.ts:255`, so a truncated or section-split inline arm is caught there rather than here.
