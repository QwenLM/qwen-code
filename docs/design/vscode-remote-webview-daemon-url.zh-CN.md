...
- 机制层复现（无需桌面客户端）：在 Docker 容器里用扩展完全相同的 argv 启动真实 CLI，并用扩展完全相同的正则抓取它的地址。容器内访问 `/capabilities` 返回 200；从宿主发起同一个 `http://127.0.0.1:<port>` 请求则被拒绝——正是报告里的失败，且没有使用任何替身进程。随后在无头 Chromium 里分别加载两种 CSP（策略从 DOM 里回读校验）：改动前的策略会以 `connect-src` 违规拦掉 `http://localhost:<port>` 与 `ws://localhost:<port>`，改动后两者都放行，而 `wss://localhost:<port>` 在两种策略下都被 `ws:` 源放行。
