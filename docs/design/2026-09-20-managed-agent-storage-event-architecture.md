...
Status: proposed, with a P0/P1 subset implemented on this branch. Date: 2026-09-20. This design uses the integration snapshot below; it does not claim production validation of the complete architecture.
...
This document does not implement SDK, schema, or deployment changes. It does not promise recovery at an arbitrary model token, exactly-once tool side effects, live database switching, or equivalent support for every MQ feature.

The current implementation slice introduces `AgentStateStore`, bounded Harness event batching, one-transaction cursor/sequence/terminal updates, and post-commit local SSE delivery with durable replay fallback. It retains one SQL row per public event for compatibility. The SQL batch-journal schema, Message/Item materializer, retention, external EventTransport, PostgreSQL adapter, and durable Harness/Runtime recovery remain proposed follow-up work.
...
| `service/HarnessCoordinator.java`                                                        | Consumes Harness SSE; deduplicates with `bootId:eventEpoch:eventId`; calls `recordHarnessEvent` for each source event. Java also generates Runtime status events.             |
| `service/HarnessEventProjector.java`                                                     | Text projections contain only `text`, without stable text Item/Part identities. Some raw events are filtered, so the public stream cannot reconstruct complete model context. |
| `service/ManagedEventStreamService.java`                                                 | Each SSE connection queries the database and polls at a default `200ms` interval when idle. The browser never accesses the database directly.                                 |
| `service/ManagedAgentService.java`                                                       | Transcript still pages through events rather than reading independently stored complete messages.                                                                             |
...
| `src/main/resources/db/migration/V1__managed_agent_core.sql` (within the service module) | Contains the core Session, Turn, Command, and Event tables; none of the proposed batch journal, Item, or Snapshot tables exist.                                               |
...
These are responsibility sketches, not implemented APIs, and do not require an interface for every table.
...
| `managed_agent_item`              | Display content, type, status, revision, and large-object references keyed by stable Item/Part identity.                                                                                        |
| `managed_agent_snapshot`          | Consistent Transcript version, `coveredSequence`, active Item state, and terminal state. A watermark alone without recoverable corresponding content is insufficient.                           |
| `managed_agent_consumer_progress` | Contiguous progress for each required projection. Publication progress is separate; Broker ACKs cannot substitute for materialization progress.                                                 |
...
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0: Contracts and baseline                              | Extract business interfaces from `ManagedAgentStore` while preserving behavior. Measure TTFT, events/second, write amplification, reconnect behavior, and source replay boundaries. Run MySQL contract tests first; add the same PostgreSQL matrix when deployment requires it.                                                                                                                                                                 |
| P1: Batching and direct push                            | Update `HarnessCoordinator` and `HarnessEventProjector`; extend stable Item/Part identity; add acceptance transactions, the Hub, and Snapshot materialization. Validate with a SQL scanner before adding MQ.                                                                                                                                                                                                                                    |
