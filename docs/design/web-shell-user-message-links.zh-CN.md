...
| ASCII 白名单而非散文字符黑名单                           | 黑名单两个方向都错：未列出的文字（emoji、泰文）会被吞进 href，被排除的 ASCII 引号会截断含引号的 URL（`/wiki/L'Aquila`）。白名单 fail closed；其边界行为参考了助手消息使用的 remark-gfm，但并不完全一致——已知差异：保留结尾 `~`；ASCII 字母数字前不要求左边界（`xhttps://…` 仍会链接）；对结尾反引号与反斜杠比 GFM 更严格（它们会终止匹配）；配对的 `a[0]` 保留其 `]`。 |
