...
> - `QWEN_CODE_MEMORY_PROJECT_SCOPE=workspace`：按精确 workspace 目录分区项目记忆。`qwen serve` 未显式设置时会注入 `workspace`；standalone CLI 仍默认按 Git 根目录共享。取值会做 trim / 小写归一，无法识别的值会告警一次并回退到 `git-root`。
