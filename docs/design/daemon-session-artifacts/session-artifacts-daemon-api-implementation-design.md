...
`artifact_changed.data.artifact` 必须是完整 `DaemonSessionArtifact`，shape 与 `GET /session/:id/artifacts` 中的单项一致；`removed` event 携带被删除前的最后完整 artifact。这样实时 UI 可以直接应用 event，不需要每条 event 后再 GET。Client 断线、丢 event 或收到未知 event type 时，再用 `GET /session/:id/artifacts` 做 snapshot sync。
...
- SDK 能 list/add，并能识别 `artifact_changed` event。
...
- 增加新的 `kind` / `status` / `source` / `storage` 字面量，但 typed SDK 必须把这些字段声明成 open union，client 必须容忍未知值：未知 `kind` 按 `other`，未知 `status` 按非 ready 状态，未知 `source` 按未分组来源，未知 `storage` 仅按可用 `url` / `workspacePath` 做保守展示。
- 增加新的 route，例如 `GET /session/:id/artifacts/:artifactId`、preview route、delete/pin route。
...
- 删除或改名现有字段。
- 改变 `artifact_changed` 的 `created` / `updated` / `removed` 语义。
...
- 唯一例外是 `storage: 'published'`：`url` 是 primary locator，`managedId` 可作为可选 managed reference 一起返回，用于未来下载/预览；此时 identity 只按 `url` 计算，`managedId` 不参与 identity。
...
- Daemon 应对 `title` / `description` 做 defense-in-depth validation，拒绝明显 HTML/script payload，例如 `<script`、`<iframe`、`javascript:`；client 仍必须把这些字段当作 untrusted text 渲染或 HTML escape。
...
- ACP `Session.runTool()` 中，成功工具结果携带的 artifacts 继续附着到 `tool_call_update._meta.artifacts`；PostToolUse / PostToolUseFailure hook 返回的 artifacts 统一通过 `client.extNotification('qwen/notify/session/artifact-event', payload)` 单独发送。该 notification 必须在 hook artifacts 收集完成后同步 await；发送失败只记录 warning，不改变原工具失败/成功结果。
...
`storage: 'published'` 是唯一例外：它必须携带 `url` 作为 primary locator，可以额外携带 `managedId` 作为 managed reference。published identity 仍按 `url` 计算；`managedId` 只用于未来下载/预览，不参与去重。
...
- 同一次 `upsertMany()` 内先按 identity 合并输入；同 identity 的第一个有效输入作为本批 owner，后续重复输入只按下方安全升级和 metadata 富化规则处理。每个最终 identity 只在 `changes[]` 里产生一条 change。若该 identity 在本批之前不存在则为 `created`，否则为 `updated`。
- 展示字段 `title`、`description`、`source`、`toolCallId`、`toolName`、`hookName`、`extensionId`、`clientId` 采用 first-writer-wins，不被后续同 identity 输入覆盖。
- 资源本体字段允许安全升级：同 URL identity 从 `external_url` 升级到 `published` 时，可以更新 `storage`、补充 `managedId`、更新 `kind` / `mimeType` / `sizeBytes`。该升级只接受 `ArtifactTool` / daemon publisher 产生的 `storage: 'published'` 输入，不能由 hook 或普通 client 伪造。
...
- client POST 同 identity 不覆盖展示字段，但会把该 artifact 的内部 `retentionSource` 提升为 `client`，用于表达用户手动保留意图。
...
- 新建 artifact 导致总量超过 200 时，按以下顺序选择 eviction candidate：
  1. 优先裁剪 `status: 'missing'` 且没有 client retention 的 artifact。
  2. 其次从 `retentionSource` 数量超过 reservation 的来源中裁剪。
  3. 再裁剪没有 client retention 的最旧 artifact。
  4. 如果所有 artifact 都有 client retention，裁剪最旧的 client-retained artifact。
...
- reservation 数值、`retentionSource`、client retention 与 `insertSeq` 是 V1 实现细节，不是 wire protocol 字段；后续可在不改变 API shape 的前提下调整默认值，或增加更细的 per-producer quota。
...
- 同 identity 后续登记不覆盖首次登记者的展示字段；metadata 只允许 Section 7 定义的受控富化，避免跨来源 metadata 注入。

### 9.4 Text Fields

- `title` / `description` 是 plain text，不是 HTML，也不是 markdown。
- Daemon validation 必须做长度、trim、ASCII 控制字符拒绝，并拒绝明显 HTML/script payload，例如 `<script`、`<iframe`、`javascript:`。
- Server-side validation 只是 defense-in-depth；所有 client 仍必须把这些字段作为 untrusted text 渲染或 HTML escape。
...
- 默认 `kind` / `storage` 推断，覆盖 html/image/video/audio/pdf/notebook/file。
- workspacePath / managedId / URL identity 去重，且 identity 不包含 source，跨 source 登记同一资源会合并为一条 artifact。
- 同时携带多个 primary locator 的普通 artifact 被拒绝；`storage: 'published'` 允许 `url + managedId`，identity 只按 `url`。
...
- 同 URL identity 支持 `external_url -> published` 资源本体升级，补齐 `managedId` / `kind` / `mimeType`。
...
- soft reservation eviction：未用额度可借用，missing 优先裁剪，client retention 最后裁剪，`createdAt + insertSeq` 稳定排序，且逐条发送 removed event。
