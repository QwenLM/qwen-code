...
- store 统一执行 validation、normalization、identity 去重、source bucket eviction。
...
- 增加新的 `kind`，但 client 必须把未知 kind 当作 `other` 或通用 link/file 展示。
...
- 调整 source bucket 的内部默认值，只要总上限和 eviction event 语义不破坏现有 client。
...
- 同一次 `upsertMany()` 内先按 identity 合并输入；同 identity 的第一个有效输入作为本批 owner，后续重复输入忽略展示字段和 metadata。每个最终 identity 只在 `changes[]` 里产生一条 change。若该 identity 在本批之前不存在则为 `created`，否则为 `updated`。
- 已存在 artifact 的 `title`、`description`、`kind`、`storage`、`mimeType`、`metadata`、`source`、`toolCallId`、`toolName`、`hookName`、`extensionId`、`clientId` 均采用 first-writer-wins，不被后续同 identity 输入覆盖。
- `status` 和 `sizeBytes` 是 daemon 的 best-effort 派生字段，可以随 workspace stat 刷新；其它展示字段不做跨来源合并。
- `metadata` 只保存首次登记时通过校验的小对象，后续同 identity 输入的 metadata 整体忽略，不做 shallow merge，避免低可信入口注入新 key 或让对象持续增长。
- 实现应在单个 `SessionArtifactStore.upsertMany()` 内同步处理，避免异步读改写竞态。

配额与保留策略：

- 每 session 最多 200 个 artifacts。
- V1 使用固定 source bucket，bucket 按首次成功登记者的 `source` 归属：
  - `tool`: 100
  - `client`: 50
  - `hook`: 50
- 三个 bucket 总和等于 session 总上限，因此不会出现某一类来源填满 200 后让其它来源无法插入的情况。
- 新建 artifact 导致对应 bucket 超限时，只从同一 owner source bucket 中按 `createdAt` 升序驱逐最早创建的 artifact。
...
- bucket 数值是 V1 实现常量，不是 wire protocol 字段；后续可在不改变 API shape 的前提下调整默认值，或增加更细的 per-producer quota。
...
- 同 identity 后续登记不合并 metadata，也不覆盖首次登记者的展示字段，避免跨来源 metadata 注入。

### 9.4 Anti-spam

- 每 session 最多 200 个 artifacts。
- source bucket 默认 `tool: 100`、`client: 50`、`hook: 50`。
...
- 同 identity upsert 采用 first-writer-wins；后续输入不覆盖展示字段、来源字段或 metadata。
- 后续同 identity metadata 被忽略，store 中 metadata 始终满足 4KB 和 primitive-only 约束。
- 同一批内重复 identity 在 `changes[]` 中只产生一条最终 change。
- source bucket 上限按首次登记者 source 生效；某一来源超限只驱逐同来源最早创建的 artifact，且逐条发送 removed event。
...
- 同 identity first-writer-wins 和 source bucket eviction 有单测。
