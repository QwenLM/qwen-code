...
`artifact_changed.data.artifact` 必须是完整 `DaemonSessionArtifact`，shape 与 `GET /session/:id/artifacts` 中的单项一致；`removed` event 携带被删除前的最后完整 artifact。`removed` 必须携带 `reason`，V1 取值为 `eviction` 或 `explicit`；eviction 产生的 removed event 还可以携带 `triggeredByArtifactId`，用于标识触发本轮裁剪的新 artifact。这样实时 UI 可以直接应用 event，不需要每条 event 后再 GET。Client 断线、丢 event 或收到未知 event type 时，再用 `GET /session/:id/artifacts` 做 snapshot sync。
...
- 成功返回 `DaemonSessionArtifactMutationResult`，其中包含一条 `change: 'removed'`、`reason: 'explicit'` 的 change。
- 同步发布对应 `artifact_changed` SSE event。

错误响应复用 Section 4.4 envelope；artifact 不存在返回 `404 NOT_FOUND`。
...
- `ArtifactTool` / daemon publisher：复制 `ToolArtifact` 字段，补 `source: 'tool'`、`toolCallId`、`toolName`、`trustedPublisher: true`。
...
- bridge ingest adapter 基于真实 ArtifactTool / daemon publisher 调用路径设置内部 `trustedPublisher: true`；该标志不从模型参数、hook payload 或 client POST body 读取。
...
- Bridge 必须从 notification transport context 派生 `source: 'hook'`，payload 中的 `source` 只能作为兼容性提示；如果 payload source 与 transport context 不一致，bridge 覆盖为 `hook` 并记录 debug/warning。Notification payload 不能设置 `trustedPublisher`。
...
- 新建 artifact 导致总量超过 200 时，按以下顺序选择 eviction candidate。本批 `upsertMany()` 新创建的 artifact 默认不进入候选池；只有当没有任何旧 artifact 可裁、且本批自身超过上限时，才按同样规则裁本批新 artifact。
...
- 基于 transport context 分配 `source`、`trustedPublisher`、`receivedSeq`；不要信任外部 payload 中的同名字段。
...
- `packages/cli/src/serve/server.ts`
  ...
  - DELETE 调用 bridge 的 `removeSessionArtifact()`。
...
- ArtifactTool 路径会在 bridge ingest adapter 设置内部 `trustedPublisher: true`；`record_artifact` 和其它 tool result 不会设置。
...
- eviction 不会优先裁剪本批刚创建的 missing artifact，除非没有旧 artifact 可裁且本批自身超过上限。
...
- `DELETE /session/:id/artifacts/:artifactId` 返回 `reason: 'explicit'` 的 removed change，并发布对应 SSE event，不删除底层文件或 URL。
