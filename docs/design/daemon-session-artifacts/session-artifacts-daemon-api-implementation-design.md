...
- daemon-managed artifact storage
...
- `ArtifactTool` 发布输出。
- 后续 v1.1 的 `record_artifact` / hook / client POST 显式登记。
...
建议作为 v1.1 或独立 PR 扩展。当前 hooks 已支持 command/HTTP/function/prompt，并且 command/HTTP hook 可以返回 JSON `HookOutput`。建议扩展 `hookSpecificOutput`：
...
v1 不新增 model-visible tool，不注册 `record_artifact`。

### 8.2 v1 cli ACP session
...
v1.1 引入 `record_artifact` 后，skill 或 agent.md 可以写：
...
v1.1 引入 hook artifacts 后，extension 可在 `qwen-extension.json` 或 `hooks/hooks.json` 中提供 PostToolUse hook：
...
- SDK 能 list artifacts，能识别 `artifact_changed`。
...
1. `RecordArtifactTool`
2. hook output artifacts
3. `qwen/notify/session/artifact-event`
4. `POST /session/:id/artifacts`
5. SDK addArtifact
6. managed artifact storage / preview route（如需要）
...
1. `ToolResult.artifacts` / `ArtifactTool` 产生结构化 artifact metadata。
2. `SessionArtifactStore` 做 validation、normalization、enrichment、dedupe。
3. `GET /session/:id/artifacts` 提供 snapshot。
4. `artifact_changed` 提供增量事件。
5. SDK 提供 list 和 event 类型。

v1.1 再按独立 PR 增加三类显式登记入口：
