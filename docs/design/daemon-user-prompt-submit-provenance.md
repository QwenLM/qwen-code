...
Add `submitted_prompt` only when the existing `isFreshUserTurn` classification is true and the captured text is nonblank. Keep the legacy `prompt` value and existing hook execution policy unchanged. Retries may still run legacy hooks but must omit submission provenance; continue, restored question, and runtime goal turns retain their existing hook exclusions. Tool-result, Stop-hook, and background re-entry loops do not create another submitted prompt. Local-only slash commands that return before model execution remain outside this hook path.
...
- Unit tests pin ordinary/multiple text blocks, non-text attachments, trusted model-only content, display projections including empty text, blank input, retry, and continuation behavior.
...
Implemented and verified on 2026-09-09 against `1919ff97f5` plus this working-tree change. The global CLI and pre-fix daemon probes reproduced the missing-field failure. All 869 session tests passed, as did build, bundle, typecheck, lint and formatting. Local and real Holo acceptance each passed all four scenarios using fresh sessions and actual model-input assertions. Both synthetic Holo records were removed and confirmed absent.
