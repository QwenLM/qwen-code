...
policy keeps per-comment dispatch so an actual mention can start the pairing
flow.

The cursor additionally remembers up to 500 dispatched comment node IDs. A
notification ID is deliberately not persisted because GitHub reuses it for
later activity on the same thread.

## Verification

The focused adapter test covers each route, direct-trigger metadata, mention
stripping, aggregate authorization, and comment deduplication.
