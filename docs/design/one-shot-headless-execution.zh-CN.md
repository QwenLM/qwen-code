...
如果这次 relaunch 既不会追加 Node 参数也不会追加脚本参数（默认如此，因为 `advanced.autoConfigureMemory` 默认关闭），并且启动后没有任何 Node 启动期环境变量（如 `NODE_EXTRA_CA_CERTS`）发生变化，那么替换后的镜像只是把同一个进程再启动一遍。此时 CLI 设置 `QWEN_CODE_NO_RELAUNCH` 并原地继续，不再调用 `execve`，每次单次运行因此省掉一整轮模块加载和 settings 处理。
