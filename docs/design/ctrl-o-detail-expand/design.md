...
**qwen 的"新现状底座"= #5661 的 type-based partition 模型（本方案的起点，不是要推翻的旧基线）：** #5661 把工具组默认渲染从"靠 `compactMode` 全显/全隐"演进为 **按工具类型分区折叠**——`forceExpandAll` 为假时，`collapsibleTools`（read/search/list，经 `isCollapsibleTool(name)`，非 Canceled）折叠成 `CompactToolGroupDisplay` 分区摘要行（按 `CATEGORY_ORDER`：command/read/edit/write/search/list/agent/other 聚合，如 `Read 3 files, edited 2 files`），`nonCollapsibleTools`（edit/write/command/agent + Canceled）**始终逐个** `ToolMessage`；force 条件（确认/错误/聚焦 shell/用户发起/终端子代理）令 `forceExpandAll=true` → 全部逐个展开。**该模型与 `compactMode` 无关**——`compactMode` 在 #5661 中已不影响工具渲染（仅残留影响思考块）。本方案在此底座上**保留整个 type-based partition 机制不动**，只删除残留的全局 `compactMode` 开关，并叠加 transcript 的 `fullDetail`（置 `forceExpandAll=true`）。
...
4. `commit 4` — **鼠标点击就地展开工具块**（依赖 #5751 已在 main）：新增 per-tool 展开态 context（`toggleToolExpanded`/`isToolExpanded`），点击命中 → 该工具/组复用 `forceExpandAll=true` + `forceShowResult=true`（§4.8，与 commit 3 的 fullDetail 同一对开关，作用域单工具）。不重做思考块点击（main/#5751 已有）。
...
- **`CompactToolGroupDisplay.tsx`（分区摘要渲染器 + `isCollapsibleTool`）**：导出 `ToolCategory`、`TOOL_NAME_TO_CATEGORY`、`CATEGORY_ORDER`（`command/read/edit/write/search/list/agent/other`）、`getToolCategory`、`buildToolSummary`、以及 **`isCollapsibleTool(name)`**（判定 read/search/list）。本 PR **保留**。
