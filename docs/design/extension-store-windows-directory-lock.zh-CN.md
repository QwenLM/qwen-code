...
destinationDirectory, { recursive: true, force: true })`，并保留符号链接与时间戳
   （`dereference` 保持 false、`preserveTimestamps` 开启）。因为树形结构完全一致，相对目标的
   链接依然有效。
...
拒绝且被挪走的树保持完整；版本之间类型发生变化的条目被对齐后复制得以执行；被打断的备份留下的
`.partial` 目录树被恢复流程删除；策略无法识别的日志被隔离；不带该字段的既有日志保持原有恢复行为。
有一个分支在非 Windows 上无法做单元测试：最后一步移除因为某进程把工作目录设在该目录上而拒绝执行——
它需要真实的操作系统级占用，由 harness 与上面的 `EBUSY` 行覆盖。
