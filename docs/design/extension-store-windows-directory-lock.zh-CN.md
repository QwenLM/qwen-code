...
比较，只是调用按策略分派的回滚。**无法完成的回滚会保留它的日志**，而不是让触发恢复的那次操作失败：
因此一个拒绝接受恢复复制的持有者只让一次操作失败，而不是让所有扩展读取与变更在它释放前全部失败。
...
而这两个码否则既不重试也不解释。`EACCES` 被刻意排除：权限拒绝并不是句柄占用，把它当作占用，
只会把一个权限问题引向一条稍后以另一个错误失败的复制路径。
...
拒绝且被挪走的树保持完整；相对符号链接目标在复制式交换后仍保持相对；无法完成的回滚不阻塞 store；
版本之间类型发生变化的条目
