...
- `GitWorktreeService`（`packages/core/src/services/gitWorktreeService.ts`）：
  ...
  （`removeWorktree`）、启动时孤儿扫描（`sweepStaleWorktrees`）。
...
- `GitWorktreeService.sweepStaleWorktrees()`：启动时清理孤儿 worktree。
...
- **session 异常退出**（daemon 崩溃、kill -9）：worktree 残留在
  `.qwen/worktrees/` 下。已有的 `sweepStaleWorktrees` 在 daemon 重启时
...
- **风险**：worktree 残留占用磁盘。**控制**：session 正常退出时自动清理；
  异常退出靠 `sweepStaleWorktrees` 兜底；Web Shell 可展示残留 worktree
  列表供手动清理（后续增量）。
