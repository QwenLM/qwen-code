...
5. Keep the operation lock across leave and delete. If another navigation supersedes the leave, do not delete a different session or treat that navigation as proof that the expected session detached. The existing New task error reporting should not be duplicated by a second sidebar error.
...
- Live running state disables the current row and is rechecked on confirmation. Cancel or failed leave sends no delete request. Failed deletion retains the row and explains the failure. Other clients and active prompts remain protected by the daemon.
