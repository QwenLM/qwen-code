...
5. 操作锁覆盖离开和删除的全过程。确认启动操作后，对话框保持打开，直至操作结束；期间阻止 Cancel、Esc、遮罩和关闭按钮关闭对话框，避免让进行中的删除看起来已取消。若其他导航取代了离开操作，不能删除另一个会话，也不能把该导航当作预期会话已 detach 的证明。现有 New task 错误提示不应被侧栏重复报告。
...
- 实时运行状态禁用当前行，并在确认时再次检查。确认前取消或离开失败时不发删除请求；离开与删除进行中无法关闭对话框。删除失败时保留行并解释原因。daemon 继续保护其他客户端和活动提示。
