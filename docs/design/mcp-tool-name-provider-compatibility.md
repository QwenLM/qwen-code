...
- Each `DiscoveredMCPTool` advertises `permissionAliases`: the **exact raw identity** `mcp__<server>__<tool>` first, then the legacy `generateLegacyMcpToolName` reduction when it differs. A verbatim provider-safe registration lost nothing and advertises no alias. Both the registry (`ToolRegistry.getPermissionAliases`) and the invocation (`permissionFlow.ts`, for the L4 call-time check) read this same array. The registry-side readers are the L1 enablement gate and the agent-side declaration filters (`agent-core.ts`, `tools/agent/agent.ts`, and the deny narrowing in `workflow-orchestrator.ts`). The L1 gate resolves aliases at two independent sites, not one producer/consumer chain: `coreToolScheduler.ts` reads `ToolRegistry.getPermissionAliases`, while the ACP path's `L1: Tool enablement check` in `Session.ts` reads `tool.permissionAliases` off the `DiscoveredMCPTool` instance itself — so a policy change inside `ToolRegistry.getPermissionAliases` (vouching, filtering, or the empty-array to `undefined` normalization) does not reach the ACP gate. `getToolRegistrationStatus` is threaded too, but only by some of its callers: `PermissionManager.isToolEnabled` and the two agent-config planners (`memory-scoped-agent-config.ts`, `skillReviewAgentPlanner.ts`) forward `toolAliases`, while the registration-time call sites (`config.ts`, `tool-registry.ts`, `managed-tool-runtime.ts`, `Session.ts`) pass the tool name alone. Do not read that parameter as unwired. There is no L2 in this vocabulary — the layers marked in code are L1, L3/L4 (`permissionFlow.ts`), L5 (`coreToolScheduler.ts`) and L5.1-L5.3 (`autoMode.ts`).
