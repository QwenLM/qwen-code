...
Permission rules, `disallowedTools` blocklists, and `disabledTools` entries may be written in a legacy spelling (`mcp__foo.bar__tool`) that no longer equals the registered provider-safe name. Matching works as follows (`packages/core/src/permissions/rule-parser.ts`):
