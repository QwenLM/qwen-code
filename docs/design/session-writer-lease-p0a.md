...
The protocol is gated by `experimental.sessionWriterLease` and is disabled by default. Enabling it requires a restart. The setting affects only ACP/daemon recorders; interactive and headless recorders continue to use the legacy path even when the setting is enabled.
