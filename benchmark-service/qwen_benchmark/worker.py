...
class Worker:
    ...
    def run_once(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                LOGGER.exception("could not write artifact checksums for %s", run_id)
        return True
    ...
    def _failed_summary(self, run_id: str) -> dict[str, Any]:
        ...
        if run["runner_mode"] == "harbor":
            try:
                qwen_version = qwen_version_from_ref(run["qwen_ref"])
            except AgentError:
                pass
        return {
            "run_id": run_id,
    ...
    def _publish(
        ...
        if not current:
            return
        if not self.suites[current["suite"]]["publish"]:
            return
        try:
