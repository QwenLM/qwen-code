...
A microphone-enabled connection closes when the app leaves the foreground, including an active Qwen Live call, even if dictation already stopped. Returning does not automatically resume Live audio. This also applies when another full-screen app opens. Unsent page state may be lost; the prompt explains this before consent, and returning offers manual reconnection. Once the independent file-selection and download slices land, their document Open/Save requests use the preflight guard included here, before launching the chooser: keep editing the current page, or reconnect explicitly and retry the file operation. A blocked Save does not create a destination. Text-only connections remain open. This conservative development-client behavior prevents background capture without a new JavaScript bridge. See the [microphone design](../../docs/design/mobile-microphone-permission.md).
