  ...
  it.each([
    {
      ...
      invoke: (bridge: ReturnType<typeof makeBridge>, sessionId: string) =>
        bridge.branchSession(sessionId, { name: 'stalled-branch' }),
    },
    {
      label: 'session cwd change',
      method: SERVE_CONTROL_EXT_METHODS.sessionCd,
      timeoutMs: 30_000,
      invoke: (bridge: ReturnType<typeof makeBridge>, sessionId: string) =>
        bridge.changeSessionCwd(sessionId, { path: '/next' }),
    },
  ])(
    'retires the channel when a $label RPC exceeds its physical deadline',
    async ({ method, timeoutMs, invoke }) => {
      vi.useFakeTimers();
      const stalledResult = deferred<Record<string, unknown>>();
      const firstHandle = makeChannel({
      ...
      try {
        ...
        const request = invoke(bridge, session.sessionId);
        void request.catch(() => undefined);
        await vi.waitFor(() =>
          expect(firstHandle.agent.extMethodCalls).toEqual(
  ...
  it.each([
    {
      ...
      method: SERVE_CONTROL_EXT_METHODS.sessionBranch,
      invoke: (bridge: ReturnType<typeof makeBridge>, sessionId: string) =>
        bridge.branchSession(sessionId, { name: 'queued-branch' }),
    },
    {
      label: 'session cwd change',
      method: SERVE_CONTROL_EXT_METHODS.sessionCd,
    ...
    'does not run a queued $label against a replacement runtime epoch',
    async ({ method, invoke }) => {
      ...
      try {
        ...
        const prompt = bridge.sendPrompt(session.sessionId, {
          sessionId: session.sessionId,
          prompt: [{ type: 'text', text: 'hold the queue' }],
        });
        void prompt.catch(() => undefined);
        await promptStarted.promise;

        const queued = invoke(bridge, session.sessionId);
        firstHandle.crash();

        await expect(prompt).rejects.toBeInstanceOf(BridgeChannelClosedError);
        await expect(queued).rejects.toBeInstanceOf(SessionNotFoundError);
        expect(channelFactory).toHaveBeenCalledTimes(1);
    ...
    async (operation) => {
      ...
      const firstHandle = makeChannel({
        ...
        loadSessionImpl: async () => {
          stalledStarted.resolve();
          return (await stalled.promise) as LoadSessionResponse;
        },
        resumeSessionImpl: async () => {
          stalledStarted.resolve();
          return (await stalled.promise) as ResumeSessionResponse;
        },
      });
      const secondHandle = makeChannel();
      const channelFactory = vi
        ...
        const request =
          operation === 'new session'
            ...
            : operation === 'load session'
              ? bridge.loadSession({
                  sessionId: 'stalled-load',
                  workspaceCwd: WS_A,
                })
              : bridge.resumeSession({
                  sessionId: 'stalled-resume',
                  workspaceCwd: WS_A,
                });
        void request.catch(() => undefined);
