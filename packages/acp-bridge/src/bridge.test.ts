...
describe('createAcpSessionBridge', () => {
  ...
  describe('sendPrompt', () => {
    ...
    it('stamps titleSource manual on the rename publication (#8977)', async () => {
      // An HTTP rename is by definition a manual title; clients need the
      // source alongside the name to tell a user-chosen session name from
      // an auto-generated one (a /clear carries only manual names over).
      const factory: ChannelFactory = async () => makeChannel().channel;
      const bridge = makeBridge({ channelFactory: factory });
      const session = await bridge.spawnOrAttach({ workspaceCwd: WS_A });
      ...
      bridge.updateSessionMetadata(
        session.sessionId,
        { displayName: 'manually named' },
        { clientId: session.clientId },
      );

      const evt = await firstMeta;
      expect((evt.data as { titleSource?: string }).titleSource).toBe('manual');
