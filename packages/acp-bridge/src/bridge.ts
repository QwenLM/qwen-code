...
export function createAcpSessionBridge(opts: BridgeOptions): AcpSessionBridge {
  ...
  // Last mode explicitly supplied by a daemon caller
  // (`setSessionApprovalMode`, or an explicit `approvalMode` on
  // load/resume, including their live-attach paths — never a `POST
  // /session` create/attach, which may be a client's echo of settings).
  // Survives ACP child reaping; an agent-side or workspace-wide mode change
  // retires it, a definitively rejected replay drops it, a mode applied
  // through a `POST /session` attach retires it, and a deliberate kill
  // clears it. A plan-mode outcome is not replayable (its execution-mode
  // half would be lost), so entering plan retires the entry instead.
  // Sessions without an explicit owner still use cold-load settings.
  const sessionApprovalModeOverrides = new Map<string, ApprovalMode>();
  // Bumped by every retirement of a parked override, so a failed spawn's
  // re-park and a remembered-mode replay can tell "nothing intervened"
  // apart from "deliberately retired" — absence from the map alone cannot
  // (a kill or a workspace-wide clear also remove the entry).
  let approvalModeOverrideRetireEpoch = 0;
  // Per-session retirement generation, bumped by every retiring call for
  // that id — deleting or not — so a failed same-id spawn's re-park can
  // tell "this id was deliberately retired while the spawn was in flight"
  // apart from an unrelated session's retirement, which the bridge-wide
  // epoch cannot.
  const sessionApprovalModeRetireGeneration = new Map<string, number>();
  // Bumped only by a workspace-wide persisted write's clear(): a failed
  // spawn's re-park stays blocked by a workspace supersede without
  // inheriting every unrelated per-session retirement.
  let approvalModeOverrideClearEpoch = 0;
  // The mode the most recent workspace-wide persisted write imposed, paired
  // with the retire epoch at which it landed. A remembered-mode replay
  // whose round trip spans such a write converges to it — the map clear()
  // cannot reach a value the replay already captured.
  let workspacePersistedApprovalMode:
    | { mode: ApprovalMode; epoch: number }
    | undefined;
  // `expected` scopes the retirement to the value one caller observed: a
  // newer mode parked meanwhile belongs to someone else and stays.
  const retireApprovalModeOverride = (
    sessionId: string,
    expected?: ApprovalMode,
  ): void => {
    if (
      expected !== undefined &&
      sessionApprovalModeOverrides.get(sessionId) !== expected
    ) {
      return;
    }
    // Bump on every retiring call — deleting or not. A deliberate
    // retirement of an already-empty id (e.g. a kill landing after a
    // same-id spawn pre-deleted the park) must still block that spawn's
    // re-park.
    sessionApprovalModeRetireGeneration.set(
      sessionId,
      (sessionApprovalModeRetireGeneration.get(sessionId) ?? 0) + 1,
    );
    if (sessionApprovalModeOverrides.delete(sessionId)) {
      approvalModeOverrideRetireEpoch++;
    }
  };
  const rememberApprovalModeOverride = (
    ...
    if (mode === 'plan') {
      retireApprovalModeOverride(sessionId);
      return;
    }
      ...
      try {
        client = new BridgeClient(
          ...
          (entry, modeId, originator, planExecutionMode) => {
            ...
            // An agent-originated mode change retires the parked override:
            // the remembered mode no longer reflects the session's owner.
            // Bridge-initiated round trips never reach this callback (the
            // demux suppresses them), so the replay cannot delete itself.
            // Confined to an actual change — a no-op re-announcement (a
            // settings reload converging to the session's current mode
            // emits one) must not retire a caller-owned override. The
            // cache can also lag the child (an inconclusive replay failure
            // deliberately keeps it stale), so an announcement OF the
            // parked mode confirms the memory rather than contradicting
            // it.
            if (
              modeId !== se.currentApprovalMode &&
              sessionApprovalModeOverrides.get(se.sessionId) !== modeId
            ) {
              retireApprovalModeOverride(se.sessionId);
            }
  ...
  async function applyApprovalMode(
    ...
    originatorClientId?: string,
    planMode?: boolean,
    // Re-validated when the queued work dequeues, not when the caller
    // enqueues it: a convergence decision made at enqueue time cannot see
    // an explicit selection whose own apply is queued ahead of this one.
    skipIf?: () => boolean,
  ): Promise<{
    sessionId: string;
    mode: ApprovalMode;
    ...
    const approvalWork = entry.approvalModeQueue.then(async () => {
      if (skipIf?.() === true) {
        // The dequeue-time precondition turned this apply into a no-op:
        // report the cached state without a round trip and without
        // touching the round-trip flag or the reconcile.
        // `currentApprovalMode` is only ever written from validated
        // ApprovalMode values (publishApprovalModeChanged).
        const cached =
          (entry.currentApprovalMode as ApprovalMode | undefined) ?? mode;
        return {
          sessionId: entry.sessionId,
          mode: cached,
          previous: cached,
      ...
      try {
        ...
        // A workspace-wide persisted write supersedes every parked
        // override this bridge holds — including sessions already reaped
        // out of `byId`, which is the whole reason they are parked. The
        // retirement keys on the write alone: a `planMode`-form write stores
        // the same settings value. The marker lets a remembered-mode replay
        // whose round trip spans this write converge to the mode it imposed.
        if (persisted) {
          sessionApprovalModeOverrides.clear();
          approvalModeOverrideRetireEpoch++;
          approvalModeOverrideClearEpoch++;
          workspacePersistedApprovalMode = {
            mode,
            epoch: approvalModeOverrideRetireEpoch,
          };
        }
  ...
  async function applyApprovalModeForAttach(
    ...
    mode: ApprovalMode,
    clientId: string,
  ): Promise<{ previous: ApprovalMode; current: ApprovalMode }> {
    try {
      const result = await applyApprovalMode(entry, mode, false, clientId);
      return { previous: result.previous, current: result.mode };
    } catch (err) {
    ...
    try {
      ...
      if (target === 'model') {
        ...
        if (actual && !KNOWN_APPROVAL_MODES.has(actual)) {
          ...
          // The corrective observes a mode change the demux suppressed
          // (e.g. an approved plan exit during a bridge round trip): the
          // session left the remembered mode, so retire the parked
          // override with it. Confined to the corrective branch — this
          // reconcile also runs after a successful replay, which must not
          // drop its own override. Park-relative like the demux path: a
          // corrective that reads back the very mode that is parked (the
          // cache can lag the child after an inconclusive replay failure)
          // confirms the memory rather than contradicting it.
          if (sessionApprovalModeOverrides.get(entry.sessionId) !== actual) {
            retireApprovalModeOverride(entry.sessionId);
          }
          publishApprovalModeChanged(
            entry,
            {
  ...
  async function restoreSession(
    ...
    if (existing) {
      ...
      if (appliedApprovalMode !== undefined && existing.restoreState?.modes) {
        // Source from the live cache like the cold path below: a newer
        // mode-carrying load that completed inside the
        // `applyRestoreSourceIfMissing` round trip above owns the entry
        // now, and the captured `appliedApprovalMode` would silently
        // revert it for every later attacher reading this snapshot.
        existing.restoreState = {
          ...existing.restoreState,
          modes: {
            ...existing.restoreState.modes,
            currentModeId: existing.currentApprovalMode ?? appliedApprovalMode,
          },
        };
      }
      return {
    ...
    if (inFlight) {
      ...
      if (appliedApprovalMode !== undefined && entry.restoreState?.modes) {
        // Same live-cache sourcing as the byId attach branch above.
        entry.restoreState = {
          ...entry.restoreState,
          modes: {
            ...entry.restoreState.modes,
            currentModeId: entry.currentApprovalMode ?? appliedApprovalMode,
          },
        };
      }
      return {
    ...
    const promise = (async (): Promise<BridgeRestoredSession> => {
      ...
      try {
        ...
        restoreEvents.close();
        if (isAcpSessionResourceNotFound(err, req.sessionId)) {
          retireApprovalModeOverride(req.sessionId);
          if (
            !ci.isDying &&
      ...
      if (racedEntry) {
        ...
        if (req.approvalMode) {
          try {
            ...
            appliedApprovalMode = result.mode;
            // Remember at apply time, not after the restore's remaining
      ...
      if (
        ...
        } else if (!entry.approvalModeRoundtripInFlight) {
          // A remembered mode replays best-effort: this request never asked
          // for it, so a rejecting child (trust gate, timeout, transport
          // close) must not fail the restore — a hard failure here
          // re-tombstones the id and leaves the session unrestorable until
          // daemon restart. Drop the override and complete on the child's
          // cold-load mode. No originator is published: no client drove the
          // change (same rule as the reconcile corrective). Calling
          // `applyApprovalMode` directly — not `applyApprovalModeForAttach`
          // — keeps the client registration intact on failure.
          const replayRetireEpoch = approvalModeOverrideRetireEpoch;
          try {
            ...
            // A workspace-wide persisted write landing mid-round-trip
            // supersedes the value this replay already captured — the
            // map clear() cannot reach a closure value. Converge the live
            ...
            // must not disable the convergence, and a marker left by an
            // older write must not converge a newer replay to a stale
            // workspace mode.
            if (
              superseding !== undefined &&
              superseding.epoch > replayRetireEpoch &&
              superseding.mode !== restoreApprovalMode
            ) {
              try {
                await applyApprovalMode(
                  entry,
                ...
                writeStderrLine(
                  `qwen serve: converging session ${JSON.stringify(req.sessionId)} to the persisted workspace approval mode after a superseded replay failed: ${supersedeErr instanceof Error ? supersedeErr.message : String(supersedeErr)}`,
                );
              }
            }
            writeStderrLine(
            ...
            if (
              err instanceof TrustGateError ||
              isDefinitiveAcpRequestError(err)
            ) {
              // A definitive rejection. Forget only the value this replay
              // actually attempted — a concurrent explicit change may have
              // parked a newer mode while the round trip was in flight.
              retireApprovalModeOverride(req.sessionId, restoreApprovalMode);
              writeStderrLine(
                `qwen serve: replaying the remembered approval mode for session ${JSON.stringify(req.sessionId)} failed; dropping it and restoring on the cold-load mode: ${err instanceof Error ? err.message : String(err)}`,
              );
            } else {
              // A timeout or transport close has an unknown outcome — the
              // child may already hold the replayed mode. Keep the memory
              // so the next cold restore retries it.
              writeStderrLine(
                `qwen serve: replaying the remembered approval mode for session ${JSON.stringify(req.sessionId)} failed without a definitive answer; keeping it for the next cold restore: ${err instanceof Error ? err.message : String(err)}`,
              );
              // The failed round trip's own finally skipped the
              // post-roundtrip reconcile (reason=roundtrip_failed), so
              // re-read the child here: when it turns out to hold the
              // replayed mode, the corrective publishes the
              // `approval_mode_changed` frame the protocol doc names as
              // authoritative for a stale cold-load snapshot.
              void reconcileAfterRoundtrip(entry, 'approvalMode');
            }
          }
        } else {
          // An explicit `setSessionApprovalMode` is mid-round-trip on this
          // entry and parks its own child-confirmed mode when it resolves.
          // Replaying the value captured before its round trip would revert
          // the newer selection in the live child.
          writeStderrLine(
            `qwen serve: skipping the remembered approval mode replay for session ${JSON.stringify(req.sessionId)}: an explicit approval-mode change is already in flight`,
          );
        }
        assertAttachableSessionEntry(req.sessionId, entry);
  ...
  const bridgeApi: AcpSessionBridge = {
    ...
    async spawnOrAttach(req) {
      ...
      if (effectiveScope === 'single') {
        ...
        if (existing) {
          ...
          // client's echo of the settings-derived mode. Remembering stays
          // on load/resume attaches and `setSessionApprovalMode`.
          return {
            sessionId: existing.sessionId,
            workspaceCwd: existing.workspaceCwd,
            ...(existing.effectiveCwd !== existing.workspaceCwd
      ...
      // A caller-supplied id may still have an override parked from the
      // id's previous incarnation; the fresh spawn supersedes it. Clear
      // BEFORE doSpawn registers the entry — after `await promise` this
      // delete could clobber an override a concurrent
      // `setSessionApprovalMode` recorded while the spawn was in flight.
      const supersededApprovalMode =
        req.sessionId !== undefined
          ? sessionApprovalModeOverrides.get(req.sessionId)
          : undefined;
      const approvalModeRetireGenerationAtSpawn =
        req.sessionId !== undefined
          ? (sessionApprovalModeRetireGeneration.get(req.sessionId) ?? 0)
          : 0;
      const approvalModeClearEpochAtSpawn = approvalModeOverrideClearEpoch;
      if (req.sessionId !== undefined) {
      ...
      try {
        ...
        // The spawn never produced a session, so nothing superseded the
        // previous incarnation's parked mode — restore it unless a
        // concurrent `setSessionApprovalMode` parked a newer one while
        // the spawn was in flight, or a deliberate retirement (a
        // workspace-wide persisted write, a kill) landed in the same
        // window. Absence from the map cannot tell those apart; the
        // per-session retire generation and the workspace-clear epoch can
        // — an UNRELATED session's retirement must not block this
        // re-park.
        if (
          req.sessionId !== undefined &&
          supersededApprovalMode !== undefined &&
          !sessionApprovalModeOverrides.has(req.sessionId) &&
          (sessionApprovalModeRetireGeneration.get(req.sessionId) ?? 0) ===
            approvalModeRetireGenerationAtSpawn &&
          approvalModeOverrideClearEpoch === approvalModeClearEpochAtSpawn
        ) {
          sessionApprovalModeOverrides.set(
            req.sessionId,
            supersededApprovalMode,
          );
        }
        throw error;
      } finally {
    ...
    async killSession(sessionId, opts) {
      ...
      if (entry.closing) {
        ...
        // The kill is committed from here on. A killed session's persisted
        // id stays restorable, so the parked override dies only with a
        // destruction the CALLER owns (`retireRememberedApprovalMode` — the
        // user-facing delete paths); daemon-internal cleanup kills (an
        // undeliverable restore response, internal recovery) preserve it
        // for the next cold restore, exactly like an idle reap or crash.
        if (opts?.retireRememberedApprovalMode === true) {
          retireApprovalModeOverride(sessionId);
        }
        await killChannelWithLog(
          closingChannel,
      ...
      if (defaultEntry === entry) defaultEntry = undefined;
      byId.delete(sessionId);
      // Surviving child reaps must not extend to every kill: only a
      // caller-owned destruction (`retireRememberedApprovalMode`) drops
      // the parked override with the session.
      if (opts?.retireRememberedApprovalMode === true) {
        retireApprovalModeOverride(sessionId);
      }
      telemetry.metrics?.sessionLifecycle('die');
