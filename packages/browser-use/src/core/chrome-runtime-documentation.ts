...
- press("Enter"), press("Control+a") and type("text") send real keyboard events. type() inserts at the current selection without clearing existing text; use fill() to replace a value. If typing produces no observable change, type() raises INPUT_BLOCKED so the target state can be inspected before continuing.
