...
- Screenshots return JPEG bytes, mimeType and metadata with the original image dimensions, viewport, device pixel ratio and CSS-pixel coordinate space. In one node_repl call, capture const shot = await tab.screenshot(), output nodeRepl.write(JSON.stringify(shot.metadata)), then await nodeRepl.emitImage({ bytes: shot.bytes, mimeType: shot.mimeType }).
