...
describe('runtime shell policy admission', () => {
  ...
  it('snapshots the canonical policy and both host state roots', () => {
    ...
    expect(Object.isFrozen(policy.protectedRoots)).toBe(true);
    expect(policy.protectedRoots).toContain(path.join(root, 'runtime'));
    expect(policy.protectedRoots).toContain(path.join(root, 'config'));
  });

  it('admits normal interactive sessions', () => {
    expect(
