...
/**
 * Body of the auto-generated `<projectRoot>/.qwen/.gitignore`. The trailing
 * `/.gitignore` keeps the file itself out of `git status`; see
 * `ensureWorktreesGitignored` for why that line is load-bearing.
 */
export const WORKTREES_GITIGNORE_BODY = `# Auto-generated by qwen-code.\n${WORKTREES_DIR}/\n/.gitignore\n`;

/**
 * The body this file had before it ignored itself. Only a byte-identical
 * match is upgraded in place, so a user-edited file is never rewritten.
 */
export const LEGACY_WORKTREES_GITIGNORE_BODY = `# Auto-generated by qwen-code.\n${WORKTREES_DIR}/\n`;
...
export class GitWorktreeService {
   ...
   * the file, so a previously-clean repository starts reporting `?? .qwen/`.
   * Every caller that fail-closes on a dirty parent — the workflow tool's
   * `isolation:'worktree'` provisioning, AgentTool's — then refuses every
   * provision after the first, blaming uncommitted changes the user never
   * made. The pattern is anchored (`/.gitignore`) so it hides only this file,
  ...
  async ensureWorktreesGitignored(): Promise<void> {
    try {
      ...
      try {
        ...
        if (isNodeError(error) && error.code === 'EEXIST') {
          // Upgrade a file this method wrote before it ignored itself, so
          // existing checkouts stop reporting a dirty parent. Byte-equality
          // against the old body is the whole guard: anything a user touched
          // differs and is left exactly as it is.
          await this.upgradeLegacyWorktreesGitignore(gitignorePath);
          return;
        }
        throw error;
      }
  ...
  /** @see ensureWorktreesGitignored — best-effort, never throws. */
  private async upgradeLegacyWorktreesGitignore(
