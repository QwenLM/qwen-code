...
const PROJECT_ROOT = '/mesh-workspace-lock-test';
...
function runWorker(runtimeDir: string, count: number): Promise<number[]> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(tsx, [worker], {
      env: {
        ...process.env,
        MESH_LOCK_RUNTIME_DIR: runtimeDir,
        MESH_LOCK_PROJECT_ROOT: PROJECT_ROOT,
        MESH_LOCK_COUNT: String(count),
      },
...
describe('mesh workspace lock', () => {
  let runtimeDir: string;

  beforeEach(async () => {
    runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mesh-lock-test-'));
  });
