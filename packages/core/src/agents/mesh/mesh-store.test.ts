...
const PROJECT_ROOT = '/mesh-store-test-project';
...
describe('mesh versioned store', () => {
  let runtimeDir: string;

  beforeEach(async () => {
    runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'mesh-store-test-'));
  ...
  it('fails closed on a newer workspace schema', async () => {
    await readMeshWorkspace(PROJECT_ROOT);
    await writeRaw(getWorkspaceFilePath(PROJECT_ROOT), {
      schemaVersion: MESH_SCHEMA_VERSION + 1,
      workspaceId: 'ws_newer',
      nextRunSequence: 1,
    });

    await expect(readMeshWorkspace(PROJECT_ROOT)).rejects.toBeInstanceOf(
      MeshSchemaVersionError,
    );
  });
  ...
  it('fails closed on a malformed current workspace schema', async () => {
    ...
    await expect(readMeshWorkspace(PROJECT_ROOT)).rejects.toThrow(
      /Malformed mesh workspace record/,
    );
  });
  ...
  it('fails closed on a newer agents schema', async () => {
    ...
    await expect(readMeshAgents(PROJECT_ROOT)).rejects.toBeInstanceOf(
      MeshSchemaVersionError,
    );
  });
  ...
  it('migrates v0 agents and thread records in order', async () => {
    ...
    const workspace = await readMeshWorkspace(PROJECT_ROOT);
  ...
  it('does not silently drop a malformed v0 agent', async () => {
    ...
    await expect(readMeshAgents(PROJECT_ROOT)).rejects.toThrow(
      /malformed v0 agent/,
    );
    expect(
  ...
  it('keeps the v0 backup until the migrated file validates', async () => {
    ...
    await expect(readMeshAgents(PROJECT_ROOT)).resolves.toEqual([ALICE]);
  ...
  it('reads a hand-written complete v1 fixture', async () => {
    ...
    await expect(readMeshAgents(PROJECT_ROOT)).resolves.toMatchObject([
  ...
  it('rejects nested workspace transactions instead of deadlocking', async () => {
    await expect(
      withMeshStoreTransaction(PROJECT_ROOT, () =>
        readMeshAgents(PROJECT_ROOT),
      ),
    ).rejects.toThrow(/Nested mesh workspace transactions/);
  });
  ...
  it('persists a run counter allocation before any thread write', async () => {
    ...
    await expect(readMeshWorkspace(PROJECT_ROOT)).resolves.toMatchObject({
      nextRunSequence: 3,
    });
  });
