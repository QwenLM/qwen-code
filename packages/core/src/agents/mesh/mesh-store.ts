...
const LOCK_OPTIONS: lockfile.LockOptions = {
  retries: { retries: 10, minTimeout: 5, maxTimeout: 100, factor: 2, randomize: true },
  stale: 10_000,
};
...
async function withFileLock<T>(
  ...
  try {
    ...
    await atomicWriteJSON(filePath, filePath.endsWith(AGENTS_FILENAME) ? [] : {}, {
      noFollow: true,
    });
  }
...
/**
 * Drops the oldest posts and runs past the retention bounds. The thread body
 * and the newest history are what a resumed agent is shown, so age is the
 * right axis — but this is lossy, and a thread that hits the bound has
 * outgrown a single work item.
 */
function trimThread(thread: Thread): Thread {
  ...
  return {
    ...
    runs: thread.runs.slice(-MAX_THREAD_RUNS),
  };
}
