 ...
 *
 * A mention is the routing signal for the whole mesh: it decides who is woken
...
import { findAgentByName } from './mesh-store.js';
 ...
 */
export function mentionToken(agent: MeshAgent): string {
  return `@${agent.name}`;
}
