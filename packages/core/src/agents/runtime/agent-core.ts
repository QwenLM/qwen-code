  ...
  async prepareTools(): Promise<FunctionDeclaration[]> {
    ...
    if (this.toolConfig) {
      ...
      if (
        ...
        // remain hidden and are reached through the ToolSearch + ToolCall
        // bridge, preserving the allowlist's schema shrink without mutating
        // the declarations.
        toolsList.push(
          ...toolRegistry
      ...
      toolsList.push(
        ...onlyInlineDecls.filter((d) => {
          if (
            ...
            isExcluded(d.name) ||
            isHiddenByEagerAllowList(d.name)
          ) {
            this.runtimeContext
              .getDebugLogger()
              ?.debug(
