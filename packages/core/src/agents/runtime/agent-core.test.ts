...
describe('AgentCore.processFunctionCalls loop-detector result feed', () => {
  // The loop detector's result-aware guards must see representation-stable
  // parts (issue #9450): a frozen board whose batch oscillates around the
  // toolOutputBatchBudget boundary would otherwise alternate between a raw
  // JSON-verbatim fingerprint (under-budget batch) and a digest-reduced
  // batch-budget-fit fingerprint (over-budget batch) — two representations
  // of identical content that never collide, so every poll counts as
  // "changed" and no result-aware halt ever fires.

  const BOARD = 'task row for a frozen board\n'.repeat(80); // ~2.2KB
  const SIBLING_OUTPUT = 'sibling payload line\n'.repeat(100); // ~2.1KB
  const TASK_LIST_ARGS = { board: 'shared' };
  // BOARD fits the budget solo; BOARD + SIBLING_OUTPUT exceeds it, so
  // alternating solo/co-batched rounds oscillate across the fit boundary.
  // Both stay far below the per-result truncation threshold, so executed
  // parts carry the raw board text (no scheduler persistence).
  const BATCH_BUDGET = 3000;

  const fnDecls: FunctionDeclaration[] = [
    { name: 'task_list', description: 'list tasks' } as FunctionDeclaration,
    { name: 'big_sibling', description: 'big sibling' } as FunctionDeclaration,
  ];

  function buildAgentForExecutedTools(tmpDir: string): {
    core: AgentCore;
    config: Config;
  } {
    const boardTool = new MockTool({
      name: 'task_list',
      execute: async () => ({
        llmContent: BOARD,
        returnDisplay: 'board',
    ...
    const siblingTool = new MockTool({
      name: 'big_sibling',
      execute: async () => ({
        llmContent: SIBLING_OUTPUT,
        returnDisplay: 'sibling',
      }),
    });
    const byName = new Map<string, MockTool>([
      [boardTool.name, boardTool],
      [siblingTool.name, siblingTool],
    ]);
    const registry = {
      getTool: (name: string) => byName.get(name),
      ensureTool: async (name: string) => byName.get(name),
      getAllToolNames: () => [...byName.keys()],
      getFunctionDeclarations: () => [],
      warmAll: async () => undefined,
    ...
    const config = {
      ...
      getSessionId: () => 'session-loop-feed',
      ...
      getPlanFilePath: () => path.join(tmpDir, 'plan.md'),
      getToolOutputBatchBudget: () => BATCH_BUDGET,
      getToolResultBytesWritten: () => 500 * 1024 * 1024,
      getMaxSubagentDepth: () => 5,
      getSkipLoopDetection: () => false,
      getMaxToolCallsPerTurn: () => 100,
  ...
  const toolCallRequestEvent = (
    ...
    value: {
      ...
      prompt_id: 'prompt-loop-feed',
    },
  });

  it('feeds representation-stable parts to the guards across the budget boundary', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-loop-feed-'));
    try {
      const { core } = buildAgentForExecutedTools(tmpDir);

      const solo = await runWithAgentContext('loop-feed', () =>
        core.runInAgentFrames(() =>
          core.processFunctionCalls(
            [taskListCall('tl-solo')],
            new AbortController(),
            'prompt-loop-feed',
            1,
            fnDecls,
      ...
      const coBatched = await runWithAgentContext('loop-feed', () =>
        core.runInAgentFrames(() =>
          core.processFunctionCalls(
            [
              taskListCall('tl-co'),
              { name: 'big_sibling', args: { step: 1 }, id: 'sib-co' },
            ],
            new AbortController(),
            'prompt-loop-feed',
            2,
      ...
      const visibleOutput = (
        ...
        const parts = ((messages as Array<{ parts?: unknown[] }>)[0]?.parts ??
          []) as Array<{
          functionResponse?: {
            id?: string;
        ...
        const part = parts.find((p) =>
          p.functionResponse?.id?.endsWith(callIdSuffix),
        );
        return part?.functionResponse?.response?.output ?? '';
      };
      // Sanity: the budget actually ran — the over-budget co-batch fitted
      // the model-visible board result while the solo batch kept it raw.
      expect(visibleOutput(coBatched.messages, 'tl-co')).toContain(
        BATCH_BUDGET_FIT_PREFIX,
      );
      expect(visibleOutput(solo.messages, 'tl-solo')).not.toContain(
        BATCH_BUDGET_FIT_PREFIX,
      );

      // The guard feed must not depend on batch composition: identical
      // content fingerprints identically whether its batch fitted or not.
      const soloBoard = solo.results.find((r) => r.toolName === 'task_list');
      const coBoard = coBatched.results.find((r) => r.toolName === 'task_list');
      expect(soloBoard).toBeDefined();
      expect(coBoard).toBeDefined();
      expect(fingerprintToolResult(soloBoard!.responseParts)).toBe(
        fingerprintToolResult(coBoard!.responseParts),
      );
    } finally {
  ...
  it('halts a frozen board polled across the fit boundary (oscillating batches)', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-loop-feed-'));
    try {
      const { core, config } = buildAgentForExecutedTools(tmpDir);
      const loopDetector = new LoopDetectionService(config);
      loopDetector.reset('session-loop-feed#loop-feed-subagent');
      const handledToolCallFingerprints = new Map<string, string>();
      const duplicateProviderToolCallResponseIds = new Set<string>();

      let halted = false;
      let rounds = 0;
      for (let round = 0; round < 10 && !halted; round++) {
        rounds++;
        const coBatched = round % 2 === 1;
        // Request-time feed, exactly as the reasoning loop streams events.
        const events = [
          toolCallRequestEvent('task_list', TASK_LIST_ARGS, `tl-${round}`),
        ];
        if (coBatched) {
          events.push(
        ...
        for (const event of events) {
          if (
            loopDetector.checkAlwaysOnSafeties(event) ||
        ...
        const result = await runWithAgentContext('loop-feed', () =>
          core.runInAgentFrames(() =>
            core.processFunctionCalls(
              calls,
              new AbortController(),
        ...
        // Result-time feed, exactly as the reasoning loop records results.
      ...
      // The result-aware global-duplicate guard fires on the 6th identical
      // frozen result. Pre-fix, the raw/fitted representation oscillation
      // judged every poll "changed" and no guard fired within these rounds.
