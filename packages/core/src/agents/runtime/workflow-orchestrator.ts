...
function workflowDisallowedTools(
  baseConfig: SubagentConfig,
  requestedDenies: readonly string[] = [],
): string[] {
  return Array.from(
    new Set([
  ...
  return async (prompt, opts, dispatchId) => {
    ...
    if (opts.extensions !== undefined) {
      ...
      const skillInstruction = selectedExtensionHasSkills
        ? await resolveWorkflowSkillInstruction(
            config,
            agentIdentity,
            resolveDispatchDenies(opts.disallowedTools),
          )
        : '';
      prompt +=
  ...
  if (agentIdentity.resolvedAgentType) {
    ...
    const runtimeConfig = await config
      ...
      .convertToRuntimeConfig(
        {
          ...baseConfig,
          disallowedTools: workflowDisallowedTools(baseConfig, requestedDenies),
        },
        config,
      );
    ...
    disallowedTools = Array.from(
      new Set([...WORKFLOW_SUBAGENT_DISALLOWED_TOOLS, ...requestedDenies]),
    );
  }
