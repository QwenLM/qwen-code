...
export class BackgroundTaskRegistry {
  ...
  private emitStatusChange(entry?: BackgroundTaskEntry): void {
    // Evict before notifying so snapshot-style consumers (those that
    // re-pull `getAll()` from inside the callback) immediately see
    // the trimmed roster — otherwise a freshly-completed entry
    // arriving alongside the cap-overflow eviction would render one
    // frame with the old + new entries before the next refresh
