...
function config(options: {
  ...
  return {
    ...
    getSessionId: () => 'agent-host-session',
...
describe('agent launcher', () => {
  ...
  it('converts the definition and starts the fixed agent runtime identity', async () => {
    ...
    expect(launchProgrammaticBackgroundAgent).toHaveBeenCalledWith(
      expect.anything(),
      { description: 'Workspace agent alice', prompt: 'go' },
      expect.objectContaining({
        agentId: 'agent-ag_alice',
  ...
  it('returns agent_unavailable without creating a runtime', async () => {
    await expect(
      launchWorkspaceAgent(config({}), AGENT, 'go'),
    ).resolves.toEqual({
