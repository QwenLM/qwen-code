...
const READ_ONLY_ROOT_COMMANDS = new Set([
  'awk', 'basename', 'cat', 'cd', 'column', 'cut', 'df', 'dirname', 'du', 'echo',
  'find', 'git', 'grep', 'head', 'ls', 'printenv', 'ps', 'pwd', 'sed', 'stat',
  'tail', 'wc', 'which', 'where', 'whoami',
]);
...
const BLOCKED_GIT_REMOTE_ACTIONS = new Set([
  'add', 'remove', 'rm', 'rename', 'set-branches', 'set-head', 'set-url', 'prune', 'update',
]);
...
function containsWriteRedirection(command: string): boolean {
  ...
  for (const char of command) {
    ...
    if (char === '\\' && !inSingleQuotes) { escapeNext = true; continue; }
    if (char === "'" && !inDoubleQuotes) { inSingleQuotes = !inSingleQuotes; continue; }
    if (char === '"' && !inSingleQuotes) { inDoubleQuotes = !inDoubleQuotes; continue; }
    if (!inSingleQuotes && !inDoubleQuotes && char === '>') return true;
  }
  return false;
}
...
function normalizeTokens(segment: string): string[] {
  ...
  for (const token of parsed) {
    ...
    else if ('op' in token && token.op === 'glob') tokens.push(`\0${token.pattern}`);
  }
  return tokens;
}

function skipEnvironmentAssignments(tokens: string[]): { root?: string; args: string[] } {
  let index = 0;
  while (index < tokens.length && ENV_ASSIGNMENT_REGEX.test(tokens[index]!)) index++;
  if (index >= tokens.length) return { args: [] };
  return { root: tokens[index], args: tokens.slice(index + 1) };
}
...
function evaluateFindCommand(tokens: string[]): boolean {
  ...
  for (const token of rest) {
    ...
    if (BLOCKED_FIND_PREFIXES.some((prefix) => lower.startsWith(prefix))) return false;
  }
  return true;
}
...
function evaluateSedCommand(tokens: string[]): boolean {
  ...
  for (const token of rest) {
    if (
      ...
      token === '--in-place' || token.startsWith('--in-place=') || token === '-f' ||
      token === '--file' || (token.startsWith('-f') && token.length > 2) ||
      token.startsWith('--file=') || (token.startsWith('-') && !SAFE_SED_OPTION.test(token))
    ) return false;
  }
...
function evaluateGitRemoteArgs(args: string[]): boolean {
  ...
  if (action === 'show' && !options.some((arg) => ['-n', '--no-query'].includes(arg.toLowerCase()))) return false;
  for (const arg of options) if (BLOCKED_GIT_REMOTE_ACTIONS.has(arg.toLowerCase())) return false;
  return true;
}
...
function evaluateGitCommand(tokens: string[]): boolean {
  ...
  if (subcommand === 'remote') return evaluateGitRemoteArgs(args);
  if (subcommand === 'branch') return evaluateGitBranchArgs(args);
...
function evaluateShellSegment(segment: string): boolean {
  ...
  if (detectCommandSubstitution(segment)) return false;
  ...
  if (normalizedRoot === 'find') return evaluateFindCommand([normalizedRoot, ...args]);
  if (normalizedRoot === 'sed') return evaluateSedCommand([normalizedRoot, ...args]);
  if (normalizedRoot === 'awk') return evaluateAwkCommand([normalizedRoot, ...args]);
  if (normalizedRoot === 'git') return evaluateGitCommand([normalizedRoot, ...args]);
  return true;
}
...
export function isShellCommandReadOnly(command: string): boolean {
  ...
  for (const segment of segments) if (!evaluateShellSegment(segment)) return false;
