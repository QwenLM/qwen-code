...
export async function updateEnvironmentContext(config: Config): Promise<Part[]> {
  const directoryContext = await getDirectoryContextString(config);

  const context = `
Please note that my working directory has changed.
Here is the updated folder structure of the current working directories.
${directoryContext}
        `.trim();
