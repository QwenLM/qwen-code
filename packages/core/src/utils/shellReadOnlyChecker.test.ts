...
describe('git config probe cd tracking (#8575)', () => {
  ...
  beforeAll(() => {
    root = fs.mkdtempSync(path.join(os.tmpdir(), 'shell-regex-cd-tracking-'));
  ...
  it('probes the post-cd repository', () => {
    ...
    expect(
      isShellCommandReadOnly('cd ../dirty-repo && git status', {
        cwd: cleanRepo,
      }),
    ).toBe(false);
    expect(
  ...
  it('keeps same-repo cd read-only', () => {
    expect(
  ...
  it('downgrades git after an unresolvable cd', () => {
    ...
    expect(isShellCommandReadOnly('cd && git status', { cwd: cleanRepo })).toBe(
      false,
    );
  });
