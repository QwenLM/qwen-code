...
describe('retryWithBackoff — network error handling', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    setSimulate429(false);
    console.warn = vi.fn();
  ...
  it('should NOT retry network errors by default (normal mode)', async () => {
    const mockFn = vi.fn(async () => {
  ...
  it('should retry network errors when shouldRetryOnError includes isNetworkError (yolo mode)', async () => {
    const { isNetworkError } = await import('./rateLimit.js');
    let attempts = 0;
    const mockFn = vi.fn(async () => {
      attempts++;
      if (attempts <= 2) {
    ...
    const promise = retryWithBackoff(mockFn, {
      maxAttempts: 3,
      initialDelayMs: 10,
      shouldRetryOnError: (error) => isNetworkError(error),
    });
