...
export class Config {
  ...
  private reasoningPreference: false | ReasoningEffort | undefined;
  ...
  constructor(params: ConfigParameters) {
    ...
    if (
      ...
      const reasoning = params.generationConfig?.reasoning;
      this.reasoningPreference =
        reasoning === false ? false : reasoning?.effort;
    }
  ...
  async refreshAuth(authMethod: AuthType, isInitialAuth?: boolean) {
    ...
    this.restoreReasoningPreference();
   ...
   * Registered routes reject non-native tiers; generic provider adapters keep
   * their existing clamping. Pass undefined to use the provider default.
   *
  ...
  setReasoningEffort(effort: ReasoningEffort | undefined): void {
    ...
    if (
      effort &&
      ((configuration &&
        (configuration.toggleOnly ||
          !configuration.efforts.includes(effort))) ||
        (!configuration &&
          !supportsGenericReasoningEffort(activeConfig?.model)))
    ) {
      return;
    }
    if (effort && activeConfig?.reasoning === false) {
      return;
    }
  ...
  setReasoningDisabled(disabled: boolean): void {
    ...
    const configuration = activeConfig
      ...
      : undefined;
    const apply = (reasoning: ContentGeneratorConfig['reasoning']): void => {
      const set = (
        config: { reasoning?: ContentGeneratorConfig['reasoning'] } | undefined,
      ): void => {
        if (config) config.reasoning = reasoning;
      };
      set(this.contentGeneratorConfig);
      const runtimeConfig =
        getRuntimeContentGenerator()?.contentGeneratorConfig;
      if (runtimeConfig && runtimeConfig !== this.contentGeneratorConfig) {
        set(runtimeConfig);
      }
      set(this.modelsConfig?.getGenerationConfig());
    };
    if (
      disabled &&
      (activeConfig?.thinkingMandatory === true ||
    ...
    this.reasoningPreference = disabled ? false : undefined;
    apply(disabled ? false : undefined);
  }

  private restoreReasoningPreference(): void {
    const preference = this.reasoningPreference;
    if (preference === undefined || !this.contentGeneratorConfig) return;
    const configuration = resolveModelReasoningConfiguration({
      modelId: this.contentGeneratorConfig.model,
    ...
    if (preference === false) {
      if (
        ...
        configuration?.canDisable === false ||
        (!configuration &&
          !supportsGenericReasoningEffort(this.contentGeneratorConfig.model))
      ) {
        this.reasoningPreference = undefined;
        return;
      }
    ...
    if (
      (configuration &&
        (configuration.toggleOnly ||
          !configuration.efforts.includes(preference))) ||
      (!configuration &&
        !supportsGenericReasoningEffort(this.contentGeneratorConfig.model))
    ) {
      this.reasoningPreference = undefined;
    ...
    this.setReasoningEffort(preference);
  }
  ...
  private async handleModelChange(
    ...
    // Full refresh rebuilds the config and restores the tracked preference.
