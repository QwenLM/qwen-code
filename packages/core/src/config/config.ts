...
export class Config {
  ...
  private pendingImageModelReload?: { value: string | undefined };
  ...
  stageImageModelReload(model: string | undefined): void {
    this.pendingImageModelReload = { value: model };
  }

  async applyPendingModelProvidersReload(): Promise<boolean> {
    const pendingImageModelReload = this.pendingImageModelReload;
    try {
      const applied = await this.modelsConfig.applyPendingModelProvidersReload(
        async (selection) => {
