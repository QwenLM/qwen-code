...
export class Config {
  ...
  private startMcpDiscoveryInBackground(): void {
    ...
    this.mcpDiscoveryPromise = manager
      ...
      .then(async () => {
        ...
        // MCP tools into the active LlmChat so the next model request sees
  ...
  async createToolRegistry(
    ...
    const registerLazy = async (
      ...
      // (#9827) without the tool silently disappearing (#10075). The
      // tool_search/deferred_tool_call pairing below treats a deferred
      // registration as registered — deferred tools stay discoverable.
      let status: ToolRegistrationStatus = 'registered';
      try {
        status = this.permissionManager
