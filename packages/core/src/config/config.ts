...
export class Config {
  ...
  async createToolRegistry(
    ...
    const registerLazy = async (
      ...
      // still registered — listed in `/tools` and loadable via the
      // ToolSearch + ToolCall bridge — but its schema stays out of the eager
      // model request (#9827) without the tool silently disappearing
      // (#10075).
      let status: ToolRegistrationStatus = 'registered';
      try {
