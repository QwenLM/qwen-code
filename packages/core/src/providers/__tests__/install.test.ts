...
describe('applyProviderInstallPlan', () => {
  ...
  it('replaces an attributable baseUrl-less legacy model when its id is requested at its endpoint', async () => {
    const bBaseUrl = 'https://b.example/v1';
    const bEnvKey = generateCustomEnvKey(AuthType.USE_OPENAI, bBaseUrl);
    // The entry's key is endpoint B's original suffix-less shape, so the
    // entry is attributable to B: requesting its id at B replaces it.
    const legacyModel = {
      id: 'legacy-model',
      name: 'legacy-model',
  ...
  it('removes a deselected baseUrl-less legacy model under historical key shapes (R39-3)', async () => {
    const baseUrl = 'https://new.example/v1';
    const envKey = generateCustomEnvKey(AuthType.USE_OPENAI, baseUrl);
    // Real upgraded-user shapes: the same endpoint's key as generated by
    // earlier releases — the original suffix-less shape and the 6-hex
    // suffix shape. Deselecting them at their own endpoint must remove
    // them exactly like the current-shape entry.
    const originalShape = {
      id: 'legacy-original',
