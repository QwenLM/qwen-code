...
Budget: <user's advisory stopping agreement; otherwise stop as blocked after 20 turns, and mark that default [ASSUMPTION] in Context; use model.goalMaxTurns or model.goalMaxActiveMinutes outside the objective for a runtime-enforced ceiling>
...
- `Budget` is a stopping agreement for the model, not a runtime-enforced turn or wall-clock limit. Do not claim that writing it configures a timer or changes the Goal token budget. Name `model.goalMaxTurns` or `model.goalMaxActiveMinutes` when the user needs an enforced ceiling. Preserve a user-specified budget; otherwise mark the default `[ASSUMPTION]` in Context.
