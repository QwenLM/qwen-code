...
**Do not check the coverage. It is checked for you, from what the agents actually did.**

You no longer copy the agents' returns anywhere. Run:
...
It reads the **harness's own per-agent transcripts** — the record the CLI writes for every subagent, which you do not author, are not given the path to, and cannot revise. It reports three things, and they are different failures:

- **Agents launched blind.** A chunk agent whose launch prompt never named the diff file could not have read it. **Do not relaunch it as it was** — the second one is as blind as the first. Rebuild its prompt with `qwen review agent-prompt` and launch it with that.
...
**It exits 3 when the diff was not covered, and you may not proceed to Step 4 on a non-zero exit.** Its report is not passed to Step 7 — `compose-review` recomputes the same thing from the same transcripts, so there is nothing for you to carry, and nothing for you to get wrong.
