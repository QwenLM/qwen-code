...
All confirmed findings (from aggregation + all reverse audit rounds) proceed to Step 6. An entry still tagged `— [unverified]` when the loop ends is not among them: the last round's verification completes before Step 6, so a tag that survives means a verifier failed — treat that entry as low-confidence (terminal-only, "Needs Human Review"), never as confirmed.
