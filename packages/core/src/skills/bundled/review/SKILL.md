...
**What you must still judge: the image's content.** The command can check extensions and sizes; it cannot see that a terminal screenshot has an env dump in the scrollback. Publish only evidence the review itself produced — and `qwen review capture-tui` is the sanctioned producer for terminal renderings: it drives the command in a **private tmux server** (structurally unable to see the user's own sessions), captures `.ans` always and `.png` when `freeze` is available, and manifests which evidence rung it reached. Never publish a capture of the user's own terminal or editor. When in doubt, keep the finding's evidence as prose and local paths.
