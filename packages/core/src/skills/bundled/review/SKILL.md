...
Every agent reads the whole diff, **by walking the `chunks[]` ranges**. **Do not write that part of the prompt. Ask for it:**

```bash
qwen review agent-prompt --plan <the plan report from Step 1> --whole-diff \
  [--rules <the rules file from Step 2, if the project has any>]
```

Paste what it prints **verbatim** at the top of **every** agent's launch prompt, then add that agent's own dimension brief below it. It carries the diff path, one `read_file` line per chunk, the paging rule, the finding format, the severity definitions and the project rules — the things a launch cannot be allowed to forget, because a launch that forgets them is not detectable from the outside afterwards. Do **not** ask for the whole diff in one read: `read_file` caps a single call at ~25 000 characters. A chunk whose `maxLineChars` exceeds the read cap holds a line no paging can reach, and the agent must say so rather than review what it happened to receive.

**Coverage is proven for this topology too — see Step 3D.** It is not proven by receipts (Step 3A has none, and must not: every dimension agent walks every chunk, so "one receipt per chunk" would demand eleven). It is proven from the lines each agent was **pointed at** in the prompt the CLI built, and the harness's record of that agent **opening the diff**. Which is why the block above is not optional: an agent given a diff path and no line ranges covers nothing, and the check will say so.
...
**Verbatim means copy, not retype, and Step 3D now checks it.** The command records what it printed, and `check-coverage` compares that against the prompt the harness recorded the agent being launched with. This is not a hypothetical: dogfooded, a run invoked this command correctly for all five chunks of a PR — and then **paraphrased what it printed** before launching the agents. The delivered prompt kept the `read_file` line (so every other check passed it) and dropped the instruction not to recite a stock sentence, dropped the warning about half-read ranges, replaced the project's review rules with a three-sentence summary of its own, and invented an instruction that was never in the original. You may wrap the block; you may not edit it.
...
**Their prompts are built in code too. Do not write the diff-reading half of them:**
...
Paste that block **verbatim** at the top of each one (except Agent 7, whose job is to run the build, and Agent 0, whose job is to read the issue — neither reads the diff), then add its dimension brief below.
...
It reads the harness's own per-agent transcripts: a record you do not author, are not given the path to, and cannot revise. It reports five failures, and they are not the same:
