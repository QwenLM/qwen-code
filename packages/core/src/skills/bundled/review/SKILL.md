...
**At medium effort, cap the territory fan-out and drop the diff-specialists.** If the Step 1 plan has more than ~8 chunks, re-run `plan-diff` with a larger `--max-chunk-lines` (e.g. double the default) so the territory coarsens into roughly eight chunk agents — but never so large that a chunk's `maxLineChars` crosses the ~25 000-char uncoverable ceiling (Step 1); if the diff cannot fit into ≤8 coverable chunks, keep the finer plan (skipping the reverse audit is still the main saving). Do **not** launch the Agent 8 diff-specialists. The whole-diff agents (Agent 0, 1b, 1c, Agent 7, the invariant agents, the test-coverage matrix) run exactly as in high — they are the cross-chunk safety net medium keeps. Everything else about 3B is identical.
...
"${QWEN_CODE_CLI:-qwen}" review check-coverage \
  ...
  [--effort medium]   # medium only — matches the reduced roster you launched
```

**At medium effort, pass `--effort medium`** — the same value you gave `agent-prompt --roster`. It tells the gate to require the balanced set (no 6a/6b/6c), so a medium review is not flagged for the personas it deliberately did not run. Omit the flag at high (the full roster is required).
