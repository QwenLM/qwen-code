...
The second clause is an attention bound, not a risk one: past roughly 3200 diff lines, asking twelve agents each to read the whole diff dilutes them all, and the chunk topology's base cost (`ceil(diffLines / 400) + 5` before invariant and specialized agents) crosses twelve about there. It is not a guarantee of fewer calls — a heavy file adds `3` invariant agents and a dominant domain up to `2` specialized finders, so a barely-over-the-line changeset can cost more under 3B than 3A; what 3B buys at that size is one accountable reader per line instead of twelve diluted ones. It is the safety valve for a changeset dominated by tests or generated files.
...
gh api --paginate repos/{owner}/{repo}/pulls/{n}/reviews \
  --jq '.[] | select(.body | contains("[Critical]")) | {id, user: .user.login, state, body}' \
  > .qwen/tmp/qwen-review-{target}-crit-reviews.ndjson
```

Three things here are load-bearing and none is optional. **`--paginate`**: the endpoint returns 30 entries per page, and a body-only blocker on page 2 would otherwise never reach the gate. **`--jq` filtering to marker-bearing bodies**: it keeps unrelated review prose out of your context. And **one record per line** — the part a plain redirect gets wrong: `gh api --paginate` emits _compact_ JSON, so the file lands as a single enormous line (on a review-heavy PR, north of 150 KB of it). `read_file` truncates that line, and a larger `offset` skips past it to EOF — so "read it whole" becomes unsatisfiable, the fail-closed branch never fires, and the gate passes over a blocker it could not read. A per-object `--jq` filter emits **line-delimited** records, which page normally.
...
- **The `event`/`body` invariant, checked as arithmetic before you submit.** The two rules above are prose, they are each stated twice, and live reviews violate both — because at submit time a model is reasoning about what it wants to say rather than about what it counted. So stop reasoning and count. Let `C` be the number of Critical findings the review is posting **anywhere** — entries in `comments` plus any blocker whose only copy sits in `body` (a last-resort unmappable or 422-relocated Critical counts toward `C` exactly like an anchored one) — and `S` the number of Suggestions in `comments`. Before the downgrade flags are applied:
  ...
  **Composing the body when states stack.** Several rules each set "the" `COMMENT` body — the downgrade sentence, the context-unavailable cap, the could-not-anchor disclosure, the uncoverable-chunks disclosure — and review has shown that any two of them can co-occur. No rule may erase another's disclosure. Build the final body from these clauses, **in this order, each present iff its condition holds, joined into one short paragraph, and nothing else**:
  ...
  7. **Body Criticals** (the carve-out): the relocated/unmappable Critical descriptions, only on a `REQUEST_CHANGES` or a downgraded-from-it `COMMENT`.
...
If there are **no confirmed findings**, submit a short summary review. Use `event=APPROVE` by default; if the presubmit JSON has `downgradeApprove=true`, use `event=COMMENT` and prepend the downgrade reason to the body. A run carrying the **context-unavailable** state from Step 1 (lightweight `pr-context` failed) may never submit `APPROVE`, findings or not: submit `event=COMMENT` with the body from the **composition rule** above (here that reduces to clause 2 alone, plus the footer); the invariant table's APPROVE row simply does not apply to such a run. Separate the footer from the body with a blank line so it renders on its own line — `-f body` does not interpret `\n`, so use a real line break inside the quotes:
