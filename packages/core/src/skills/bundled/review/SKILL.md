...
- **The `body` invariant, checked before you submit.** The rule above is prose, it is stated twice, and reviews still violate it, because a model holding a finding it could not anchor would rather say it somewhere than drop it. So make it mechanical. For `COMMENT`, `body` is **exactly one** of the three sentences above, followed by the model footer, and **nothing else** — no second paragraph, no "Also:", no relocated Suggestion. For `APPROVE` it is `No issues found. LGTM! ✅` plus the footer. For `REQUEST_CHANGES` it is `""`. Read the `body` you are about to send and confirm it matches. If it does not, the extra text is a finding you failed to anchor: a Critical belongs in `body` and nothing else does, so if it is a Suggestion, **delete it** — it is already in the terminal output and the Step 8 report, where the author will see it without it becoming a public review paragraph that no line of code answers to.
