...
A `C=0` outcome — Approve, or a Comment with no Critical — is a claim that nothing blocks the merge. It is not the default you fall back to when your own agents surfaced nothing. Before you commit to it, take **each live `**[Critical]**` already on the PR — from the context file's "Open inline comments" section, from "Already discussed", AND from "Review summaries"** — and check it against the code as it stands at the reviewed commit. Review-level bodies matter because an unmappable or 422-relocated blocker lives **only** there, and `pr-context` truncates each summary to ~240 characters: when a summary shows a `**[Critical]**` marker (or is cut where one could hide), fetch the full body with `gh api repos/{owner}/{repo}/pulls/{n}/reviews` before ruling on it — never rule on a truncated blocker. A reply alone does not retire a blocker: `pr-context` moves a thread to "Already discussed" on **any** reply, and "I disagree" or "wontfix" is a reply. Only the code decides — a replied-to Critical counts as closed exactly when the re-check below lands on "fixed by this diff", never because the thread has an answer. Record one verdict per Critical:
...
- **The `event`/`body` invariant, checked as arithmetic before you submit.** The two rules above are prose, they are each stated twice, and live reviews violate both — because at submit time a model is reasoning about what it wants to say rather than about what it counted. So stop reasoning and count. Let `C` be the number of Critical findings in `comments` and `S` the number of Suggestions. Before the downgrade flags are applied:
  ...
  A **context-unavailable** run (Step 1's lightweight fallback) caps **every** `C=0` row, not just the empty one: the `APPROVE` row becomes `COMMENT` with body `Reviewed diff-only — the PR's existing discussion could not be fetched, so this is not an approval.`, and the Suggestion-only row's body becomes `Reviewed diff-only — existing discussion unavailable, so this is not a no-blockers claim. Suggestions are inline.` A run that could not see the PR's open threads may post its findings, but it may never certify the absence of blockers — neither as an LGTM nor as a "no blockers" sentence. `C≥1` rows are unaffected (blockers stand on their own evidence).
