...
**What the hunks cover is a stated scope, not a certification.** `fix-delta --since` records what `git add -A` records in the reviewed repository — tracked files, and untracked files no ignore rule hides — and prints that scope on every run, with one detected exception beside it: a HEAD that moved between the two moments, since a change that landed by commit alone leaves no hunk. An edit inside a submodule or a nested repository, to a gitignored file HEAD does not track (one HEAD tracks is re-hashed like any tracked file), or to any path in the review's `.qwen/tmp` name families, tracked or not, is named as outside the scope rather than probed for; so is the fact that a hunk shows a file as git stores it — a binary file as `Binary files … differ`, a Git LFS file as its pointer.
