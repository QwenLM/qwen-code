...
**What `fix-delta` will not certify.** The hunks are the audit's whole input, so the command's second job is to say when they are not the whole edit — the failure direction over-warns, and it never silences a blind spot. Four surfaces get that treatment. **Content the superproject tree cannot record**: a submodule or nested repository enters the tree as its gitlink alone, so an edit inside one moves nothing — the command probes each and records, per path it can answer for, a DIGEST of the state it found: the uncommitted entries AND the repository's identity (HEAD, branch, stash count). The first half makes "already dirty at snapshot time" a checkable claim rather than a boolean (a bare boolean let a level-2 gitlink that merely moved stamp its parent, after which a real edit inside the parent was filed as dirt that was already there); the second half is why clean paths carry a digest too — a fix COMMITTED or stashed inside a repository an ignored directory hides leaves its status empty at both moments and the superproject trees record nothing of it, and the moved identity is the one trace it leaves. Paths the probe cannot answer for at all — an unreadable directory, a repository whose status will not run, an ignored tree past the walk budget — are disclosed rather than assumed clean, and never enter the baseline. What the probe prunes it prunes by what the ORCHESTRATOR says, never by anything in the tree: a review worktree is a path this run created and named with `--review-worktree`, walked for planted repositories rather than skipped; every other repository — a `git init` under the `review-pr-*` name, a gitfile pointing into git's worktree registry, a directory recreated over a stale registry entry, a concurrent PR review's genuine worktree — is content and is probed, its dirt disclosed. Each tree-side classification was an entrance in turn (the family name, then the registry, then the registry with a back-link), because each is something the reviewed code can write, and the orchestrator's own arguments are the one thing it cannot; a `--fix` run names no worktree at all. A link that resolves to the audited repository itself, or a walk that re-enters it through a link to an ancestor, is recognised by filesystem identity (`dev`/`ino`, never a resolved spelling — a case-insensitive filesystem resolves a differently-cased link to a different string for the same directory) and never probed as its own nested repository — its edits are exactly what the tree comparison sees. The comparison is a closed 4×4 matrix: at each moment every probed path is exactly one of DIRTY (a digest, and named dirty), CLEAN (a digest), UNRESOLVED (no digest — persisted in the snapshot for disclosure, never as a baseline) or ABSENT, and each of the sixteen cells routes to one disclosure or to the all-clear (dirty→dirty-same is pre-existing; dirty→dirty-moved, clean→dirty, unresolved→dirty and absent→dirty are fresh dirt; dirty→clean and dirty→absent are cleaned; clean→clean-moved is the interior-moved note; unresolved→clean and absent→clean appeared; clean→absent and unresolved→absent vanished; anything→unresolved is the unresolved disclosure), so a repository created between the moments with the fix committed inside, a baseline repository renamed away, or a path the snapshot could not answer for that is gone since, is disclosed rather than dropped for lacking a second digest. A status-discovered path whose interior probe fails is UNRESOLVED, never "named dirty with no digest" — a cell the matrix does not have. A nested repository's own ignored entries (`--ignored=matching`) ride its digest but are not dirt — `dist/`, `node_modules/` are the everyday shape of a repository git itself reports clean — so an ignored entry appearing or vanishing is disclosed by the interior-moved note; a nested repository's collapsed entries take the top level's routing entry for entry — a collapsed directory that IS a repository is probed (the everyday `.gitignore` names the vendored checkout itself), one that merely holds repositories is walked, an untracked directory git will not descend into is the same shape, a slashless entry may be a link reaching a repository, and the interior index's own mode-160000 gitlinks take the root's index-sweep ruling (absent checkout skipped, checkout without a git dir unresolved, checkout with one probed) — so a level-2 repository is a working tree nested in this one with a cell of its own, and an ignored file's bytes are outside the model at every level. A nested repository whose merged config (read with `--includes --show-scope -z`, so an `include.path` is followed and a value holding a newline cannot forge a scope) carries a `filter.*.clean`/`.process` key in any scope but the user's own (`global`, `system`) — its own config, an `include.path`/`includeIf` it pulls in, its per-worktree file — is unresolved, never certified — a repo-local `git lfs install --local` in a nested checkout is that shape, and is disclosed on every run: no `-c` neutralises a filter wired by attributes, and a single-scope read would not follow includes. A global filter bound by a nested repository's own attributes is disclosed once, by the root's steering note off the merged config, not re-probed per repository. The walk has a per-walk budget and a per-run cap of ten walks' worth, charged per DIRECTORY OPENED — a repository is a directory holding `.git`, so an opened directory is the unit of discovery, while the plain files a directory holds are 90% of its entries and none of the search. Charged per entry, this repository's own `node_modules` (8,174 directories, 84,566 entries) spent the ceiling on every run, so `node_modules` was disclosed as unresolvable every time and the all-clear could never print; a disclosure that always fires is one nobody reads. The walk is also bounded to the audited repository, by filesystem identity rather than by a resolved spelling and on the path's BYTES: a directory link whose target lies outside the root is named as SCOPE — content out there is outside this model the way a gitignored file's bytes are — and never WALKED, so no enumeration spends the run's budget on a tree the capture does not record; it is probed all the same, on every route that reaches it, and the scope bucket is what keeps a commit inside it (nothing a fix could have made) from reading as a move inside this tree. Scope does not spend the all-clear, since `npm link`, a `file:../sibling` dependency and a hoisted package store put one in ordinary runs and a qualification that fires every time is one nobody reads; a link that was NOT there at the snapshot does, because the fix may have made it. The maps are null-prototyped so a repository named `__proto__` is an ordinary key. The scope is stated with the all-clear: the model is the working tree and the working trees nested in it — a gitignored file's edit, or anything inside a git directory, is outside every working tree and is not something the command claims to see. The same distinction reaches the capture: the families are excluded because the flow writes them UNTRACKED, so the tracked half of a family match is user content and is recorded after the exclusion has run. What counts as the flow's own inside a family is DERIVED, from the plan path the orchestrator already carries — the plan file and everything under its prompt-record directory, wholesale — rather than enumerated by name: the record keys are bare (`chunk-8`, `run-sessions.json`) as often as they carry a content digest, so a suffix list and a digest-shaped regex classified most of what the directory holds as user content. The staged half of a family match, which nothing structural separates from a user's own staged note, fails closed instead of guessing: the second capture re-includes only the paths the first one re-included — recorded in the fingerprinted snapshot — and names on stderr what it held back, because a family path that entered the index BETWEEN the moments got there by a `git add` this flow never made. The tree comparison's rendering probe asks `check-attr diff` about a rename's SOURCE name as well as its folded new name, because git renders the pair off the old name's attribute. The capture is additionally pinned to `core.autocrlf=false` (with `core.safecrlf=false`, whose round-trip check means nothing for a throwaway index) so both moments store the bytes on disk and an edit confined to line endings stays visible; the nested-repository probe's status is pinned to `core.quotePath=true`, so a non-UTF-8 name is C-quoted to ASCII before the digest is taken and two interior states cannot collapse into one hash; the nested-repository probes keep git's own view of "modified" — pinning them would make every text file of an `autocrlf=true` checkout read permanently modified and file a real content edit as pre-existing dirt — so an edit confined to line endings inside a nested repository, and normalisation wired by a `text`/`eol` attribute in a tracked `.gitattributes`, are the two shapes this does not close. A third is `core.ignoreCase`: on a case-insensitive filesystem a case-only rename is invisible to git itself — `status` shows nothing and a commit records nothing — and the key is true in every repository git creates there, so a disclosure keyed on it would print on every macOS run; by git's own definition that rename is outside "the content `git add -A` captures". On a case-SENSITIVE filesystem a `true` is never git's own, and under it `add -A` folds a case-colliding sibling into the tracked name — both captures identical, a bare all-clear over the edit — so every spawn that measures a working tree — the capture's and the nested-repository probes' alike, since a nested repository's own `status` folds the same sibling into silence and, unlike `autocrlf`, the pin changes nothing for a legitimate repository on such a filesystem — pins `core.ignoreCase=false` wherever that repository's filesystem tells `Aa` from `aA` — probed read-only, once per root, by lstat'ing the tree's own `.git` entry under the spelling `.GIT` (ENOENT, a different inode, or an inode the filesystem cannot vouch for is a sensitive filesystem; the same verifiable inode is not — provided the entry could not have been given a second name: a directory, or a gitfile with a link count of one, since `ln .git .GIT` shares the inode on a sensitive filesystem), on the volume `add -A` walks rather than the git dir's, since a linked worktree's git dir sits under the main checkout and a `--separate-git-dir` wherever the user put it; a probe that cannot run pins — and leaves the key alone elsewhere. Hooks are pinned off with the config — `core.hooksPath` at a path that cannot exist, on every spawn that measures a working tree — because a `post-index-change` hook inherits the capture's throwaway `GIT_INDEX_FILE` and can reset the index being recorded, and a hook that merely prints makes `assertCompleteCapture` refuse a capture that was complete. A family-named path the user staged without committing is captured from the user's own index as well, since the throwaway index seeded from HEAD does not hold it and `add -u` cannot reach it. **Every spawn is pinned against the tree it measures**, the discipline `worktree.ts` states for its tripwire — the probe children inside discovered repositories and the outer `read-tree`/`add`/`write-tree`/`status`/`ls-files`/`check-attr` against the audited repository alike, since the audited repository's `.git/config` is the same writable surface one level up: `-c core.fsmonitor=`, because `core.fsmonitor` runs a command from that config (measured: on git 2.43 `add`/`status`/`ls-files`/`write-tree` all executed a planted hook; on 2.55 `write-tree` alone did), and `--work-tree <path>`, because a repo-local `core.worktree` otherwise redirects the command at a pristine decoy. The one plant a pin cannot meet — a `core.worktree` in the audited repository that steers `rev-parse --show-toplevel` itself — the pin's own argument — is met by a gate at root derivation: the tree git names must contain the directory the command runs in AND belong to the repository discovered from it (a `core.worktree` naming an enclosing checkout passes containment while switching the whole measurement to the outer repository), and a root that fails either is refused, disclosed by the flow as `Fix audit: not run`. A `.git` entry planted INSIDE the working tree is the same class from the other side: one `sub/.git` reading `gitdir: <repo>/.git` narrows every reading to `sub` — git answers `--show-toplevel` there, the honest walk stops at the plant, and both git-dir readings agree — so the run is refused when an enclosing checkout answers for the same git dir, which no legitimate submodule (`<super>/.git/modules/<name>`) or linked worktree (`<common>/worktrees/<name>`) does. That gate is config-blind since round 18: the honest working tree is the directory of the first `.git` at or above the working directory that git's own discovery would accept (a gitfile, or a directory holding `HEAD`, `objects/` and `refs/` — an empty `.git` directory or a dangling `.git` link is climbed past, as git climbs past it), and the derived root must be that directory, so a subtree `core.worktree` and an enclosing directory holding a planted gitfile back to this repository are both refused. The side-path redirect check runs again immediately before each write — the capture executes the repository's filters, which are disclosed, not refused — and covers the whole ancestor chain of an in-repository side path, bounded by the checkout. **Repo-local config that shapes the capture and the rendering** cannot be overridden the same way, so it is named instead: a `filter.<name>.clean` or `filter.<name>.process` decides what `add -A` stores for a path (a real edit absent from both trees, or bytes the worktree never held attested as an edit), `.git/info/exclude` and `core.excludesFile` drop a new file from the capture (and an ignore rule that APPEARS between the moments — the `.gitignore` line a fix writes — makes the second capture read a still-present file as deleted, so a deletion whose path git now reports IGNORED and whose file is still on disk is dropped from the hunks and disclosed instead of reaching the auditor as an edit the fix never made — the ignore test is what keeps a file the fix REPLACED with a directory of the same name in the hunks, since that path is a deletion and a real edit at once), and a `diff` attribute resolving to `-diff` or a driver turns a text fix's hunks into an opaque binary patch the auditor cannot read. Each is read fresh at BOTH moments — every one of these files is plantable between the snapshot and the `--since` — and printed on stderr beside the blind-spot lines; Step 6B has the orchestrator repeat every such line under the **Fix audit** heading, so the qualification is disclosed with the audit rather than certified past. A configured filter also changes how the capture's own `git add` notes are ruled on: the filter's child shares git's stderr, so once one is configured every note is judged on its own line — no multi-line reassembly, because a forged opener could otherwise absorb git's real failure notes into one tolerated shape. The single-line zero-commit pairing is kept even then: git never skips a path on the "unable to index file" note alone (the specific failure is printed first and stays unexplained), so a forged pair explains nothing away, while dropping it would refuse the capture for every Git-LFS user — whose filter keys are global — over a freshly initialised repository on git ≥ 2.55. **And the baseline record itself is anchored outside the tree's write surface.** The snapshot file lives inside the tree at a path the reviewed code can write to and is excluded from both captures, so a rewritten `tree` field would diff the fix against a baseline fabricated after a hidden edit; `--snapshot` therefore prints a SHA-256 fingerprint of the record, the orchestrator keeps it, and `--since` refuses the file without it or when the bytes no longer match — the orchestrator's own argument construction being the one channel a planted process cannot rewrite. **The hunks `--since` writes carry the same anchor, for the same reason**: they are the audit's whole input, they land in the same writable directory, and a SEPARATE process reads them back, so `--since` prints `hunks fingerprint <hex>` on the empty-diff path as well as the non-empty one, and `agent-prompt --role fix-audit` requires `--hunks-fingerprint` and refuses a file that no longer matches it — which is the only thing that makes a forgery keeping every `diff --git` header byte-identical while substituting the bodies detectable at all. The second capture is seeded from the SNAPSHOT'S OWN TREE, never from HEAD: seeded from HEAD at both moments, the comparison also carried every path the seed's tracked set gained or lost between the moments (a commit in the window — a tracked path untracked, an ignored path force-added) and every path an ignore rule written or deleted in the window admitted to or hid from `add -A`, and a classifier had to guess, per record, whether the comparison or the fix had made it — an entrance a round at a time (a rule that hides an embedded repository's gitlink, a HEAD that moved before the question was asked). Seeded from the first tree, an entry the first capture recorded is an entry the second one holds whatever the rules say now (ignore rules never touch an index entry), so a deletion in the hunks is a path gone from the disk and nothing else; what the seed cannot carry — a path HEAD tracked that was absent at the snapshot and is on disk again — is re-admitted off the snapshot's recorded HEAD, and a rule REMOVED in the window, which admits pre-existing content the first capture never saw, is still ruled on against the snapshot's record of what its rules hid — what it hid is recorded by KIND — files, symlinks, and nested repositories (`ls-files --others` prints one as `sub/` and does not descend; a staged-but-uncommitted one is printed without the slash and is told from a file only by its index mode) on lists of their own — and an addition is the rule's removal only when its kind matches what stood there: a blob where a file was, a link where a link was, a gitlink where a repository was. Any other pairing is a replacement the fix made and rides the hunks. The staged half of the family re-inclusion hands `add -f` only a path that lstats — git dies on any other pathspec — and names what it left out; a gitlink under a family name is never re-included by either half, tracked or staged (an explicit gitlink pathspec to `add -u` or `add -A -f` runs a status inside the checkout, measured): one the seed holds is refreshed by the seed pass, one only the index holds is in neither tree, and the blind-spot probe answers for the interior of either — named on stderr, once per gitlink. The re-admission skips a path that now sits behind a link, inside a repository the capture just recorded as a gitlink, or under a component that became a plain file (git refuses each pathspec outright, and the link, the gitlink or the file is what the capture records), and a gitlink whose `.git` git does not accept as the checkout's own — a hollow directory, a gitfile naming an enclosing checkout's git dir — stands rather than being refreshed off the HEAD `rev-parse` would find by walking up; a removed checkout is removed with a null OID as long as the repository's object names (64 hex under `--object-format=sha256`). The scratch index lives in the git dir, resolved as bytes with only git's one trailing newline removed (a git dir's name can hold one); a git dir whose name is not valid UTF-8 cannot be spelled into `GIT_INDEX_FILE`, so the scratch falls back to the system temp directory when that is outside the working tree, and refuses otherwise. A commit in the window is then not an edit the trees can carry: the snapshot records HEAD, and a HEAD that moved — or a record that names none, or a recorded commit the repository no longer holds — is disclosed and withholds the bare all-clear, because a change that landed by commit alone leaves no hunk. The capture runs inside no nested repository at all: `git add -A` decides whether a gitlink is modified by running a status INSIDE the checkout, a child that reads and executes that checkout's own config (a `filter.*.clean|process` under `.git/modules/<name>/config`, a `core.fsmonitor` command, its hooks — measured, and no `-c` on the parent reaches it), so the seed's gitlinks are excluded from `add -A` and refreshed by hand off `rev-parse HEAD`, entry for entry as git rules them (gone: removed; replaced by a file: the replacement; empty, dead, unreadable, unborn, corrupt: the entry stands and the probe answers for it — where `add -A` would have died on the corrupt one and cost the whole capture), and both discovery statuses run `--ignore-submodules=all`, which spawns nothing, while every gitlink is probed by the index sweep after its own config has been screened. A gitlink whose checkout exists EMPTY — the directory a clone creates, or a non-recursive `submodule update --init` leaves at level 2 — holds nothing an edit could hide in and is skipped like an absent one, not disclosed on every run. A nested repository's skip-worktree bits are read the way `local-anchor.ts` reads the root's: an out-of-cone tracked path of a sparse checkout is tagged by design and absent, exempt only when git's own `sparse-checkout check-rules` says it is out of the rules and ENOENT says it is gone; a hand-set bit, an in-rules absence, a present out-of-rules path, and a name the spawn cannot carry keep the flag — and only where the rules GOVERN the worktree: `check-rules` answers about the rules, not about who set a bit, so an unflagged tracked path that is out of the rules (rules written but never applied) proves the sparse checkout owns no bit here, and every flag stands — with the index's gitlinks kept out of both sets, since git never sets a sparse bit on one and `check-rules` never lists one, so a submodule inside a sparse checkout would otherwise read as ungoverned on every run. The bits are asked about before dirt is ruled on: a hand-set bit hides an edit whether or not another file is dirty, and a repository already dirty at the snapshot would otherwise file the hidden edit as pre-existing dirt. The residual is a checkout left mid-narrowing — git refuses to remove an out-of-cone file with local modifications and leaves it present and unflagged — which reads as ungoverned, and so unresolved, until a `sparse-checkout reapply`; fail-closed over a state the user has yet to resolve, never a certification. And every name that reaches a probed repository is recorded against its filesystem identity, so `--since` can tell a route that changed (a workspace link into a submodule that an install step created in the fix window, reached by the status route before the index sweep reaches the submodule) from a repository that appeared or vanished: a baseline recorded through a link OUT of the tree is a baseline for the repository behind the link, not for the name, so when the probe no longer reaches the name as scope (the link removed, or replaced by an in-tree repository) that entry is withdrawn and the name routes as vanished or appeared like any other — left in, a dirty external baseline was exempted from every list on scope and named nowhere; the baseline is read by identity as well as by name — only where both names resolve to the same place now, since a renamed directory keeps its inode and a filesystem can hand a fresh clone the inode number a deleted repository freed, and neither is the same repository under a new name — and one unchanged repository reached by a new route is the all-clear, not two disclosures.
