...
**What `fix-delta` will not certify.** The hunks are the audit's whole input, so the command's second job is to say when they are not the whole edit — the failure direction over-warns, and it never silences a blind spot. Three surfaces get that treatment. **Content the superproject tree cannot record**: a submodule or nested repository enters the tree as its gitlink alone, so an edit inside one moves nothing — the command probes each for uncommitted content and records, per path, a DIGEST of the state it found, so that "already dirty at snapshot time" is a checkable claim rather than a boolean (a bare boolean let a level-2 gitlink that merely moved stamp its parent, after which a real edit inside the parent was filed as dirt that was already there). Paths the probe cannot answer for at all — an unreadable directory, a repository whose status will not run, an ignored tree past the walk budget — are disclosed rather than assumed clean, and never enter the baseline. **The probe's own children are pinned against the tree they measure**, the discipline `worktree.ts` states for its tripwire: `-c core.fsmonitor=` on the `status` and the `ls-files -v`, because a discovered repository's `.git/config` is writable by anything running as this user and `core.fsmonitor` runs a command; and `--work-tree <path>` beside `-C <path>`, because a repo-local `core.worktree` otherwise redirects the probe at a pristine decoy and it answers clean over an edit on disk. **Repo-local config that shapes the capture and the rendering** cannot be overridden the same way, so it is named instead: a `filter.<name>.clean` decides what `add -A` stores for a path (a real edit absent from both trees, or bytes the worktree never held attested as an edit), `.git/info/exclude` drops a new file from the capture, and a `diff` attribute resolving to `-diff` or a driver turns a text fix's hunks into an opaque binary patch the auditor cannot read. Each is read fresh at BOTH moments — every one of these files is plantable between the snapshot and the `--since` — and printed on stderr beside the blind-spot lines, so the orchestrator discloses the qualification with the audit rather than certifying past it.
