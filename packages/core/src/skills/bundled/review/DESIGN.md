...
The backward base-move smuggle (see the roster's heavy-interaction-file comment) is closed at the gate rather than priced as exposure: a seam-bounded round sheds hunks on the premise a prior round published them, and that premise holds only while the merge base holds still between rounds. Each round that POSTS carries its merge base in the machine-ledger marker beside the anchor (`mb`), `pr-context`'s recovery persists it into the side file the next round's posture read already touches, and the bound engages only when that base matches the round's own — a retarget mid-loop (or no vouched base at all, which is every round until the first posted marker carries one) keeps the bound off and the full-range republication is the floor, so smuggled hunks outside the seam are shown once and re-anchored past. The stamp is written at the POSTING boundary, not by the capture, and that is the whole of its soundness: a capture proves only that a diff was published — the anchor-recovery path runs `fetch-pr` twice in one round, and an `upToDate`/`emptyDiff` stop publishes without launching an agent at all, so a capture-written stamp let a round vouch for its own discarded capture. It rides the anchor's rung end to end (withheld by a fail-closed or truncated round, stripped from a foreign or anonymous winner, never grafted), and it lives on the pull request rather than under `.qwen/`, which the review workflow deletes before every run — a stamp that cannot outlive the round is a gate that can never be proven, and a bound that never engages. Both of the bound's deployment conditions are recorded rather than silent — `seamOracle: 'unavailable'` where no parser resolves, `baseContinuity: 'unproven'` where no posted round vouched a matching base — because a bound that did not run renders exactly like a bound that ran and kept everything. The heavy-file invariant agents keep their coverage regardless, as before.
