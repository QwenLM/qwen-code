...
The reserve a plan-sourced wall keeps for its tail scales like the CI workflow's — a third of the wall, capped at 4800 seconds — rather than the flat 4800 the environment path assumes, so a short `--deadline` is not spent entirely on reserve. It floors at the compose floor (1200 seconds) rather than the workflow's 600, because the verifier's gate rests on the floor sitting strictly inside the reserve; a `--deadline` under about fifty minutes therefore cannot admit round 1 at all (the round-1 estimate is thirty minutes), which is the arithmetic the workflow's shortest budgets already meet — and such a flag is refused at capture rather than discovered at round 1. At the floor the reserve _equals_ the compose floor rather than exceeding it; the verifier's gate still fires after the round gate, because the round gate prices the round on top of the reserve. An explicit `QWEN_REVIEW_DEADLINE_RESERVE_SECONDS` still overrides both. Two readers of the clock stay on the environment alone: the core review-workflow runner's launch allowance, and `qwen review run`, whose own `--timeout-minutes` kill is not yet exported as a deadline — a plan wall bounds the round builder and the verifier, not those.
