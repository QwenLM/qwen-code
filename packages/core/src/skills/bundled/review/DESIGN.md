...
- **It produces no verdict, and its output never enters the findings artifact.** An unpinned assumption is a disclosure: it rides the affected finding's `outcomeNote` and a terminal block, and nothing downstream — the verdict, the fix-induced census, the posted comments — reads it. The one route by which the rule could be violated through the back door is closed by keeping the output out of the one file everything downstream reads.
