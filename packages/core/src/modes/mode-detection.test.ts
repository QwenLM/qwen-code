...
describe('ModeDetector', () => {
  ...
  describe('detect()', () => {
    it('should suggest tester mode for "test" keyword', () => {
    ...
    it('should suggest reviewer mode for "review" keyword', () => {
    ...
    it('should suggest debugger mode for "bug" and "fix" keywords', () => {
      const suggestions = detector.detect(
    ...
    it('should suggest optimizer mode for "optimize performance" keywords', () => {
      const suggestions = detector.detect(
    ...
    it('should suggest security mode for "vulnerability" keyword', () => {
      const suggestions = detector.detect(
    ...
    it('should suggest architect mode for "architecture" keyword', () => {
      const suggestions = detector.detect(
        'Help me design the system architecture',
      );
      expect(suggestions[0].mode).toBe('architect');
      expect(suggestions[0].confidence).toBe(0.8);
      expect(suggestions[0].reason).toContain('design');
    });

    it('should suggest developer mode for "implement" keyword', () => {
      const suggestions = detector.detect(
    ...
    it('should suggest product mode for "acceptance criteria" keywords', () => {
      const suggestions = detector.detect(
    ...
    it('should match patterns case-insensitively', () => {
      const upper = detector.detect('Write TESTS for the module');
      const lower = detector.detect('write tests for the module');
      expect(upper[0].mode).toBe(lower[0].mode);
  ...
  describe('recordUsage and getPersonalizedSuggestions', () => {
    ...
    it('should retain only last 1000 usage entries', () => {
      ...
      expect(detector.getUsageHistory().length).toBe(1000);
    });
  });

  describe('addRule / resetRules / clearHistory', () => {
    it('should detect modes using custom rules', () => {
      detector.addRule({
        patterns: ['migrate', 'migration'],
        mode: 'developer',
        confidence: 0.85,
    ...
    it('should support regex patterns in custom rules', () => {
      ...
      const match = suggestions.find((s) => s.confidence === 0.95);
