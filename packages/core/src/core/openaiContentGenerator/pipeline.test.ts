...
describe('ContentGenerationPipeline', () => {
  ...
  describe('execute', () => {
    ...
    it('resolves the capability via the registry key with an unscoped fallback', async () => {
      // An env/flag/settings endpoint override repoints the live base URL
      // away from the registry entry that declares the capability. The wire
      // must apply the same registry-key + unscoped-fallback rule as the
      // pickers, or the two surfaces disagree about the declared tiers.
      const capability = {
        thinking: true,
        efforts: ['high', 'max'],
        defaultEffort: 'high',
      ...
      const getResolvedModelConfig = vi.fn(
        (_authType: unknown, _modelId: unknown, baseUrl?: string) =>
          baseUrl === undefined
            ? { capabilities: { reasoning: capability } }
            : undefined,
      );
      ...
      const apiCall = (mockClient.chat.completions.create as Mock).mock
        .calls[0][0];
      expect(apiCall.reasoning_effort).toBe('max');
    });
