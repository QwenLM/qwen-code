...
const TOOL_GUIDANCE_LINE_GATES: ReadonlyArray<{
  ...
  { prefix: '  - To search for files, use', tools: [ToolNames.SHELL] },
  {
    prefix: '  - If the workspace becomes multi-root',
    tools: [ToolNames.GREP, ToolNames.GLOB],
  },
  { prefix: '  - To search the content of files', tools: [ToolNames.GREP] },
  { prefix: '  - To search file contents', tools: [ToolNames.SHELL] },
  { prefix: '  - Reserve using the', tools: [ToolNames.SHELL] },
  {
    prefix: `  - Use '${ToolNames.SHELL}'`,
    tools: [ToolNames.SHELL],
  },
...
function gateToolGuidance(
  ...
  const kept = section.split('\n').filter((line) => {
    const gate = line.startsWith('- **Codebase Search:**')
      ? {
          tools: hasBashSearch
            ? [ToolNames.AGENT, ToolNames.SHELL]
            : [ToolNames.AGENT, ToolNames.GREP, ToolNames.GLOB],
        }
      : TOOL_GUIDANCE_LINE_GATES.find((entry) => line.startsWith(entry.prefix));
...
`.trim();
  ...
  - If the workspace becomes multi-root and \`rg\` is unavailable, use '${ToolNames.GREP}' or '${ToolNames.GLOB}'.
...
- **Respect Tool Decisions:** Tool permissions are enforced by the runtime. If a call is denied or canceled, respect that decision and do _not_ try the same action through another path. Retry only if the user subsequently requests that action.
`.trim();
  return gateToolGuidance(directGuidance, surface, hasBashSearch);
}
...
${codeModeOnly ? codeModeToolCallExamples : filterToolCallExamples(getToolCallExamples(model || '', hasBashSearch), surface)}
...
export function getCoreSystemPrompt(
  ...
  const effectiveCodeModeOnly =
    typeof searchConfig === 'boolean' ? todoWriteEnabled : codeModeOnly;
