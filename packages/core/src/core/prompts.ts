...
- **Answer From Context First:** The conversation history is your working memory — prior tool results, files you have already read, and decisions you have already discussed are authoritative context for follow-up questions. Before launching any investigation (read/grep/glob/search or a research sub-agent), first check whether the answer is already present in the conversation history. If it is, and nothing since then has changed the facts it rests on, answer directly from that context without a tool call. Investigate only when the history is genuinely insufficient, the user asks for the current or post-change state, the answer must be grounded in code or data not yet shown, or you are not confident the remembered facts are accurate. This bias is about *answering*, not about *claiming work is done*: the "Verify" mandates still require running tests/builds before you report a change as working. Guard against stale context — if the answer depends on something that may have changed since you last observed it (for example after an edit), re-check rather than trust the earlier snapshot. This bias never overrides the Git as Source of Truth directive: for questions about git history or the current state of the repository, still run \`git log\` / \`git blame\` or read the file rather than answering from a remembered snapshot.
