 ...
 * ARE the user's structured payload é—?already emitted on stdout via
...
const OUTPUT_RECOVERY_MESSAGE =
  'Output token limit hit. Resume directly é—?no apology, no recap of what ' +
...
export class GeminiChat {
   ...
   * threshold check sees `0` and refuses to compress é—?so the first API call
  ...
  async tryCompress(
    ...
    if (info.compressionStatus === CompressionStatus.COMPRESSED && newHistory) {
      ...
      // Auto-compaction replaces history in place é—?no env-context refresh
      ...
      try {
        ...
        for (
          ...
          try {
            ...
            // Transient budget exhausted é—?stop immediately.
   ...
   * does not clone é—?use this when you only need the count and would
      ...
      while (true) {
        ...
        if (chunk.usageMetadata) {
          ...
          if (lastPromptTokenCount) {
            ...
            // Mirror to the global telemetry only when wired éˆ?subagents
