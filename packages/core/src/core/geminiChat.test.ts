...
describe('GeminiChat', async () => {
  ...
  describe('sendMessageStream with retries', () => {
    ...
    it('reports all eligible fallback failures after exhausting the chain', async () => {
      vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
        authType: AuthType.USE_GEMINI,
        model: 'test-model',
        maxRetries: 0,
      ...
      fallbackAGenerateContentStream.mockRejectedValueOnce(capacityError);
      fallbackBGenerateContentStream.mockRejectedValueOnce(capacityError);

      const stream = await chat.sendMessageStream(
        'test-model',
        { message: 'test' },
        'prompt-all-fallbacks-fail',
      );
      await expect(
        (async () => {
          for await (const _ of stream) {
        ...
        'fallback chain failed after trying: fallback-a (rate-limit), fallback-b (rate-limit).',
      );
    ...
    it('stops the fallback chain on non-eligible fallback errors before output', async () => {
      vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
        authType: AuthType.USE_GEMINI,
        model: 'test-model',
        maxRetries: 0,
      ...
      fallbackAGenerateContentStream.mockRejectedValueOnce(authError);

      const stream = await chat.sendMessageStream(
        'test-model',
        { message: 'test' },
        'prompt-fallback-auth-stops-chain',
      );
      await expect(
        (async () => {
          for await (const _ of stream) {
      ...
      ).rejects.toThrow('fallback chain failed after trying: fallback-a');
      ...
      fallbackAGenerateContentStream.mockResolvedValueOnce(
        (async function* () {
          yield {
            candidates: [
              {
                content: { parts: [{ text: 'partial fallback output' }] },
              },
            ],
          } as unknown as GenerateContentResponse;
          throw abortError;
    ...
    it('rolls back fallback partial tool calls before throwing final fallback errors', async () => {
      ...
      try {
        ...
        const resultPromise = (async () => {
          try {
            await collecting;
            throw new Error('Expected fallback stream to fail.');
          } catch (error) {
    ...
    it('skips fallback aliases that resolve to the primary backend model', async () => {
      vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
        authType: AuthType.USE_GEMINI,
        model: 'test-model',
        maxRetries: 0,
      ...
      expect(resolveForModel).toHaveBeenCalledWith('test-model', {
        failClosed: true,
      });
      expect(resolveForModel).toHaveBeenCalledWith('fallback-alias', {
        failClosed: true,
      });
      expect(fallbackGenerateContentStream).not.toHaveBeenCalled();
    });

    it('does not re-resolve fallback aliases that already failed to resolve', async () => {
      vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
        authType: AuthType.USE_GEMINI,
        model: 'test-model',
        maxRetries: 0,
      ...
      const resolveError = new Error('unknown model alias');
      const resolveForModel = vi
        .fn()
      ...
      const stream = await chat.sendMessageStream(
        'test-model',
        { message: 'test' },
        'prompt-duplicate-unresolved-fallback',
      );
      const events: StreamEvent[] = [];
      for await (const event of stream) {
        events.push(event);
      ...
      expect(
        resolveForModel.mock.calls.filter(
          ([modelId]) => modelId === 'bad-alias',
        ),
      ).toHaveLength(1);
      expect(resolveForModel).toHaveBeenCalledWith('fallback-model', {
        failClosed: true,
