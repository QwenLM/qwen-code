...
describe('GeminiChat', async () => {
  ...
  describe('sendMessageStream', () => {
    ...
    it('uses the normalized function call ID for preparation metadata', async () => {
      ...
      let releaseFunctionCall!: () => void;
      const functionCallReady = new Promise<void>((resolve) => {
        releaseFunctionCall = resolve;
      });
      vi.mocked(mockContentGenerator.generateContentStream).mockResolvedValue(
        (async function* () {
          yield preparationResponse;
          await functionCallReady;
          yield {
            candidates: [
              {
                content: {
      ...
      expect(
        !first.done && first.value.type === StreamEventType.CHUNK
          ? getToolCallPreparations(first.value.value)
          : [],
      ).toHaveLength(1);
      releaseFunctionCall();
      const events: StreamEvent[] = first.done ? [] : [first.value];
      for (;;) {
  ...
  it('discards an empty content-only thinking block followed by another opening tag', async () => {
      ...
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
            candidates: [
              {
                content: { parts: [{ text: 'Successful final response' }] },
                finishReason: 'STOP',
              },
            ],
          } as unknown as GenerateContentResponse;
    ...
    const stream = await chatWithRecording.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-content-only-thinking-tag-leak',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
  ...
  it('preserves a leading literal thinking block', async () => {
    vi.mocked(
      ...
      (async function* () {
        yield {
          candidates: [{ content: { parts: [{ text: '<thi' }] } }],
        } as unknown as GenerateContentResponse;
        yield {
          candidates: [
            {
              content: {
                parts: [{ text: 'nk>literal example</think> remains visible' }],
              },
              finishReason: 'STOP',
            },
          ],
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-literal-thinking-block',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
    ...
    const emittedText = events
      ...
      .map((part) => part.text ?? '')
      .join('');
    expect(emittedText).toBe('<think>literal example</think> remains visible');
    expect(chat.getLastModelMessageText()).toBe(
      '<think>literal example</think> remains visible',
    );
  });

  it('preserves an empty thinking block when the finish chunk has no content', async () => {
    vi.mocked(
      mockContentGenerator.generateContentStream,
    ).mockImplementationOnce(async () =>
      (async function* () {
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-empty-thinking-block',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
    ...
    const emittedText = events
      ...
      .map((part) => part.text ?? '')
      .join('');
    expect(emittedText).toBe('<think>\n\n</think>');
    expect(chat.getLastModelMessageText()).toBe('<think>\n\n</think>');
  });

  it('discards a partial repeated thinking tag at the end of the stream', async () => {
    vi.mocked(mockContentGenerator.generateContentStream)
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
            candidates: [
              {
                content: { parts: [{ text: '<think></think><thi' }] },
                finishReason: 'STOP',
              },
            ],
          } as unknown as GenerateContentResponse;
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-partial-thinking-tag-leak',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
  ...
  it('discards an overlong undecided protocol prefix', async () => {
    vi.mocked(mockContentGenerator.generateContentStream)
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-overlong-protocol-prefix',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
  ...
  it('preserves a structured part that follows an undecided text prefix', async () => {
    vi.mocked(
      ...
      (async function* () {
          ...
          candidates: [
            {
              content: {
                parts: [
                  {
                    functionCall: {
                      id: 'call_after_literal_thinking_prefix',
                      name: 'read_file',
                      args: { path: '/tmp/literal.txt' },
                    },
                  },
                ],
              },
        ...
        yield {
          candidates: [
            {
              content: { parts: [{ text: 'literal</think>' }] },
              finishReason: 'STOP',
            },
          ],
        } as unknown as GenerateContentResponse;
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-structured-part-after-thinking-prefix',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
  ...
  it('discards a repeated thinking tag that spans structured parts', async () => {
    vi.mocked(mockContentGenerator.generateContentStream)
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
            candidates: [
              {
                content: {
                  parts: [{ text: 'structured thought', thought: true }],
                },
              },
            ],
          } as unknown as GenerateContentResponse;
            ...
            candidates: [
              {
                content: {
                  parts: [
                    {
                      functionCall: {
                        id: 'call_inside_thinking_tag_leak',
                        name: 'read_file',
          ...
          yield {
            candidates: [
              {
                content: {
                  parts: [{ text: '</think><think>leaked content' }],
                },
                finishReason: 'STOP',
              },
            ],
    ...
    const stream = await chat.sendMessageStream(
      'test-model',
      { message: 'test' },
      'prompt-id-structured-thinking-tag-leak',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
    ...
    expect(
      emittedParts
        ...
        .map((part) => part.text ?? '')
        .join(''),
    ).toBe('Successful final response');
  });
