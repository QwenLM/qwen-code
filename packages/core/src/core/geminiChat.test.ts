...
describe('GeminiChat', async () => {
  ...
  it('retries visible think tags when the same response has separate reasoning', async () => {
    ...
    try {
      ...
      const emittedThoughts = events
        .filter((event) => event.type === StreamEventType.CHUNK)
        .flatMap((event) => event.value.candidates?.[0]?.content?.parts ?? [])
        .filter((part) => part.thought);
      expect(emittedThoughts).toEqual([]);
      expect(chatWithRecording.getLastModelMessageText()).toBe(
  ...
  it('suppresses a tool call while a thinking tag prefix is unresolved', async () => {
    vi.useFakeTimers();
    try {
      vi.mocked(mockContentGenerator.generateContentStream)
        .mockImplementationOnce(async () =>
        ...
        .mockImplementationOnce(async () =>
          (async function* () {
            yield {
              candidates: [
                {
                  content: { parts: [{ text: 'Recovered without tool call' }] },
                  finishReason: 'STOP',
                },
              ],
            } as unknown as GenerateContentResponse;
      ...
      const stream = await chat.sendMessageStream(
        'qwen3.7-max',
        { message: 'test' },
        'prompt-id-thinking-tag-tool-call',
      );
      const events = await collectStreamWithFakeTimers(stream);
  ...
  it('does not replay after a tool call was emitted', async () => {
    vi.mocked(mockContentGenerator.generateContentStream)
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
            candidates: [
              {
                content: {
                  parts: [
                    {
                      functionCall: {
                        id: 'call_already_emitted',
                        name: 'read_file',
                        args: { path: '/tmp/already-emitted' },
                      },
                    },
                  ],
                },
      ...
      .mockImplementationOnce(async () =>
        (async function* () {
          yield {
            candidates: [
              {
                content: { parts: [{ text: 'Unexpected replay' }] },
                finishReason: 'STOP',
              },
            ],
          } as unknown as GenerateContentResponse;
    ...
    const stream = await chat.sendMessageStream(
      'qwen3.7-max',
      { message: 'test' },
      'prompt-id-thinking-tag-after-output',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
    ...
    const emittedParts = events
      .filter((event) => event.type === StreamEventType.CHUNK)
      .flatMap((event) => event.value.candidates?.[0]?.content?.parts ?? []);
    expect(emittedParts.filter((part) => part.functionCall)).toHaveLength(1);
    expect(emittedParts.map((part) => part.text ?? '').join('')).toBe(
      '<think>late',
    );
  });

  it.each(['split', 'coalesced'] as const)(
    'stops thinking-tag detection at the leading guard boundary (%s)',
    async (framing) => {
            ...
            candidates: [
              {
                content: {
                  parts: [
                    {
                      text:
                        (framing === 'coalesced' ? prefix : '') + '<think>late',
                    },
                  ],
                },
                finishReason: 'STOP',
      ...
      const stream = await chat.sendMessageStream(
        'qwen3.7-max',
        { message: 'test' },
        `prompt-id-thinking-tag-boundary-${framing}`,
      );
      const events: StreamEvent[] = [];
      ...
      const emittedText = events
        ...
        .map((part) => part.text ?? '')
        .join('');
      expect(emittedText).toBe(`${prefix}<think>late`);
    },
  );
  ...
  it.each(['Short <', 'Short </', 'Short <t'])(
    'preserves a short response ending in a literal tag prefix: %s',
    async (response) => {
      vi.mocked(
        mockContentGenerator.generateContentStream,
      ).mockImplementationOnce(async () =>
      ...
      const stream = await chat.sendMessageStream(
        'qwen3.7-max',
        { message: 'test' },
        'prompt-id-short-literal-tag-prefix',
      );
      const events: StreamEvent[] = [];
      for await (const event of stream) events.push(event);
  ...
  it('holds reasoning only while a literal think tag is structurally unresolved', async () => {
    vi.mocked(
      mockContentGenerator.generateContentStream,
    ).mockImplementationOnce(async () =>
      (async function* () {
          ...
          candidates: [
            {
              content: {
                parts: [
                  {
                    text: 'Reasoning remains visible.',
                    thought: true,
        ...
        yield {
          candidates: [
            {
              content: {
                parts: [{ text: 'nk>example</think> literally.' }],
              },
              finishReason: 'STOP',
            },
          ],
    ...
    const stream = await chat.sendMessageStream(
      'qwen3.7-max',
      { message: 'test' },
      'prompt-id-cross-chunk-thinking-literal',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
    ...
    expect(chunks[0]?.value.candidates?.[0]?.content?.parts).toEqual([
      ...
      { text: 'Use <think>example</think> literally.' },
    ]);
    expect(events.some((event) => event.type === StreamEventType.RETRY)).toBe(
      false,
    );
  ...
  it('preserves non-text parts when a partial thinking tag precedes them', async () => {
    const inlineData = { mimeType: 'image/png', data: 'generated-image' };
    vi.mocked(
      mockContentGenerator.generateContentStream,
    ).mockImplementationOnce(async () =>
      (async function* () {
        ...
        yield {
          candidates: [
            {
              content: { parts: [{ text: 'nk>example</think>' }] },
              finishReason: 'STOP',
            },
          ],
        } as unknown as GenerateContentResponse;
    ...
    const stream = await chat.sendMessageStream(
      'qwen3.7-max',
      { message: 'test' },
      'prompt-id-thinking-tag-inline-data',
    );
    const events: StreamEvent[] = [];
    for await (const event of stream) events.push(event);
  ...
  it('streams healthy separate reasoning without waiting for the final answer', async () => {
    ...
    if (first.value?.type !== StreamEventType.CHUNK) {
      throw new Error('Expected reasoning chunk before releasing final');
    }
    expect(first.value.value.candidates?.[0]?.content?.parts).toEqual([
      { text: 'Reasoning in progress', thought: true },
    ]);
    releaseFinal!();
    const second = await iterator.next();
    expect(second.done).toBe(false);
    expect(second.value?.type).toBe(StreamEventType.CHUNK);
    if (second.value?.type !== StreamEventType.CHUNK) {
      throw new Error('Expected final answer chunk');
    }
    expect(second.value.value.candidates?.[0]?.content?.parts).toEqual([
