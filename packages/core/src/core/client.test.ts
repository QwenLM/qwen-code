...
describe('Gemini Client (client.ts)', () => {
  ...
  describe('continueInterruptedTurn', () => {
    /** Stand-in for the sendMessageStream generator; never consumed. */
    const fakeStream = (async function* () {
      yield undefined as never;
      return undefined as never;
    })() as unknown as ReturnType<GeminiClient['sendMessageStream']>;

    it('returns a null stream and sends nothing when the last turn ended cleanly', () => {
      client.setHistory([
      ...
      const result = client.continueInterruptedTurn(
        new AbortController().signal,
        'prompt-id-1',
      );

      expect(result.detection.kind).toBe('none');
      expect(result.stream).toBeNull();
      expect(sendSpy).not.toHaveBeenCalled();
    });

    it('re-submits an orphaned trailing user prompt with Retry semantics', () => {
      client.setHistory([
        { role: 'model', parts: [{ text: 'earlier answer' }] },
        { role: 'user', parts: [{ text: 'do the thing' }] },
      ]);
      const sendSpy = vi
        .spyOn(client, 'sendMessageStream')
        .mockReturnValue(fakeStream);
      const signal = new AbortController().signal;

      const result = client.continueInterruptedTurn(signal, 'prompt-id-2');

      expect(result.detection.kind).toBe('interrupted_prompt');
    ...
    it('re-submits every consecutive trailing user entry', () => {
      const trailingUsers: Content[] = Array.from(
        { length: 41 },
        (_, index) => ({
          role: 'user',
      ...
      const result = client.continueInterruptedTurn(signal, 'prompt-id-long');

      expect(result.detection.kind).toBe('interrupted_prompt');
      expect(sendSpy).toHaveBeenCalledWith(
        trailingUsers.flatMap((entry) => entry.parts ?? []),
        signal,
        'prompt-id-long',
        { type: SendMessageType.Retry },
      );
    ...
    it('closes dangling tool calls with a synthesized ToolResult submission', () => {
      client.setHistory([
      ...
      const result = client.continueInterruptedTurn(signal, 'prompt-id-3');

      expect(result.detection.kind).toBe('interrupted_turn');
      expect(result.stream).toBe(fakeStream);
      expect(sendSpy).toHaveBeenCalledTimes(1);
      const [request, sentSignal, promptId, options] = sendSpy.mock.calls[0]!;
      expect(sentSignal).toBe(signal);
      expect(promptId).toBe('prompt-id-3');
      expect(options).toEqual({ type: SendMessageType.ToolResult });
