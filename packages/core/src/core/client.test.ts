...
describe('Gemini Client (client.ts)', () => {
  ...
  describe('startChat — deferred tools', () => {
    ...
    // #12029: the percentage alone scales with the window, which is not what
    // decides the trade. These four pin the absolute cap that bounds it. The
    // shared mockConfig deliberately does not define the getter, so every test
    // above measures the uncapped percentage — the pre-#12029 behaviour.
    function setMaxPreloadTokens(value: number | undefined) {
      (
        mockConfig as unknown as {
          getToolSearchMaxPreloadTokens?: () => number | undefined;
        }
      ).getToolSearchMaxPreloadTokens = () => value;
    }

    it('caps the preload budget at the absolute token ceiling', async () => {
      const reg = getRegistryMock();
      reg.getTool.mockImplementation((n: string) =>
        n === 'tool_search' ? ({} as never) : null,
      );
      // 10% of the default window is far above the cap, so the cap binds.
      setMaxPreloadTokens(8_000);
      reg.preloadDeferredToolsWithinBudget.mockClear();
    ...
    it('keeps the percentage when it is the smaller of the two', async () => {
      const reg = getRegistryMock();
      reg.getTool.mockImplementation((n: string) =>
        n === 'tool_search' ? ({} as never) : null,
      );
    ...
    it('skips deferred preload when the cap is 0', async () => {
      const reg = getRegistryMock();
      reg.getTool.mockImplementation((n: string) =>
        n === 'tool_search' ? ({} as never) : null,
      );
      // Same meaning as a threshold of 0: never preload.
      setMaxPreloadTokens(0);
      reg.preloadDeferredToolsWithinBudget.mockClear();
    ...
    it('treats a negative cap as 0 rather than inverting the bound', async () => {
      const reg = getRegistryMock();
      reg.getTool.mockImplementation((n: string) =>
        n === 'tool_search' ? ({} as never) : null,
      );
      setMaxPreloadTokens(-5_000);
      reg.preloadDeferredToolsWithinBudget.mockClear();
    ...
    it('ignores a non-finite cap and uses the percentage', async () => {
      const reg = getRegistryMock();
      reg.getTool.mockImplementation((n: string) =>
        n === 'tool_search' ? ({} as never) : null,
      );
      setMaxPreloadTokens(NaN);
      reg.preloadDeferredToolsWithinBudget.mockClear();
