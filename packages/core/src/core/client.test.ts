...
describe('Gemini Client (client.ts)', () => {
  ...
  describe('sendMessageStream', () => {
    ...
    describe('SendMessageType.Peer is a top-level interaction', () => {
      // The enum's own doc promises Peer behaves "like Teammate". These
      // three branches are where that promise is kept or silently broken;
      // each one fails differently, so each gets its own assertion.
      beforeEach(() => {
        mockTurnRunFn.mockReturnValue(
          (async function* () {
            yield { type: GeminiEventType.Content, value: 'response' };
