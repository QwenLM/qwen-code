...
describe('QwenLogger', () => {
  ...
  describe('createRumPayload', () => {
    ...
    describe('runtime and client attribution', () => {
      const SERVE_ENV = 'QWEN_CODE_SERVE';
      const DESKTOP_ENV = 'QWEN_CODE_DESKTOP';
      let savedServeEnv: string | undefined;
      let savedDesktopEnv: string | undefined;

      const createPayload = async (config: Config) =>
        (
          QwenLogger.getInstance(config)! as unknown as {
            createRumPayload(): Promise<RumPayload>;
          }
        ).createRumPayload();

      beforeEach(() => {
        savedServeEnv = process.env[SERVE_ENV];
        savedDesktopEnv = process.env[DESKTOP_ENV];
        delete process.env[SERVE_ENV];
        delete process.env[DESKTOP_ENV];
      });

      afterEach(() => {
        if (savedServeEnv === undefined) delete process.env[SERVE_ENV];
        else process.env[SERVE_ENV] = savedServeEnv;
        if (savedDesktopEnv === undefined) delete process.env[DESKTOP_ENV];
        else process.env[DESKTOP_ENV] = savedDesktopEnv;
      });

      it('reports runtime=cli and no client for a plain CLI session', async () => {
        const payload = await createPayload(mockConfig);

        expect(payload.properties).toEqual(
          expect.objectContaining({ runtime: 'cli' }),
        );
        expect(payload.properties).not.toHaveProperty('client');
        expect(payload.properties).not.toHaveProperty('channel');
      });

      it('reports runtime=acp and keeps channel for direct ACP launches', async () => {
        const payload = await createPayload(
          makeFakeConfig({ getChannel: () => 'ACP' }),
        );

        expect(payload.properties).toEqual(
          expect.objectContaining({ runtime: 'acp', channel: 'ACP' }),
        );
        expect(payload.properties).not.toHaveProperty('client');
      });

      it('keeps the VSCode channel and adds client=vscode', async () => {
        const payload = await createPayload(
          makeFakeConfig({ getChannel: () => 'VSCode' }),
        );
      ...
      it('reports runtime=daemon for daemon-spawned sessions', async () => {
        ...
        expect(payload.properties).toEqual(
          expect.objectContaining({ runtime: 'daemon', channel: 'ACP' }),
        );
        expect(payload.properties).not.toHaveProperty('client');
      });

      it('attributes the Tauri desktop shell via QWEN_CODE_DESKTOP', async () => {
        process.env[SERVE_ENV] = '1';
        process.env[DESKTOP_ENV] = '1';
        const payload = await createPayload(
          makeFakeConfig({ getChannel: () => 'ACP' }),
        );
