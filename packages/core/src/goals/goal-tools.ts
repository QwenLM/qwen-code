...
> {
  ...
  constructor(private readonly config: GoalToolConfig) {
    super(
      ...
      'Read the current Goal identity, objective, evidence cursor, and the verifier feedback the last rejected proposal left for this permitted Goal turn. The Goal is verified from the transcript itself, so there is nothing here to cite: this call exists to re-read the objective and the feedback, not to gather evidence. Outside a permitted Goal turn it reports "active": false together with "lastGoal", a scalar summary (goalId, revision, status, turnCount, activeTimeMs, tokensUsed, plus tokenBudget, turnBudget, activeTimeBudgetMs and lastReason when recorded) of the session\'s most recent Goal, so a Goal that has already stopped can still be inspected. It never returns transcript history or changes Goal state. Use the result silently; do not narrate or acknowledge the retrieval to the user.',
      Kind.Read,
      {
        type: 'object',
        properties: {},
  ...
  async execute(signal: AbortSignal): Promise<GoalToolResult> {
    ...
    const payload = {
      ...
      nextAction: receipt.readyForVerification
        ...
        : 'Continue this turn without claiming the Goal is complete or blocked. A repeated-blocker audit requires the same blocker mode and exact same reason text across three consecutive Goal turns, each showing the blocker in its transcript.',
    };
      ...
      Kind.Think,
      {
        ...
        properties: {
          ...
          blockerKind: {
            ...
            description:
              'authority: a user or maintainer decision or permission is required; external: an evidenced external resource or capability is unavailable; repeated: the same evidenced blocker with the exact same reason text across three consecutive Goal turns; infeasible: a tool result shows the objective cannot be satisfied as written (self-contradictory, names a target that verifiably does not exist, or needs an action no tool can perform) -- not difficulty, uncertainty, or obtainable information. Omission uses the repeated-blocker audit.',
          },
        },
