...
> {
  ...
  async execute(signal: AbortSignal): Promise<GoalToolResult> {
    ...
    signal.throwIfAborted();
    // Ending the turn to wait for a checkpoint only makes sense when one
    // can run. A runtime that compresses nothing would answer the same way
    // every turn, and three such turns read as no progress; recording the
    // proposal instead lets verification stop the Goal as evidence-limited,
    // with the real reason, and a resume then starts a fresh window.
    if (
      view.evidenceCatalog?.truncated &&
      view.compactsEvidence &&
