...
function recoverLegacyGoal(records: readonly ChatRecord[]): GoalRecovery {
  for (
...
export interface GoalStateRecorder {
  recordGoalState(
    recordUuid: string,
...
export async function recordMigratedGoalState(
  recorder: GoalStateRecorder,
  input: MigratedGoalStateInput,
): Promise<{ record: ChatRecord; payload: GoalStateRecordPayloadV2 }> {
  const payload = createMigratedGoalState(input);
  const record = await recorder.recordGoalState(input.recordUuid, payload);
