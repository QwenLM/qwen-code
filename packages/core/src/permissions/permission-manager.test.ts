...
describe('PermissionManager', () => {
  ...
  describe('tools.eager allowlist (#9827, #10075)', () => {
    ...
    it('an explicitly empty list is active and defers everything', async () => {
      ...
      // ToolSearch and ToolCall together keep the deferred tools reachable,
      // and exec stays registered so CodeModeOnly sessions keep their one
      // control tool (#10414).
      expect(await pm.getToolRegistrationStatus('tool_search')).toBe(
        'registered',
      );
      expect(await pm.getToolRegistrationStatus('tool_call')).toBe(
        'registered',
      );
      expect(await pm.getToolRegistrationStatus('exec')).toBe('registered');
    });

    it('exec and the tool_call bridge ride with tool_search under a narrow eager list', async () => {
      // CodeModeOnly narrows the model-visible surface to exec({ source })
      // plus the audited direct-only controls, and deferred tools stay
      // reachable through ToolSearch + ToolCall. A narrow tools.eager list
      // must therefore never demote exec or either bridge half (#10414).
      pm = new PermissionManager(
        makeConfig({ eagerTools: ['ReadFile'] }),
      );
      pm.initialize();
      expect(await pm.getToolRegistrationStatus('exec')).toBe('registered');
      expect(await pm.getToolRegistrationStatus('tool_search')).toBe(
      ...
      // An unlisted, non-exempt built-in stays deferred, not disabled.
      expect(await pm.getToolRegistrationStatus('send_message')).toBe(
        'deferred',
      );
    });
