...
export class PermissionManager {
   ...
   * discoverable and callable through the ToolSearch + ToolCall bridge —
   * but its schema is kept out of the eager model request, which is the
   * #9827 guarantee. Call-time approval for such a tool falls back to the
   * normal permission evaluation (ask / approval-mode), so nothing loses
   * capability silently (#10075).
   *
   ...
   * - `exec`: CodeModeOnly's structured control tool. That mode narrows
   *   the model-visible surface to `exec({ source })` plus the audited
   *   direct-only controls, so the one tool the mode is built around must
   *   survive any `tools.eager` shrink (#10414).
