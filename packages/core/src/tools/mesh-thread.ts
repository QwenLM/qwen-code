...
/**
 * @fileoverview The tools a mesh agent uses to work a shared thread.
 *
 * One rule shapes every schema here: **no mutating tool accepts a thread,
 * author, run, or idempotency id from the model.** A mesh agent is one
...
> extends BaseToolInvocation<TParams, ToolResult> {
  ...
  async execute(): Promise<ToolResult> {
    try {
      ...
      if (error instanceof MeshCloseRejectedError) {
...
> {
  ...
  async execute(): Promise<ToolResult> {
    try {
      const context = requireMeshRunContext('thread_read');
      ...
      const agents = await readMeshAgents(this.config.getProjectRoot());
...
/** Every mesh thread tool, in the order the run frame lists them. */
export const MESH_THREAD_TOOLS = [
  ThreadPostTool,
  ThreadWaitTool,
  ThreadBlockTool,
  ThreadReviewTool,
