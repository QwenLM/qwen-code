...
describe('AskUserQuestionTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should surface the pipeline cancel reason instead of a fake user decline', async () => {
      const params = {
        questions: [
          {
            question: 'Test?',
      ...
      // The host could not render the approval surface, so the confirmation
      // pipeline auto-cancelled with a reason. The user never saw the question
      // and made no decision.
      const hostReason =
        'The host could not present the required approval for "ask_user_question".';
      await confirmation.onConfirm(ToolConfirmationOutcome.Cancel, {
        cancelMessage: hostReason,
      });

      const result = await invocation.execute(new AbortController().signal);
      expect(result.llmContent).toBe(hostReason);
      expect(result.returnDisplay).toBe(hostReason);
      expect(result.llmContent).not.toContain('declined to answer');
    });

    it('should keep the generic decline text when the cancel carries no reason', async () => {
      const params = {
        questions: [
          {
            question: 'Test?',
    ...
    it('should not report a stale pipeline reason on a later genuine decline', async () => {
      const params = {
        questions: [
          {
            question: 'Test?',
      ...
      // First the pipeline auto-cancels with a reason, then the question is
      // re-presented and the user genuinely declines (no reason carried).
      await confirmation.onConfirm(ToolConfirmationOutcome.Cancel, {
