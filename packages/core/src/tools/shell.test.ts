...
describe('ShellTool', () => {
  ...
  describe('execute', () => {
    ...
    describe('simulated sed edit', () => {
      ...
      it('keeps a user-confirmed sed edit across a re-entrant preview (ask bounce)', async () => {
        // A PreToolUse ask bounce re-fetches getConfirmationDetails for the
        // SAME sed target after the user edited + approved the content.
        // The confirmed content must survive the re-fetch, the
        // re-confirmation view must present exactly that content, and
        // execution must write it (#9434 R3-1/R4-2/R4-5).
        mockFileSystemService.readTextFile.mockResolvedValue({
          content: 'foo\n',
          _meta: { bom: false, encoding: 'utf-8', lineEnding: 'lf' },
        });
        ...
        // First confirmation: the user edits the proposed content.
        const first = await invocation.getConfirmationDetails(mockAbortSignal);
        expect(first.type).toBe('edit');
        if (first.type !== 'edit') {
          throw new Error('expected edit confirmation');
        }
        await first.onConfirm(ToolConfirmationOutcome.ProceedOnce, {
          newContent: 'USER_EDITED_CONTENT\n',
        });

        // Re-entrant fetch for the same target (the ask bounce).
        const second = await invocation.getConfirmationDetails(mockAbortSignal);
        expect(second.type).toBe('edit');
        if (second.type !== 'edit') {
          throw new Error('expected edit confirmation');
        }
        // The bounced view shows the CONFIRMED content, not the freshly
        // simulated one — view and write must agree.
        expect(second.newContent).toBe('USER_EDITED_CONTENT\n');
        expect(second.fileDiff).toContain('+USER_EDITED_CONTENT');
        await second.onConfirm(ToolConfirmationOutcome.ProceedOnce);
      ...
      it('keeps the prepared sed edit when a re-entrant preview fails (ask bounce)', async () => {
        // A FAILED re-entrant getConfirmationDetails during an ask bounce
        // must not flip sedEditPreviewFailed: execution still runs the
        // guarded executeSedEdit path (not the raw sed command) and keeps
        // the user-confirmed content (#9434 R4-3).
        const meta = { bom: false, encoding: 'utf-8', lineEnding: 'lf' };
        mockFileSystemService.readTextFile
          .mockResolvedValueOnce({ content: 'foo\n', _meta: meta })
          .mockRejectedValueOnce(
            Object.assign(new Error('EIO: read error'), { code: 'EIO' }),
          )
          .mockResolvedValue({ content: 'foo\n', _meta: meta });
        ...
        // The failed re-fetch propagates to the caller (the bounce falls
        // back to a synthetic confirmation) instead of corrupting state.
        await expect(
        ...
        // No raw shell fallback: the guarded sed edit path writes the
      ...
      it('clears the confirmed sed content when the prepared edit changes', async () => {
        // A genuinely new sed target (the file changed on disk between
        // previews) must drop the stale user-confirmed content instead of
        // writing it over the new simulation.
        mockFileSystemService.readTextFile
          .mockResolvedValueOnce({
            content: 'foo\n',
            _meta: { bom: false, encoding: 'utf-8', lineEnding: 'lf' },
        ...
        // The file changed on disk → the prepared edit differs → the
        // confirmed content is dropped and the view shows the new
        // simulation.
        const second = await invocation.getConfirmationDetails(mockAbortSignal);
        expect(second.type).toBe('edit');
        if (second.type !== 'edit') {
        ...
        expect(second.newContent).toBe('bar bar\n');
        await second.onConfirm(ToolConfirmationOutcome.ProceedOnce);
        ...
        expect(mockShellExecutionService).not.toHaveBeenCalled();
        expect(mockFileSystemService.writeTextFile).toHaveBeenCalledWith(
          expect.objectContaining({ content: 'bar bar\n' }),
        );
        expect(result.llmContent).toContain('sed edit applied');
      });
