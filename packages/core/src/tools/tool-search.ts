...
The returned <functions> block is informational. After reviewing the schema, invoke the deferred tool through tool_call with its exact name and schema-shaped arguments. Do not call the deferred tool directly: its declaration remains hidden so the model-facing tool list and prompt-cache prefix stay stable.
