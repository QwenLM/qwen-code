...
- **Do not poll for subagent completion.** After dispatching background agents (\`run_in_background: true\`), do NOT repeatedly call tools like \`${ToolNames.READ_FILE}\`, \`${ToolNames.GLOB}\`, or \`${ToolNames.RUN_SHELL_COMMAND}\` to check whether they finished — this busy-wait pattern consumes tokens, bloats context, and can trigger the API's repetitive-call detection. Instead: wait for the \`<task-notification>\` XML block that arrives automatically when a background agent completes, or use \`${ToolNames.SEND_MESSAGE}\` to communicate with a running agent. If you need to wait for multiple agents before proceeding, state your intent clearly and the notification mechanism will deliver results as they arrive.
${
  isForkSubagentEnabled(this.config)
    ? `
## When to fork
