...
describe('McpClientManager', () => {
  ...
  it('routes discovery through the pool when one is injected (F2 commit 4)', async () => {
    ...
    const acquireSpy = vi.fn().mockResolvedValue({
      ...
      transportId: connectionIdOf('srv', {}),
      serverName: 'srv',
      entryIndex: 0,
    });
    const fakePool = {
  ...
  it('readResource self-heals when pooled handle is dead (R24 T19)', async () => {
    ...
    (await acquireSpy.mock.results[0].value).state = 'failed';
...
describe('McpClientManager — addRuntimeMcpServer / removeRuntimeMcpServer (T2.8)', () => {
  ...
  function mkRuntimeConfig(
    ...
    const runtimeServers = { ...opts.runtimeServers };
    const addSpy = vi.fn((name: string, config: MCPServerConfig) => {
      opts.runtimeAddSpy?.(name, config);
      runtimeServers[name] = config;
    });
    const removeSpy = vi.fn((name: string) => {
      opts.runtimeRemoveSpy?.(name);
      const present = name in runtimeServers;
      delete runtimeServers[name];
      return present;
    });
    return {
      isTrustedFolder: () => true,
      getMcpServers: () => ({ ...opts.settingsServers, ...runtimeServers }),
      getMcpServerCommand: () => undefined,
      getTargetDir: () => '/session/worktree',
      getResourceRegistry: () => ({ removeResourcesByServer: vi.fn() }),
      getPromptRegistry: () => ({ removePromptsByServer: vi.fn() }),
      ...
      getRuntimeMcpServers: () => runtimeServers,
  ...
  it('case 1: happy fresh add → replaced=false, correct transport and toolCount', async () => {
    ...
    expect(acquireSpy).toHaveBeenCalledTimes(1);
    expect(updateConfig).toHaveBeenCalledExactlyOnceWith(sessionConfig);
  ...
  it.each(['untrusted', 'pending approval'])(
    'rejects a fresh runtime add before mutation when the server is %s',
    async (restriction) => {
      let allowed = false;
      const config = mkRuntimeConfig();
      Object.assign(config, {
        isTrustedFolder: () => restriction !== 'untrusted' || allowed,
        isMcpServerPendingApproval: () =>
          restriction === 'pending approval' && !allowed,
      });
      ...
      const conn = {
        id: 'guarded::lifecycle-1',
        transportId: connectionIdOf('guarded', serverConfig),
        client: { getStatus: () => MCPServerStatus.CONNECTED },
        on: vi.fn(),
        release: vi.fn(),
        updateConfig: vi.fn(),
      ...
      await expect(
        manager.addRuntimeMcpServer('guarded', serverConfig, 'client'),
      ).rejects.toThrow('workspace trust or server approval is required');
      expect(config.addRuntimeMcpServer).not.toHaveBeenCalled();
      expect(config.removeRuntimeMcpServer).not.toHaveBeenCalled();
      expect(config.getRuntimeMcpServers()).toEqual({});
      expect(budget.tryReserve).not.toHaveBeenCalled();
      ...
      await expect(
        manager.addRuntimeMcpServer('guarded', serverConfig, 'client'),
      ).resolves.toMatchObject({ name: 'guarded', replaced: false });
      expect(budget.tryReserve).toHaveBeenCalledExactlyOnceWith('guarded');
      expect(pool.acquire).toHaveBeenCalledOnce();
      expect(config.getRuntimeMcpServers()).toEqual({ guarded: serverConfig });
      expect(manager.getServerStatus('guarded')).toBe(
        MCPServerStatus.CONNECTED,
      );
    },
  );
  ...
  it('reacquires a failed same-fingerprint runtime handle and uses its new tools and resources', async () => {
    ...
    const { DiscoveredMCPTool } = await import('./mcp-tool.js');
    const config = mkRuntimeConfig();
    const resources = new ResourceRegistry();
    const serverConfig = { command: 'node', args: ['server.js'] };
    const makeConnection = (version: string) => {
      const client = {
        getStatus: vi.fn((): MCPServerStatus => MCPServerStatus.CONNECTED),
        callTool: vi.fn().mockResolvedValue({
          content: [{ type: 'text', text: `${version} tool result` }],
        }),
        readResource: vi.fn().mockResolvedValue({
          contents: [{ uri: 'test://resource', text: `${version} resource` }],
        }),
      };
      const tool = new DiscoveredMCPTool(
        {} as ConstructorParameters<typeof DiscoveredMCPTool>[0],
        'srv',
        'echo',
        `${version} tool`,
        { type: 'object' },
    ...
    const original = makeConnection('original');
    ...
    Object.assign(config, {
      ...
      getDisabledTools: () => new Set(),
      getResourceRegistry: () => resources,
    });
    const registry = new ToolRegistry(config);
    const manager = registry.getMcpClientManager();
    for (const conn of [original, replacement]) {
      pool.acquire.mockImplementationOnce(async () => {
        registry.registerTool(conn.tool);
        resources.registerResource(conn.resource);
        return conn;
      });
    }

    await manager.addRuntimeMcpServer('srv', serverConfig, 'client');
    const signal = new AbortController().signal;
    await expect(
      registry.getTool('mcp__srv__echo')!.build({}).execute(signal),
    ).resolves.toMatchObject({ returnDisplay: 'original tool result' });
    await expect(
      manager.readResource('srv', 'test://resource'),
    ).resolves.toEqual({
      contents: [{ uri: 'test://resource', text: 'original resource' }],
    });
    original.client.callTool.mockClear();
    ...
    ).resolves.toMatchObject({ name: 'srv', replaced: true, toolCount: 1 });
    expect(pool.acquire).toHaveBeenCalledTimes(2);
    expect(original.release).toHaveBeenCalledOnce();
    expect(original.updateConfig).not.toHaveBeenCalled();
    expect(replacement.updateConfig).toHaveBeenCalledExactlyOnceWith(
      serverConfig,
    );
    expect(registry.getToolsByServer('srv')).toEqual([replacement.tool]);
    expect(resources.getResourcesByServer('srv')).toEqual([
      replacement.resource,
    ]);
    await expect(
      registry.getTool('mcp__srv__echo')!.build({}).execute(signal),
    ).resolves.toMatchObject({ returnDisplay: 'replacement tool result' });
    await expect(
      manager.readResource('srv', 'test://resource'),
    ).resolves.toEqual({
      contents: [{ uri: 'test://resource', text: 'replacement resource' }],
    });
    expect(original.client.callTool).not.toHaveBeenCalled();
    expect(original.client.readResource).not.toHaveBeenCalled();
    expect(replacement.client.callTool).toHaveBeenCalledOnce();
    expect(replacement.client.readResource).toHaveBeenCalledExactlyOnceWith(
      'test://resource',
      undefined,
    );
    await manager.stop();
  ...
  it.each([
    ...
    'runtime %s preserves only held workspace budget (same-name holder: %s)',
    async (action, anotherHolder) => {
      const { WorkspaceMcpBudget } = await import('./mcp-workspace-budget.js');
      const budget = new WorkspaceMcpBudget({
        clientBudget: 1,
        mode: 'enforce',
      });
      const holders = new Set(anotherHolder ? ['sibling-entry'] : []);
      if (anotherHolder) expect(budget.tryReserve('srv')).toBe('reserved');
      const serverConfig = { command: 'node', args: ['server.js'] };
      const config = mkRuntimeConfig();
      const conn = {
        id: 'srv::runtime-entry',
        transportId: connectionIdOf('srv', serverConfig),
        client: { getStatus: () => MCPServerStatus.CONNECTED },
        on: vi.fn(),
      ...
      const pool = {
        getBudget: () => budget,
        acquire: async () => {
          reservedDuringAcquire = budget.getReservedSlots();
          if (action === 'failed add') throw new Error('spawn failed');
      ...
      if (action === 'failed add') {
        ...
        expect(budget.tryReserve('next-server')).toBe('refused');
        await expect(
          manager.removeRuntimeMcpServer('srv', 'client'),
        ).resolves.toMatchObject({ name: 'srv', removed: true });
      }
...
describe('pooled session recovery', () => {
  function fixture(eventEmitter?: EventEmitter) {
    let trusted = true;
    let disabled = false;
    ...
    const connection = (name = 'srv') =>
      Object.assign(new EventEmitter(), {
        id: connectionIdOf(name, servers[name]),
        transportId: connectionIdOf(name, servers[name]),
        client: {
          getStatus: (): MCPServerStatus => MCPServerStatus.CONNECTED,
          callTool: vi.fn(),
          readResource: vi.fn().mockResolvedValue({ contents: [] }),
        },
        release: vi.fn(),
  ...
  function runtimeFixture() {
    ...
    Object.assign(f.config, {
      getSettingsMcpServers: () => ({}),
      getRuntimeMcpServers: () => f.servers,
      addRuntimeMcpServer: (name: string, config: MCPServerConfig) => {
        f.servers[name] = config;
      },
      removeRuntimeMcpServer: (name: string) => {
        const present = name in f.servers;
    ...
    Object.assign(f.initial, { toolsSnapshot: [] });
    Object.assign(f.replacement, { toolsSnapshot: [] });
    return f;
  }

  it.each(['disconnect', 'stop', 'trust', 'approval', 'config'])(
    'rejects a late initial discovery after %s supersedes it',
    async (action) => {
      const f = fixture();
      let complete!: (value: typeof f.initial) => void;
      f.pool.acquire.mockReturnValueOnce(
        new Promise((resolve) => {
          complete = resolve;
        }),
      );
      ...
      if (action === 'trust') f.revokeTrust();
      if (action === 'approval') f.requireApproval();
      if (action === 'config') f.servers['srv'] = { command: 'replacement' };
      complete(f.initial);
      await discovery;
      await stopping;
      expect(f.initial.release).toHaveBeenCalledOnce();
      expect(f.manager.getServerStatus('srv')).toBe(
        MCPServerStatus.DISCONNECTED,
      );
  ...
  it('refreshes policy changed during initial acquisition before tracking its handle', async () => {
    const f = fixture();
    let complete!: (value: typeof f.initial) => void;
    f.pool.acquire.mockReturnValueOnce(
      new Promise((resolve) => {
    ...
    complete(f.initial);
    await discovery;
    expect(f.initial.updateConfig).toHaveBeenCalledWith(f.servers['srv']);
    expect(f.manager.getServerStatus('srv')).toBe(MCPServerStatus.CONNECTED);
    await f.manager.stop();
  });

  it('recovers only the server requested by a resource read', async () => {
    const f = fixture();
    f.servers['other'] = { command: 'other' };
    const other = f.connection('other');
    f.pool.acquire.mockImplementation(async (name) =>
      name === 'srv' ? f.initial : other,
    );
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    other.emit('event', { kind: 'failed', serverName: 'other', generation: 0 });
    await expect(
      f.manager.readResource('srv', 'test://resource'),
    ).resolves.toEqual({ contents: [] });
    expect(f.pool.acquireForRecovery.mock.calls.map(([name]) => name)).toEqual([
      'srv',
    ]);
    expect(f.replacement.client.readResource).toHaveBeenCalledExactlyOnceWith(
      'test://resource',
      undefined,
    );
    await f.manager.stop();
  ...
  it('waits for in-flight recovery before a runtime re-add uses its session seat', async () => {
    const f = runtimeFixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    let complete!: (value: typeof f.replacement) => void;
    ...
    const added = f.manager.addRuntimeMcpServer(
      'srv',
      f.servers['srv'],
      'client',
    );
  ...
  it('releases a runtime acquire that finishes after removal', async () => {
    const f = runtimeFixture();
    const config = f.servers['srv'];
    delete f.servers['srv'];
    let complete!: (value: typeof f.initial) => void;
    ...
    const added = f.manager.addRuntimeMcpServer('srv', config, 'client');
    await f.manager.removeRuntimeMcpServer('srv', 'client');
    complete(f.initial);
    await expect(added).rejects.toThrow('superseded');
    expect(f.initial.release).toHaveBeenCalledOnce();
  ...
  it('runs configuration reconciliation queued behind a runtime add', async () => {
    const f = runtimeFixture();
    const config = f.servers['srv'];
    delete f.servers['srv'];
    let complete!: (value: typeof f.initial) => void;
    ...
    const added = f.manager.addRuntimeMcpServer('srv', config, 'client');
    f.servers['other'] = new MCPServerConfig('other');
    const refresh = f.manager.discoverAllMcpToolsIncremental(f.config);
    complete(f.initial);
    await added;
    await refresh;
    expect(f.pool.acquire.mock.calls.map(([name]) => name)).toEqual([
      'srv',
      'other',
    ]);
  });

  it('drops retained failure notices when a runtime server is removed', async () => {
    const f = runtimeFixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    f.pool.acquireForRecovery.mockRejectedValue(new Error('unavailable'));
    await f.manager.recoverFailedConnections(new AbortController().signal, {
    ...
    Object.assign(f.pool, { getBudget: () => budget });
    expect(
      await f.manager.recoverFailedConnections(new AbortController().signal),
    ).toEqual([]);
    expect(budget.beginBulkPass).not.toHaveBeenCalled();
  });

  it('rejects a runtime name reserved by mcp.serverCommand', async () => {
    const f = runtimeFixture();
    Object.assign(f.config, {
      getMcpServerCommand: () => 'node configured-server',
    });
    await expect(
  ...
  it('recovers an evicted handle on next demand without executing any tool', async () => {
    ...
    expect(notices[0].message).toContain('reconnected');
    expect(f.replacement.client.callTool).not.toHaveBeenCalled();
    await f.manager.recoverFailedConnections(new AbortController().signal);
    expect(f.pool.acquireForRecovery).toHaveBeenCalledTimes(1);
  });

  it('recovers another server when one candidate fails in the same pass', async () => {
    const f = fixture();
    f.servers['other'] = new MCPServerConfig('other-command');
    const other = f.connection('other');
    const recoveredOther = f.connection('other');
    f.pool.acquire.mockImplementation(async (name) =>
      name === 'srv' ? f.initial : other,
    );
    f.pool.acquireForRecovery.mockImplementation(async (name) => {
      if (name === 'srv') throw new Error('unavailable');
      return recoveredOther;
    });
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    other.emit('event', {
    ...
    expect(notices).toEqual(
      expect.arrayContaining([
        {
          serverName: 'srv',
          message: expect.stringContaining('remains disconnected'),
        },
        {
          serverName: 'other',
    ...
    expect(f.manager.getServerStatus('srv')).toBe(MCPServerStatus.DISCONNECTED);
    expect(f.manager.getServerStatus('other')).toBe(MCPServerStatus.CONNECTED);
    expect(recoveredOther.client.callTool).not.toHaveBeenCalled();
    await f.manager.stop();
  });

  it('drops an obsolete failed candidate during configuration reconciliation', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    delete f.servers['srv'];
    await f.manager.discoverAllMcpToolsIncremental(f.config);
    const budget = {
      beginBulkPass: vi.fn(),
      endBulkPass: vi.fn(),
      clearRefusal: vi.fn(),
  ...
  it('retains child-agent recovery notices for the owning ACP session', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    const notices = await f.manager.recoverFailedConnections(
    ...
    expect(notices).toEqual([
      { serverName: 'srv', message: expect.stringContaining('reconnected') },
    ]);
    expect(
      await f.manager.recoverFailedConnections(new AbortController().signal),
    ).toEqual(notices);
  ...
  it('runs a queued configuration refresh even when a recovery listener throws', async () => {
    ...
    f.servers['added'] = new MCPServerConfig('other');
    events.once('mcp-client-update', () => {
    ...
    expect(f.pool.acquire.mock.calls.some(([name]) => name === 'added')).toBe(
      true,
    );
  });

  it('preserves unrelated workspace refusals on successful recovery', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    const { WorkspaceMcpBudget } = await import('./mcp-workspace-budget.js');
    const budget = new WorkspaceMcpBudget({ clientBudget: 1, mode: 'enforce' });
    budget.beginBulkPass();
    budget.recordRefusal('unrelated', 'stdio');
    budget.recordRefusal('srv', 'stdio');
    budget.endBulkPass();
    Object.assign(f.pool, { getBudget: () => budget });
    await f.manager.recoverFailedConnections(new AbortController().signal);
    expect(budget.getRefusedServerNames()).toEqual(['unrelated']);
  });

  it.each([undefined, '/explicit-cwd'])(
    'uses the discovery fingerprint for runtime adds with cwd %s',
    async (cwd) => {
      const actual =
        await vi.importActual<typeof import('./mcp-client.js')>(
          './mcp-client.js',
        );
      vi.mocked(populateMcpServerCommand).mockImplementation(
        actual.populateMcpServerCommand,
      );
      try {
        const f = fixture();
        delete f.servers['srv'];
        Object.assign(f.config, {
          getSettingsMcpServers: () => ({}),
          getRuntimeMcpServers: () => f.servers,
          addRuntimeMcpServer: (name: string, cfg: MCPServerConfig) => {
            f.servers[name] = cfg;
          },
        });
        f.pool.acquire.mockImplementation(async (_name, cfg) =>
          Object.assign(f.initial, {
            id: connectionIdOf('srv', cfg),
            transportId: connectionIdOf('srv', cfg),
            toolsSnapshot: [],
          }),
        );
        const raw = { command: 'node', cwd };
        await f.manager.addRuntimeMcpServer('srv', raw, 'client');
        const expected = { ...raw, cwd: cwd ?? '/workspace' };
        expect(f.pool.acquire.mock.calls[0][1]).toEqual(expected);
        expect(f.servers['srv']).toEqual(raw);
        // Re-adding the raw recipe must retain the same pooled process.
        await f.manager.addRuntimeMcpServer('srv', raw, 'client');
        ...
        expect(f.pool.acquireForRecovery).toHaveBeenCalledWith(
          'srv',
          expected,
          'session',
          expect.anything(),
  ...
  it('releases a failed resource handle even without a failed event', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    Object.assign(f.initial, { state: 'failed' });
    vi.spyOn(f.initial.client, 'getStatus').mockReturnValue(
      MCPServerStatus.DISCONNECTED,
    );
    await expect(
      f.manager.readResource('srv', 'test://resource'),
    ).rejects.toThrow('pool entry disconnected');
    expect(f.initial.release).toHaveBeenCalledTimes(1);
    const notices = await f.manager.recoverFailedConnections(
      new AbortController().signal,
    );
    expect(f.pool.acquireForRecovery).toHaveBeenCalledOnce();
  ...
  it('keeps a force-closed pool entry outside automatic resource recovery', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    Object.assign(f.initial, { state: 'closed' });
    await expect(
      f.manager.readResource('srv', 'test://resource'),
    ).rejects.toThrow('pool entry disconnected');
    expect(f.initial.client.readResource).not.toHaveBeenCalled();
    expect(f.initial.release).toHaveBeenCalledOnce();
    await f.manager.recoverFailedConnections(new AbortController().signal);
    expect(f.pool.acquireForRecovery).not.toHaveBeenCalled();
    f.pool.acquire.mockResolvedValueOnce(f.replacement);
    await f.manager.discoverAllMcpTools(f.config);
    expect(f.pool.acquire).toHaveBeenCalledTimes(2);
    expect(f.manager.getServerStatus('srv')).toBe(MCPServerStatus.CONNECTED);
    await f.manager.stop();
  });

  it('recovers resource reads through the pool and leaves the notice for ACP', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    const standaloneCount = vi.mocked(McpClient).mock.calls.length;
    await expect(
      f.manager.readResource('srv', 'test://resource'),
    ).resolves.toEqual({ contents: [] });
    expect(f.pool.acquireForRecovery).toHaveBeenCalledOnce();
    ...
    expect(vi.mocked(McpClient).mock.calls.length).toBe(standaloneCount);
    expect(
      (
        await f.manager.recoverFailedConnections(new AbortController().signal)
      )[0].message,
  ...
  it.each(['never acquired', 'explicit disconnect'])(
    'does not create a standalone resource client in pool mode: %s',
    async (state) => {
      const f = fixture();
      if (state === 'explicit disconnect') {
        await f.manager.discoverAllMcpTools(f.config);
  ...
  it.each(['trust', 'approval', 'disabled'])(
    'rechecks %s before reading a resource after recovery finishes',
    async (change) => {
      const f = fixture();
      await f.manager.discoverAllMcpTools(f.config);
      f.fail();
      const recover = f.manager.recoverFailedConnections.bind(f.manager);
      vi.spyOn(f.manager, 'recoverFailedConnections').mockImplementation(
        async (...args) => {
          const result = await recover(...args);
          // Another server can keep the bulk recovery pending after srv has
          // reconnected. Revoke policy before the waiting read resumes.
      ...
      expect(f.replacement.client.readResource).not.toHaveBeenCalled();
      await f.manager.stop();
    },
  );

  it.each(['trust', 'approval', 'disabled', 'removed', 'changed'])(
    'rejects a pooled resource read after %s changes',
    async (change) => {
      const f = fixture();
      await f.manager.discoverAllMcpTools(f.config);
      if (change === 'trust') f.revokeTrust();
      if (change === 'approval') f.requireApproval();
      if (change === 'disabled') f.disable();
      if (change === 'removed') delete f.servers['srv'];
      if (change === 'changed')
        f.servers['srv'] = new MCPServerConfig('different-command');
      await expect(
        f.manager.readResource('srv', 'test://resource'),
      ).rejects.toThrow();
      expect(f.initial.client.readResource).not.toHaveBeenCalled();
  ...
  it.each(['disconnected', 'registration failure'])(
    ...
    async (failure) => {
      ...
      expect(f.pool.recordRecoveryFailure).toHaveBeenCalledExactlyOnceWith(
        f.initial.transportId,
      );
    },
  );
  ...
  it.each(['stop', 'disconnect'])(
    '%s invalidates a config reconciliation queued behind recovery',
    async (action) => {
      const f = fixture();
      await f.manager.discoverAllMcpTools(f.config);
      f.fail();
      ...
      const stopping =
        action === 'stop'
          ? f.manager.stop()
          : f.manager.disconnectServer('srv');
      complete(f.replacement);
      await Promise.all([recovering, refresh, stopping]);
      expect(f.replacement.release).toHaveBeenCalledTimes(1);
      expect(f.pool.acquire).toHaveBeenCalledTimes(1);
      // A subsequent explicit refresh remains an authorized new operation.
      await f.manager.discoverAllMcpTools(f.config);
      expect(f.pool.acquire).toHaveBeenCalledTimes(2);
    },
  );

  it.each(['reacquired', 'removed', 'disabled'])(
    'drops a retained recovery failure after the server is %s',
    async (action) => {
      const f = fixture();
      await f.manager.discoverAllMcpTools(f.config);
      f.fail();
      const controller = new AbortController();
      f.pool.acquireForRecovery.mockImplementationOnce(async () => {
        controller.abort();
        throw new Error('connection refused');
      });
      await expect(
        f.manager.recoverFailedConnections(controller.signal),
      ).resolves.toEqual([]);
  ...
  it.each(['trust', 'disabled'])(
    'disconnect preserves a queued sibling %s policy update',
    async (policy) => {
      const f = fixture();
      f.servers['other'] = { command: 'other', trust: true };
      const other = Object.assign(new EventEmitter(), {
        id: connectionIdOf('other', f.servers['other']),
        transportId: connectionIdOf('other', f.servers['other']),
        client: f.initial.client,
        release: vi.fn(),
        updateConfig: vi.fn(),
      });
      f.pool.acquire.mockImplementation(async (name) =>
        name === 'other' ? other : f.initial,
      );
      await f.manager.discoverAllMcpTools(f.config);
      f.fail();
      let complete!: (conn: typeof f.replacement) => void;
      ...
      if (policy === 'trust')
        f.servers['other'] = { ...f.servers['other'], trust: false };
      else
        vi.spyOn(f.config, 'isMcpServerDisabled').mockImplementation(
          (name) => name === 'other',
        );
      const refresh = f.manager.discoverAllMcpTools(f.config);
      await f.manager.disconnectServer('srv');
      ...
      expect(f.replacement.release).toHaveBeenCalledTimes(1);
      if (policy === 'trust') {
        expect(other.updateConfig).toHaveBeenCalledWith(
  ...
  it('releases a late acquire when disconnect supersedes a queued refresh', async () => {
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    let completeRecovery!: (conn: typeof f.replacement) => void;
    f.pool.acquireForRecovery.mockReturnValue(
      new Promise((resolve) => {
    ...
    let completeAcquire!: (conn: typeof other) => void;
    f.pool.acquire.mockReturnValue(
      new Promise((resolve) => {
    ...
    const refresh = f.manager.discoverAllMcpTools(f.config);
    completeRecovery(f.replacement);
    await vi.waitFor(() => expect(f.pool.acquire).toHaveBeenCalledTimes(2));
    await f.manager.disconnectServer('other');
    completeAcquire(other);
    await Promise.all([recovering, refresh]);
    expect(other.release).toHaveBeenCalledTimes(1);
    await f.manager.stop();
    expect(other.release).toHaveBeenCalledTimes(1);
  ...
  it('reports workspace budget refusal and completes its refusal batch', async () => {
    const { BudgetExhaustedError } = await import('./mcp-client-manager.js');
    const f = fixture();
    await f.manager.discoverAllMcpTools(f.config);
    f.fail();
    ...
    Object.assign(f.pool, { getBudget: () => budget });
    f.pool.acquireForRecovery.mockRejectedValue(
      new BudgetExhaustedError('srv', 1, 1),
    );
    const notices = await f.manager.recoverFailedConnections(
      new AbortController().signal,
    );
    expect(notices[0].message).toContain(
      'workspace MCP client budget is exhausted',
    );
    expect(notices[0].message).not.toContain('authentication');
    expect(budget.beginBulkPass).toHaveBeenCalledOnce();
    expect(budget.endBulkPass).toHaveBeenCalledOnce();
  });
