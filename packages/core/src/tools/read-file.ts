...
> {
    ...
    super(
      ...
      Kind.Read,
      {
        properties: {
          ...
          offset: {
            description:
              "Optional: For text files, the 0-based line number to start reading from. Requires 'limit' to be set. Use for paginating through large files. Omit for Jupyter notebooks (.ipynb).",
            type: 'integer',
          },
          limit: {
            description:
              "Optional: For text files, maximum number of lines to read. Use with 'offset' to paginate through large files. If omitted, reads the entire file (if feasible, up to a default limit). Omit for Jupyter notebooks (.ipynb).",
            type: 'integer',
          },
          pages: {
            description: `Optional: For PDF files, the page range to extract as text (e.g., '1-5', '3', '10-20'). Pages are 1-indexed. Max ${PDF_MAX_PAGES_PER_READ} pages per request. Open-ended ranges like '3-' are not supported. Use this for large PDFs or when the model does not support native PDF input. Omit for Jupyter notebooks (.ipynb).`,
            type: 'string',
          },
        },
        required: ['file_path'],
  ...
  protected override validateToolParamValues(
    ...
    if (
      ...
      return `For Jupyter notebooks (.ipynb), omit 'offset', 'limit', and 'pages' entirely; do not use 0 or null. Retry with only: ${JSON.stringify({ file_path: filePath })}`;
    }
