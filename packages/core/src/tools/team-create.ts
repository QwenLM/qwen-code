...
- **Do not treat idle as an error.** A teammate sending a message and then going idle is the normal flow—they sent their message and are now waiting for a response. When a teammate's turn ends, the runtime forwards that teammate's final answer to you automatically; if they also called send_message earlier, that earlier report is delivered too. There is no summary of teammate-to-teammate messages; ask the teammate directly when you need peer-collaboration detail.
