...
- **Do not treat idle as an error.** A teammate sending a message and then going idle is the normal flow—they sent their message and are now waiting for a response. If a teammate's turn ends without an explicit report, the runtime forwards that teammate's final answer to you automatically—that is what you receive on idle. There is no summary of teammate-to-teammate messages; ask the teammate directly when you need peer-collaboration detail.
