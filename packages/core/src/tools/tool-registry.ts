...
import { isModelVisibleTool } from './tool-exposure.js';
...
export class ToolRegistry {
  ...
  isDeferredTool(name: string): boolean {
    ...
    return tool ? this.isEffectivelyDeferred(tool) : false;
  }
  ...
  getFunctionDeclarations(options?: {
    ...
    const mode =
      options?.mode ?? this.config.getEffectiveToolMode?.() ?? ToolMode.Direct;
    return Array.from(this.tools.values())
      .filter(
        (tool) =>
          isModelVisibleTool(tool.name, mode) &&
  ...
  getFunctionDeclarationsFiltered(toolNames: string[]): FunctionDeclaration[] {
    ...
    const mode = this.config.getEffectiveToolMode?.() ?? ToolMode.Direct;
    for (const name of toolNames) {
      const tool = this.tools.get(name);
      if (tool && isModelVisibleTool(tool.name, mode)) {
