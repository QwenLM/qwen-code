...
import { runWithMeshRunContext } from '../agents/mesh/run-context.js';
...
import type { MeshAgent, Thread, ThreadRun } from '../agents/mesh/types.js';

const PROJECT_ROOT = '/mesh-tools-test';
...
describe('mesh thread tools', () => {
  ...
  it('exposes no thread, author, run or idempotency id in any mutating schema', () => {
    ...
    for (const Tool of MESH_THREAD_TOOLS) {
  ...
  it('refuses to act outside a mesh run', async () => {
  ...
  it('posts as the bound agent with its run recorded as the source', async () => {
    ...
    const result = await runWithMeshRunContext(frame(thread), () =>
  ...
  it('creates a sub-thread under the ambient thread, not a remembered one', async () => {
    ...
    await runWithMeshRunContext(frame(second), () =>
      new ThreadCreateTool(config)
        .build({ title: 'from the second turn' })
        .execute(new AbortController().signal),
    );

    const { threads } = await import('../agents/mesh/mesh-store.js').then((m) =>
      m.listThreads(PROJECT_ROOT),
    );
  ...
  it('assigning a sub-thread books the assignee through admission', async () => {
    ...
    const result = await runWithMeshRunContext(frame(parent), () =>
      new ThreadCreateTool(config)
  ...
  it('rejects an unknown or disabled assignee by name', async () => {
    ...
    const unknown = await runWithMeshRunContext(frame(parent), () =>
      new ThreadCreateTool(config)
    ...
    const disabled = await runWithMeshRunContext(frame(parent), () =>
      new ThreadCreateTool(config)
  ...
  it('reads another thread in the workspace, marked as untrusted', async () => {
    ...
    const result = await runWithMeshRunContext(frame(mine), () =>
