 ...
 * to — or undefined for a frame that names none. A host holding one
 * session has one set of settings and ignores the argument. A host of
 * several (see `resolveSessionId`) can hold sessions from different
...
export interface InboundGateOptions {
  ...
  /**
   * For a process hosting several sessions: the one name it keeps for the
   * session `id` names, or undefined for a session it does not hold.
   *
   ...
   *
   * It answers with a name rather than yes-or-no because a host can hold
   * one session under more than one spelling — it publishes a session
   * under one id and `/clear` swaps the id it answers to — and the host
   * is the only party that knows the two are one session. The gate
   * judges, buckets and meters by the name it gets back, so a sender
   * alternating between the spellings gets one session's allowance and
   * one session's settings, not two. Case folding belongs here too: the
   * gate compares the names as given.
   */
  ...
  /**
   * Whether this host can put a parked message in front of someone.
   * Defaults to true. False turns every hold into a refusal — see
   * `admit` — for a host with no review surface of its own.
   */
...
export class InboundGate {
  ...
  constructor(private readonly options: InboundGateOptions) {
    ...
    this.answersForSeveralSessions = options.resolveSessionId !== undefined;
  }
  ...
  resolvePolicy(
    ...
    try {
      const configured = this.options.getPolicySetting(addressed);
      if (configured !== undefined && !isInboundPolicy(configured)) {
        ...
        return this.hold('policy-unreadable', this.policyScope(addressed));
      }
      explicit = configured;
    } catch (error) {
      debugLogger.debug(
    ...
    if (explicit === 'hold') {
      return this.hold('explicit-setting', this.policyScope(addressed));
    }
    ...
    try {
      mode = this.options.getApprovalMode(addressed);
    } catch (error) {
      debugLogger.debug(
  ...
  admit(
    ...
    // wires `resolveSessionId` instead, has no single id to compare, and
    ...
    if (settled !== undefined) {
      ...
      this.forgetAdmittedBody(frame, origin, this.dedupScope(frame, lookup));
    ...
    // sender would have been told `held`, which promises a person will
    // look. `refused` is the truth: this receiver will not take it.
    //
    // Decided here rather than by the host answering `refuse` to
    // `getPolicySetting`, because that reader cannot tell a message that
    // would be held from one that would be accepted: an explicit setting
    // outranks both a trusted controller and this session's own
  ...
  reevaluate(reason: string): number {
    ...
    // Carries the pin answer the judging loop already paid for, so the
    // release below does not ask the host a second time — and cannot get
    // a different answer than the one the message was judged on.
    const release: Array<{
      entry: HeldMessage;
      pin?: 'here' | 'gone' | 'unknown';
    }> = [];
    let dropped = 0;
    ...
    for (const entry of this.held) {
      let pin: 'here' | 'gone' | 'unknown' | undefined;
      if (dropMessagesForSessionsThatLeft) {
        pin = this.pinStatus(entry.frame);
        if (pin === 'gone') {
          misaddressed += 1;
          this.settleMisaddressed(entry);
          continue;
      ...
      // Judged for the session it was parked for, not for whatever the
      // host answers now. Re-reading the name here would let one sweep
      // read this message's lifetime for one session and its policy for
      // another — and a resolver that throws mid-sweep would rewrite an
      // explicit hold into `policy-unreadable` and lose its scope.
      const decision = this.resolvePolicy(
        entry.frame,
        originOf(entry),
        this.heldLookup(entry),
      );
      const { policy } = decision;
      if (policy === 'accept') {
        release.push({ entry, pin });
    ...
    let released = 0;
    for (const { entry, pin: judgedPin } of release) {
      const pin = judgedPin ?? this.pinStatus(entry.frame);
      if (pin === 'gone') {
        misaddressed += 1;
        this.settleMisaddressed(entry);
        continue;
  ...
  /**
   * Ask the host which session a frame is addressed to.
   *
   * An empty answer means there was nobody to ask — no resolver wired, or
   * an unpinned frame — or that the host does not hold that session;
   * callers fall back to the id on the frame, which is then all there is.
   *
   * `unreadable` is not the same as an empty answer. A resolver that
   * threw did not say "no", it said nothing, and a message whose
   * addressee is unknown is held rather than judged by a spelling that
   * may answer to some other session.
   */
  ...
  private forgetAdmittedBody(
    frame: PeerUserFrame,
    origin: PeerOrigin,
    recordedScope: string | undefined,
  ): void {
    this.admission.forgetBody(
      peerSenderKey(frame, origin),
      frame.message.content,
      frame.msgId,
      // Always the scope the record was written under: a parked entry
      // hands back what it was parked with, and `admit` hands back the
      // lookup it opened with. Deriving one here instead would ask the
      // host again — after the settings readers have run, which is a
      // chance for the session map to have moved — and a rollback keyed
      // differently from its record is a silent no-op.
      recordedScope,
    );
  }
