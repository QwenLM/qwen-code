const toCsv = (xs) => (xs && xs.length ? xs.join(',') : undefined);
export async function checkText({ server = 'http://localhost:8081', text, language = 'pt-BR', motherTongue, enabledRules, disabledRules, level = 'default', }) {
    const body = new URLSearchParams();
    body.set('text', text);
    body.set('language', language);
    if (motherTongue)
        body.set('motherTongue', motherTongue);
    if (enabledRules?.length)
        body.set('enabledRules', toCsv(enabledRules));
    if (disabledRules?.length)
        body.set('disabledRules', toCsv(disabledRules));
    if (level)
...
export function applySuggestions(original, matches, strategy = 'first') {
    ...
    for (const m of sorted) {
        ...
        if (offset < cursor || offset > original.length)
            continue;
...
//# sourceMappingURL=ltClient.js.map