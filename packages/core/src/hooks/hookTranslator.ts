...
/**
 * Decoupled LLM request format - stable across Qwen Code versions
 */
...
/**
 * Decoupled LLM response format - stable across Qwen Code versions
 */
...
/**
 * Decoupled tool configuration - stable across Qwen Code versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_QWEN_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
