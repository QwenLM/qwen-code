...
// `defaultSpawnChannelFactory` (the production spawn factory + its
// `killChild` / `SCRUBBED_CHILD_ENV_KEYS` helpers) lifted to
// `@qwen-code/acp-bridge/spawnChannel` in #4175 PR F1 so
// `channels/base/AcpBridge.ts` and the VSCode IDE companion can share
// one spawn implementation. Imported (the factory below uses it as
// the default when `opts.channelFactory` is omitted) and re-exported
// for backward compatibility with `cli/src/serve/index.ts` and bridge
// tests that mock the factory.
import { defaultSpawnChannelFactory } from '@qwen-code/acp-bridge/spawnChannel';
export { defaultSpawnChannelFactory };

// `BridgeClient` class + `PendingPermission` + `PermissionResolutionRecord`
// + `MAX_RESOLVED_PERMISSION_RECORDS` lifted to
// `@qwen-code/acp-bridge/bridgeClient` in #4175 PR F1 (step 2). The
// factory below imports `BridgeClient` as a value (constructor),
// `PendingPermission` / `PermissionResolutionRecord` as types for its
// `pendingPermissions` / `resolvedPermissions` bookkeeping maps, and
// `MAX_RESOLVED_PERMISSION_RECORDS` for the duplicate-vote cache cap.
// Re-exported for backward compat (`ChannelInfo.client: BridgeClient`
// is declared below and `server.ts` doesn't reach into these types
// directly today, but the public re-export keeps the option open for
// embedders).
import {
  BridgeClient,
...
export { MAX_RESOLVED_PERMISSION_RECORDS };
