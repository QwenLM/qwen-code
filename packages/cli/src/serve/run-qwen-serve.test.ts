...
describe('runQwenServe startup observability', () => {
  ...
  it('forwards CDP tunnel auto-wire env only when the child can register tools', async () => {
    tmpDir = fs.realpathSync(
      fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-cdp-env-')),
    );
    ...
    try {
      ...
      const childEnvOverrides = (
        createBridge.mock.calls[0]?.[0] as {
          childEnvOverrides?: Record<string, string | undefined>;
        }
      ).childEnvOverrides;
      expect(childEnvOverrides).toMatchObject({
    ...
    const authPort = await getFreeLoopbackPort();
    const authHandle = await runQwenServe(
      {
        port: authPort,
        hostname: '127.0.0.1',
        mode: 'http-bridge',
    ...
    try {
      ...
      expect(childEnvOverrides).toMatchObject({
        QWEN_SERVE_CDP_TUNNEL_OVER_WS: undefined,
