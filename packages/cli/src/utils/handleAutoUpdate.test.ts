...
describe('handleAutoUpdate', () => {
  ...
  it('should show manual update message when enableAutoUpdate is false', () => {
    ...
    mockGetInstallationInfo.mockReturnValue({
      updateCommand: 'npm i -g @protolabs/proto@latest',
      updateMessage: 'Please run npm i -g @protolabs/proto@latest to update',
      isGlobal: true,
    ...
    expect(mockUpdateEventEmitter.emit).toHaveBeenCalledWith(
      ...
      {
        message:
          'An update is available!\nPlease run npm i -g @protolabs/proto@latest to update',
      },
    );
  ...
  it('should emit "update-failed" when the update process fails', async () => {
    ...
    expect(mockUpdateEventEmitter.emit).toHaveBeenCalledWith('update-failed', {
      message:
        'Automatic update failed. Please try updating manually. (command: npm i -g @protolabs/proto@2.0.0, stderr: An error occurred)',
    });
  });
  ...
  it('should use the "@nightly" tag for nightly updates', async () => {
    ...
    expect(mockSpawn).toHaveBeenCalledWith(
      ...
      expect.arrayContaining([
        ...
        'npm i -g @protolabs/proto@nightly',
      ]),
      {
        stdio: 'pipe',
      },
