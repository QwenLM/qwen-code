...
describe('startChildHeapProbe', () => {
  it('sums old-generation spaces rather than old_space alone', () => {
    ...
    try {
      expect(probe.snapshot().peakOldGenerationBytes).toBe(186 * 1024 * 1024);
    } finally {
      probe.stop();
    }
  });
  ...
  it('holds the high-water mark when usage falls back', () => {
    ...
    try {
      committed = 9_000;
      expect(probe.snapshot().peakOldGenerationBytes).toBe(9_000);
      committed = 10;
      expect(probe.snapshot().peakOldGenerationBytes).toBe(9_000);
    } finally {
      probe.stop();
  ...
  it('keeps the last good values when V8 readings throw', () => {
    ...
    try {
      expect(probe.snapshot().peakOldGenerationBytes).toBe(4_096);
      failing = true;
      const snapshot = probe.snapshot();
  ...
  it('classifies every heap space the running Node actually reports', () => {
    ...
    const probe = startChildHeapProbe();
    try {
      expect(probe.snapshot().unclassifiedSpaceNames).toEqual([]);
    } finally {
      probe.stop();
    }
  });
  ...
  it('tracks a real allocation upward against the real V8', () => {
    ...
    const probe = startChildHeapProbe();
    try {
      const before = probe.snapshot().peakOldGenerationBytes;
      const held: unknown[] = [];
      ...
      const after = probe.snapshot().peakOldGenerationBytes;
