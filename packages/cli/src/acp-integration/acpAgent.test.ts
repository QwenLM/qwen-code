...
describe('QwenAgent MCP SSE/HTTP support', () => {
  ...
  it('projects qwen3.8-max reasoning controls through one ACP option', async () => {
    const sessionId = 'qwen38-standard-reasoning-session';
    const innerConfig = await setupSessionMocks(sessionId);
    const generation: {
      reasoning?: false | { effort?: string };
  ...
  it('supports explicit off and reset on the fallback reasoning surface', async () => {
    ...
    try {
      ...
      expect(option).toMatchObject({ currentValue: 'none' });
  ...
  it('keeps the fallback default selection sticky across a rebuild', async () => {
    // The fallback surface's 'default' is an explicit session selection, so
    // later model/config reconciliation can reapply the canonical default.
    ...
    try {
      ...
      expect(generation.reasoning).toBeUndefined();
      expect(reasoningOverride).toBeUndefined();
  ...
  it('keeps a configured budget when the fallback surface selects default', async () => {
    // Models outside the reasoning registry fall back to the generic
    // surface; selecting 'Default' there must record the sticky default
    // override without replacing the reasoning block, or a configured
    // sibling budget_tokens (honored e.g. on the Anthropic wire) is silently
    // destroyed while the selection reports success.
    const sessionId = 'fallback-reasoning-budget-session';
    const innerConfig = await setupSessionMocks(sessionId);
    const generation: {
      reasoning?: false | { effort?: string; budget_tokens?: number };
  ...
  it('keeps qwen3.8-max effort controls when thinking is mandatory', async () => {
    ...
    innerConfig.getReasoningEffort = vi.fn(() => generation.reasoning.effort);
    innerConfig.setReasoningEffort = vi.fn((effort: string | undefined) => {
      if (effort) generation.reasoning = { effort };
    });
