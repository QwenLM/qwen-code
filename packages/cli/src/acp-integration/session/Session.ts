  ...
  async #buildMidTurnParts(
    ...
    for (const resolved of audioCheckedMessages) {
      ...
      if (
        ...
        resolved.message.mediaReferences
      ) {
        const everyMediaBlockHasAReference =
          resolved.message.mediaReferences.length ===
          resolved.message.content.filter((block) => block.type === 'image')
            .length;
        if (everyMediaBlockHasAReference) {
          recorder?.recordMidTurnUserMessage(
            stripReferencedInlineDataParts(finalized, resolved.message.content),
