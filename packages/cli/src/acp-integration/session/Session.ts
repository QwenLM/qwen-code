        ...
        return await withInteractionSpan(
          ...
          async () => {
            ...
            if (isSlashCommand(inputText)) {
              ...
              // (it returns only the content). The caller fires it after the
              // turn so an interactive `/auto-improve start` run isn't stranded.
  ...
  async #executeCronPromptInner(prompt: string): Promise<void> {
    return Storage.runWithRuntimeBaseDir(
      ...
      async () => {
        ...
        // stream) from a genuine error so the run is recorded 'cancelled'
        // rather than 'failed'.
        ...
        await withInteractionSpan(
          ...
          async () => {
            let turnCount = 0;
            try {
              // The cron prompt for auto-improve is a slash command
              // (`/auto-improve tick ...`); resolve it to the real prompt and
              // capture its onComplete (markRunCompleted) so the run is
              // recorded after the turn.
              ...
              if (isSlashCommand(prompt)) {
                ...
                if (processedParts === null) {
                  // No prompt was produced — don't let the finally record this
                  // as a successful run.
              ...
              // the turn finishes, even on abort/error paths, so an auto-improve
              // run isn't stranded. cancelled vs errored distinguished above.
              if (slashOnComplete) {
                try {
