  ...
  async #runStopContinuation(
    ...
    while (nextMessage !== null) {
      ...
      if (functionCalls.length > 0) {
        ...
        // need no rollback and are skipped by setting this after them. The
        // repeated-tool-failure stop below preserves the run into history
        // inside #buildNextMessageAfterToolRun, so a discarded snapshot
        // there is harmless too.
        pendingPresentationSnapshot = toolRun.presentationSnapshot;
        pendingPresentationGeneration =
          toolRun.presentationSnapshotGeneration ?? 0;
        if (nextAfterTools.stoppedByRepeatedToolFailure) {
