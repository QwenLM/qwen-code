...
export default {
  ...
  'Basics:': '基礎功能：',
  'Add context': '添加上下文',
  'Use {{symbol}} to specify files for context (e.g., {{example}}) to target specific files or folders.': '使用 {{symbol}} 指定文件作爲上下文（例如，{{example}}），用於定位特定文件或文件夾',
  '@': '@',
  ...
  'Execute shell commands via {{symbol}} (e.g., {{example1}}) or use natural language (e.g., {{example2}}).': '通過 {{symbol}} 執行 shell 命令（例如，{{example1}}）或使用自然語言（例如，{{example2}}）',
  '!': '!',
  '!npm run start': '!npm run start',
  'start server': 'start server',
  'Commands:': '命令：',
  'shell command': 'shell 命令',
  'Model Context Protocol command (from external servers)': '模型上下文協議命令（來自外部服務器）',
  ...
  'New line': '換行',
  'New line (Alt+Enter works for certain linux distros)': '換行（某些 Linux 發行版支持 Alt+Enter）',
  'Clear the screen': '清屏',
  ...
  'For a full list of shortcuts, see {{docPath}}': '完整快捷鍵列表，請參閱 {{docPath}}',
  ...
  'Memory Usage': '內存使用',
  'IDE Client': 'IDE 客戶端',
  'Analyzes the project and creates a tailored QWEN.md file.': '分析項目並創建定製的 QWEN.md 文件',
  'List available Qwen Code tools. Usage: /tools [desc]': '列出可用的 Qwen Code 工具。用法：/tools [desc]',
  'List available skills.': '列出可用技能。',
  'Available Qwen Code CLI tools:': '可用的 Qwen Code CLI 工具：',
  'No tools available': '沒有可用工具',
  'View or change the approval mode for tool usage': '查看或更改工具使用的審批模式',
  'Invalid approval mode "{{arg}}". Valid modes: {{modes}}': '無效的審批模式 "{{arg}}"。有效模式：{{modes}}',
  'Approval mode set to "{{mode}}"': '審批模式已設置爲 "{{mode}}"',
  'View or change the language setting': '查看或更改語言設置',
  'change the theme': '更改主題',
  'Select Theme': '選擇主題',
  'Preview': '預覽',
  '(Use Enter to select, Tab to configure scope)': '（使用 Enter 選擇，Tab 配置作用域）',
  '(Use Enter to apply scope, Tab to go back)': '（使用 Enter 應用作用域，Tab 返回）',
  'Theme configuration unavailable due to NO_COLOR env variable.': '由於 NO_COLOR 環境變量，主題配置不可用。',
  ...
  'Copy the last result or code snippet to clipboard': '將最後的結果或代碼片段複製到剪貼板',
  'Manage subagents for specialized task delegation.': '管理用於專門任務委派的子智能體',
  'Manage existing subagents (view, edit, delete).': '管理現有子智能體（查看、編輯、刪除）',
  ...
  'Enter to select, ↑↓ to navigate, Esc to go back': 'Enter 選擇，↑↓ 導航，Esc 返回',
  ...
  'Edit color': '編輯顏色',
  '❌ Error:': '❌ 錯誤:',
  'Are you sure you want to delete agent "{{name}}"?': '您確定要刪除智能體 "{{name}}" 嗎？',
  'Project Level (.qwen/agents/)': '項目級 (.qwen/agents/)',
  'User Level (~/.qwen/agents/)': '用戶級 (~/.qwen/agents/)',
  '✅ Subagent Created Successfully!': '✅ 子智能體創建成功！',
  'Subagent "{{name}}" has been saved to {{level}} level.': '子智能體 "{{name}}" 已保存到 {{level}} 級別。',
  'Name: ': '名稱: ',
  'Location: ': '位置: ',
  '❌ Error saving subagent:': '❌ 保存子智能體時出錯:',
  'Warnings:': '警告:',
  'Name "{{name}}" already exists at {{level}} level - will overwrite existing subagent': '名稱 "{{name}}" 在 {{level}} 級別已存在 - 將覆蓋現有子智能體',
  'Name "{{name}}" exists at user level - project level will take precedence': '名稱 "{{name}}" 在用戶級別存在 - 項目級別將優先',
  'Name "{{name}}" exists at project level - existing subagent will take precedence': '名稱 "{{name}}" 在項目級別存在 - 現有子智能體將優先',
  'Description is over {{length}} characters': '描述超過 {{length}} 個字符',
  'System prompt is over {{length}} characters': '系統提示超過 {{length}} 個字符',
  ...
  'Describe what this subagent should do and when it should be used. (Be comprehensive for best results)': '描述此子智能體應該做什麼以及何時使用它。（爲了獲得最佳效果，請全面描述）',
  'e.g., Expert code reviewer that reviews code based on best practices...': '例如：專業的代碼審查員，根據最佳實踐審查代碼...',
  ...
  'Press Enter to save, e to save and edit, Esc to go back': '按 Enter 保存，e 保存並編輯，Esc 返回',
  'Press Enter to continue, {{navigation}}Esc to {{action}}': '按 Enter 繼續，{{navigation}}Esc {{action}}',
  'cancel': '取消',
  'go back': '返回',
  '↑↓ to navigate, ': '↑↓ 導航，',
  ...
  'e.g., Reviews code for best practices and potential bugs.': '例如：審查代碼以查找最佳實踐和潛在錯誤。',
  'Description cannot be empty.': '描述不能爲空。',
  'Failed to launch editor: {{error}}': '啓動編輯器失敗: {{error}}',
  'Failed to save and edit subagent: {{error}}': '保存並編輯子智能體失敗: {{error}}',
  ...
  'Version:': '版本：',
  'Status:': '狀態：',
  'Are you sure you want to uninstall extension "{{name}}"?': '確定要卸載擴展 "{{name}}" 嗎？',
  ...
  'Failed to update extension "{{name}}": {{error}}': '更新擴展 "{{name}}" 失敗：{{error}}',
  ...
  'Settings': '設置',
  'To see changes, Qwen Code must be restarted. Press r to exit and apply changes now.': '要查看更改，必須重啓 Qwen Code。按 r 退出並立即應用更改。',
  ...
  'check session stats. Usage: /stats [model|tools]': '檢查會話統計信息。用法：/stats [model|tools]',
  'Show model-specific usage statistics.': '顯示模型相關的使用統計信息',
  'Show tool-specific usage statistics.': '顯示工具相關的使用統計信息',
  'exit the cli': '退出命令行界面',
  'Open MCP management dialog, or authenticate with OAuth-enabled servers': '打開 MCP 管理對話框，或在支持 OAuth 的服務器上進行身份驗證',
  'List configured MCP servers and tools, or authenticate with OAuth-enabled servers': '列出已配置的 MCP 服務器和工具，或使用支持 OAuth 的服務器進行身份驗證',
  'Manage workspace directories': '管理工作區目錄',
  'Add directories to the workspace. Use comma to separate multiple paths': '將目錄添加到工作區。使用逗號分隔多個路徑',
  'Show all directories in the workspace': '顯示工作區中的所有目錄',
  'set external editor preference': '設置外部編輯器首選項',
  'Select Editor': '選擇編輯器',
  'Editor Preference': '編輯器首選項',
  'These editors are currently supported. Please note that some editors cannot be used in sandbox mode.': '當前支持以下編輯器。請注意，某些編輯器無法在沙箱模式下使用。',
  'Your preferred editor is:': '您的首選編輯器是：',
  'Manage extensions': '管理擴展',
  'Manage installed extensions': '管理已安裝的擴展',
  'List active extensions': '列出活動擴展',
  'Update extensions. Usage: update <extension-names>|--all': '更新擴展。用法：update <extension-names>|--all',
  'Disable an extension': '禁用擴展',
  'Enable an extension': '啓用擴展',
  'Install an extension from a git repo or local path': '從 Git 倉庫或本地路徑安裝擴展',
  'Uninstall an extension': '卸載擴展',
  'No extensions installed.': '未安裝擴展。',
  'Usage: /extensions update <extension-names>|--all': '用法：/extensions update <擴展名>|--all',
  ...
  'Installing extension from "{{source}}"...': '正在從 "{{source}}" 安裝擴展...',
  'Extension "{{name}}" installed successfully.': '擴展 "{{name}}" 安裝成功。',
  'Failed to install extension from "{{source}}": {{error}}': '從 "{{source}}" 安裝擴展失敗：{{error}}',
  'Usage: /extensions uninstall <extension-name>': '用法：/extensions uninstall <擴展名>',
  'Uninstalling extension "{{name}}"...': '正在卸載擴展 "{{name}}"...',
  'Extension "{{name}}" uninstalled successfully.': '擴展 "{{name}}" 卸載成功。',
  'Failed to uninstall extension "{{name}}": {{error}}': '卸載擴展 "{{name}}" 失敗：{{error}}',
  'Usage: /extensions {{command}} <extension> [--scope=<user|workspace>]': '用法：/extensions {{command}} <擴展> [--scope=<user|workspace>]',
  'Unsupported scope "{{scope}}", should be one of "user" or "workspace"': '不支持的作用域 "{{scope}}"，應爲 "user" 或 "workspace"',
  'Extension "{{name}}" disabled for scope "{{scope}}"': '擴展 "{{name}}" 已在作用域 "{{scope}}" 中禁用',
  'Extension "{{name}}" enabled for scope "{{scope}}"': '擴展 "{{name}}" 已在作用域 "{{scope}}" 中啓用',
  'Do you want to continue? [Y/n]: ': '是否繼續？[Y/n]：',
  'Do you want to continue?': '是否繼續？',
  'Installing extension "{{name}}".': '正在安裝擴展 "{{name}}"。',
  '**Extensions may introduce unexpected behavior. Ensure you have investigated the extension source and trust the author.**': '**擴展可能會引入意外行爲。請確保您已調查過擴展源並信任作者。**',
  'This extension will run the following MCP servers:': '此擴展將運行以下 MCP 服務器：',
  'local': '本地',
  'remote': '遠程',
  'This extension will add the following commands: {{commands}}.': '此擴展將添加以下命令：{{commands}}。',
  'This extension will append info to your QWEN.md context using {{fileName}}': '此擴展將使用 {{fileName}} 向您的 QWEN.md 上下文追加信息',
  ...
  'You are installing an extension from {{originSource}}. Some features may not work perfectly with Qwen Code.': '您正在安裝來自 {{originSource}} 的擴展。某些功能可能無法完美兼容 Qwen Code。',
  '--ref and --auto-update are not applicable for marketplace extensions.': '--ref 和 --auto-update 不適用於市場擴展。',
  'Extension "{{name}}" installed successfully and enabled.': '擴展 "{{name}}" 安裝成功並已啓用。',
  'Installs an extension from a git repository URL, local path, or claude marketplace (marketplace-url:plugin-name).': '從 Git 倉庫 URL、本地路徑或 Claude 市場（marketplace-url:plugin-name）安裝擴展。',
  'The github URL, local path, or marketplace source (marketplace-url:plugin-name) of the extension to install.': '要安裝的擴展的 GitHub URL、本地路徑或市場源（marketplace-url:plugin-name）。',
  ...
  'Acknowledge the security risks of installing an extension and skip the confirmation prompt.': '確認安裝擴展的安全風險並跳過確認提示。',
  'The source argument must be provided.': '必須提供來源參數。',
  'Extension "{{name}}" successfully uninstalled.': '擴展 "{{name}}" 卸載成功。',
  'Uninstalls an extension.': '卸載擴展。',
  'The name or source path of the extension to uninstall.': '要卸載的擴展的名稱或源路徑。',
  'Please include the name of the extension to uninstall as a positional argument.': '請將要卸載的擴展名稱作爲位置參數。',
  'Enables an extension.': '啓用擴展。',
  'The name of the extension to enable.': '要啓用的擴展名稱。',
  'The scope to enable the extenison in. If not set, will be enabled in all scopes.': '啓用擴展的作用域。如果未設置，將在所有作用域中啓用。',
  'Extension "{{name}}" successfully enabled for scope "{{scope}}".': '擴展 "{{name}}" 已在作用域 "{{scope}}" 中啓用。',
  'Extension "{{name}}" successfully enabled in all scopes.': '擴展 "{{name}}" 已在所有作用域中啓用。',
  'Invalid scope: {{scope}}. Please use one of {{scopes}}.': '無效的作用域：{{scope}}。請使用 {{scopes}} 之一。',
  'Disables an extension.': '禁用擴展。',
  'The name of the extension to disable.': '要禁用的擴展名稱。',
  'The scope to disable the extenison in.': '禁用擴展的作用域。',
  'Extension "{{name}}" successfully disabled for scope "{{scope}}".': '擴展 "{{name}}" 已在作用域 "{{scope}}" 中禁用。',
  'Extension "{{name}}" successfully updated: {{oldVersion}} → {{newVersion}}.': '擴展 "{{name}}" 更新成功：{{oldVersion}} → {{newVersion}}。',
  'Unable to install extension "{{name}}" due to missing install metadata': '由於缺少安裝元數據，無法安裝擴展 "{{name}}"',
  'Extension "{{name}}" is already up to date.': '擴展 "{{name}}" 已是最新版本。',
  'Updates all extensions or a named extension to the latest version.': '將所有擴展或指定擴展更新到最新版本。',
  ...
  'Extension "{{name}}" linked successfully and enabled.': '擴展 "{{name}}" 鏈接成功並已啓用。',
  'Links an extension from a local path. Updates made to the local path will always be reflected.': '從本地路徑鏈接擴展。對本地路徑的更新將始終反映。',
  ...
  'The setting to configure (name or env var).': '要配置的設置（名稱或環境變量）。',
  'The scope to set the setting in.': '設置配置的作用域。',
  'List all settings for an extension.': '列出擴展的所有設置。',
  'Name of the extension.': '擴展名稱。',
  'Extension "{{name}}" has no settings to configure.': '擴展 "{{name}}" 沒有可配置的設置。',
  ...
  'Select a plugin to install from marketplace "{{name}}":': '從市場 "{{name}}" 中選擇要安裝的插件：',
  'Plugin selection cancelled.': '插件選擇已取消。',
  'Select a plugin from "{{name}}"': '從 "{{name}}" 中選擇插件',
  'Use ↑↓ or j/k to navigate, Enter to select, Escape to cancel': '使用 ↑↓ 或 j/k 導航，回車選擇，Esc 取消',
  ...
  'install required IDE companion for {{ideName}}': '安裝 {{ideName}} 所需的 IDE 配套工具',
  'enable IDE integration': '啓用 IDE 集成',
  'disable IDE integration': '禁用 IDE 集成',
  'IDE integration is not supported in your current environment. To use this feature, run Qwen Code in one of these supported IDEs: VS Code or VS Code forks.': '您當前環境不支持 IDE 集成。要使用此功能，請在以下支持的 IDE 之一中運行 Qwen Code：VS Code 或 VS Code 分支版本。',
  'Set up GitHub Actions': '設置 GitHub Actions',
  'Configure terminal keybindings for multiline input (VS Code, Cursor, Windsurf, Trae)': '配置終端按鍵綁定以支持多行輸入（VS Code、Cursor、Windsurf、Trae）',
  'Please restart your terminal for the changes to take effect.': '請重啓終端以使更改生效。',
  'Failed to configure terminal: {{error}}': '配置終端失敗：{{error}}',
  'Could not determine {{terminalName}} config path on Windows: APPDATA environment variable is not set.': '無法確定 {{terminalName}} 在 Windows 上的配置路徑：未設置 APPDATA 環境變量。',
  '{{terminalName}} keybindings.json exists but is not a valid JSON array. Please fix the file manually or delete it to allow automatic configuration.': '{{terminalName}} keybindings.json 存在但不是有效的 JSON 數組。請手動修復文件或刪除它以允許自動配置。',
  'File: {{file}}': '文件：{{file}}',
  'Failed to parse {{terminalName}} keybindings.json. The file contains invalid JSON. Please fix the file manually or delete it to allow automatic configuration.': '解析 {{terminalName}} keybindings.json 失敗。文件包含無效的 JSON。請手動修復文件或刪除它以允許自動配置。',
  ...
  'Existing keybindings detected. Will not modify to avoid conflicts.': '檢測到現有按鍵綁定。爲避免衝突，不會修改。',
  'Please check and modify manually if needed: {{file}}': '如有需要，請手動檢查並修改：{{file}}',
  'Added Shift+Enter and Ctrl+Enter keybindings to {{terminalName}}.': '已爲 {{terminalName}} 添加 Shift+Enter 和 Ctrl+Enter 按鍵綁定。',
  'Modified: {{file}}': '已修改：{{file}}',
  '{{terminalName}} keybindings already configured.': '{{terminalName}} 按鍵綁定已配置。',
  'Failed to configure {{terminalName}}.': '配置 {{terminalName}} 失敗。',
  'Your terminal is already configured for an optimal experience with multiline input (Shift+Enter and Ctrl+Enter).': '您的終端已配置爲支持多行輸入（Shift+Enter 和 Ctrl+Enter）的最佳體驗。',
  ...
  'Press Escape, Ctrl+C, or Ctrl+D to cancel': '按 Escape、Ctrl+C 或 Ctrl+D 取消',
  ...
  'This menu is read-only. To add or modify hooks, edit settings.json directly or ask Qwen Code.': '此菜單爲只讀。要添加或修改 Hook，請直接編輯 settings.json 或詢問 Qwen Code。',
  'Enter to select · Esc to cancel': 'Enter 選擇 · Esc 取消',
  'Exit codes:': '退出碼：',
  'Configured hooks:': '已配置的 Hook：',
  'No hooks configured for this event.': '此事件未配置 Hook。',
  'To add hooks, edit settings.json directly or ask Qwen.': '要添加 Hook，請直接編輯 settings.json 或詢問 Qwen。',
  ...
  'To modify or remove this hook, edit settings.json directly or ask Qwen to help.': '要修改或刪除此 Hook，請直接編輯 settings.json 或詢問 Qwen。',
  'Hook Configuration - Disabled': 'Hook 配置 - 已禁用',
  'All hooks are currently disabled. You have {{count}} that are not running.': '所有 Hook 當前已禁用。您有 {{count}} 未運行。',
  ...
  'Tool operations will proceed without hook validation': '工具操作將在沒有 Hook 驗證的情況下繼續',
  'To re-enable hooks, remove "disableAllHooks" from settings.json or ask Qwen Code.': '要重新啓用 Hook，請從 settings.json 中刪除 "disableAllHooks" 或詢問 Qwen Code。',
  'Project': '項目',
  'User': '用戶',
  'System': '系統',
  'Extension': '擴展',
  ...
  'When a subagent (Agent tool call) is started': '子智能體（Agent 工具調用）啓動時',
  ...
  'Input to command is JSON of tool call arguments.': '命令輸入爲工具調用參數的 JSON。',
  'Input to command is JSON with fields "inputs" (tool call arguments) and "response" (tool call response).': '命令輸入爲包含 "inputs"（工具調用參數）和 "response"（工具調用響應）字段的 JSON。',
  'Input to command is JSON with tool_name, tool_input, tool_use_id, error, error_type, is_interrupt, and is_timeout.': '命令輸入爲包含 tool_name、tool_input、tool_use_id、error、error_type、is_interrupt 和 is_timeout 的 JSON。',
  'Input to command is JSON with notification message and type.': '命令輸入爲包含通知消息和類型的 JSON。',
  'Input to command is JSON with original user prompt text.': '命令輸入爲包含原始用戶提示文本的 JSON。',
  'Input to command is JSON with session start source.': '命令輸入爲包含會話啓動來源的 JSON。',
  'Input to command is JSON with session end reason.': '命令輸入爲包含會話結束原因的 JSON。',
  'Input to command is JSON with agent_id and agent_type.': '命令輸入爲包含 agent_id 和 agent_type 的 JSON。',
  'Input to command is JSON with agent_id, agent_type, and agent_transcript_path.': '命令輸入爲包含 agent_id、agent_type 和 agent_transcript_path 的 JSON。',
  'Input to command is JSON with compaction details.': '命令輸入爲包含壓縮詳情的 JSON。',
  'Input to command is JSON with tool_name, tool_input, and tool_use_id. Output JSON with hookSpecificOutput containing decision to allow or deny.': '命令輸入爲包含 tool_name、tool_input 和 tool_use_id 的 JSON。輸出包含 hookSpecificOutput 的 JSON，其中包含允許或拒絕的決定。',
  'stdout/stderr not shown': 'stdout/stderr 不顯示',
  'show stderr to model and continue conversation': '向模型顯示 stderr 並繼續對話',
  ...
  'show stderr to user only but continue with tool call': '僅向用戶顯示 stderr 但繼續工具調用',
  'block processing, erase original prompt, and show stderr to user only': '阻止處理，擦除原始提示，僅向用戶顯示 stderr',
  'stdout shown to Qwen': '向 Qwen 顯示 stdout',
  'show stderr to user only (blocking errors ignored)': '僅向用戶顯示 stderr（忽略阻塞錯誤）',
  'command completes successfully': '命令成功完成',
  'stdout shown to subagent': '向子智能體顯示 stdout',
  'show stderr to subagent and continue having it run': '向子智能體顯示 stderr 並繼續運行',
  'stdout appended as custom compact instructions': 'stdout 作爲自定義壓縮指令追加',
  'block compaction': '阻止壓縮',
  'show stderr to user only but continue with compaction': '僅向用戶顯示 stderr 但繼續壓縮',
  'use hook decision if provided': '如果提供則使用 Hook 決定',
  'Config not loaded.': '配置未加載。',
  'Hooks are not enabled. Enable hooks in settings to use this feature.': 'Hook 未啓用。請在設置中啓用 Hook 以使用此功能。',
  'No hooks configured. Add hooks in your settings.json file.': '未配置 Hook。請在 settings.json 文件中添加 Hook。',
  'Configured Hooks ({{count}} total)': '已配置的 Hook（共 {{count}} 個）',
  'Export current session message history to a file': '將當前會話的消息記錄導出到文件',
  'Export session to HTML format': '將會話導出爲 HTML 文件',
  'Export session to JSON format': '將會話導出爲 JSON 文件',
  'Export session to JSONL format (one message per line)': '將會話導出爲 JSONL 文件（每行一條消息）',
  'Export session to markdown format': '將會話導出爲 Markdown 文件',
  'generate personalized programming insights from your chat history': '根據你的聊天記錄生成個性化編程洞察',
  'Resume a previous session': '恢復先前會話',
  'Restore a tool call. This will reset the conversation and file history to the state it was in when the tool call was suggested': '恢復某次工具調用。這將把對話與文件歷史重置到提出該工具調用建議時的狀態',
  'Could not detect terminal type. Supported terminals: VS Code, Cursor, Windsurf, and Trae.': '無法檢測終端類型。支持的終端：VS Code、Cursor、Windsurf 和 Trae。',
  'Terminal "{{terminal}}" is not supported yet.': '終端 "{{terminal}}" 尚未支持。',
  'Invalid language. Available: {{options}}': '無效的語言。可用選項：{{options}}',
  'Language subcommands do not accept additional arguments.': '語言子命令不接受額外參數',
  ...
  'LLM output language rule file generated at {{path}}': 'LLM 輸出語言規則文件已生成於 {{path}}',
  'Please restart the application for the changes to take effect.': '請重啓應用程序以使更改生效。',
  'Failed to generate LLM output language rule file: {{error}}': '生成 LLM 輸出語言規則文件失敗：{{error}}',
  'Invalid command. Available subcommands:': '無效的命令。可用的子命令：',
  'Available subcommands:': '可用的子命令：',
  'To request additional UI language packs, please open an issue on GitHub.': '如需請求其他 UI 語言包，請在 GitHub 上提交 issue',
  ...
  'Approval mode changed to: {{mode}} (saved to {{scope}} settings{{location}})': '審批模式已更改爲：{{mode}}（已保存到{{scope}}設置{{location}}）',
  'Usage: /approval-mode <mode> [--session|--user|--project]': '用法：/approval-mode <mode> [--session|--user|--project]',
  'Scope subcommands do not accept additional arguments.': '作用域子命令不接受額外參數',
  'Plan mode - Analyze only, do not modify files or execute commands': '規劃模式 - 僅分析，不修改文件或執行命令',
  'Default mode - Require approval for file edits or shell commands': '默認模式 - 需要批准文件編輯或 shell 命令',
  'Auto-edit mode - Automatically approve file edits': '自動編輯模式 - 自動批准文件編輯',
  'YOLO mode - Automatically approve all tools': 'YOLO 模式 - 自動批准所有工具',
  '{{mode}} mode': '{{mode}} 模式',
  'Settings service is not available; unable to persist the approval mode.': '設置服務不可用；無法持久化審批模式。',
  ...
  'Analyze only, do not modify files or execute commands': '僅分析，不修改文件或執行命令',
  'Require approval for file edits or shell commands': '需要批准文件編輯或 shell 命令',
  'Automatically approve file edits': '自動批准文件編輯',
  'Automatically approve all tools': '自動批准所有工具',
  'Workspace approval mode exists and takes priority. User-level change will have no effect.': '工作區審批模式已存在並具有優先級。用戶級別的更改將無效。',
  'Apply To': '應用於',
  ...
  'Usage: /memory add --project <text to remember>': '用法：/memory add --project <要記住的文本>',
  'Usage: /memory add --global <text to remember>': '用法：/memory add --global <要記住的文本>',
  'Attempting to save to project memory: "{{text}}"': '正在嘗試保存到項目記憶："{{text}}"',
  'Attempting to save to global memory: "{{text}}"': '正在嘗試保存到全局記憶："{{text}}"',
  'Current memory content from {{count}} file(s):': '來自 {{count}} 個文件的當前記憶內容：',
  ...
  'Usage: /memory add [--global|--project] <text to remember>': '用法：/memory add [--global|--project] <要記住的文本>',
  'Attempting to save to memory {{scope}}: "{{fact}}"': '正在嘗試保存到記憶 {{scope}}："{{fact}}"',
  'Open auto-memory folder': '打開自動記憶文件夾',
  'Auto-memory: {{status}}': '自動記憶：{{status}}',
  'Auto-dream: {{status}} · {{lastDream}} · /dream to run': '自動整理：{{status}} · {{lastDream}} · /dream 立即運行',
  'never': '從未',
  'on': '開',
  'off': '關',
  '✦ dreaming': '✦ 整理中',
  'Remove matching entries from managed auto-memory.': '從託管自動記憶中刪除匹配的條目。',
  'Usage: /forget <memory text to remove>': '用法：/forget <要刪除的記憶文本>',
  'No managed auto-memory entries matched: {{query}}': '沒有匹配的託管自動記憶條目：{{query}}',
  'Show managed auto-memory status.': '顯示託管自動記憶狀態',
  'Run managed auto-memory extraction for the current session.': '爲當前會話運行託管自動記憶提煉',
  'Managed auto-memory root: {{root}}': '託管自動記憶根目錄：{{root}}',
  'Managed auto-memory topics:': '託管自動記憶主題：',
  'No extraction cursor found yet.': '尚未找到提煉遊標。',
  'Cursor: session={{sessionId}}, offset={{offset}}, updated={{updatedAt}}': '遊標：session={{sessionId}}，offset={{offset}}，updated={{updatedAt}}',
  'No chat client available to extract memory.': '沒有可用於提煉記憶的聊天客戶端。',
  'Managed auto-memory extraction is already running.': '託管自動記憶提煉已在運行中。',
  'Managed auto-memory extraction found no new durable memories.': '託管自動記憶提煉未發現新的持久記憶。',
  'Consolidate managed auto-memory topic files.': '整理託管自動記憶主題文件',
  'Managed auto-memory dream found nothing to improve.': '託管自動記憶 dream 未發現可改進內容。',
  'Deduplicated entries: {{count}}': '去重條目數：{{count}}',
  'Save a durable memory using the save_memory tool.': '使用 save_memory 工具保存一條持久記憶',
  'Usage: /remember [--global|--project] <text to remember>': '用法：/remember [--global|--project] <要記住的文本>',
  'Authenticate with an OAuth-enabled MCP server': '使用支持 OAuth 的 MCP 服務器進行認證',
  ...
  'Make sure to copy the COMPLETE URL - it may wrap across multiple lines.': '⚠️  請確保複製完整的 URL —— 它可能跨越多行。',
  ...
  'Unknown step': '未知步驟',
  'Esc to back': 'Esc 返回',
  '↑↓ to navigate · Enter to select · Esc to close': '↑↓ 導航 · Enter 選擇 · Esc 關閉',
  '↑↓ to navigate · Enter to select · Esc to back': '↑↓ 導航 · Enter 選擇 · Esc 返回',
  '↑↓ to navigate · Enter to confirm · Esc to back': '↑↓ 導航 · Enter 確認 · Esc 返回',
  'User Settings (global)': '用戶設置（全局）',
  'Workspace Settings (project-specific)': '工作區設置（項目級）',
  'Disable server:': '禁用服務器：',
  'Select where to add the server to the exclude list:': '選擇將服務器添加到排除列表的位置：',
  ...
  'server': '個服務器',
  'servers': '個服務器',
  'Add MCP servers to your settings to get started.': '請在設置中添加 MCP 服務器以開始使用。',
  'Run qwen --debug to see error logs': '運行 qwen --debug 查看錯誤日誌',
  'OAuth Authentication': 'OAuth 認證',
  'Press Enter to start authentication, Esc to go back': '按 Enter 開始認證，Esc 返回',
  'Authenticating... Please complete the login in your browser.': '認證中... 請在瀏覽器中完成登錄。',
  'Press c to copy the authorization URL to your clipboard.': '按 c 複製授權 URL 到剪貼板。',
  'Copy request sent to your terminal. If paste is empty, copy the URL above manually.': '已向終端發送複製請求；若粘貼爲空，請手動複製上方 URL。',
  'Cannot write to terminal — copy the URL above manually.': '無法寫入終端，請手動複製上方 URL。',
  ...
  '(unnamed)': '(未命名)',
  'Warning: This tool cannot be called by the LLM': '警告：此工具無法被 LLM 調用',
  'Reason': '原因',
  'Tools must have both name and description to be used by the LLM.': '工具必須同時具有名稱和描述才能被 LLM 使用。',
  ...
  'Save the current conversation as a checkpoint. Usage: /chat save <tag>': '將當前對話保存爲檢查點。用法：/chat save <tag>',
  ...
  'A checkpoint with the tag {{tag}} already exists. Do you want to overwrite it?': '標籤爲 {{tag}} 的檢查點已存在。您要覆蓋它嗎？',
  'No chat client available to save conversation.': '沒有可用的聊天客戶端來保存對話',
  'Conversation checkpoint saved with tag: {{tag}}.': '對話檢查點已保存，標籤：{{tag}}',
  'No conversation found to save.': '未找到要保存的對話',
  'No chat client available to share conversation.': '沒有可用的聊天客戶端來分享對話',
  'Invalid file format. Only .md and .json are supported.': '無效的文件格式。僅支持 .md 和 .json 文件',
  ...
  'Share the current conversation to a markdown or json file. Usage: /chat share <file>': '將當前對話分享到 markdown 或 json 文件。用法：/chat share <file>',
  'Generate a project summary and save it to .qwen/PROJECT_SUMMARY.md': '生成項目摘要並保存到 .qwen/PROJECT_SUMMARY.md',
  'No chat client available to generate summary.': '沒有可用的聊天客戶端來生成摘要',
  'Already generating summary, wait for previous request to complete': '正在生成摘要，請等待上一個請求完成',
  'No conversation found to summarize.': '未找到要總結的對話',
  'Failed to generate project context summary: {{error}}': '生成項目上下文摘要失敗：{{error}}',
  'Saved project summary to {{filePathForDisplay}}.': '項目摘要已保存到 {{filePathForDisplay}}',
  ...
  'Switch the model for this session (--fast for suggestion model)': '切換此會話的模型（--fast 可設置建議模型）',
  ...
  'No models available for the current authentication type ({{authType}}).': '當前認證類型 ({{authType}}) 沒有可用的模型',
  'Starting a new session, resetting chat, and clearing terminal.': '正在開始新會話，重置聊天並清屏。',
  'Starting a new session and clearing.': '正在開始新會話並清屏。',
  'Already compressing, wait for previous request to complete': '正在壓縮中，請等待上一個請求完成',
  ...
  'Chat history compressed from {{originalTokens}} to {{newTokens}} tokens.': '聊天曆史已從 {{originalTokens}} 個 token 壓縮到 {{newTokens}} 個 token。',
  'Compression was not beneficial for this history size.': '對於此歷史記錄大小，壓縮沒有益處。',
  'Chat history compression did not reduce size. This may indicate issues with the compression prompt.': '聊天曆史壓縮未能減小大小。這可能表明壓縮提示存在問題。',
  'Could not compress chat history due to a token counting error.': '由於 token 計數錯誤，無法壓縮聊天曆史。',
  ...
  'Tool: {{tool}}': '工具：{{tool}}',
  'Allow execution of MCP tool "{{tool}}" from server "{{server}}"?': '允許執行來自服務器 "{{server}}" 的 MCP 工具 "{{tool}}"？',
  'Yes, always allow tool "{{tool}}" from server "{{server}}"': '是，總是允許來自服務器 "{{server}}" 的工具 "{{tool}}"',
  'Yes, always allow all tools from server "{{server}}"': '是，總是允許來自服務器 "{{server}}" 的所有工具',
  'Shell Command Execution': 'Shell 命令執行',
  'A custom command wants to run the following shell commands:': '自定義命令想要運行以下 shell 命令：',
  ...
  'Current Plan:': '當前計劃：',
  'Progress: {{done}}/{{total}} tasks completed': '進度：已完成 {{done}}/{{total}} 個任務',
  ...
  '👋 Welcome back! (Last updated: {{timeAgo}})': '👋 歡迎回來！（最後更新：{{timeAgo}}）',
  '🎯 Overall Goal:': '🎯 總體目標：',
  'Get started': '開始使用',
  'Select Authentication Method': '選擇認證方式',
  'OpenAI API key is required to use OpenAI authentication.': '使用 OpenAI 認證需要 OpenAI API 密鑰',
  'You must select an auth method to proceed. Press Ctrl+C again to exit.': '您必須選擇認證方法才能繼續。再次按 Ctrl+C 退出',
  'Terms of Services and Privacy Notice': '服務條款和隱私聲明',
  'Qwen OAuth': 'Qwen OAuth (免費)',
  'Discontinued — switch to Coding Plan or API Key': '已停用 — 請切換到 Coding Plan 或 API Key',
  ...
  'API-KEY': 'API-KEY',
  'Use coding plan credentials or your own api-keys/providers.': '使用 Coding Plan 憑證或您自己的 API 密鑰/提供商。',
  'OpenAI': 'OpenAI',
  'Failed to login. Message: {{message}}': '登錄失敗。消息：{{message}}',
  'Authentication is enforced to be {{enforcedType}}, but you are currently using {{currentType}}.': '認證方式被強制設置爲 {{enforcedType}}，但您當前使用的是 {{currentType}}',
  'Qwen OAuth authentication timed out. Please try again.': 'Qwen OAuth 認證超時。請重試',
  ...
  'OAuth token expired (over {{seconds}} seconds). Please select authentication method again.': 'OAuth 令牌已過期（超過 {{seconds}} 秒）。請重新選擇認證方法',
  'Press any key to return to authentication type selection.': '按任意鍵返回認證類型選擇',
  'Waiting for Qwen OAuth authentication...': '正在等待 Qwen OAuth 認證...',
  'Note: Your existing API key in settings.json will not be cleared when using Qwen OAuth. You can switch back to OpenAI authentication later if needed.': '注意：使用 Qwen OAuth 時，settings.json 中現有的 API 密鑰不會被清除。如果需要，您可以稍後切換回 OpenAI 認證。',
  'Note: Your existing API key will not be cleared when using Qwen OAuth.': '注意：使用 Qwen OAuth 時，現有的 API 密鑰不會被清除。',
  'Authentication timed out. Please try again.': '認證超時。請重試。',
  'Waiting for auth... (Press ESC or CTRL+C to cancel)': '正在等待認證...（按 ESC 或 CTRL+C 取消）',
  'Missing API key for OpenAI-compatible auth. Set settings.security.auth.apiKey, or set the {{envKeyHint}} environment variable.': '缺少 OpenAI 兼容認證的 API 密鑰。請設置 settings.security.auth.apiKey 或設置 {{envKeyHint}} 環境變量。',
  '{{envKeyHint}} environment variable not found.': '未找到 {{envKeyHint}} 環境變量。',
  '{{envKeyHint}} environment variable not found. Please set it in your .env file or environment variables.': '未找到 {{envKeyHint}} 環境變量。請在 .env 文件或系統環境變量中進行設置。',
  '{{envKeyHint}} environment variable not found (or set settings.security.auth.apiKey). Please set it in your .env file or environment variables.': '未找到 {{envKeyHint}} 環境變量（或設置 settings.security.auth.apiKey）。請在 .env 文件或系統環境變量中進行設置。',
  'Missing API key for OpenAI-compatible auth. Set the {{envKeyHint}} environment variable.': '缺少 OpenAI 兼容認證的 API 密鑰。請設置 {{envKeyHint}} 環境變量。',
  'Anthropic provider missing required baseUrl in modelProviders[].baseUrl.': 'Anthropic 提供商缺少必需的 baseUrl，請在 modelProviders[].baseUrl 中配置。',
  'ANTHROPIC_BASE_URL environment variable not found.': '未找到 ANTHROPIC_BASE_URL 環境變量。',
  'Invalid auth method selected.': '選擇了無效的認證方式。',
  'Failed to authenticate. Message: {{message}}': '認證失敗。消息：{{message}}',
  'Authenticated successfully with {{authType}} credentials.': '使用 {{authType}} 憑據成功認證。',
  'Invalid QWEN_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}': '無效的 QWEN_DEFAULT_AUTH_TYPE 值："{{value}}"。有效值爲：{{validValues}}',
  'OpenAI Configuration Required': '需要配置 OpenAI',
  'Please enter your OpenAI configuration. You can get an API key from': '請輸入您的 OpenAI 配置。您可以從以下地址獲取 API 密鑰：',
  'API Key:': 'API 密鑰：',
  'Invalid credentials: {{errorMessage}}': '憑據無效：{{errorMessage}}',
  'Failed to validate credentials': '驗證憑據失敗',
  'Press Enter to continue, Tab/↑↓ to navigate, Esc to cancel': '按 Enter 繼續，Tab/↑↓ 導航，Esc 取消',
  'Select Model': '選擇模型',
  ...
  'Permission rules are a tool name, optionally followed by a specifier in parentheses.': '權限規則是一個工具名稱，可選地後跟括號中的限定符。',
  'e.g.,': '例如',
  'or': '或',
  ...
  'Are you sure you want to delete this permission rule?': '確定要刪除此權限規則嗎？',
  'Permissions:': '權限：',
  '(←/→ or tab to cycle)': '（←/→ 或 tab 切換）',
  'Press ↑↓ to navigate · Enter to select · Type to search · Esc to cancel': '按 ↑↓ 導航 · 回車選擇 · 輸入搜索 · Esc 取消',
  'Search…': '搜索…',
  'Use /trust to manage folder trust settings for this workspace.': '使用 /trust 管理此工作區的文件夾信任設置。',
  'Add directory…': '添加目錄…',
  'Add directory to workspace': '添加工作區目錄',
  'Qwen Code can read files in the workspace, and make edits when auto-accept edits is on.': 'Qwen Code 可以讀取工作區中的文件，並在自動接受編輯模式開啓時進行編輯。',
  'Qwen Code will be able to read files in this directory and make edits when auto-accept edits is on.': 'Qwen Code 將能夠讀取此目錄中的文件，並在自動接受編輯模式開啓時進行編輯。',
  'Enter the path to the directory:': '輸入目錄路徑：',
  'Enter directory path…': '輸入目錄路徑…',
  'Tab to complete · Enter to add · Esc to cancel': 'Tab 補全 · 回車添加 · Esc 取消',
  'Remove directory?': '刪除目錄？',
  'Are you sure you want to remove this directory from the workspace?': '確定要將此目錄從工作區中移除嗎？',
  ...
  '⏳ MCP servers are starting up ({{count}} initializing)...': '⏳ MCP 服務器正在啓動（{{count}} 個正在初始化）...',
  'Note: First startup may take longer. Tool availability will update automatically.': '注意：首次啓動可能需要更長時間。工具可用性將自動更新',
  'Configured MCP servers:': '已配置的 MCP 服務器：',
  'Ready': '就緒',
  'Starting... (first startup may take longer)': '正在啓動...（首次啓動可能需要更長時間）',
  'Disconnected': '已斷開連接',
  ...
  'Start a fresh idea with /clear or /new; the previous session stays available in history.': '用 /clear 或 /new 開啓新思路；之前的會話會保留在歷史記錄中。',
  'Use /bug to submit issues to the maintainers when something goes off.': '遇到問題時，用 /bug 將問題提交給維護者。',
  'Switch auth type quickly with /auth.': '用 /auth 快速切換認證方式。',
  'You can run any shell commands from Qwen Code using ! (e.g. !ls).': '在 Qwen Code 中使用 ! 可運行任意 shell 命令（例如 !ls）。',
  'Type / to open the command popup; Tab autocompletes slash commands and saved prompts.': '輸入 / 打開命令彈窗；按 Tab 自動補全斜槓命令和保存的提示詞。',
  'You can resume a previous conversation by running qwen --continue or qwen --resume.': '運行 qwen --continue 或 qwen --resume 可繼續之前的會話。',
  ...
  'Add a QWEN.md file to give Qwen Code persistent project context.': '添加 QWEN.md 文件，爲 Qwen Code 提供持久的項目上下文。',
  ...
  'Long conversation? /compress summarizes history to free context.': '對話太長？用 /compress 總結歷史，釋放上下文。',
  ...
  'of input tokens were served from the cache, reducing costs.': '從緩存載入 token ，降低了成本',
  'Tip: For a full token breakdown, run `/stats model`.': '提示：要查看完整的令牌明細，請運行 `/stats model`',
  ...
  'Tool': '工具',
  'Output': '輸出',
  'No API calls have been made in this session.': '本次會話中未進行任何 API 調用',
  'Tool Name': '工具名稱',
  'Calls': '調用次數',
  'Success Rate': '成功率',
  'Avg Duration': '平均耗時',
  ...
  'No tool calls have been made in this session.': '本次會話中未進行任何工具調用',
  'Session start time is unavailable, cannot calculate stats.': '會話開始時間不可用，無法計算統計信息',
  ...
  'The TOML format is deprecated. Would you like to migrate them to Markdown format?': 'TOML 格式已棄用。是否將它們遷移到 Markdown 格式？',
  ...
  'Markdown file already exists: {{filename}}': 'Markdown 文件已存在：{{filename}}',
  'TOML Command Format Deprecation Notice': 'TOML 命令格式棄用通知',
  'Found {{count}} command file(s) in TOML format:': '發現 {{count}} 個 TOML 格式的命令文件：',
  'The TOML format for commands is being deprecated in favor of Markdown format.': '命令的 TOML 格式正在被棄用，推薦使用 Markdown 格式。',
  'Markdown format is more readable and easier to edit.': 'Markdown 格式更易讀、更易編輯。',
  'You can migrate these files automatically using:': '您可以使用以下命令自動遷移這些文件：',
  ...
  'TOML format will continue to work for now, but migration is recommended.': 'TOML 格式目前仍可使用，但建議遷移。',
  'Open extensions page in your browser': '在瀏覽器中打開擴展市場頁面',
  'Unknown extensions source: {{source}}.': '未知的擴展來源：{{source}}。',
  'Would open extensions page in your browser: {{url}} (skipped in test environment)': '將在瀏覽器中打開擴展頁面：{{url}}（測試環境中已跳過）',
  'View available extensions at {{url}}': '在 {{url}} 查看可用擴展',
  'Opening extensions page in your browser: {{url}}': '正在瀏覽器中打開擴展頁面：{{url}}',
  'Failed to open browser. Check out the extensions gallery at {{url}}': '打開瀏覽器失敗。請訪問擴展市場：{{url}}',
  'Rate limit error: {{reason}}': '觸發限流：{{reason}}',
  'Retrying in {{seconds}} seconds… (attempt {{attempt}}/{{maxRetries}})': '將於 {{seconds}} 秒後重試…（第 {{attempt}}/{{maxRetries}} 次）',
  ...
  'You can get your Coding Plan API key here': '您可以在這裏獲取 Coding Plan API Key',
  'API key is stored in settings.env. You can migrate it to a .env file for better security.': 'API Key 已存儲在 settings.env 中。您可以將其遷移到 .env 文件以獲得更好的安全性。',
  'New model configurations are available for Alibaba Cloud Coding Plan. Update now?': '阿里雲百鍊 Coding Plan 有新模型配置可用。是否立即更新？',
  'Coding Plan configuration updated successfully. New models are now available.': 'Coding Plan 配置更新成功。新模型現已可用。',
  'Coding Plan API key not found. Please re-authenticate with Coding Plan.': '未找到 Coding Plan API Key。請重新通過 Coding Plan 認證。',
  'Failed to update Coding Plan configuration: {{message}}': '更新 Coding Plan 配置失敗：{{message}}',
  ...
  'Choose based on where your account is registered': '請根據您的賬號註冊地區選擇',
  ...
  '{{region}} configuration updated successfully. Model switched to "{{model}}".': '{{region}} 配置更新成功。模型已切換至 "{{model}}"。',
  'Authenticated successfully with {{region}}. API key and model configs saved to settings.json (backed up).': '成功通過 {{region}} 認證。API Key 和模型配置已保存至 settings.json（已備份）。',
  'Context Usage': '上下文使用情況',
  '% used': '% 已用',
  '% context used': '% 上下文已用',
  'Context exceeds limit! Use /compress or /clear to reduce.': '上下文超出限制！請使用 /compress 或 /clear 來減少上下文。',
  'Use /compress or /clear': '使用 /compress 或 /clear',
  'No API response yet. Send a message to see actual usage.': '暫無 API 響應。發送消息以查看實際使用情況。',
  ...
  'Run /context detail for per-item breakdown.': '運行 /context detail 查看詳細分解。',
  'body loaded': '內容已加載',
  'memory': '記憶',
  '{{region}} configuration updated successfully.': '{{region}} 配置更新成功。',
  'Authenticated successfully with {{region}}. API key and model configs saved to settings.json.': '成功通過 {{region}} 認證。API Key 和模型配置已保存至 settings.json。',
  'Tip: Use /model to switch between available Coding Plan models.': '提示：使用 /model 切換可用的 Coding Plan 模型。',
  'Please answer the following question(s):': '請回答以下問題：',
  'Cannot ask user questions in non-interactive mode. Please run in interactive mode to use this tool.': '無法在非交互模式下詢問用戶問題。請在交互模式下運行以使用此工具。',
  ...
  '↑/↓: Navigate | ←/→: Switch tabs | Enter: Select': '↑/↓: 導航 | ←/→: 切換標籤頁 | Enter: 選擇',
  '↑/↓: Navigate | ←/→: Switch tabs | Space/Enter: Toggle | Esc: Cancel': '↑/↓: 導航 | ←/→: 切換標籤頁 | Space/Enter: 切換 | Esc: 取消',
  '↑/↓: Navigate | Space/Enter: Toggle | Esc: Cancel': '↑/↓: 導航 | Space/Enter: 切換 | Esc: 取消',
  '↑/↓: Navigate | Enter: Select | Esc: Cancel': '↑/↓: 導航 | Enter: 選擇 | Esc: 取消',
  'Configure Qwen authentication information with Qwen-OAuth or Alibaba Cloud Coding Plan': '使用 Qwen OAuth 或阿里雲百鍊 Coding Plan 配置 Qwen 認證信息',
  'Authenticate using Qwen OAuth': '使用 Qwen OAuth 進行認證',
  'Authenticate using Alibaba Cloud Coding Plan': '使用阿里雲百鍊 Coding Plan 進行認證',
  ...
  'Failed to authenticate with Qwen OAuth: {{error}}': 'Qwen OAuth 認證失敗：{{error}}',
  'Processing Alibaba Cloud Coding Plan authentication...': '正在處理阿里雲百鍊 Coding Plan 認證...',
  'Successfully authenticated with Alibaba Cloud Coding Plan.': '已成功通過阿里雲百鍊 Coding Plan 認證。',
  'Failed to authenticate with Coding Plan: {{error}}': 'Coding Plan 認證失敗：{{error}}',
  '中国 (China)': '中國 (China)',
