...
interface ClientConnection {
  ...
  authRateLimit?: AuthRateLimitState;
}
...
export class RemoteControlServer {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.httpServer.listen(this.config.port, this.config.host, () => {
          ...
          debug.debug(
            'Remote control server started on %s://%s:%d',
            protocol,
            this.config.host,
            this.config.port,
          );
  ...
  private handleHttpRequest(
    ...
    // Connection info endpoint - requires token query param
    ...
    // QR code data endpoint - requires token query param
...
<body>
  <div class="container">
    ...
    <div class="security-warning" id="security-warning" style="display: none;">
      ...
      Only use this on trusted networks. For production use, enable WSS (WebSocket Secure).
    </div>

    <div class="status">
      <div class="status-row">
        <span class="status-label">Session</span>
        <span class="status-value">${escapeHtml(this.sessionState.sessionName)}</span>
      </div>
      <div class="status-row">
        <span class="status-label">Session ID</span>
        <span class="status-value session-id">${escapeHtml(this.sessionState.sessionId.slice(0, 8))}...</span>
      </div>
      <div class="status-row">
        <span class="status-label">Status</span>
        <span class="status-value" id="connection-status">Waiting for connection</span>
      </div>
    </div>
    ...
    <div class="qr-section" id="qr-section">
      ...
      <p class="instructions">
        ...
        <ol>
          ...
          <li>Connect directly via WebSocket: <code>${escapeHtml(connectionUrl)}</code></li>
        </ol>
      </p>
    </div>
  ...
  <script>
    const token = '${escapeHtml(this.authToken)}';
    const wsUrl = '${escapeHtml(connectionUrl)}';
    const isSecure = window.location.protocol === 'https:' || wsUrl.startsWith('wss://');

    // Show security warning for non-secure connections
    if (!isSecure) {
   ...
   */
  private getQRConnectionData(): QRConnectionData {
    const protocol = this.config.secure ? 'wss' : 'ws';
    return {
  ...
  private handleWebSocketConnection(
    ...
    const client: ClientConnection = {
      ...
      authRateLimit: this.authRateLimits.get(clientIp),
    };
