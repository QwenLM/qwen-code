...
export async function parseArguments(): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .option('proxy', {
      type: 'string',
      description: 'Proxy for proto, like schema://user:password@host:port',
    })
    .deprecateOption(
      'proxy',
    ...
    .command('$0 [query..]', 'Launch proto CLI', (yargsInstance: Argv) =>
      yargsInstance
        .positional('query', {
          description:
...
export async function loadCliConfig(
  ...
  if (argv.continue || argv.resume) {
    ...
    if (argv.resume) {
      ...
      if (!sessionData) {
        const message = `No saved session found with ID ${argv.resume}. Run \`proto --resume\` without an ID to choose from existing sessions.`;
