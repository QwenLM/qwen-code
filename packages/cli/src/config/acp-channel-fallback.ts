 ...
 * inherit the marker, so those sessions report `channel=desktop-shell`.
 *
...
export function resolveAcpChannelFallback(
  ...
  // The desktop-shell marker wins: Tauri sessions are daemon-spawned too,
  // but the launcher identity is the finer-grained signal.
