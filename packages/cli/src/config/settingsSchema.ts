  ...
  tools: {
    ...
    properties: {
      ...
      mode: {
        ...
        description:
          'Choose how tools are exposed to the model. Direct uses ordinary tool calls; Code Mode also exposes the isolated exec JavaScript tool; Code Mode Only exposes ordinary tools only through exec. Safe and bare modes always use Direct. Container execution warns and uses direct tools for Code Mode, and rejects Code Mode Only. Code Mode Only ignores eager/visible schema deferral on the session surface: callable tools retain full nested schemas there and tool_search is hidden; agent surfaces narrow nested bindings by their own allowlists.',
        showInDialog: true,
        options: [
      ...
      visible: {
        ...
        default: undefined as string[] | undefined,
        description:
          'Deferred tool names made visible at startup without requiring the ToolSearch + ToolCall bridge. Listed tools appear alongside core tools in the initial session. Code Mode Only already includes full nested schemas in exec and hides both bridge tools.',
        showInDialog: false,
        mergeStrategy: MergeStrategy.UNION,
      },
      eager: {
