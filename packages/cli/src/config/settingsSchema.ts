      ...
      generationConfig: {
        ...
        properties: {
          ...
          toolParametersMandatory: {
            ...
            default: false,
            description:
              'When true, a tool that declares no arguments is still sent with `"parameters": { "type": "object", "properties": {} }` instead of omitting the field. Servers that type `tools[].function.parameters` as a required field reject the request with a validation error when a parameterless tool omits it (for example TabbyAPI `Field required`). Only a tool that omits the field is rewritten: a no-argument tool whose schema reaches the wire as a bare `{ "type": "object" }` with no `properties` key keeps its declared shape, while an MCP tool registered with an empty argument list publishes `{ "type": "object", "properties": {} }` — which the converter reduces to an omission — so it is rewritten too. Leave false by default: llama.cpp / LM Studio / vLLM reject the replacement shapes (QwenLM/qwen-code#11431, #11410), so enable it only for an endpoint that rejects the omitted field. A route whose hostname matches a MiniMax host (`api.minimaxi.com`, `api.minimax.io`, or any `*.minimaxi.com` / `*.minimax.io` subdomain) always sends the field because MiniMax rejects its absence (QwenLM/qwen-code#11834); this setting cannot turn that off. The setting is read only on the OpenAI Chat Completions wire: the OpenAI Responses API (`openai-responses`) and the Anthropic and Gemini generators ignore it. It applies on whichever provider serves the route, including one selected because the model id looks like a vendor model (`deepseek`, `glm-`, `mimo-`, `mistral`), and is read when the request provider for that route is built: an edit under `modelProviders` is picked up by the settings hot reload and takes effect on the next request, while an edit under `model.generationConfig` is read only once, when the CLI builds the generation config at startup, so it takes effect on restart — a model switch does not re-read that block, it re-applies the generation config of the selected provider entry over it. Neither applies to a request already in flight. A side model pointed at its own baseUrl does not inherit the setting of another route, and its own entry can override it, including with false. A side model that shares the parent\'s baseUrl keeps the parent\'s value unless its own entry sets one, so set false on that entry to opt out.',
            parentKey: 'generationConfig',
            showInDialog: false,
          },
          schemaCompliance: {
