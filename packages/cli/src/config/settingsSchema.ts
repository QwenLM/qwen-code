  ...
  model: {
    ...
    properties: {
      ...
      goalCheckpointTimeoutSeconds: {
        ...
        description:
          'Legacy Goal evidence-checkpoint timeout, in seconds. Accepted for compatibility with older settings; automatic Goal evidence checkpoints no longer run, so this setting has no effect on current Goal verification. Valid values remain integers from 1 to 900; invalid values are rejected at startup. The legacy default is 180.',
        showInDialog: false,
      },
      maxToolCalls: {
        type: 'number',
