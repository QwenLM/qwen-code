  ...
  review: {
    ...
    properties: {
      attribution: {
        ...
        default: true,
        description:
          'Append the attribution footer naming the model and CLI version (e.g. "_— qwen3-coder via Qwen Code /review (v0.21.2)_") to review bodies and inline comments posted to GitHub. Disable to post reviews without AI attribution: no footer, and no "**[Critical]**"/"**[Suggestion]**" severity markers on posted comments and body lists. Two consequences to know: qwen-autofix\'s Critical-only mode (after round 5) no longer recognizes the posted findings as Critical and defers them; and each posted comment carries an invisible "<!-- qwen-review -->" marker — that, not the footer, is what presubmit duplicate detection recognizes earlier posts by, from any account. Only honored from User, System, and SystemDefaults settings scopes; values set in Workspace settings are ignored, so a repository cannot set review policy for its reviewers.',
        showInDialog: false,
      },
      effort: {
        type: 'enum',
