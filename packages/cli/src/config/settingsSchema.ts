  ...
  tools: {
    ...
    properties: {
      ...
      toolSearch: {
        ...
        properties: {
          ...
          threshold: {
            ...
            default: 10,
            description:
              'Context-window percentage used as the session-start budget for preloading ordinary deferred tools (bundled built-ins and MCP alike). When every eligible deferred tool schema fits within the budget, all are declared upfront for direct calls instead of loaded on demand, keeping the prompt prefix stable for KV caching; otherwise they stay behind the ToolSearch + ToolCall bridge. Tools demoted by tools.eager are excluded from this preload and stay reachable on demand through that bridge. Set 0 to always keep deferred tools behind the bridge.',
            showInDialog: true,
      ...
      codeModeOnly: {
        ...
        default: false,
        description:
          'Experimental. Replace ordinary direct tool declarations with a sandboxed exec({ source }) bridge while retaining audited control-plane tools. Disabled by default.',
        showInDialog: true,
      },
      listDirectory: {
        type: 'object',
