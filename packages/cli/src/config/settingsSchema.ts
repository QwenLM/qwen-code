  ...
  model: {
    ...
    properties: {
      ...
      toggleModel: {
        ...
        default: undefined as string | undefined,
        description:
          'Alternate model ID for quick toggle via the toggleModel hotkey. When set, pressing the configured hotkey switches between the current model and this model. Use the qualified syntax "model-id(authType)" (e.g. "gpt-4(openai)") to disambiguate when multiple providers offer the same model ID; without qualification the current provider is preferred, and if it does not offer the model, the first other provider in enum order wins. Leave empty to disable the hotkey.',
        showInDialog: false,
      },
      baseUrl: {
        type: 'string',
