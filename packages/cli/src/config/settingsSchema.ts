      ...
      brand: {
        ...
        properties: {
          name: {
            ...
            default: '' as string,
            description:
              "Product name shown in the Web Shell sidebar, welcome header, About panel and browser tab title. Sanitized to a single line; capped at 80 characters. Environment variable placeholders are NOT expanded in this key — the substitution source is process-wide, so a workspace could supply the value; a placeholder value is ignored with a warning on the daemon's stderr. Leave empty to use the built-in name. The terminal banner has its own separate setting, `ui.customBannerTitle`.",
            showInDialog: false,
          },
          logoPath: {
            type: 'string',
