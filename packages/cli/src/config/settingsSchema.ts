  ...
  security: {
    ...
    properties: {
      ...
      allowedInsecureVoiceBaseUrls: {
        ...
        default: [] as string[],
        description:
          'Complete voice base URLs that may use HTTP or private-network addresses. Entries must include an explicit http:// or https:// scheme and the full provider path; only URL serialization and trailing slashes are normalized. Wildcards are not supported; metadata, link-local, and IPv6 transition-mechanism addresses remain blocked even when listed. Only honored from User, System, and SystemDefaults settings scopes; values set in Workspace settings are ignored. Enable only for trusted endpoints in managed private networks. An allowlisted hostname is only as trustworthy as its DNS; prefer IP-literal entries when the gateway address is stable.',
        showInDialog: false,
        items: {
          type: 'string',
          description:
