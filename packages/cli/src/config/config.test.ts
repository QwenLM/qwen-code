...
describe('mergeExcludeTools', () => {
  ...
  it('passes tools.codeModeOnly into the effective tool mode', async () => {
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments();
    const settings: Settings = { tools: { codeModeOnly: true } };

    const config = await loadCliConfig(settings, argv, undefined, []);

    expect(config.getEffectiveToolMode()).toBe(
      ServerConfig.ToolMode.CodeModeOnly,
    );
  });
