...
function applyScripts(base: MathBox, sub?: MathBox, sup?: MathBox): MathBox {
  ...
  if (base.lines.length > 1) {
    const baseWidth = Math.max(boxWidth(base), 1);
    const suffixWidth = visibleWidth(suffix);
    const lines = base.lines.map((line, index) => {
      const padded = padRight(line, baseWidth);
      return index === base.baseline
        ? `${padded}${suffix}`
        : `${padded}${spaces(suffixWidth)}`;
    });
    return {
      lines,
      baseline: base.baseline,
      inline: `${base.inline}${suffix}`,
    };
  }
...
class TeXParser {
  ...
  private parseCommand(depth: number): MathBox {
    ...
    switch (command) {
      ...
      case 'begin': {
        ...
        this.skipEnvironmentArguments(environmentName);
  ...
  private readEnvironmentContent(environmentName: string): string {
    ...
    for (let cursor = this.position; cursor < this.input.length; ) {
      ...
      if (marker?.name === environmentName && braceDepth === 0) {
  ...
  private skipEnvironmentArguments(environmentName: string): void {
    const normalizedName = environmentName.replace(/\*$/, '');
    const environmentsWithSpec = new Set(['alignat', 'array', 'tabular']);
    if (!environmentsWithSpec.has(normalizedName)) return;

    this.skipSpaces();
    const group = readBalanced(this.input, this.position, '{', '}');
    if (group) this.position = group.end;
  }
...
function applyUnderline(box: MathBox): MathBox {
  if (box.lines.length <= 1) return textBox(`${box.inline}\u0332`);
  const width = Math.max(boxWidth(box), 1);
  return {
    lines: [
      ...box.lines.map((line) => centerLine(line, width)),
...
export function renderInlineMathInText(text: string): string {
  return splitInlineMathSegments(text)
    .map((segment) => {
      ...
      try {
        return renderTerminalMathInline(segment.text);
      } catch {
        return segment.raw;
      }
    })
