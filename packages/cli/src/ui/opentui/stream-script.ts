...
export type StreamEvent =
  ...
  | { type: "task-end"; id: string; tools: number; seconds: number; tokens: string }
  | { type: "done" };
...
export function buildScenario(): StreamEvent[] {
  ...
  for (const delta of tokenize(THINKING)) events.push({ type: "thinking", delta });
  events.push({ type: "thinking-end" });

  for (const delta of tokenize(TEXT_1)) events.push({ type: "text", delta });

  events.push({ type: "tool-start", id: "t1", tool: "Read", title: "Read packages/cli/src/ui/AppContainer.tsx" });
  for (const delta of tokenize(TOOL_READ_OUTPUT)) events.push({ type: "tool-output", id: "t1", delta });
  events.push({ type: "tool-end", id: "t1", success: true, summary: "4779 lines" });

  events.push({ type: "tool-start", id: "t2", tool: "Bash", title: "rg -n \"clearTerminal\" packages/cli/src node_modules/ink/build" });
  for (const delta of tokenize(TOOL_GREP_OUTPUT)) events.push({ type: "tool-output", id: "t2", delta });
  events.push({ type: "tool-end", id: "t2", success: true, summary: "6 matches" });

  for (const delta of tokenize(TEXT_2)) events.push({ type: "text", delta });

  events.push({ type: "task-start", id: "s1", name: "researcher", description: "benchmark ink frame write patterns" });
  for (const line of TASK_LINES) events.push({ type: "task-progress", id: "s1", line });
  events.push({ type: "task-end", id: "s1", tools: 3, seconds: 12.4, tokens: "2.1k" });
