 ...
 * option details, and footer hints. The pure helpers and the state machine
...
export function OpentuiRewindSelector(props: OpentuiRewindSelectorProps) {
  ...
  useKeyboard((key) => {
    ...
    if (state.phase === 'restore-options') {
      ...
      if (original.name === 'up' || original.sequence === 'k') {
        dispatch({ type: 'option-up' });
        return;
      }
      if (original.name === 'down' || original.sequence === 'j') {
        dispatch({
          type: 'option-down',
  ...
  if (state.phase !== 'pick' && selectedTurn) {
    ...
    if (state.phase === 'confirm') {
      return (
        <DialogFrame>
          ...
          <FooterHint text={t('Enter/Y to confirm · Esc/N to go back')} />
        </DialogFrame>
      );
    }
    ...
    return (
      <DialogFrame>
        ...
        <FooterHint
          text={t('↑↓ to navigate · Enter to select · Esc to go back')}
        />
      </DialogFrame>
    );
  }
  ...
  return (
    <DialogFrame>
      ...
      <box marginTop={1} flexDirection="column">
        {visibleTurns.map((turn, visibleIndex) => {
          ...
          const showDownIndicator =
            visibleIndex === visibleTurns.length - 1 && window_.showScrollDown;
      ...
      <FooterHint
        text={t('↑↓ to navigate · Enter to select · Esc to cancel')}
      />
    </DialogFrame>
  );
}
