...
function down(
  x: number,
  y: number,
  opts?: { ctrl?: boolean; button?: number },
): MultiClickPointer {
  return {
    x,
    y,
...
/** Records updateSelection calls; `selection` models the framework state. */
function makeHost(selection: { isDragging: boolean } | null) {
  const calls: Array<[number, number]> = [];
  const host: SelectionHost = {
    getSelection: () => selection,
    updateSelection: (_r, x, y) => {
      calls.push([x, y]);
    },
  };
  return { host, calls };
...
describe('MultiClickSelectionController', () => {
  ...
  it('upgrades the selection from word to line span across clicks', () => {
    ...
    // 3rd click upgrades the same selection to the whole line.
    expect(calls).toEqual([
      [2, 0],
      [6, 0],
      [0, 0],
