...
export const remoteControlCommand: SlashCommand = {
  ...
  action: async (
    ...
    // FIX: Parse stop arg BEFORE checking existing server
    // This fixes the bug where "stop" was unreachable when server was already running
    const stopArg = args.trim().toLowerCase() === 'stop';

    // Check if already running
    const existingServer = getServer();
    if (existingServer && !stopArg) {
    ...
    // FIX: Handle stop command - now reachable even when server is running
    if (stopArg) {
    ...
    try {
      ...
      // FIX: Generate QR code from URL only - token NOT in URL
      const qrCodeUrl = connectionInfo.url; // No token query param
