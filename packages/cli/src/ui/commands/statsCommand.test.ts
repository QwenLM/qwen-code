...
describe('statsCommand', () => {
  ...
  describe('non-interactive mode', () => {
    ...
    it('stats model shows cost per source when Advisor usage is attributed separately', async () => {
      const modelSubCommand = statsCommand.subCommands?.find(
        (sc) => sc.name === 'model',
      );
      if (!modelSubCommand?.action) throw new Error('Subcommand has no action');
      ...
      const mainMetrics: ModelMetricsCore = {
        tokens: {
          prompt: 1_000_000,
          candidates: 500_000,
          cached: 0,
      ...
      const advisorMetrics: ModelMetricsCore = {
        tokens: {
          prompt: 200_000,
          candidates: 100_000,
          cached: 0,
      ...
      expect(result.type).toBe('message');
      expect(result.content).toContain('advisor-model (main)');
      expect(result.content).toContain('Estimated cost: $2.0000');
      expect(result.content).toContain('advisor-model (advisor)');
      expect(result.content).toContain('Estimated cost: $0.4000');
    });
