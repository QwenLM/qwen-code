...
describe('copyCommand', () => {
  ...
  describe('non-interactive mode', () => {
    ...
    it('should return content directly without calling clipboard', async () => {
      if (!copyCommand.action) throw new Error('Command has no action');

      const aiText = 'Hello from model';
      mockGetHistory.mockReturnValue([
        { role: 'model', parts: [{ text: aiText }] },
      ]);

      const result = await copyCommand.action(nonInteractiveContext, '');
    ...
    it('should return info message when no history in non-interactive mode', async () => {
      if (!copyCommand.action) throw new Error('Command has no action');
