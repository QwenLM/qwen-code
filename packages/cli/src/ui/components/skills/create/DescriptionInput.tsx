...
import { type Config, skillGenerator } from '@qwen-code/qwen-code-core';
...
export function DescriptionInput({
  ...
  const handleGenerate = useCallback(
    async (
      ...
      try {
        const generated = await skillGenerator(
          userDescription,
          config,
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        ...
        errors: [
          t('skills.create.error.generator: {{error}}', {
  ...
  const placeholder = t('skills.create.description.placeholder');

  return (
    <Box flexDirection="column" gap={1}>
      <Box>
        <Text color={theme.text.secondary}>
          {t('skills.create.description.llmDescription')}
        </Text>
      </Box>
      ...
      {state.isGenerating ? (
        <Box>
          ...
          <Text color={theme.text.accent}>{t('skills.create.generating')}</Text>
        </Box>
      ) : (
        <TextInput
