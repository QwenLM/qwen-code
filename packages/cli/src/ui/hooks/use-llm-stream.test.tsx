...
describe('useLlmStream', () => {
  ...
  it('polices UPPERCASE-MIME nested tool-result media case-insensitively', async () => {
    // MIME types are case-insensitive (RFC 6838) and MCP servers supply the
    // string verbatim: an UPPERCASE-MIME audio tool result must stay on the
    // exact route of the audio-capable override (the detector sees it), and
    // an UPPERCASE-MIME image must fail closed exactly like its lowercase
    // twin instead of slipping past the gate into silent route slimming.
    const audioPart = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    mockConfig.getModel = vi.fn(() => 'session-model');
        ...
        [
          {
            functionResponse: {
              ...
              parts: [
                {
                  inlineData: { mimeType: 'AUDIO/WAV', data: 'UklGRkFVZA==' },
                },
              ],
            },
          } as never,
    ...
    const audioSent = JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0]);
    expect(audioSent).toContain('AUDIO/WAV');
    expect(audioSent).toContain('UklGRkFVZA==');
    expect(audioSent).not.toContain('was not sent');
    expect(mockSendMessageStream.mock.calls[1]?.[3]?.modelOverride).toBe(
      'audio-model\0',
    );
    ...
    const imageSent = JSON.stringify(mockSendMessageStream.mock.calls[2]?.[0]);
    expect(imageSent).not.toContain('IMAGE/PNG');
    expect(imageSent).not.toContain('aW1hZ2U=');
    expect(imageSent).toContain('was not sent');
    expect(mockAddItem).toHaveBeenCalledWith(
      expect.objectContaining({
        type: MessageType.ERROR,
        text: expect.stringContaining('Image returned by a tool was not sent'),
  ...
  it('fails closed fileData tool-result media an audio-only override cannot see', async () => {
    // Core's convertToFunctionResponse nests `fileData` parts into
    // functionResponse.parts exactly like `inlineData` (and core's
    // slimming/microcompact treat both as media), so the gate must police
    // both carriers — keyed on fileData.mimeType for the URI-backed one.
    const audioPart = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    mockConfig.getModel = vi.fn(() => 'session-model');
              ...
              parts: [
                {
                  fileData: {
                    mimeType: 'image/png',
                    fileUri: 'gs://bucket/screenshot.png',
                  },
                },
              ],
            },
    ...
    const imageSent = JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0]);
    expect(imageSent).not.toContain('gs://bucket/screenshot.png');
    expect(imageSent).toContain('was not sent');
    expect(mockAddItem).toHaveBeenCalledWith(
      expect.objectContaining({
        type: MessageType.ERROR,
    ...
    // A fileData AUDIO part under the audio-capable override passes the gate
    // and keeps the exact route — the carrier counts, not just inlineData.
    await act(async () => {
      await result.current.submitQuery(
        [
          {
            functionResponse: {
              id: 'tool-call-filedata-audio',
              name: 'record',
              response: { output: 'recording attached' },
              parts: [
                {
    ...
    const audioSent = JSON.stringify(mockSendMessageStream.mock.calls[2]?.[0]);
    expect(audioSent).toContain('gs://bucket/recording.wav');
    expect(audioSent).not.toContain('was not sent');
    expect(mockSendMessageStream.mock.calls[2]?.[3]?.modelOverride).toBe(
      'audio-model\0',
    );
  });
  ...
  it('preserves a non-object pass-through response while fail-closing its nested image', async () => {
    // Core's convertToFunctionResponse passes a tool-supplied functionResponse
    // part through verbatim, so `response` itself is not guaranteed to be a
    // plain object — untyped custom tool adapters (the boundary
    // coreToolScheduler's tests exercise) return arrays, scalars, and raw
    // strings. Spreading such a response into the rebuilt object mangles it:
    // an array becomes a numeric-keyed object, a scalar spreads to `{}` (erasing
    // the tool output entirely), a string scatters into char-indexed entries.
    // The replacer must instead fold the stringified response into the
    // appended note, preserving the tool result.
    const audioPart = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    mockConfig.getModel = vi.fn(() => 'session-model');
    ...
    const cases: Array<[string, unknown]> = [
      ['array', [{ probeKey: 'PROBE_ARRAY_VALUE_42' }]],
      ['number', 42],
      ['string', 'raw scalar output'],
    ];
    for (let i = 0; i < cases.length; i++) {
      const [label, responseValue] = cases[i];
      await act(async () => {
        await result.current.submitQuery(
          [
            {
              functionResponse: {
                id: `tool-call-${label}-response`,
                name: 'custom_tool',
      ...
      const sentParts = mockSendMessageStream.mock.calls[i + 1]?.[0] as Part[];
      const sentJson = JSON.stringify(sentParts);
      // The image bytes are stripped and the fail-closed note is present...
      expect(sentJson).not.toContain('aW1hZ2U=');
      expect(sentJson).toContain('was not sent');
      // ...and the non-object response is PRESERVED (folded into the note as
      // stringified JSON), not spread into a mangled object.
      const frPart = sentParts.find((p) => p.functionResponse);
      const response = frPart?.functionResponse?.response as Record<
        string,
        unknown
      >;
      expect(typeof response['output']).toBe('string');
      expect(response['output']).toContain('was not sent');
      // No numeric-key / char-indexed mangling of the original response.
      expect(response['0']).toBeUndefined();
      // The original content survives in stringified form.
      if (label === 'array') {
        expect(response['output']).toContain(
          '[{"probeKey":"PROBE_ARRAY_VALUE_42"}]',
        );
      } else if (label === 'number') {
        expect(response['output']).toContain('42');
      } else {
        expect(response['output']).toContain('raw scalar output');
      }
    }
    expect(mockAddItem).toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it('clamps oversized nested tool-result media the route supports', async () => {
    ...
    // applies: an oversized nested audio blob on an audio-capable route is
    // placeholder-substituted by the size clamp, not shipped raw.
    const audioPart = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    mockConfig.getModel = vi.fn(() => 'session-model');
    ...
    const oversizedData = 'UklGRgAAAAAAAAAAAAAAAAAAAAAAAA=='; // ~22 bytes decoded
    const previousLimit = process.env['QWEN_CODE_MAX_INLINE_MEDIA_BYTES'];
    process.env['QWEN_CODE_MAX_INLINE_MEDIA_BYTES'] = '16';
    try {
      await act(async () => {
        await result.current.submitQuery(
          [
            {
              functionResponse: {
                ...
                parts: [
                  {
                    inlineData: { mimeType: 'audio/wav', data: oversizedData },
                  },
                ],
              },
            } as never,
      ...
      if (previousLimit === undefined) {
        ...
        process.env['QWEN_CODE_MAX_INLINE_MEDIA_BYTES'] = previousLimit;
      }
    }

    const sent = JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0]);
    expect(sent).toContain('Media omitted');
    expect(sent).not.toContain(oversizedData);
    // The route SUPPORTS audio — this is a size clamp, not a capability
    // fail-closed: no 'was not sent' note.
  ...
  it('clamps resolution-failed images destined for an image-capable session model', async () => {
    vi.stubEnv('QWEN_CODE_MAX_INLINE_MEDIA_BYTES', '1');
    const audioPart = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    ...
    mockConfig.getModel = vi.fn(() => 'session-model');
    // The session model accepts images, so the vision bridge gate is false and
    // images flow straight through to it — the size clamp is their only guard.
    ...
    mockHandleSlashCommand.mockResolvedValue({
      type: 'submit_prompt',
      content: [{ text: 'look and listen' }, audioPart, imagePart],
      modelOverride: 'voice-model',
    });
    const { result, mockSendMessageStream } = renderTestHook();

    await act(async () => {
      await result.current.submitQuery('/model voice-model look');
    });

    // The audio fails closed (marker) and the vision bridge is skipped because
    // the session model is image-capable. Without the resolution-failed clamp
    // the oversized image would be forwarded raw, bypassing the size limit.
    expect(mockRunAudioBridge).not.toHaveBeenCalled();
    expect(mockRunVisionBridge).not.toHaveBeenCalled();
    const sent = JSON.stringify(mockSendMessageStream.mock.calls[0]?.[0]);
    expect(sent).toContain('Media omitted');
  ...
  it('delivers a failed drained steer exactly once when the bridge failed', async () => {
    ...
    await act(async () => {
      if (capturedOnComplete) {
        await capturedOnComplete(
          makeCompletedToolCall('call1', 'prompt-id-drain-single-recovery'),
        );
      }
    });
    ...
    await act(async () => {
      if (capturedOnComplete) {
        await capturedOnComplete(
          makeCompletedToolCall('call2', 'prompt-id-drain-single-recovery-2'),
        );
      }
    });
    await waitFor(() => {
  ...
  it('keeps the pristine retry payload when settle accepted the drained steer before the failure', async () => {
    ...
    mockSendMessageStream
      .mockReturnValueOnce(
        ...
        (async function* () {
          // Emulate core settle for a push that LANDED with NO model content:
          // accept() settles the steer input before the stream failure
          // reaches the CLI's failure handler, so the restore() that handler
          // triggers must be a no-op. (No Content event first — the steer
          // never reached the model, so the undo below must re-arm the
          // composite; with content landed the steer would be consumed and
          // the store would keep only the dedup-safe tool responses.)
          (
  ...
  it('hands the retry store back to the outer payload once a hook-path steer is accepted', async () => {
    // Hook-path twin of the client-driven onAccept test. A tool continuation
    // drains a steered voice note whose bridge failed; the push LANDED (settle
    // accepts) and a later failure of the same request surfaces. With an outer
    // payload present — always the case in production, since a tool
    // continuation is preceded by the outer turn's prompt — settle's onAccept
    // must hand the retry store back to that outer payload, symmetric with
    // handleResolvedSteer. Otherwise Ctrl+Y re-submits the stored composite and
    // re-injects the already-accepted steer. (The already-landed tool
    // responses themselves are deduped by core's repairOrphanedToolUseTurns;
    // the steer segment is what this guards.)
    const queuedPrompt = 'listen @/tmp/recording.wav';
    const resolvedAudioPart: Part = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    ...
    mockSendMessageStream
      ...
      // Emulate core settle for a push that LANDED: accept() settles the steer
      // before the stream failure reaches the CLI's failure handler.
      .mockReturnValueOnce(
        (async function* () {
          yield {
            type: ServerLlmEventType.Content,
    ...
    await act(async () => {
      if (capturedOnComplete) {
        await capturedOnComplete(
          makeCompletedToolCall('call1', 'prompt-id-hook-accept'),
        );
      }
    });
    await waitFor(() => {
    ...
    // The push landed (accept fired) and the stream then failed: restore()
    // must have stayed a no-op (no re-queue).
    expect(restoreSteer).not.toHaveBeenCalled();
    ...
    // The retry restarts the outer payload — it does NOT re-deliver the
    // already-accepted steer audio.
    const retrySent = JSON.stringify(mockSendMessageStream.mock.calls[2][0]);
    expect(retrySent).toContain('start the analysis');
    expect(retrySent).not.toContain('UklGRg==');
  });

  it('strips a consumed steer from the retry store in goal-first sessions', async () => {
    // R47-4 negative control: goal-first sessions have nothing to hand back —
    // `/goal` is intercepted by the message queue and Goal submits skip the
    // lastPromptRef store, so `previousPayload` is still null when the first
    // goal tool continuation drains a steer. The old code installed no
    // onAccept at all for null previousPayload, so the composite (tool
    // responses + steer) stayed in the store after the model CONSUMED the
    // steer, and a later retryable failure let Ctrl+Y re-inject the
    // already-consumed steer segment (double delivery — the orphan strip
    // keeps the consumed steer push because the model answered it). With
    // nothing to hand back, accept must strip the steer segment instead:
    // only the dedup-safe tool responses stay retryable.
    const queuedPrompt = 'listen @/tmp/recording.wav';
    const resolvedAudioPart: Part = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    };
    ...
    mockSendMessageStream
      // Send 1: the GOAL tool continuation draining the steer. Goal submits
      // skip the lastPromptRef store, so previousPayload is null when this
      // send stores its composite — no outer prompt precedes it. The model
      // CONSUMES the steer (content lands), settle accepts, then a later
      // retryable failure of the same request surfaces.
      .mockReturnValueOnce(
        (async function* () {
          yield {
            type: ServerLlmEventType.Content,
            value: 'answer that consumed the steer',
          };
          (
            mockSendMessageStream.mock.calls[0]?.[3] as {
              steerInput?: SteerInput;
      ...
      // Send 2: the Ctrl+Y retry.
      .mockReturnValueOnce(finished());

    // No preceding submitQuery: goal-first session, lastPromptRef is null.
    await act(async () => {
      if (capturedOnComplete) {
        await capturedOnComplete(
          makeCompletedToolCall('call1', 'prompt-id-goal-first-steer'),
        );
      }
    });
    await waitFor(() => {
    ...
    // The consumed steer segment left the store at accept time: the retry
    // re-sends only the dedup-safe tool responses — never the steer audio,
    // its text, or its marker (re-injecting any of them would deliver the
    // consumed steer content to the model a second time).
    const retrySent = JSON.stringify(mockSendMessageStream.mock.calls[1][0]);
    expect(retrySent).toContain('testTool');
    expect(retrySent).not.toContain('UklGRg==');
    expect(retrySent).not.toContain('listen @/tmp/recording.wav');
    expect(retrySent).not.toContain('could not transcribe');
  });
  ...
  it('re-bridges a client-driven steer drain via Ctrl+Y when the Steer send fails', async () => {
    ...
    //
    // Faithful to core's ordering (R47-5): the turn streams content BEFORE
    // the boundary drain resolves (every takeSteerInput site is post-turn),
    // the drain resolves (onSteerResolved), settle ACCEPTS once the push
    // lands, and only then does the enclosing failure surface. The old shape
    // of this test never invoked settle (bare vi.fn() accept/restore) and
    // blessed the stranding regression.
    mockRunAudioBridge.mockResolvedValue({
      status: 'ok',
    ...
    mockSendMessageStream
      .mockReturnValueOnce(
        (async function* () {
          // Production ordering: the turn streams content before the
          // boundary drain resolves.
          yield {
            type: ServerLlmEventType.Content,
            value: 'partial response',
          };
          ...
          // The Steer push lands (settle accepts) before the enclosing
          // failure surfaces — core always settles in a finally first.
          input.accept();
    ...
    expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
    // Accepted-settle, no re-queue: the retry store is the single channel.
    expect(restoreSteer).not.toHaveBeenCalled();
    ...
    // No model content landed SINCE the drain (the enclosing turn's earlier
    // content does not count — the steer was not in it), so the accepted
    // steer never reached the model: Ctrl+Y re-delivers the stored pristine
    // payload (re-bridged), not the handed-back outer prompt.
    const retrySent = JSON.stringify(mockSendMessageStream.mock.calls[1][0]);
  ...
  it('restores the outer retry payload when the client-steer drain is restored', async () => {
    ...
    //
    // R47-1 negative control: settle restores BEFORE the enclosing failure
    // surfaces (core settles in a finally), and no model content lands after
    // the drain — the exact shape where an undo keyed only on lastPromptRef
    // identity would re-arm the stored payload alongside the re-queue and
    // deliver the steer twice. The retry must stay the outer payload.
    const restoreSteer = vi.fn();
    mockSendMessageStream.mockReturnValueOnce(
      (async function* () {
        // Production ordering: content streams before the boundary drain.
        yield {
          type: ServerLlmEventType.Content,
          value: 'partial response',
        };
        ...
        opts.onSteerResolved?.(input);
        // The Steer push never lands: settle restores before the enclosing
        // failure surfaces.
        input.restore();
        throw new Error('steer send failed');
      })(),
    ...
    expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
    // The re-queue channel fired (restore settled the drain).
    expect(restoreSteer).toHaveBeenCalledTimes(1);
  ...
  it('hands the retry store back to the outer payload once the steer is accepted', async () => {
    // Once the steer's push has landed (settle fires accept) AND the model
    // answered it (content lands after the drain), the steer content is
    // owned by the history — so the retry store must hand back the outer
    // payload the drain superseded. Leaving the steer payload in the store
    // would let Ctrl+Y re-deliver the already-accepted steer when a later
    // core-driven continuation of the same request fails (the exact
    // twice-delivery the restore channel prevents for the re-queue).
    mockSendMessageStream.mockReturnValueOnce(
      (async function* () {
        const opts = mockSendMessageStream.mock.calls[0]?.[3] as {
          onSteerResolved?: (steerInput: SteerInput) => void;
        ...
        input.accept = vi.fn(() => input.onAccept?.());
        opts.onSteerResolved?.(input);
        // The Steer push lands (settle accepts) and the model answers the
        // steer: content lands AFTER the drain, proving delivery.
        input.accept();
        yield {
          type: ServerLlmEventType.Content,
          value: 'answer to the steer',
        };
        throw new Error('outer turn failed');
      })(),
    );
    ...
    // The errored outer turn stays retryable with ITS payload: Ctrl+Y
    // re-sends the outer prompt exactly once — never the accepted steer.
    expect(mockSendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockSendMessageStream.mock.calls[1]?.[0]).toBe('start the analysis');
    expect(
      JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0]),
  ...
  it('keeps a client-steer payload retryable when the enclosing send fails before any model content', async () => {
    ...
    // accepts) but no model content lands SINCE the drain. The accept
    ...
    // re-delivers it. The enclosing send's failure must undo the hand-back
    // and keep the steer payload in the retry store.
    //
    // Production-faithful ordering (R47-3): core only resolves client-driven
    // drains at turn boundaries, AFTER the turn streamed content — so a
    // Content event lands before onSteerResolved. The enclosing send's
    // tracker is already true by then; the undo must key on the per-drain
    // tracker, not the send's.
    mockRunAudioBridge.mockResolvedValue({
      status: 'ok',
    ...
    mockSendMessageStream.mockReturnValueOnce(
      (async function* () {
        // The turn streams content before the boundary drain resolves
        // (every takeSteerInput site is post-turn) — the enclosing tracker
        // is already true by drain-resolution time.
        yield {
          type: ServerLlmEventType.Content,
          value: 'outer turn content',
        };
        // Core resolves the drain (onSteerResolved stores the pristine
        // payload), the Steer push lands (settle fires accept via onAccept),
        // then the send errors with no content landed since the drain.
        const opts = mockSendMessageStream.mock.calls[0]?.[3] as {
          onSteerResolved?: (steerInput: SteerInput) => void;
        };
        const input: SteerInput = {
        ...
        opts.onSteerResolved?.(input);
        input.accept();
        throw new Error('steer send failed before its own content');
      })(),
    );
  ...
  it('keeps a retryParts-less client steer retryable when the enclosing send fails', async () => {
    // R47-2 negative control: the COMMON drain shape has no retryParts —
    // every text steer and every cleanly-transcribed media steer. The old
    // handleResolvedSteer early-returned for them (no store swap, no
    // onAccept/onRestore, no undo record), so an accepted steer whose
    // enclosing send errored was stranded: Ctrl+Y retried the outer prompt
    // while the orphan strip popped the landed steer push. The bookkeeping
    // must not gate on retryParts — the drain's own parts are the payload.
    mockSendMessageStream
      .mockReturnValueOnce(
        (async function* () {
          // Production ordering: the turn streams content before the boundary
          // drain resolves.
          yield {
            type: ServerLlmEventType.Content,
            value: 'outer turn content',
          ...
          const input: SteerInput = {
            // No retryParts: every bridge succeeded, `parts` IS the payload.
            parts: [{ text: 'steered question' }],
            accept: vi.fn(),
            restore: vi.fn(),
          };
    ...
    await act(async () => {
      await result.current.submitQuery(
        'start the analysis',
        SendMessageType.UserQuery,
        'prompt-id-client-steer-no-retryparts',
      );
    });
    expect(mockSendMessageStream).toHaveBeenCalledTimes(1);
    ...
    // The accepted steer never reached the model (no content landed since
    // the drain): Ctrl+Y re-delivers the drain's parts, not the outer prompt.
    expect(JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0])).toContain(
      'steered question',
    );
    expect(mockSendMessageStream.mock.calls[1]?.[0]).not.toBe(
  ...
  it('fails closed a top-level image an image-incapable media-routed override cannot see', async () => {
    // A skill tool installs a non-inline audio-capable override; the drain's
    // audio routes to it and stamps it. A top-level image in the same drain
    // must be capability-probed against that same route and fail closed
    // visibly when the route cannot see it — instead of passing raw into the
    // exact route, where slimming would silently placeholder-substitute it.
    const audioPrompt = 'listen @/tmp/recording.wav';
    const imagePrompt = 'inspect @/tmp/screenshot.png';
    const audioPart: Part = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        request: {
          callId: 'call-img-route',
          name: 'testTool',
          args: {},
          isClientInitiated: false,
          prompt_id: 'prompt-id-image-incapable-route',
        },
        status: 'success',
        responseSubmittedToLlm: false,
        response: {
    ...
    const send = mockSendMessageStream.mock.calls[0];
    const sent = JSON.stringify(send?.[0]);
    // The routed audio stays raw on its exact route; the image failed closed
    // visibly with its pristine bytes captured for Retry — never raw in the
    // exact-routed payload.
    expect(sent).toContain('audio/wav');
    expect(sent).toContain('UklGRg==');
    expect(sent).toContain('Image was not sent');
    expect(sent).not.toContain('aW1hZ2U=');
    expect(send?.[3]).toMatchObject({
      modelOverride: 'audio-skill-model\0',
    });
    expect(mockAddItem).toHaveBeenCalledWith(
      expect.objectContaining({
        type: MessageType.ERROR,
  ...
  it('routes a top-level image an image-capable media-routed override can see', async () => {
    // Complement of the fail-closed test: when the media-routed override
    // DOES support images, the capability probe (extended to non-inline
    // overrides) must route the top-level image raw on the exact route —
    // not fail it closed or bridge it away. Without the probe extension the
    // no-bridge arm would fail-close a perfectly deliverable image.
    const audioPrompt = 'listen @/tmp/recording.wav';
    const imagePrompt = 'inspect @/tmp/screenshot.png';
    const audioPart: Part = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    ...
    const resolveForModel = vi.fn(async () => ({
      contentGeneratorConfig: { modalities: { audio: true, image: true } },
    }));
    Object.assign(mockConfig, {
      getEffectiveInputModalities: () => ({}),
      getDefaultVisionBridgeModel: () => undefined,
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        request: {
          callId: 'call-img-capable-route',
          name: 'testTool',
          args: {},
          isClientInitiated: false,
          prompt_id: 'prompt-id-image-capable-route',
        },
        status: 'success',
        responseSubmittedToLlm: false,
        response: {
          ...
          errorType: undefined,
          modelOverride: 'omni-skill-model',
        },
        tool: { displayName: 'MockTool' },
        invocation: {
          getDescription: () => 'Mock description',
    ...
    await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalledOnce());
    expect(resolveForModel).toHaveBeenCalledWith('omni-skill-model', {
      failClosed: true,
    });
    const send = mockSendMessageStream.mock.calls[0];
    const sent = JSON.stringify(send?.[0]);
    // Both media routed raw on the exact route — nothing failed closed.
    expect(sent).toContain('UklGRg==');
    expect(sent).toContain('aW1hZ2U=');
    expect(sent).not.toContain('was not sent');
    expect(send?.[3]).toMatchObject({
      modelOverride: 'omni-skill-model\0',
    });
  });
  ...
  it('keeps the media-routed override when a later segment\'s audio probe fails', async () => {
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        request: {
          callId: 'call-sticky-route',
          name: 'testTool',
          args: {},
          isClientInitiated: false,
          prompt_id: 'prompt-id-sticky-route',
        },
        status: 'success',
        responseSubmittedToLlm: false,
        response: {
    ...
    const midTurnDrainRef = {
      current: vi
        .fn<() => string[]>()
        .mockReturnValueOnce([firstAudio, secondAudio])
        .mockReturnValue([]),
    };
    let capturedOnComplete:
      | ((completedTools: TrackedToolCall[]) => Promise<void>)
  ...
  it('stores the drain pristine capture when the tool-result gate also captured', async () => {
    ...
    // holds the drain's marker-substituted steer segment) while the drain
    // supplies the fully pristine payload via metadata. The pristine capture
    // must win — storing the gate capture would strand the steer segment's
    // pristine media, so Ctrl+Y resends the marker instead of re-deriving.
    const audioPrompt = 'listen @/tmp/recording.wav';
    const imagePrompt = 'inspect @/tmp/screenshot.png';
    const audioPart: Part = {
      inlineData: { mimeType: 'audio/wav', data: 'UklGRg==' },
    ...
    const topLevelImageMarkerPrefix =
      'Image was not sent: the active model override does not support images';
    const resolveForModel = vi.fn(async () => ({
      contentGeneratorConfig: { modalities: { audio: true } },
    }));
    ...
    // The tool result nests an image the audio-only route cannot see (gate
    // capture); the drain's audio establishes the route and its image fails
    // closed for lack of a vision bridge (metadata capture).
    const toolResponseWithNestedImage: Part[] = [
      {
        functionResponse: {
          id: 'call-dual-capture',
          name: 'testTool',
          response: { output: 'screenshot attached' },
          parts: [
            {
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        request: {
          callId: 'call-dual-capture',
          name: 'testTool',
          args: {},
          isClientInitiated: false,
          prompt_id: 'prompt-id-dual-capture',
        },
        status: 'success',
        responseSubmittedToLlm: false,
        response: {
          callId: 'call-dual-capture',
          responseParts: toolResponseWithNestedImage,
          errorType: undefined,
          modelOverride: 'audio-skill-model',
        },
        tool: { displayName: 'MockTool' },
    ...
    mockSendMessageStream
      .mockReturnValueOnce(
        ...
        (async function* () {
          ...
          // accept() settles the steer input before the stream failure
          // reaches the CLI's failure handler, so the restore() that handler
          // triggers is a no-op and the undo re-arms the stored composite —
          // keeping its steer segment for the capture-precedence comparison.
          // (No Content event first: content landed since the push would
          // mean the model consumed the steer, and the store would keep only
          // the tool responses.)
          (
            mockSendMessageStream.mock.calls[0]?.[3] as {
              steerInput?: SteerInput;
    ...
    await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalledOnce());
    // The drained audio exact-routes raw; the drained image and the nested
    // tool image both fail closed visibly (no vision bridge, audio-only
    // route) — no raw image bytes in the payload.
    const firstSent = JSON.stringify(mockSendMessageStream.mock.calls[0]?.[0]);
    expect(firstSent).toContain('UklGRg==');
    expect(firstSent).not.toContain('aW1hZ2U=');
    expect(firstSent).not.toContain(nestedImageData);
    expect(firstSent).toContain('was not sent');
    expect(mockSendMessageStream.mock.calls[0]?.[3]).toMatchObject({
      modelOverride: 'audio-skill-model\0',
    });
    ...
    await waitFor(() => expect(mockSendMessageStream).toHaveBeenCalledTimes(2));
    // The retry re-derived from the drain's fully pristine capture: the
    // steered image is re-failed-closed from its pristine bytes (a second
    // visible ERROR) instead of resending the drain's marker text, and the
    // nested tool image is re-gated. Under the `??` precedence bug the gate
    // capture won and Ctrl+Y resent the marker without re-deriving.
    const retried = JSON.stringify(mockSendMessageStream.mock.calls[1]?.[0]);
    expect(retried).toContain('UklGRg==');
    expect(retried).not.toContain('aW1hZ2U=');
    expect(retried).not.toContain(nestedImageData);
    expect(retried).toContain('was not sent');
    const topLevelImageFailCloses = mockAddItem.mock.calls.filter(
      ([item]) =>
        (item as { type?: string; text?: string })?.type ===
          MessageType.ERROR &&
        typeof (item as { text?: string }).text === 'string' &&
        (item as { text: string }).text.startsWith(topLevelImageMarkerPrefix),
    );
    // Once in the drain, once again when the retry re-derives the pristine
    // image — proof the pristine capture (not the marker) was stored.
    expect(topLevelImageFailCloses).toHaveLength(2);
  });
  ...
  describe('Slash Command Handling', () => {
    ...
    describe('inline model override', () => {
      ...
      it('does not re-apply the exact-route marker to a media-free notification turn', async () => {
        allowInlineModel();
        // Production drains Notifications with no prompt_id (submitQuery
        // mints one from the prompt counter); mirror that shape — the
        // minted id must not collide with the stamped turn's id. Make the
        // counter advance on startNewPrompt like the real SessionContext.
        let promptCount = 5;
        const previousImplementation = vi
          .mocked(useSessionStats)
          .getMockImplementation();
        vi.mocked(useSessionStats).mockImplementation(
          () =>
            ({
              ...
              addUsage: vi.fn(),
              getPromptCount: vi.fn(() => promptCount),
              stats: { sessionId: 'test-session-id' },
            }) as never,
        );
        try {
          const imagePart = {
          ...
          // A background completion drains afterwards as a standalone,
          // text-only Notification turn with NO explicit prompt id —
          // production passes undefined and submitQuery mints a fresh id. It
          // inherits the override (per #7114 it must not be cleared) but
          // keeps the bare selector — the routed media belongs to turn 1, so
          // the marker must not be re-applied to this media-free turn.
          await act(async () => {
            await result.current.submitQuery(
              '<task-notification>completed</task-notification>',
              SendMessageType.Notification,
      ...
      it('does not exact-route a media-free notification onto a retry stamp', async () => {
        // Retry submits with no prompt_id (submitQuery mints one from the
        // prompt counter) and — unlike UserQuery/Cron/Teammate — used not to
        // advance the counter, so a media-free Notification drained right
        // after minted the SAME id, matched the retry's media-routed stamp,
        // and was exact-routed into the stale override. Retry must advance
        // the counter so every minted id is unique.
        let promptCount = 5;
        const previousImplementation = vi
          .mocked(useSessionStats)
          .getMockImplementation();
        ...
        try {
          ...
          const completedToolCalls: TrackedToolCall[] = [
            {
              request: {
                callId: 'call-retry-stamp',
                name: 'testTool',
                args: {},
                isClientInitiated: false,
                prompt_id: 'prompt-id-retry-stamp',
              },
              status: 'success',
              responseSubmittedToLlm: false,
              response: {
          ...
          mockSendMessageStream
            .mockReturnValueOnce(
              // eslint-disable-next-line require-yield
              (async function* () {
                // Settle the drained steer as push-landed with NO model
                // content before the failure surfaces: restore() must be a
                // no-op and the undo re-arms the composite so the retry store
                // keeps the routed audio for Ctrl+Y. (No Content event first:
                // content landed since the push would mean the model consumed
                // the steer and the store would keep only the tool responses.)
                (
                  mockSendMessageStream.mock.calls[0]?.[3] as {
                    steerInput?: SteerInput;
                  }
            ...
            .mockReturnValueOnce(
              (async function* () {
                ...
                throw new Error('retry failed too');
              })(),
            )
            .mockReturnValueOnce(
              (async function* () {
          ...
          // The media-routed continuation exact-routes and stamps its prompt.
          expect(mockSendMessageStream.mock.calls[0]?.[3]).toMatchObject({
            modelOverride: 'audio-skill-model\0',
          });

          // Ctrl+Y re-routes the stored media and re-stamps with its minted
          // id — and must advance the counter so the id stays unique.
          await act(async () => {
            await result.current.retryLastPrompt();
          });
          await waitFor(() =>
          ...
          // A media-free Notification then mints its own id; it must NOT
          // match the retry's stamp and must keep the bare selector.
          await act(async () => {
            await result.current.submitQuery(
              '<task-notification>completed</task-notification>',
              SendMessageType.Notification,
