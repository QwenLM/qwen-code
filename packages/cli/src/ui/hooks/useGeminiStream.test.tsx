...
describe('useGeminiStream', () => {
  ...
  it('unwinds never-executed not_started scheduler responses before submitting them (issue #9450)', async () => {
    // A scheduler response with executionStatus 'not_started' — the
    // plan-mode-entry sibling skip, pre-validation cancellations,
    // permission / tool-not-found / validation rejections — never executed,
    // so its fabricated constant error carries no result evidence. The hook
    // must unwind the loop guards' request-side reservations before
    // submission: client.ts's recording feed excludes only the
    // duplicate-provider synthetic class, so without the unwind the
    // fabricated error pairs with the streamed request in requestByCallId
    // and records as a "changed" result — resetting frozen-board streaks
    // and disarming every result-aware halt (the daemon twin excludes this
    // class via its not_started filter).
    const noteSuppressedToolCallByCallId = vi.fn();
    const client = new MockedGeminiClientClass(mockConfig);
    client.getLoopDetectionService = vi.fn().mockReturnValue({
    ...
    const completedToolCalls: TrackedToolCall[] = [
      {
        request: {
          callId: 'skipped-sibling',
          name: 'task_list',
          args: {},
          isClientInitiated: false,
          prompt_id: 'prompt-not-started',
        },
        status: 'error',
        responseSubmittedToGemini: false,
        response: {
          ...
          responseParts: [
            {
              functionResponse: {
                id: 'skipped-sibling',
                name: 'task_list',
                response: { error: 'plan mode entry sibling skip' },
              },
            },
          ],
      ...
      } as unknown as TrackedCompletedToolCall,
      {
        ...
        response: {
          callId: 'executed-tool',
          responseParts: [{ text: 'executed output' }],
          errorType: undefined,
    ...
    // Only the never-executed synthetic is unwound; the executed response
    // keeps its result evidence.
    await waitFor(() => {
      expect(noteSuppressedToolCallByCallId).toHaveBeenCalledTimes(1);
    });
    expect(noteSuppressedToolCallByCallId).toHaveBeenCalledWith(
      'skipped-sibling',
    );
  });
