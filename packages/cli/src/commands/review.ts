...
export const reviewCommand: CommandModule = {
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .demandCommand(
        1,
        'Specify a subcommand: run, parse-args, fetch-pr, capture-local, plan-diff, pr-context, comment-status, load-rules, agent-prompt, build-test, base-tree, test-delta, drive, extract-step, script-lint, resolve-anchors, check-coverage, presubmit, test-efficacy, test-plan, findings, publish-assets, compose-review, submit, or cleanup.',
      )
      .version(false),
  handler: () => {
