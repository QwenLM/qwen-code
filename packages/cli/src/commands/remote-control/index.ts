...
export const remoteControlCommand = {
  ...
  handler: async (argv: Arguments) => {
    ...
    try {
      ...
      // FIX: Generate QR code from URL only - token entered manually, not in URL
      try {
        await new Promise<void>((resolve) => {
          qrcode.generate(
            connectionInfo.url, // No token in URL
            { small: true },
            (code: string) => {
              // eslint-disable-next-line no-console
...
// FIX: ESM-compatible entrypoint check (package is "type": "module")
