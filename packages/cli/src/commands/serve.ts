  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option('pairing-qr', {
        type: 'boolean',
        description:
          'Print the token-bearing pairing QR even when stdout is captured (not an interactive terminal) and the bearer is an operator-supplied (stable) token, which is withheld by default to keep stable credentials out of collected logs. Enable only when the log pipeline is as trusted as the daemon host. Can also be set via the serve.pairingQr setting (user, system, and system-defaults scopes only); the flag wins when passed, and --no-pairing-qr vetoes the setting for that run.',
      })
  ...
  handler: async (argv) => {
    ...
    try {
      const serveOptions = {
        ...
        ...(argv['pairing-qr'] !== undefined
          ? { pairingQr: argv['pairing-qr'] }
          : {}),
