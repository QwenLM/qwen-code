...
describe('runTestPlan', () => {
  ...
  describe('command claims', () => {
    ...
    const mavenCmd = (
      ...
      // produce (it joins first, then wraps the whole selector).
      const selector =
        modules === null ? null : (shellSelector(modules) ?? modules.join(','));
      const narrowing =
    ...
    it('recognizes Maven wrapper command spellings', () => {
      for (const command of [
        ...
        '..\\..\\mvnw test',
        '../../mvnw.cmd test',
        // The Maven daemon and the debug launcher are Maven too.
        'mvnd test',
        'mvnDebug test',
      ]) {
