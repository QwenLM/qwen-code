...
- **The defect lives in a file the diff does not touch** (a feature-docs page the change falsifies, a contract that sits elsewhere): the resolver only searches the diff's new-side lines, so you cannot anchor the target file itself. Anchor on the changed line that MAKES it wrong — set **File** to that diff file and quote that line — and name the real target file, and why the change breaks it, in the **Issue** text. The comment hangs off the relevant change; the body tells the author where the defect actually is.
