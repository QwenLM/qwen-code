...
export interface Ledger {
   ...
   *
   * Withheld is not lost: a fail-closed round's marker carries no `sha`, but
   * recovery (`pr-context`) grafts the anchor forward from the most recent
   * EARLIER own marker that carries one — the withhold is about the
   * fail-closed round's own range, while an earlier round's "clean up to
   * sha" stays true, and scoping the next round `sha..HEAD` re-covers the
   * gap. One non-clean round therefore degrades the next round's range, not
   * every later round's (issue #9902).
   *
