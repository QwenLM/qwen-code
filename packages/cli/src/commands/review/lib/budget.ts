...
/**
 * Edge punctuation and balanced wrapper pairs stripped to a fixpoint, for
 * the normalize and fold keys: punctuation rides OUTSIDE wrappers (`(-).`
...
export function budgetGapDisclosures(finalText: string): string[] {
  ...
  for (const line of finalText.split(/\r?\n/)) {
    ...
    // The normalized form sheds edge punctuation and wrapping brackets,
    // quotes, and parens — balanced pairs only, never one side: `(none)`
    // and `"none"` reached a real posted body as phantom gaps because the
    // wrapping defeated the match below. Judged bare and wrapped alike, by
    // the one strict classifier — its doc names why the shapes are narrow.
    const normalized = stripGapWrappers(raw);
    if (normalized.length === 0 || PLACEHOLDER_GAP_RE.test(normalized)) {
      continue;
    }
    // Folded on the wrapper-stripped text, so one gap restated with and
    // without wrappers — `(auth flow untested.)` and `auth flow untested`
    // — discloses once.
