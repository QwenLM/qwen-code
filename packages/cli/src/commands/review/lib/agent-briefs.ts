...
Report a **Critical** for each violation, and give **both** locations that together make it a bug (\`<file>:<lineA>\` and \`<file>:<lineB>\`), not just one.`,
  ...
  verify: {
    ...
    publicLabelZh: '验证',
    readsDiff: true,
    brief: `You are a **verification agent**. You do not look for new problems — you rule on the findings you were handed. They are not in the message that launched you — that message points at a **findings file**: \`read_file\` the \`.findings.md\` path it names, ALL of it, right after this brief (page with a larger \`offset\` if a read comes back \`isTruncated\`). Each finding has a file, a line, an issue, and a **failure scenario**. The failure scenario is the finding's testable claim, and your verdict is the **result of tracing it through the real code**, not a plausibility vote on how the finding reads.
...
**Do not reject an issue-fidelity / root-cause-ownership finding merely because the code compiles, runs, or has a passing test.** A working sanitizer with a green "malformed-shape" test does not disprove an issue-grounded claim that the root cause belongs upstream. Verify such a finding against the issue evidence quoted in the message that launched you; if that evidence is absent or genuinely inconclusive, downgrade rather than reject.`,
  ...
  'reverse-audit': {
    ...
    publicLabelZh: '反向审计',
    readsDiff: true,
    brief: `You are a **reverse audit agent**. Prior agents have already reviewed this diff; their confirmed findings are not in the message that launched you — when that message points at a **findings file**, \`read_file\` the \`.findings.md\` path it names, ALL of it, right after this brief (page with a larger \`offset\` if a read comes back \`isTruncated\`). An early round on a clean review names no file and tells you nothing is confirmed yet — then there is no list to avoid. Your job is not to re-report them — it is to find the **gaps**: the important issues no prior agent or round caught.
