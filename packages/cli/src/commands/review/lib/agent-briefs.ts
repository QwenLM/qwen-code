...
- \`toolchain: "maven"\` → read its \`build[]\` / \`test[]\` results exactly like the npm line: a failure in a file **the diff changed** is a **Critical**, a failure in a file it did not touch is pre-existing, and a non-empty \`timedOut\` is infrastructure — informational, never a Critical. The scope is \`mvn -pl <modules> -am\` — the modules the diff touches plus everything they compile against, through \`./mvnw\` when the repo pins one. A command that timed out may be retried ONCE with \`--timeout 550\` before ruling it infrastructure; a second timeout stands. When \`test[]\` failed, SKIP the base-tree/test-delta measurement below — the A/B is unavailable for this toolchain — the path rule stands, say so.
