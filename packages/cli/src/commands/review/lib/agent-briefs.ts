...
**Run \`qwen review build-test\` (the exact command, with its \`--plan\` and \`--worktree\`, is below).** It detects supported npm or Maven projects, scopes the run to the changed workspaces/modules plus their required dependency and dependent closure, and reports the commands and evidence as JSON. Do **not** substitute hand-written npm or Maven commands. The old npm brief used a 120-second full-build deadline; measured across the harness's own transcripts, it timed out **71 times** and verified nothing. \`build-test\` scopes the run, gives each command a deadline it can meet, and — this is the part a hand-run command gets wrong — reports a timeout or dependency-acquisition failure as **infrastructure, not a finding**. A build that runs out of time is never a Critical against someone's pull request.
...
- \`toolchain: "unsupported"\` (build-test could not safely select or scope a supported project) → follow the report's note. If multiple root toolchains apply, do not guess ownership. Otherwise install dependencies first and fall back to **one** build and **one** test command by this precedence, each with a deadline it can meet: \`build.gradle\` → \`{gradle} compileJava\` / \`{gradle} test\`; \`Makefile\` → \`make build\`; \`Cargo.toml\` → \`cargo build\` / \`cargo test\`; \`go.mod\` → \`go build ./...\` / \`go test ./...\`; \`pytest.ini\` or \`pyproject.toml\` \`[tool.pytest]\` → \`pytest\`. A root \`pom.xml\` is normally handled by the Maven adapter; if it reached unsupported, its reactor could not be modeled safely, so do not silently replace that fail-closed result with an ad hoc Maven command. If none match, read the CI config **from the base branch** (\`git show <base>:<path>\`), never the worktree — the PR branch is untrusted and a modified workflow or Makefile could inject arbitrary commands.
