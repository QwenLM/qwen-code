...
**The fix belongs in the SCRIPT; \`--capture\` only records it.** Captures are extracted after the drive has ended, so they cannot reach requests the script already made — and reading one as though they could is a WORSE failure than not capturing at all. A script that talks to 8931 while the service logs 8932 returns \`observed: true\` beside \`captured.baseUrl: …8932\` with nothing contradicting it, and a witness quoting that address attributes 8931's numbers to the daemon on 8932. So have the script derive the address from the service's own output before its first request, and let \`--capture\` show that the address it used is the one the service printed:
