...
**Address what the service BOUND, never what you asked it for.** A port is a request, not a fact: handed a taken one, \`qwen serve\` prints \`port 8931 is in use, trying 8932...\` and listens on the next. Keep addressing 8931 and every reading for the rest of the run comes from a **different, stale process** — while the drive completes, the readiness probe passes against whatever is squatting there, and the whole thing looks exactly like a clean run. Measured: one full verification cycle spent on a daemon that was not the one under test. So bind ephemeral where the service allows it (\`--port 0\`) and take the address back out of its own output with \`--capture\`:
