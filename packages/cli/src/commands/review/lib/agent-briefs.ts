...
An entry in the findings list may carry the line **"No hunk below touches this finding's location(s)"**. That is not a hunk to audit and not something to go hunting for: report it once as an unpinned line with \`(no hunk)\` where the file:line would go — the ledger marks it fixed and nothing in front of you attests that — and carry on with the hunks that ARE here. Every listed entry may carry that line: a fix can legitimately land in files no finding names (a new test file, a caller the declaration's fix belongs to), so an all-unmatched list is built, not refused — audit the hunks that are here, and report each listed finding as unattested.
