...
- Fetch the issue evidence with the \`review issue-context\` command your task context names — it resolves the closing-issue metadata, then fetches each issue's title, body, and **full comment thread** from the issue's OWN repository (a PR can close an issue in another repo; the subcommand takes the repository each reference carries). The closing-issue set is a discovery hint, not proof the author linked the right issue. If it is empty (the evidence file says so explicitly), do **not** treat every \`#123\` mentioned in the PR description as a target issue: references phrased as prior incidents, examples, regressions, comparisons, or “what happened on #123” are motivating evidence, not the requested scope. Fetch an unlinked reference as a target issue only when the PR context explicitly says this PR fixes, closes, resolves, or implements it — re-run the command with \`--issue <n>\` to add it (extras are fetched from the PR's OWN repository — if the referenced issue lives in a different repo, this command cannot fetch it: declare that evidence unavailable instead of judging the PR repo's same-numbered, unrelated issue). You may fetch a motivating incident for evidence, but label it as such and do not claim the PR is required to satisfy that referenced PR's own scope.
