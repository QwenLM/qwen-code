...
**When your prompt carries an "Accumulation candidates" section, adjudicating every entry in it is REQUIRED, not advisory.** Each candidate is an added line that writes into a container that may outlive the write. A diff hunk shows the single write, never the accumulation — the leak lives downstream, in wherever the written value finally RESTS. For EACH candidate your findings record must answer three questions: **(a)** how often does this write fire — per turn, per tool call, per event, or once? **(b)** what bounds the container the value ultimately rests in — a per-turn or staging local is NOT a container, and clearing on one is the measured failure mode; follow the value through every hand-off (arguments, aliases, returns) until it reaches long-lived state or provably dies. **(c)** who reclaims old entries from that resting place, and does the reclaimer actually cover this kind of entry? A candidate whose answers are "recurs per turn / container unbounded / nothing reclaims it" is a finding — unbounded accumulation is a leak, and a leak is Critical. A candidate you clear must name the resting container and its bound or reclaimer; "looks fine" clears nothing.
