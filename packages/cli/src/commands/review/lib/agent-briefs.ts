...
- \`toolchain: "refused"\` (the operator set \`review.sandbox: required\` and no container runtime answered) → the build and test evidence is **unavailable for this run**. Report the dimension as unmeasured, quoting the report's \`note\`. **Do not install, build or test by hand to fill the gap** — the whole point of that setting is that this repository's own commands do not execute outside a container, and running them from your shell is the one route around it. This is the same discipline as a probe that cannot get an isolated tree: absent evidence, never substituted evidence.
