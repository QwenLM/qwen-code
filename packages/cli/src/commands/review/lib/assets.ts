  ...
  if (v === '') {
    return {
      error:
        ...
        'a fork or scratch repo rather than the repository under review) to ' +
        'publish review evidence images. The PR-review workflow blanks a ' +
        'self-targeting designation before the CLI sees it, so it reads as ' +
        'unset here.',
    };
  }
  const parts = v.split('/');
