...
async function runPrContext(args: PrContextArgs): Promise<void> {
  ...
  let sawOwnReview = false;
  if (reviews.length && me === '') {
    // The identity lookup failed without failing closed (nothing gated on
    // it): recovery cannot run, and an unknown identity is not proof of
    // absence — take the strip path, like a thrown recovery, so the round
    // counter survives.
    recoveryThrew = true;
  }
  try {
    if (reviews.length && me !== '') {
      const outcome = recoverOwnLedger(reviews, me);
