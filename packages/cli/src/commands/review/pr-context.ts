 ...
 * foreign work list as this account's own certified round. `merged` refines
 * that: when the foreign winner was merged OVER this account's own findings
 * (the union), the list is MIXED — calling it all "THEIR claims" gave false
 * provenance for the own subset and inverted the exact trust distinction the
 * author sentence exists to enforce — and its `dropped` sum spans two
 * markers plus the merge re-cap, so the PARTIAL note must not pin the loss
 * on one round's size cap (a Step 6 reader cross-referencing that round's
 * body finds it complete and dismisses the warning as stale).
 */
export function renderLedgerSection(
  ...
  return [
    ...
    '',
    `Round ${ledger.round}${ledger.sha ? `, reviewed at \`${code(ledger.sha)}\`` : ''}, recovered from ${author ? (merged ? `**@${cell(author)}**'s round-${ledger.round} marker MERGED over this account's own latest findings — entries this account certified are its own claims, the rest are @${cell(author)}'s, and no incremental anchor travelled with the foreign marker (the sha never crosses accounts; this round is full-range unless a local cache supplies one)` : `the marker **@${cell(author)}**'s last posted review carried — another account, so these are THEIR claims and no incremental anchor travelled with them (the sha never crosses accounts; this round is full-range unless a local cache supplies one)`) : `the marker this account's last posted review carried`}. **Every entry below is owed a this-round ruling** (fixed / still stands / cannot tell / superseded by <class-id>) under Step 6's previous-round rules — the ledger is a work list, not a verdict; re-assert each claim against the code before repeating or retiring it.${ledger.sha ? ` The reviewed-at sha is the incremental anchor Step 1's recovered-anchor check reads from the side file — when Step 1's recovered-anchor check rules a re-run admissible, pass it as \`--since <sha>\` on a \`fetch-pr\` re-run, which validates it against the fetched history and scopes the diff and plan; never run git against an anchor yourself.` : ''}`,
...
export function buildMarkdown(
  ...
  if (prevLedger) {
    parts.push(
      renderLedgerSection(prevLedger, prevLedgerAuthor, prevLedgerMerged),
    );
  }
