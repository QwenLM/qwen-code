...
export function renderLedgerSection(ledger: Ledger): string {
  ...
  return [
    ...
    '',
    `Round ${ledger.round}${ledger.sha ? `, reviewed at \`${code(ledger.sha)}\`${ledger.model ? ` by \`${code(ledger.model)}\`` : ''}` : ''}, recovered from the marker this account's last posted review carried. **Every entry below is owed a this-round ruling** (fixed / still stands / cannot tell / superseded by <class-id>) under Step 6's previous-round rules — the ledger is a work list, not a verdict; re-assert each claim against the code before repeating or retiring it.${ledger.sha ? ` The reviewed-at sha is the incremental anchor Step 1's recovered-anchor check reads from the side file — validate it is an ancestor of the fetched head, and scope to it only if the \`model\` beside it matches the model running THIS review (an absent \`model\` counts as a mismatch — the marker predates the field): "clean up to that sha" is the recorded model's verdict, and an anchor another model certified would scope this round past code the current model never reviewed.` : ''}`,
