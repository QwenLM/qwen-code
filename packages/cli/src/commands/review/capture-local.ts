 ...
 * identity: a rendering-only move — an `.git/info/attributes` edit, a
 * repo-local `diff.<driver>.binary` flip — changes none of the file's
 * bytes and appears in no `git diff HEAD`, yet dating on the suffix read
 * the blocker as "moved" and passed `--fail-on` while the stop still
 * rendered the Critical as open. Rendering moves keep their consequence
 * where it belongs — the incremental scope re-reviews them already.
 *
 * …but both sides of the byte comparison must come from one REGIME.
 * `hash-object` cleans under the CURRENT attributes while the recorded oid
 * was cleaned under the cache-time ones, and the lstat reports the CURRENT
 * mode while the tree stores the cached one: a `.gitattributes` `text`
 * entry plus `git add --renormalize` moves the computed oid while every
 * worktree byte stands still and `git diff HEAD` stays empty, and under
 * `core.fileMode=false` git itself stops trusting mode bits while the
 * stored and lstated modes disagree. Dating across either disagreement
 * read standing blockers as "moved" and passed `--fail-on` over them — the
 * normalisation-commit shape this very comparison was documented to close
 * was the shape that triggered it. The date is therefore regime-aware: the
 * recorded suffix names the clean-filter regime the recorded oid was
 * produced under (`text` drives normalisation), so a `text` state that
 * disagrees across the comparison is UNDATABLE — said out loud, never a
 * confident "moved" — and under `core.fileMode=false` the exec bit is
 * folded away on both sides (the file↔symlink type is not; git reports
 * typechanges under every fileMode). The residual is the documented
 * pass-lean beside the rendered list, same as every undatable blocker.
 *
 ...
 *
 * A fix can also land where the per-blocker date cannot see, and a stop
 * cannot attribute what it does not date — so two vetoes withhold every
 * blocker. A file ADDED to the captured population since the cached round
 * may BE the fix: a whole-population cache dates added files tracked AND
 * untracked, and a scoped capture (`--file`, `--no-untracked`) — which
 * cannot reconstruct the untracked half it never listed — dates the
 * TRACKED delta, which the cached HEAD tree reconstructs either way (see
 * `filesAddedSince`). And a file the cached round hashed whose bytes MOVED
 * since may CARRY the fix — the per-blocker date reads only the blocker's
 * own file, so a fix committed to a sibling left the gate red for ever.
 * The residual is a false PASS — an unrelated new file or an unrelated
 * moved file clears standing blockers — beside the rendered blocker list;
...
function blockerStateStillMatchesTree(
  ...
  // The added-file veto — see the docstring. A whole-population cache dates
  // added files tracked AND untracked; a scoped capture cannot reconstruct
  // the untracked half it never listed — files already present at cache
  // time would read as "added" and permanently disarm every blocker — but
  // the TRACKED delta reconstructs from the cached HEAD tree either way.
  const populationScoped =
    cache.source !== undefined || cache.untracked === false;
  const added = filesAddedSince(repoRoot, cache, populationScoped);
  if (added === null || added.length > 0) return false;
  const population = Object.keys(cache.files);
  const current = hashWorktreeFiles(repoRoot, [
    ...new Set([...population, ...blockers]),
  ]);
  // `core.fileMode=false` makes git itself ignore the exec bit — the
  // stored tree keeps one mode while the worktree lstat reports another and
  // `git diff HEAD` stays empty — so a date that still compared it would
  // read every standing blocker as "moved". Read once; the veto below and
  // the per-blocker date both key on it.
  const fileModeTrusted =
    gitOpt('-C', repoRoot, 'config', '--get', 'core.fileMode') !== 'false';
  // The moved-file veto: a fix committed to a file the cached round hashed
  // that is NOT the blocker's own — the per-blocker date below reads only
  // the blocker's file, and left the gate red for ever over that landed
  // fix. A DEFINITE move only: an undatable state (a clean-regime change,
  // UNHASHABLE on both sides) is not fix material — the per-blocker date
  // owns those, with the regime change said out loud.
  for (const p of population) {
    if (dateVerdict(cache.files[p], current[p], fileModeTrusted) === 'moved') {
      return false;
    }
  }
  ...
  return blockers.some((p) => {
    const before = Object.hasOwn(cache.files, p)
      ...
      : Object.hasOwn(atRevision, p)
        ? atRevision[p]
        : undefined;
    return blockerStillStands(p, before, current[p], fileModeTrusted);
  });
}

/**
 * One recorded-vs-fresh identity comparison, regime-aware. Both sides must
 * come from one regime for either half of the comparison to mean anything:
 * under `core.fileMode=false` the exec bit is folded away on both sides
 * (the file↔symlink type is not — git reports typechanges under every
 * fileMode), and a clean-filter regime change (`text` normalisation
 * switched on or off) leaves the oid comparison undatable — never a
 * confident "moved". `UNHASHABLE` on both sides is a stable unreadable
 * state, not a move.
 */
function dateVerdict(
  before: string | undefined,
  after: string | undefined,
  fileModeTrusted: boolean,
): 'stands' | 'moved' | 'undatable' {
  if (before === undefined) return 'undatable';
  if (before === UNHASHABLE) {
    return after === UNHASHABLE ? 'undatable' : 'moved';
  }
  if (after === undefined || after === UNHASHABLE) return 'moved';
  const b = before.split(':');
  const a = after.split(':');
  // The clean-filter regime — comparable only when both sides carry one: a
  // cache from before the rendering suffix records no regime to check.
  if (
    b.length > 2 &&
    a.length > 2 &&
    normalises(b.slice(2).join(':')) !== normalises(a.slice(2).join(':'))
  ) {
    return 'undatable';
  }
  // Under `core.fileMode=false` the exec bit stops meaning anything (see
  // above), but the mode still carries the file↔symlink TYPE, which git
  // reports under every fileMode: fold the one, keep the other.
  if (
    b[0] !== a[0] &&
    (fileModeTrusted ||
      !(
        (b[0] === '100644' || b[0] === '100755') &&
        (a[0] === '100644' || a[0] === '100755')
      ))
  ) {
    return 'moved';
  }
  return b[1] === a[1] ? 'stands' : 'moved';
}

/**
 * Whether one blocker's recorded identity still stands — the per-blocker
 * date. Undatable never stands (the documented pass-lean); a regime change
 * says WHY, out loud, rather than reading the blocker as "moved".
 */
function blockerStillStands(
  path: string,
  before: string | undefined,
  after: string | undefined,
  fileModeTrusted: boolean,
): boolean {
  const verdict = dateVerdict(before, after, fileModeTrusted);
  if (
    verdict === 'undatable' &&
    before !== undefined &&
    before !== UNHASHABLE &&
    ...
    // A regime change — the only undatable shape over two hashable sides.
    // `display()`: the path is ledger-written — model-controlled input.
    writeStderrLine(
      `Blocker date: ${display(path)} cannot be dated — the attributes ` +
        `that drive content normalisation changed since the cached round, ` +
        `and the two identities were produced under different clean ` +
        `filters. It does not stand; the stop's rendered list still ` +
        `names it.`,
    );
  }
...
/**
 * Whether the suffix's `text` state enables content normalisation — `set`
 * and `auto` clean on add/hash; `unset` and `unspecified` do not. The
 * comparison is a regime BIT, not a string equality: a `binary` mark moves
 * the state from `unspecified` to `unset` while the clean behaviour stands
 * still, and reading that move as a regime change would unseat every
 * blocker under a rendering-only attribute edit.
 */
function normalises(suffix: string): boolean {
  const entry = suffix.split(',').find((kv) => kv.startsWith('text='));
  if (entry === undefined) return false;
  const v = entry.slice('text='.length);
  return v !== 'unset' && v !== 'unspecified';
}

/**
 * Files in the captured population now that were not there when the cache
 * was written: tracked — or, unless `trackedOnly`, untracked-but-not-ignored
 * as well — minus the cached HEAD's tree and every path the cache hashed.
 * `.gitignore`d names are not subjects and never count, and neither is the
 * review's own plumbing: the capture excludes it from the hashed population,
 * so the reconstruction of "what was there at cache time" must too — in a
 * repo that does not gitignore `.qwen/` the review's artifacts (the cache
 * file Step 8 wrote among them) are untracked-and-not-ignored, and counting
 * them as "added" disarmed every blocker in every round.
 *
 * `trackedOnly` is what a SCOPED cache (`--file`, `--no-untracked`) takes:
 * the cached round never enumerated the untracked half, so dating that half
 * against it would read files already present at cache time as "added" and
 * permanently disarm every blocker — while the tracked population
 * reconstructs from the cached HEAD tree either way, so a fix committed to
 * a brand-new file is still attributed.
 *
...
function runCaptureLocal(args: CaptureLocalArgs): void {
  ...
  if (nothingToReview) {
    // Date against the PUBLISHED ledger — the plan's `cachePath`, the one
    // file Step 8 writes and SKILL's stop branches read. Re-resolving the
    // caller's `--cache` here let a non-canonical value (an empty string, a
    // typo, a directory holding no cache for this target) read zero
    // blockers over a standing Critical while the round's own rendering
    // still named it from the published file.
    const resolvedCache = cachePathFor(target, sourcePath);
