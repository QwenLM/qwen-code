...
import { isPathProvablyAbsent } from './lib/local-anchor.js';
...
export interface FixSnapshot {
  ...
  hiddenPaths: string[];
  /**
   * The ignored NESTED REPOSITORIES the same moment's rules hid, by the
   * name a tree entry gives them (`ls-files --others` prints `sub/` and
   * does not descend). Kept apart from `hiddenPaths` because the two
   * answer different additions: a rule removed in the window admits the
   * repository to the second capture as a GITLINK, and only a gitlink
   * addition at this name is that admission — a blob or a link recorded
   * at the same name is a replacement the fix made, and rides the hunks.
   */
  hiddenRepos: string[];
  /**
   * The ignored SYMLINKS the same moment's rules hid, kept apart from
   * `hiddenPaths` for the same reason `hiddenRepos` is: a rule removed in
   * the window admits the link as a mode-120000 addition, and only a link
   * added at this name is that admission — a blob recorded where a hidden
   * link stood, or a link where a hidden file stood, is a replacement the
   * fix made and rides the hunks.
   */
  hiddenLinks: string[];
  /**
   * The family-named paths this capture RE-INCLUDED as user content — the
   * tracked half and the staged half together — keyed on the raw path bytes
   * (latin1-encoded) like `hiddenPaths`. The staged half is the ambiguous
   * one: nothing structural tells a user's staged note under a family name
   * from the flow's own artifact that a `git add -A` staged into the same
   * index, so `--since` re-includes only what the snapshot already
   * re-included and discloses what it held back. A family path that
   * entered the user's index BETWEEN the moments is the flow's own
   * bookkeeping until something proves otherwise, and re-including it put
   * a rewrite no outcome owns into the hunks.
   */
  reincluded: string[];
  /**
   * The commit HEAD named at snapshot time, or null under an unborn HEAD.
   * `--since` seeds its capture from the snapshot's own TREE, never from
   * HEAD, so a commit made between the moments moves nothing in the
   * comparison; it is disclosed instead, because a change that landed by
   * commit alone (an ignored path force-added, a tracked path untracked)
   * leaves no hunk and is one this command cannot see. The head also
   * names the paths the second capture re-admits: one HEAD tracked that
   * was absent at the snapshot and is on disk again (see
   * `restoreAbsentTrackedPaths`).
   */
  head: string | null;
  /**
   * The filesystem identity (`dev:ino`, or the lexical `p:` fallback) of
   * every probed path, keyed like `digests`. One physical repository
   * reached under two names — a link beside its target, a workspace link
   * into a submodule — is one baseline entry however it was named, and
   * `--since` uses these to recognise a name that arrived or left between
   * the moments as the same repository, not as one that appeared or
   * vanished. Absent from a record this command did not write; then no
   * name is collapsed and the comparison over-warns, as elsewhere.
   */
...
/**
 * The family paths the flow's own PLAN makes bookkeeping: the plan file and
 * everything under its prompt-record directory. The family re-inclusion
 * (user content the index holds under a family name) must not extend to
 * them: a `git add -A` the user ran in a checkout that does not ignore
 * `.qwen` stages them into the user's index, and re-including their two
 * moments puts the flow's own bookkeeping in the hunks as an edit no
 * outcome owns — the exact harm the exclusion exists to prevent, arriving
 * through the index instead of the tree.
 *
 * Derived from the plan path the orchestrator already carries, never from a
 * hand-listed name set: every prompt record the flow writes lives in that
 * one directory (`agent-prompt`'s briefs and launch prompts, the run and
 * resume ledgers, the budget-stop marker), and their keys are NOT
 * name-shaped — a chunk agent's is a bare `chunk-8`, a territory agent's
 * embeds the file it owns — so a list of suffixes and a digest-shaped
 * regex classified most of what the directory holds as user content. A
 * directory the flow owns wholesale needs no enumeration.
 */
function planBookkeeping(root: string, plan: string | undefined): string[] {
  if (plan === undefined || plan === '') return [];
  const rels: string[] = [];
  for (const candidate of [plan, promptRecordDir(plan)]) {
    // Canonical, like every other side-path ruling here: the plan path is
    // prose-chosen and may be spelled through a symlinked prefix.
...
/**
 * True when a family-matched path (raw bytes, `/`-joined) is the plan's own
 * record — the plan file, or anything under its prompt-record directory.
 */
function isFlowBookkeeping(
  path: Buffer,
  bookkeeping: readonly string[],
): boolean {
  const rel = path.toString('latin1');
  return bookkeeping.some((p) => rel === p || rel.startsWith(`${p}/`));
}
 ...
 * bookkeeping (or an attacker's content) as fix edits. Ruled at every level
 * the exclusion reaches: the `.qwen`/`.qwen/tmp` prefixes, the
 * family-matching entries under them, and the command's own side paths — a
 * link planted at the deterministic `--out` name itself would redirect the
 * write, or in `--since` mode the baseline read, through it. Refuse, the way
...
function assertNoRedirectedExcludes(
  ...
  // …and the family directories themselves, the level the exclusion is
  // actually keyed on: the prefixes above stop two levels short of the
  // glob, so a link planted at a family-matching NAME — committed by the
  // reviewed PR, created by the project's own install step between the two
  // moments, or left by a global filter child of the shape the side-path
  // check above already meets — passed both and redirected every side file
  // the flow writes under it into a physical path no pathspec matches.
  // Containment is no defence at this level: a link back INSIDE the
  // checkout redirects the write just as far outside the exclusion, so what
  // is ruled on is the entry, never where its target sits. Read as bytes,
  // so a name the UTF-8 decode cannot carry is still lstat'd by the bytes
  // it has. Last, because a link at this run's own `--out` is more
  // precisely the refusal above.
  const tmpDir = join(root, '.qwen', 'tmp');
  const tmpRel = Buffer.from('.qwen/tmp');
  let familyEntries: Buffer[] = [];
  try {
    familyEntries = readdirSync(tmpDir, 'buffer');
  } catch {
    // No `.qwen/tmp` to plant in — the prefix loop above ruled on it.
  }
  for (const name of familyEntries) {
    const rel = joinRel(tmpRel, name);
    if (!inNameFamily(rel)) continue;
    const abs = joinBytes(Buffer.from(tmpDir), name);
    let link = false;
    try {
    ...
    if (!link) continue;
    // …and only one that resolves to a DIRECTORY: the redirect that harms
    // is one the flow writes UNDER, so every side file below the family
    // name lands at a physical path no pathspec excludes. A family-named
    // link that is a FILE entry is user content the capture records like
    // any other — a tracked dangling symlink's deletion is an edit, not a
    // redirect — and a link above THIS run's own side path is refused by
    // the ancestor walk above whatever its target resolves to, an
    // unresolvable one included.
    let directory = false;
    try {
      directory = statSync(abs).isDirectory();
    } catch {
      // Nothing can be written under a target that does not resolve.
    }
    if (!directory) continue;
    throw new Error(
      `fix-delta: excluded directory ${escapeNoteToken(decodePath(rel))} ` +
        'is a symlink; refusing to write side files outside the exclusion.',
    );
  }
}
...
/**
 * The env pair that blanks the tree's repo-local content filters for one
 * spawn — or a refusal when the screen cannot certify there are none.
 *
 * The ROOT's config is the whole surface this screens, because no spawn
 * of this command runs inside a nested repository before that repository
 * has been screened by its own probe (`probeNestedRepoState`), and the
 * capture runs inside none at all: `git add -A` decides whether a gitlink
 * is modified by running a `status` INSIDE every submodule checkout the
 * index holds — a child that reads and EXECUTES the checkout's own
 * `filter.<name>.clean|process` under `.git/modules/<name>/config`, which
 * a screen of the root never enumerates and no `-c` on the parent reaches
 * (measured: `diff.ignoreSubmodules`, `submodule.<name>.ignore` and
 * `submodule.recurse` all left the inner status running) — so the seed's
 * gitlinks are excluded from `add -A` and refreshed by hand off `rev-parse
 * HEAD`, which reads no config a filter or hook rides on (see
 * `refreshSeedGitlinks`), and both discovery statuses run with
 * `--ignore-submodules=all`, which spawns nothing (measured: `none` ran
 * the planted filter, `all` did not).
 *
 * `screenForTree` answers in two halves and both are load-bearing here.
 * `unread` is every config file the walk could NOT read to the bottom:
 * another user's `~user/`, a non-regular target, a parse failure, an include
 * nesting past git's limit, or a fan-out past the screen's own
 * `MAX_INCLUDE_FILES` — and git has NO fan-out cap, so a filter planted in
 * the 65th include is one git reads and executes while the screen reports
 * nothing. A filter it cannot see it cannot blank, so the capture would run
 * it as the reviewing user: the exact execution channel the blanking exists
 * to close. The same argument the sibling measurement in `worktree.ts`
 * already refuses on.
 *
 * `dangling` is deliberately NOT consulted: git skips an include whose target
 * does not exist, so it delivers nothing to the checkout and executes
 * nothing, and refusing on it darkened the whole efficacy phase on every
 * standard CI checkout, where `actions/checkout`'s persisted credentials
 * leave `includeIf` directives whose per-job target is already gone.
 */
function blankedFilterEnv(root: string, what: string): Record<string, string> {
  const screen = screenForTree(root);
  if (screen === null) {
    throw new Error(
      `fix-delta: ${what} cannot run — the filter screen could not read the ` +
        `config of ${escapeNoteToken(root)} at all, so a ` +
        '`filter.*.clean|process` the reviewed tree planted may be invisible ' +
        'to it and would execute as the reviewing user. A config that cannot ' +
        'be read is not a config with no filters in it.',
    );
  }
  if (screen.unread.length > 0) {
    throw new Error(
      `fix-delta: ${what} cannot run — the filter screen could not read ` +
        `${screen.unread.length} config file(s) to the bottom (` +
        `${screen.unread.map((entry) => escapeNoteToken(entry)).join(', ')}), ` +
        `so a ` +
        '`filter.*.clean|process` behind one of them is invisible to the ' +
        'blanking and would execute as the reviewing user.',
    );
  }
  return filterBlankEnv(screen.filters) as Record<string, string>;
}
/**
 * Record the working tree under `root` as a tree object and return its sha,
 * with the family paths the capture re-included. Runs through a throwaway
 * index so the user's index is untouched.
 *
 * `bookkeeping` is the plan-derived set `isFlowBookkeeping` rules against,
 * and `reincludable` is the staged half's fail-closed gate: null at the
 * snapshot moment (nothing to gate against yet), the snapshot's own recorded
 * set at `--since`.
 *
 * `seed` is what the throwaway index starts from. The snapshot moment seeds
 * from HEAD (the everyday meaning of "tracked"); the `--since` moment seeds
 * from the SNAPSHOT'S OWN TREE, so the two captures differ only by what is
 * on disk. Seeded from HEAD at both moments, the comparison also carried
 * two things that are not edits — every path the seed's tracked set gained
 * or lost between the moments (a commit in the window), and every path an
 * ignore rule written or deleted in the window admitted to or hid from
 * `add -A` — and a classifier had to guess, per record, whether the tree
 * comparison or the fix had made it. With the first tree as the second
 * seed, an entry the first capture recorded is an entry the second one
 * holds, whatever the rules say about it now (ignore rules never touch an
 * index entry), so a deletion in the hunks is a path that is GONE from the
 * disk, and nothing else. What the seed cannot re-admit is a path HEAD
 * tracked that was absent at the snapshot and is on disk again — that half
 * is `restoreAbsentTrackedPaths`; the addition-side classifier
 * (`ghostAdditions`) still rules on a rule REMOVED in the window, which
 * admits pre-existing content the first capture never saw.
 */
export function snapshotWorkingTree(
  root: string,
  sidePaths: readonly string[] = [],
  bookkeeping: readonly string[] = [],
  reincludable: ReadonlySet<string> | null = null,
  seed: { tree: string; head: string | null | undefined } | null = null,
): { tree: string; reincluded: string[]; seedHeadMissing: boolean } {
  ...
  // globally). A screen that could not be read to the bottom REFUSES
  // instead of blanking nothing: it cannot certify the config holds no
  // filter, and an unblanked one executes. Screened before the scratch dir
  ...
  const seedRef =
    seed !== null
      ...
      : gitOpt('-C', root, 'rev-parse', '--verify', '--quiet', 'HEAD') !== null
        ? 'HEAD'
        : null;
  const blankFilters = blankedFilterEnv(root, 'the capture');
  const scratch = mkdtempSync(join(scratchParent(root), 'qwen-fix-delta-'));
  const env = { GIT_INDEX_FILE: join(scratch, 'index'), ...blankFilters };
  try {
    // The `--since` moment starts from the snapshot's own tree (see the
    // docblock); the snapshot moment from HEAD.
    // An unborn HEAD (a repo with no commit yet) has no tree to seed from —
    // start empty, and `add -A` records everything as added.
    if (seedRef !== null) {
      gitWithEnv(env, ['-C', root, ...capturePins(root), 'read-tree', seedRef]);
    } else {
      gitWithEnv(env, [
        '-C',
        root,
    ...
    // that predicts what `add -A` accepts.
    // The seed's gitlinks are kept out of `add -A` and refreshed by hand
    // (see `refreshSeedGitlinks`): the capture never runs a status inside
    // a nested repository.
    const seedGitlinks = gitlinkPathsIn(
      gitWithEnvRaw(env, [
        '-C',
        root,
        ...probePins(root),
    ...
    const add = gitWithEnvReport(
      ...
      capturePathspecBytes(
        root,
        capturableSidePaths(root, sidePaths),
        seedGitlinks,
      ),
    );
    assertCompleteCapture(add);
    refreshSeedGitlinks(env, root, seedGitlinks);
    ...
    const tracked = trackedFamilyPaths(env, root, sidePaths, bookkeeping);
    ...
    const stagedListing = stagedFamilyPaths(
      root,
      sidePaths,
      [...tracked.paths, ...tracked.gitlinks],
      bookkeeping,
    );
    // A gitlink under a family name is a nested repository, never handed
    // to `add -u`/`add -f` as an explicit pathspec — that spawn runs a
    // status inside the checkout and executes its config (measured, for
    // both `-u` and `-A -f`), the channel this capture keeps closed. One
    // the SEED holds is refreshed off its HEAD by `refreshSeedGitlinks`;
    // one only the user's index holds is in neither tree, and its interior
    // is the blind-spot probe's to answer for. Named on stderr either way,
    // once each: the staged listing excludes what the tracked one holds.
    const gitlinksHeld = [...tracked.gitlinks, ...stagedListing.gitlinks];
    if (gitlinksHeld.length > 0) {
      writeStderrLine(
        `fix-delta: left ${gitlinksHeld.length} nested repositor${gitlinksHeld.length === 1 ? 'y' : 'ies'} ` +
          'under a review name family out of the re-inclusion: ' +
          gitlinksHeld
            .map((path) => escapeNoteToken(decodePath(path)))
            .join(', ') +
          ' — a gitlink the seed holds is refreshed off its HEAD by the seed ' +
          'pass, one only the index holds is in neither tree, and the ' +
          "interior of either is the blind-spot probe's to answer for; the " +
          'capture never runs inside it.',
      );
    }
    const unaddable: Buffer[] = [];
    const stagedNow = stagedListing.paths.filter((path) => {
      ...
      // all-clear. Only a path that LSTATS is handed to `add -f`: git
      // runs the same lstat as this user, so any error here is a pathspec
      // it dies on (`did not match any files` — a family directory
      // replaced by a plain file answers ENOTDIR), and one unexplained
      // refusal fails the whole capture. What is not handed over is
    ...
    if (unaddable.length > 0) {
      writeStderrLine(
        `fix-delta: left ${unaddable.length} staged path(s) under a review ` +
          'name family out of this capture: ' +
          unaddable
            .map((path) => escapeNoteToken(decodePath(path)))
            .join(', ') +
          ' — staged, but nothing the capture can stat sits at the name ' +
          'now (its directory replaced, or unreadable), so `add` would ' +
          'refuse the pathspec; an edit there rides no hunk.',
      );
    }
    // This half is the AMBIGUOUS one, and it fails closed: a family path
    // the snapshot did not re-include is not re-included here either. The
    // committed half needs no gate — the flow writes its side files
    // untracked and never commits, so a path HEAD's tree holds is user
    // content by that contract — but the staged half is exactly where the
    // flow's own artifacts arrive when a checkout does not ignore `.qwen`
    // and something ran `git add -A` between the two moments (a fixer
    // staging its own work, a hook, the user). Nothing structural tells
    // such a path from a user's staged note, so the snapshot's own answer
    // is what rules: what it re-included was user content before the fix
    // ran, and what it did not is bookkeeping until proven otherwise.
    const held = (path: Buffer): boolean =>
      reincludable === null || reincludable.has(path.toString('latin1'));
    const stagedOnly = stagedNow.filter(held);
    const withheld = stagedNow.filter((path) => !held(path));
    ...
    const reincluded = [...tracked.paths, ...stagedOnly];
      ...
      writeStderrLine(
        `fix-delta: re-included ${reincluded.length} tracked/staged ` +
          ...
          reincluded
            .map((path) => escapeNoteToken(decodePath(path)))
            .join(', ') +
          " — the plan's own record is excluded wholesale, so these are " +
          'treated as user content.',
      );
    }
    // …and every path the fail-closed gate held back, for the same reason:
    // a dropped path is an edit the hunks do not carry, and silence about
    // it is the certification this module never makes.
    if (withheld.length > 0) {
      writeStderrLine(
        `fix-delta: left ${withheld.length} staged path(s) under a review ` +
          'name family out of this capture: ' +
          withheld.map((path) => escapeNoteToken(decodePath(path))).join(', ') +
          ' — tracked or staged now, but not re-included at snapshot time, ' +
          'so a family path that entered the index between the two moments ' +
          "is the flow's own bookkeeping until something proves it user " +
          'content, and its rewrite rides no hunk.',
      );
    }
    ...
    if (tracked.paths.length > 0) {
      const update = gitWithEnvReport(
        ...
        Buffer.concat(
          tracked.paths.flatMap((path) => [path, Buffer.from([0])]),
        ),
      );
      assertCompleteCapture(update);
    }
    const seedHeadMissing =
      seed !== null && typeof seed.head === 'string'
        ? !restoreAbsentTrackedPaths(env, root, seed.tree, seed.head, sidePaths)
        : false;
    const tree = gitWithEnv(env, [
      '-C',
      root,
      ...capturePins(root),
      'write-tree',
    ]);
    // latin1, like every other path-keyed record field: a byte<->char
    // bijection, so the set `--since` gates on compares the RAW bytes and
    // a name the display decode cannot carry still round-trips.
    return {
      tree,
      reincluded: reincluded.map((p) => p.toString('latin1')),
      seedHeadMissing,
    };
  } finally {
...
/**
 * Where the capture's scratch directory goes: the git dir, which no capture
 * can record. Read as BYTES — `rev-parse --absolute-git-dir` prints the
 * name as it is on disk, and a `--separate-git-dir` holding a byte that is
 * not valid UTF-8 decoded to U+FFFD and named a directory that does not
 * exist, so every capture died on ENOENT. `GIT_INDEX_FILE` is an
 * environment string, which cannot carry such a byte at all; when the git
 * dir's name cannot be spelled as a string the scratch goes to the system
 * temp directory instead — provided that is OUTSIDE the working tree (a
 * hermetic sandbox pointing it inside made the scratch itself a captured
 * path) — and otherwise the capture refuses, disclosed, rather than run
 * against a location it cannot name.
 */
function scratchParent(root: string): string {
  const raw = gitRaw('-C', root, 'rev-parse', '--absolute-git-dir');
  // One trailing newline is git's line end; an interior one is part of the
  // name (a `--separate-git-dir` can hold one), never a place to cut.
  const gitDirBytes =
    raw.length > 0 && raw[raw.length - 1] === 0x0a
      ? raw.subarray(0, raw.length - 1)
      : raw;
  const gitDir = gitDirBytes.toString('utf8');
  if (Buffer.from(gitDir, 'utf8').equals(gitDirBytes)) return gitDir;
  let temp: string;
  try {
  ...
  if (!isInside(temp, rootReal)) return temp;
  throw new Error(
    "fix-delta: the capture cannot run — the git dir's name is not valid " +
      'UTF-8, so the throwaway index cannot be placed in it, and the ' +
      'system temp directory sits inside the working tree, where the ' +
      'capture would record it.',
  );
}

/** The mode-160000 paths of an `ls-files -s -z` listing, as raw bytes. */
function gitlinkPathsIn(listing: Buffer): Buffer[] {
  const out: Buffer[] = [];
  for (const entry of splitNul(listing)) {
    if (!entry.subarray(0, 7).equals(GITLINK_MODE_PREFIX)) continue;
    const tab = entry.indexOf(0x09);
    if (tab !== -1) out.push(entry.subarray(tab + 1));
  }
  return out;
}

/**
 * What `git add -A` would do to the seed's gitlinks, done by hand — and
 * without the one thing `add -A` does on the way, which is to run a
 * `status` inside every such checkout to decide whether it is modified. That
 * child reads the checkout's OWN config and executes what it finds there:
 * a `filter.<name>.clean|process` under `.git/modules/<name>/config`, a
 * `core.fsmonitor` command, the hooks its `core.hooksPath` names — none of
 * which the root's screen enumerates or a `-c` on the parent reaches. The
 * gitlink's value is the checkout's HEAD, and `rev-parse HEAD` reads
 * nothing a command rides on; so each entry is refreshed from that and
 * written back with `update-index --index-info`, bytes through stdin.
 *
 * Entry for entry, git's own ruling: a checkout that is GONE is a removed
 * entry (a deletion in the hunks, the fix's); one replaced by a file or a
 * link is the replacement (removed here, then recorded by `add -f`); one
 * that is empty, holds no `.git`, cannot be read, has no commit yet, or
 * carries a name the spawn cannot carry stands as the seed recorded it —
 * `add -A` leaves such a gitlink alone too (`ce_compare_gitlink` reads an
 * unresolvable HEAD as unchanged), as does one whose `.git` git does not
 * accept as its own, and the blind-spot probe is what answers for its
 * state. A corrupt checkout that `add -A` would have DIED
 * on ("'git status --porcelain=2' failed in submodule") stands the same
 * way, and is disclosed by the probe as unresolved rather than costing the
 * whole capture.
 */
function refreshSeedGitlinks(
  env: Record<string, string>,
  root: string,
  gitlinks: readonly Buffer[],
): void {
  if (gitlinks.length === 0) return;
  const rootBuf = Buffer.from(root);
  const records: Buffer[] = [];
  const replaced: Buffer[] = [];
  // The null OID is as long as the repository's object names: 64 hex in
  // a `--object-format=sha256` repository, where `update-index` rejects a
  // 40-hex record as malformed and the whole capture would have died on
  // the first removed checkout. `--show-object-format` arrived with
  // `--object-format` (git 2.29); a git that cannot answer is SHA-1.
  const zeros = '0'.repeat(
    gitOpt('-C', root, 'rev-parse', '--show-object-format') === 'sha256'
      ? 64
      : 40,
  );
  const removal = (rel: Buffer): Buffer =>
    Buffer.concat([Buffer.from(`0 ${zeros}\t`), rel, Buffer.from([0])]);
  for (const rel of gitlinks) {
    ...
    try {
      ...
      if (errnoOf(err) === 'ENOENT') records.push(removal(rel));
      continue;
    }
    if (!st.isDirectory()) {
      records.push(removal(rel));
      replaced.push(rel);
      continue;
    }
    if (gitlinkCheckoutState(abs) !== 'repo') continue;
    const path = abs.toString('utf8');
    if (!Buffer.from(path, 'utf8').equals(abs)) continue;
    // …and only a checkout whose `.git` git accepts as ITS OWN: over a
    // hollow `.git` (an empty directory, a gitfile pointing back at the
    // superproject) `rev-parse HEAD` walks up and answers with the
    // superproject's commit, and recording that would put a `Subproject
    // commit` hunk the fix never made in front of the auditor. The entry
    // stands; the probe discloses the checkout as unresolved.
    if (!answersForOwnGitDir(path)) continue;
    const head = gitOpt(
      '-C',
      path,
      ...probePins(path),
    ...
    // A checkout of the OTHER hash algorithm names its HEAD at the other
    // length, and `update-index` rejects the record as malformed (as
    // `add -A` refuses "a submodule of a different hash algorithm"): the
    // entry stands, and the probe answers for the checkout.
    if (head.length !== zeros.length) continue;
    records.push(
      Buffer.concat([Buffer.from(`160000 ${head}\t`), rel, Buffer.from([0])]),
    );
  }
  if (records.length > 0) {
    const update = gitWithEnvReport(
      env,
      ['-C', root, ...capturePins(root), 'update-index', '-z', '--index-info'],
      Buffer.concat(records),
    );
    if (!update.completed || update.status !== 0) {
      throw new Error(
        `fix-delta: git update-index could not refresh the gitlinks (exit ${update.status}): ` +
          escapeNoteToken(update.stderr.trim()),
      );
    }
  }
  if (replaced.length > 0) {
    const added = gitWithEnvReport(
      env,
      [
        '-C',
        root,
      ...
      Buffer.concat(replaced.flatMap((path) => [path, Buffer.from([0])])),
    );
    assertCompleteCapture(added);
  }
}

/**
 * The half of "tracked" the snapshot tree cannot carry into the second
 * capture: a path HEAD tracked at snapshot time that was ABSENT from the
 * disk then (deleted by the change under review and not staged, a build
 * artifact cleaned away) is in no first tree, so a seed taken from that
 * tree never asks `add -A` about it — and a fix that writes it back (a
 * regenerated `dist/index.js` under an ignore rule, a file the fixer
 * restored) would leave no hunk. Asked of the snapshot's HEAD, never of
 * HEAD now: the paths are the ones the first tree LACKS beside that commit,
 * and every one that is on disk now — a file or a link, never a directory,
 * whose non-ignored content `add -A` already recorded — is recorded with
 * `-f`, because the rule that hid it from the capture is exactly why it
 * needs recording. Returns false when the recorded commit is no longer in
 * the repository: the caller discloses that the half was not re-captured.
 */
function restoreAbsentTrackedPaths(
  env: Record<string, string>,
  root: string,
  seedTree: string,
  head: string,
  ...
  if (gitOpt('-C', root, 'cat-file', '-e', `${head}^{commit}`) === null) {
    return false;
  }
  const raw = gitRaw(
    '-C',
  ...
  const side = new Set(sidePaths.map((p) => p.split(sep).join('/')));
  const rootBuf = Buffer.from(root);
  // The gitlinks the scratch index holds NOW — the seed's, refreshed, and
  // every embedded repository `add -A` just recorded: a path beneath one
  // is a pathspec git refuses (`is in submodule`), and its content is the
  // blind-spot probe's to answer for, not this pass's.
  const gitlinks = gitlinkPathsIn(
    gitWithEnvRaw(env, [
      '-C',
      root,
      ...probePins(root),
  ...
  const underGitlink = (rel: Buffer): boolean =>
    gitlinks.some(
      (g) =>
        rel.length > g.length &&
        rel[g.length] === 0x2f /* / */ &&
        rel.subarray(0, g.length).equals(g),
    );
  const restore: Buffer[] = [];
  for (const rel of splitNul(raw)) {
    if (rel.length === 0 || side.has(rel.toString('utf8'))) continue;
    // …and not behind a link or inside a repository: `lstat` follows every
    // LEADING component, so a path whose directory the fix replaced with a
    // symlink reads as present while `add` refuses the pathspec outright
    // (`beyond a symbolic link`, `is in submodule`), and one unexplained
    // refusal fails the whole capture. The link or the gitlink is what
    // `add -A` recorded; what sits behind it is outside the tree this
    // command measures.
    if (behindSymlink(rootBuf, rel) || underGitlink(rel)) continue;
    // Only an ENTRY that is a file or a link now is re-admitted. A leading
    // component that became a plain file answers ENOTDIR; a directory is
    // content `add -A` already walked; anything the probe cannot stat is
    // not proven present. Each is a pathspec `add -f` would die on or
    // record nothing for, so each is left out rather than tried.
    try {
      const st = lstatSync(joinBytes(rootBuf, rel));
  ...
  if (restore.length === 0) return true;
  const added = gitWithEnvReport(
    env,
    [
      '-C',
    ...
    Buffer.concat(restore.flatMap((path) => [path, Buffer.from([0])])),
  );
  assertCompleteCapture(added);
  return true;
}
...
/**
 * Whether any LEADING component of `rel` is a symlink (or cannot be
 * stat'd): the path then sits behind a link, and git's pathspec refuses it.
 */
function behindSymlink(rootBuf: Buffer, rel: Buffer): boolean {
  let at = -1;
  for (;;) {
    const next = rel.indexOf(0x2f /* / */, at + 1);
    if (next === -1) return false;
    try {
      if (
        lstatSync(joinBytes(rootBuf, rel.subarray(0, next))).isSymbolicLink()
      ) {
        return true;
      }
    } catch {
 ...
 * before rather than recording the side file.
 */
...
function trackedFamilyPaths(
  ...
  return splitFamilyListing(raw, bookkeeping);
}

/** A family listing split by recorded mode: the paths to re-include, and the gitlinks never to. */
interface FamilyListing {
  paths: Buffer[];
  gitlinks: Buffer[];
}

/**
 * Split an `ls-files -s -z` family listing on git's RECORDED mode — never
 * the filesystem's, which follows a link (fix-delta keeps the entry, not
 * the target): a mode-160000 entry is a nested repository and goes to
 * `gitlinks`; every other path that is not the flow's own bookkeeping goes
 * to `paths`.
 */
function splitFamilyListing(
  raw: Buffer,
  bookkeeping: readonly string[],
): FamilyListing {
  const paths: Buffer[] = [];
  const gitlinks: Buffer[] = [];
  for (const entry of splitNul(raw)) {
    const tab = entry.indexOf(0x09);
    if (tab === -1) continue;
    const path = entry.subarray(tab + 1);
    if (path.length === 0) continue;
    if (entry.subarray(0, 7).equals(GITLINK_MODE_PREFIX)) {
      gitlinks.push(path);
      continue;
    }
    if (isFlowBookkeeping(path, bookkeeping)) continue;
    paths.push(path);
  }
...
function stagedFamilyPaths(
  ...
  const listing = splitFamilyListing(raw, bookkeeping);
  const staged = (path: Buffer): boolean => !seen.has(path.toString('latin1'));
  return {
    paths: listing.paths.filter(staged),
    gitlinks: listing.gitlinks.filter(staged),
  };
}
...
export function assertCompleteCapture(add: {
  ...
  if (unexplained.length > 0 || (add.status !== 0 && lines.length === 0)) {
    // Escaped at the render boundary: a note is git's text around a path
    // or a value the tree chose, and the fallback is the raw stream.
    throw new Error(
      `fix-delta: git add could not capture the whole tree (exit ${add.status}): ` +
        escapeNoteToken(unexplained.join(' | ') || add.stderr.trim()),
    );
  }
}
...
export function capturePathspecBytes(
  root: string,
  extraLiteral: readonly string[] = [],
  extraLiteralBytes: readonly Buffer[] = [],
): Buffer {
  ...
  // The seed's gitlinks, as the bytes `ls-files -z` printed them (see
  // `refreshSeedGitlinks`).
  for (const rel of extraLiteralBytes) {
    specs.push(Buffer.concat([Buffer.from(':(exclude,literal)'), rel]));
  }
...
function captureSteeringSurfaces(root: string): string[] {
  ...
  // Named rather than neutralised. The CAPTURE pins `core.fileMode=true`, but
  // the nested-repository probes deliberately keep git's own view of
  // "modified" — pinning them would make every mode-carrying file of a
  // `fileMode=false` checkout read modified at both moments, permanent dirt
  // behind which a real edit hides — so a repo-local setting still steers
  // what a nested probe reports, and is disclosed.
  // Only the value that STEERS is named. `git init` writes `core.filemode`
  // into an ordinary repository's own config, and `true` is git's default on
  // a filesystem that honours the exec bit — naming that would disclose a
  // surface every ordinary repository has. `false` is the one that suppresses
  // mode detection, and so the one worth a note.
  const fileMode = gitOpt('-C', root, 'config', '--get', 'core.fileMode');
  if (
    fileMode !== null &&
    ['false', 'no', 'off', '0'].includes(fileMode.trim().toLowerCase())
  ) {
    surfaces.push(`core.fileMode (${escapeNoteToken(fileMode)})`);
  }
  return surfaces;
}
...
/**
 * The paths the ignore rules hide at THIS moment AND that are on disk, per
 * file (no directory collapse: a collapsed entry prefixes content its rule
 * may not cover). Recorded into the snapshot so the addition-side
 * classifier can ask the question of the FIRST moment's rule set — by
 * `--since` time the rules that answered (a `.git/info/exclude` line, a
 * `core.excludesFile` entry) may no longer exist anywhere to be re-asked.
 * Listed individually; on a checkout with a package store ignored this is
 * the one list the snapshot pays for in proportion to the store.
 */
function hiddenIgnoredPaths(root: string): {
  files: string[];
  links: string[];
  repos: string[];
} {
  ...
  // recognise. Union the two enumerations.
  //
  // Membership is what `ghostAdditions` reads as "already on disk at
  // snapshot time", so the `--cached` half is filtered to what IS on disk:
  // `ls-files --cached --ignored` lists an index entry whose worktree copy
  // is gone (a committed artifact under an ignore rule added later, cleaned
  // or deleted by the change under review), and recorded here that entry
  // ruled the file's REAL return during the fix window a capture invention
  // — a landed addition dropped from the hunks and from the count. A
  // `--cached --ignored` entry that is also in HEAD and on disk sits in
  // both trees and produces no `A` record for the classifier to rule on;
  // one whose copy is missing is absent from the first tree and produces
  // exactly that record, honestly. `--others` is on disk by construction.
  // The test is `lstatSync` with only ENOENT meaning absent, the module's
  // rule: the ENTRY, never its target.
  const files: string[] = [];
  const links: string[] = [];
  const repos: string[] = [];
  const rootBuf = Buffer.from(root);
  // The `--cached` half is typed off the user's index: `ls-files -s` with
  // `--cached --ignored` prints nothing, and a staged-but-uncommitted
  // gitlink under an ignore rule is printed by the plain listing WITHOUT
  // the trailing slash `--others` gives a repository — so its mode is the
  // only thing that tells it from a file.
  const cachedModes = new Map<string, string>();
  for (const entry of splitNul(
    gitRaw('-C', root, ...probePins(root), 'ls-files', '-s', '-z'),
  )) {
    const tab = entry.indexOf(0x09);
    if (tab === -1) continue;
    cachedModes.set(
      entry.subarray(tab + 1).toString('latin1'),
      entry.subarray(0, 6).toString('latin1'),
    );
  }
  for (const selector of ['--others', '--cached'] as const) {
    ...
    for (const listed of splitNul(raw)) {
      if (listed.length === 0) continue;
      // `--others` prints an ignored nested repository as `sub/` — it does
      // not descend into one — while the capture records it as the
      // gitlink `sub`, the spelling the classifier looks an `A` record up
      // by. Such an entry goes on its own list, keyed as the tree spells
      // it, and matches only a gitlink addition (see `FixSnapshot.
      // hiddenRepos`); the byte/latin1 key space stays.
      if (listed[listed.length - 1] === 0x2f /* / */) {
        const name = listed.subarray(0, listed.length - 1);
        if (name.length > 0) repos.push(name.toString('latin1'));
        continue;
      }
      const key = listed.toString('latin1');
      if (selector === '--cached') {
        // Membership means "on disk at snapshot time", for a gitlink as
        // for a file: a staged checkout whose directory is gone would
        // otherwise attest a repository re-created in the window as
        // pre-existing, and drop that addition from the hunks.
        try {
          lstatSync(joinBytes(rootBuf, listed));
        } catch (err) {
          if (errnoOf(err) === 'ENOENT') continue;
          // Any other error: not proven absent, so it stays in the set.
        }
        const mode = cachedModes.get(key);
        (mode === '160000' ? repos : mode === '120000' ? links : files).push(
          key,
        );
        continue;
      }
      // `--others` is on disk by construction; the ENTRY's kind decides
      // which list, never its target's.
      let link = false;
      try {
        link = lstatSync(joinBytes(rootBuf, listed)).isSymbolicLink();
      } catch {
        // Unreadable now: a file until proven otherwise (the addition-side
        // lookup then over-matches a blob, never invents).
      }
      (link ? links : files).push(key);
    }
  }
  return { files, links, repos };
}
 ...
 * comparison reads the admission as an addition. Ruled against the
 * snapshot's record because the first moment's rule set is gone by the
 * time the question is asked. There is no deletion-side mirror: the second
 * capture is seeded from the first tree, so a rule that APPEARED in the
 * window hides nothing the first capture recorded (see
 * `snapshotWorkingTree`), and a `D` record is a path gone from the disk.
...
function ghostAdditions(
  ...
  // `--raw`, not `--name-status`: the record's KIND decides which list a
  // name is looked up in. `:<src mode> <dst mode> <src sha> <dst sha> A`
  // NUL `<path>` NUL, and only the destination mode is read.
  const raw = gitRaw(
    '-C',
    root,
    ...probePins(root),
  ...
  for (let i = 0; i + 1 < fields.length; i += 2) {
    const meta = fields[i].toString('latin1').split(' ');
    if (meta.length < 5 || meta[4][0] !== 'A') continue;
    const rel = fields[i + 1];
    const key = rel.toString('latin1');
    // The record's kind must match what the rules hid at that name: a
    // gitlink where a REPOSITORY was hidden, a link where a LINK was, a
    // blob where a FILE was — each is the rule's removal admitting what
    // was already there. Any other pairing (a blob or link where the
    // repository stood, a link where a file stood, a repository where a
    // file stood) is a replacement the fix made, and rides the hunks.
    const hidden =
      meta[1] === '160000'
        ? hiddenRepos
        : meta[1] === '120000'
...
function probePins(path: string): string[] {
  return [
    ...
    // `core.ignoreCase=false` wherever the filesystem is case-SENSITIVE: git
    // sets the key true only where it detects a case-insensitive filesystem,
    // so a `true` on a sensitive one is a plant (or a linked worktree on a
    // sensitive volume inheriting the main checkout's key), and under it
    // `add -A` folds a case-colliding sibling (`A.txt` tracked, `a.txt`
    // edited) into the tracked name — both captures identical, a bare
    // all-clear over the edit — and a nested repository's `status` folds
    // the same sibling into silence. Unlike `autocrlf`, the pin changes
    // nothing for a legitimate repository on such a filesystem (git never
    // writes `true` there), so the probes carry it too. On a
    // case-insensitive filesystem the key is git's own and stays
    // (DESIGN.md: a case-only rename is invisible to git itself there).
    // Probed once per path, read-only, on the repository's own volume.
    ...(filesystemIsCaseSensitive(path) ? ['-c', 'core.ignoreCase=false'] : []),
  ];
}
...
function capturePins(path: string): string[] {
  ...
  //
  // `core.fileMode=true` for the same reason as `autocrlf`: a
  // `core.fileMode=false` in the audited repository's OWN `.git/config`
  // makes git read no mode difference at all, so a landed exec-bit edit
  // produces two identical trees and a bare all-clear over it. It is git's
  // own default wherever the filesystem honours the bit, so it is a no-op
  // absent that setting, and it is symmetric across both moments — on a
  // filesystem that genuinely cannot store the bit every entry reads 0644 at
  // both moments and the phantom cancels in the tree-to-tree delta, while a
  // real content or mode edit still shows.
  return [
    ...probePins(path),
    '-c',
    'core.autocrlf=false',
...
const caseSensitivity = new Map<string, boolean>();

/**
 * Whether the filesystem the WORKING TREE sits on tells `Aa` from `aA`,
 * read-only and once per root: the tree's own `.git` entry (a directory,
 * or a linked worktree's gitfile — every root has one) is lstat'd under
 * its case-swapped spelling `.GIT`. ENOENT is a sensitive filesystem; an
 * entry that is the SAME inode is an insensitive one — provided the inode
 * could not have been given a second name: a directory cannot be hard-
 * linked, and a gitfile (a linked worktree's or a submodule's `.git`) with
 * a link count of one has none, while one with more has been `ln`'d, and
 * a `.GIT` hard-linked to it on a sensitive filesystem would otherwise
 * read as the same entry; an entry that is a DIFFERENT inode is two names
 * differing only by case coexisting, which only a sensitive filesystem
 * can hold — so a planted `.GIT` cannot pass the probe off as
 * insensitive either way. Nothing is written, and the probe is on
 * the volume `add -A` walks, not the git dir's (a linked worktree's git
 * dir is under the main checkout, a `--separate-git-dir` wherever the user
 * put it). A probe that cannot run answers SENSITIVE: the pin then only
 * over-reports a case-only rename as a deletion and an addition on an
 * insensitive filesystem, where silence over a fold is the direction this
 * module never fails in.
 */
function filesystemIsCaseSensitive(root: string): boolean {
  const known = caseSensitivity.get(root);
  if (known !== undefined) return known;
  let sensitive = true;
  try {
    const own = lstatSync(join(root, '.git'));
    try {
      const swapped = lstatSync(join(root, '.GIT'));
      // An inode the filesystem cannot vouch for (0 on both sides) is no
      // proof the two spellings are one entry, and neither is one a
      // gitfile shares with a hard link: sensitive, and pinned.
      const oneName = own.isDirectory() || (own.isFile() && own.nlink === 1);
      sensitive = !(
        oneName &&
        hasVerifiableInode(own.ino) &&
        swapped.dev === own.dev &&
        swapped.ino === own.ino
      );
    } catch {
  ...
  caseSensitivity.set(root, sensitive);
  return sensitive;
}
...
function probeNestedRepoState(
  ...
  /**
   * The interior index's own mode-160000 entries — this repository's
   * submodules — routed by the caller as the top level's gitlinks are.
   */
  interiorGitlinks?: Buffer[];
  ...
  if (!answersForOwnGitDir(path)) return { state: 'failed' };
  ...
  const inner = gitWithEnvReport({}, [
    ...
    '--branch',
    '--show-stash',
    // `=all` for the same reason the root's discovery status runs with it:
    // under `none` this status spawns inside every level-2 checkout before
    // that checkout's own config has been screened. The level-2 state is
    // not lost — the interior gitlinks are swept below and each is probed
    // (and screened) on its own, with a digest of its own.
  ...
  // level down. The same holds for the index's mode-160000 gitlinks: a
  // submodule whose checkout lost its git dir emits no entry at all (git
  // has nothing to compare it against), so the sweep collects both modes
  // and the caller routes the gitlinks as the root's index sweep does.
  // The probe's own pins ride; a sweep that cannot run makes the interior
  // unanswerable — failed, never clean.
  ...
  // The bits are asked about BEFORE dirt is ruled on: a hand-set
  // skip-worktree or assume-unchanged bit hides an edit from this status
  // whether or not some other file is dirty, and a repository already
  // dirty at the snapshot would otherwise file the hidden edit as
  // "pre-existing dirt, nothing newly dirtied it" — the same status text
  // at both moments, the same digest.
  if (indexBitsHideEntries(path, probeState, display ?? path)) {
    return { state: 'failed' };
  }
 ...
 *
 * The status this confirms ran with `--ignore-submodules=all`, whose
...
function indexBitsHideEntries(
  ...
  let tags: Buffer;
  try {
    tags = gitRaw('-C', path, ...probePins(path), 'ls-files', '-v', '-z');
  } catch {
    return true;
  }
  let listing: Buffer;
  ...
  // The index's gitlinks, by name: git never sets a sparse bit on one
  // (`sparse-checkout set` leaves an out-of-cone submodule present and
  // `H`), so a submodule inside a sparse checkout would read as an
  // unflagged out-of-rules path — an ungoverned control — on every run.
  // They are neither controls nor candidates below; the interior sweep
  // further down probes them.
  const gitlinkNames = new Set<string>();
  for (const entry of splitNul(listing)) {
    if (!entry.subarray(0, 7).equals(GITLINK_MODE_PREFIX)) continue;
    const tab = entry.indexOf(0x09);
    if (tab !== -1)
      gitlinkNames.add(entry.subarray(tab + 1).toString('latin1'));
  }
  // `<tag> <path>` records: lowercase tags are the assume-unchanged family
  // (the combined bits render lowercase `s`), `S` is skip-worktree alone.
  const flagged: Buffer[] = [];
  const unflagged: Buffer[] = [];
  for (const record of splitNul(tags)) {
    if (record.length < 2) continue;
    const name = record.subarray(2);
    if (gitlinkNames.has(name.toString('latin1'))) continue;
    const tag = record[0];
    if (tag === 0x53 /* S */ || (tag >= 0x61 && tag <= 0x7a)) {
      flagged.push(name);
    } else {
      unflagged.push(name);
    }
  ...
  if (
    flagged.length > 0 &&
    !sparseCheckoutOwnsBits(path, flagged, unflagged, state, display)
  ) {
    return true;
  }
  for (const entry of splitNul(listing)) {
    ...
    try {
      if (
        indexBitsHideEntries(
          subPath.toString('utf8'),
          state,
          `${display}/${decodePath(name)}`,
        )
      ) {
        return true;
      }
...
/**
 * Whether EVERY flagged index entry is one a sparse checkout set the bit
 * on: an out-of-cone tracked path is skip-worktree-tagged by design and
 * absent from the worktree, so counting it wedged every sparse nested
 * repository as unresolved on every run — and a disclosure that always
 * fires is one nobody reads. The recipe is `local-anchor.ts`'s, paid for
 * there one review round at a time: the flag is read `--worktree
 * --type=bool` (a global `core.sparseCheckout=true` inherited through HOME
 * must not switch the exemption on for a repository that is not sparse,
 * and `--get` alone echoes `yes`/`on`/`1`); membership comes from
 * `sparse-checkout check-rules`, not from the bit or the tag's case (git
 * versions differ on whether a manual bit survives inside a cone); and
 * only an ENOENT-proven ABSENT, out-of-rules path is exempt — an in-rules
 * path absent with a bit set is a deletion the bit hides, a present
 * out-of-rules path can hold an edit, and a name the spawn cannot carry
 * (not valid UTF-8) or that the rules cannot be asked about (an older git)
 * keeps the flag. Fail closed, never certified.
 */
function sparseCheckoutOwnsBits(
  path: string,
  flagged: readonly Buffer[],
  unflagged: readonly Buffer[],
  state: ProbeState,
  display: string,
): boolean {
  const sparse =
  ...
  // A name the spawn cannot carry (not valid UTF-8) cannot be asked about
  // at all: the exemption is not granted, and the reader is told which
  // name kept it — the generic unresolved note names none of this.
  const unaskable = (rel: Buffer): boolean =>
    !Buffer.from(rel.toString('utf8'), 'utf8').equals(rel);
  const cannotAsk = [...flagged, ...unflagged].find(unaskable);
  if (cannotAsk !== undefined) {
    const note =
      `fix-delta: the sparse-checkout rules of ${escapeNoteToken(display)} ` +
      `cannot be asked about ${escapeNoteToken(decodePath(cannotAsk))} — the ` +
      'name is not valid UTF-8 and cannot reach `sparse-checkout ' +
      'check-rules` — so its skip-worktree bits are read as hand-set.';
  ...
  for (const rel of flagged) {
    const name = rel.toString('utf8');
    if (!isPathProvablyAbsent(path, name, ancestorProbes)) return false;
    absent.push(name);
  }
  // `check-rules` answers about the RULES, not about who set a bit. The
  // rules are proof only where they govern the worktree — where every
  // out-of-rules tracked path is flagged, as git leaves a sparse checkout.
  // A repository whose rules exist but were never applied (a
  // `sparse-checkout` file written by hand, `core.sparseCheckout` flipped
  // on) has out-of-rules paths PRESENT and unflagged, and there a bit over
  // a deleted file is a hand's, so the whole set keeps its flag. The
  // control is the unflagged set: one out-of-rules member and the rules
  // govern nothing.
  const controls = unflagged.map((rel) => rel.toString('utf8'));
  let inRules: Set<string>;
  try {
    const out = gitWithInputRaw(
      Buffer.from(
        [...absent, ...controls].map((name) => `${name}\0`).join(''),
        'utf8',
      ),
      ['-C', path, ...probePins(path), 'sparse-checkout', 'check-rules', '-z'],
    );
    inRules = new Set(out.split('\0').filter((name) => name !== ''));
  } catch {
    return false;
  }
  const ungoverned = controls.filter((name) => !inRules.has(name));
  if (ungoverned.length > 0) {
    // Say WHY, and what would end it: the generic unresolved note names a
    // directory that cannot be opened, a config that steers — not this. A
    // `sparse-checkout set` leaves an out-of-cone file with local edits
    // present and unflagged ("left despite sparse patterns"), and until a
    // `reapply` the checkout reads as ungoverned on every run.
    // Named the way every other note names a path — root-relative, never
    // the absolute spelling — and once, however many routes reach the
    // repository.
    const note =
      `fix-delta: the sparse-checkout rules of ${escapeNoteToken(display)} do ` +
      `not govern its worktree — ${ungoverned
        .slice(0, 5)
        .map((name) => escapeNoteToken(name))
        .join(
          ', ',
        )}${ungoverned.length > 5 ? `, and ${ungoverned.length - 5} more` : ''} ` +
      `${ungoverned.length === 1 ? 'is' : 'are'} tracked, present and outside the rules ` +
      '(what `sparse-checkout set` leaves behind when a file has local ' +
      'edits), so its skip-worktree bits are read as hand-set until ' +
      '`git sparse-checkout reapply` runs there.';
    if (!state.notes.includes(note)) state.notes.push(note);
    return false;
  }
  return absent.every((name) => !inRules.has(name));
}

/**
 * Whether git, run inside `path`, answers for the repository `path`'s own
 * `.git` names — and not for an enclosing one. A `.git` git itself rejects
 * — an empty directory, an empty file, a gitfile pointing nowhere or back
 * at the superproject — passes the `.git`-exists gate every discovery
 * route uses, and git's discovery then walks UP and answers with the
 * SUPERPROJECT under this path: its status under `--work-tree`, or its
 * HEAD as the checkout's own. The git dir git resolves from inside must be
 * the one this path's own `.git` names.
 */
function answersForOwnGitDir(path: string): boolean {
  const expectedGitDir = ownGitDirOf(path);
  if (expectedGitDir === null) return false;
  ...
  // …and not an ENCLOSING checkout's git dir either: a gitfile reading
  // `gitdir: ../.git` names the superproject's own git dir, so the two
  // sides agree while every answer from inside — HEAD, status — is the
  // superproject's, not this checkout's.
  return !enclosingGitDirs(path).some((dir) => {
    const theirs = ownGitDirOf(dir);
    return theirs !== null && samePath(discovered, theirs);
  });
}

/**
 * The git dir a directory's own `.git` entry names — the directory
 * itself, or a gitfile's `gitdir:` target resolved against the directory
 * — or null when it holds none git could read.
 */
function ownGitDirOf(dir: string): string | null {
  ...
  try {
    ...
    if (statSync(dotGit).isFile()) {
      ...
      return pointer === null ? null : resolve(dir, pointer[1]);
    }
    return dotGit;
  } catch {
    return null;
  ...
  for (let qi = 0; qi < queue.length; qi++) {
    ...
    for (const e of entries) {
      ...
      if (e.isSymbolicLink()) {
        ...
        // directory: a link out of the tree led the walk to ENUMERATE a
        // directory the capture never records, spending the run's budget
        // on content the model does not cover; and a link reaching a FILE
        // outside the tree is the same scope fact in a shape that cannot
        // hold a repository. Out of root is disclosed, never walked; the
        // link itself is what `add -A` records, and the disclosure is what
        // says an edit through it would leave no record here. What the
        // scope ruling does NOT do is stand down on the repository behind
        // it — see the probe below.
        ...
        if (reach === 'outside') {
          ...
          // Recorded AND probed, the way every other route that reaches
          // this shape rules it (`probeLinkedRepo`, and the funnel
          // comment in `probeNestedRepo`): a repository's uncommitted
          // dirt is a blind spot whether the link that reaches it sits
          // inside an ignored directory or at a top-level path, and
          // routing only the latter let a fix land unreported in the
          // everyday `.qwen` shape. The outOfRoot bucket is what keeps a
          // commit inside it — nothing a fix could have made — from
          // reading as this tree's transition, so probing it does not
          // restore the false transition the walk's enumeration did.
          if (isDir && existsSync(joinBytes(childAbs, DOT_GIT))) {
            probeNestedRepo(childAbs, childRel, ctx, state);
          }
          continue;
        }
        if (isDir) {
...
interface ProbeState {
  ...
  probed: Set<string>;
  /**
   * Key -> filesystem identity, for every name that reached a repository —
   * the probed one and every later name that resolved to the same place.
   * Persisted so `--since` can tell a route that changed from a repository
   * that appeared or vanished (see `FixSnapshot.identities`).
   */
  identities: Record<string, string>;
  /**
   * Free-form disclosure lines a probe wants printed beside its verdict —
   * the WHY of an unresolved answer where the generic note would send the
   * reader hunting (a sparse checkout whose rules do not govern its
   * worktree, and what would make them). Printed by the caller at both
   * moments, never persisted.
   */
  notes: string[];
...
function probeNestedRepo(
  ...
  // baseline entry, under the first name that reached it — but EVERY name
  // that reached it is recorded against its identity, because which name
  // reaches a repository first depends on the route, and the route can
  // change between the moments (a link into a submodule that an install
  // step created in the fix window is reached by the status route before
  // the index sweep reaches the submodule). Keyed by name alone, one
  // unchanged repository read as vanished under its old name and appeared
  // under its new one; `--since` collapses the two by identity.
  const identity = identityOf(abs);
  if (identity !== null) {
  ...
  const result = probeNestedRepoState(abs, state, decodePath(rel));
 ...
 * disclosed under its raw spelling, and the interior index's own gitlinks
 * take the root's index-sweep ruling.
 */
function probeNestedInterior(
  ...
  // This repository's own gitlinks take the ROOT's index-sweep ruling
  // verbatim (see `probeBlindSpotState`): a checkout that does not exist
  // or is empty holds no content an edit could hide in and is skipped —
  // probing it answered 'failed' and over-warned on every run, and a
  // non-recursive `submodule update --init` leaves every level-2 checkout
  // empty — one that holds content without its git dir is a dead gitlink
  // nobody can answer for (the inner probe would walk UP into this
  // repository and report it), and one with a git dir is a working tree
  // nested in this one whose status the level-1 digest does not carry.
  for (const name of probe.interiorGitlinks ?? []) {
    const gitlinkRel = joinRel(rel, name);
    const gitlinkAbs = joinBytes(rootBuf, gitlinkRel);
    const excluded = probeExcluded(gitlinkAbs, gitlinkRel, ctx);
    if (excluded === 'git-dir') continue;
    if (excluded === 'review-worktree') {
      walkAndProbe(gitlinkAbs, gitlinkRel, ctx, state);
      continue;
    }
    const checkout = gitlinkCheckoutState(gitlinkAbs);
    if (checkout === 'absent' || checkout === 'empty') continue;
    if (checkout !== 'repo') {
      state.unresolved.add(gitlinkRel.toString('latin1'));
      continue;
    }
    probeNestedRepo(gitlinkAbs, gitlinkRel, ctx, state);
  }
...
/**
 * What a gitlink's checkout directory holds, read directly rather than
 * through `existsSync` (which folds "cannot see" into "nothing there"):
 * absent (ENOENT), empty (created and never populated — the shape a clone
 * or a non-recursive `submodule update --init` leaves), a repository (a
 * `.git` entry, file or directory), dead content (entries but no `.git`),
 * or unreadable (any other error). Only the first two hold nothing.
 */
function gitlinkCheckoutState(
  abs: Buffer,
): 'absent' | 'empty' | 'repo' | 'dead' | 'unreadable' {
  let entries: Buffer[];
  try {
    entries = readdirSync(abs, 'buffer');
  } catch (err) {
    return errnoOf(err) === 'ENOENT' ? 'absent' : 'unreadable';
  }
  if (entries.length === 0) return 'empty';
  return entries.some((entry) => entry.equals(DOT_GIT)) ? 'repo' : 'dead';
}
...
function probeBlindSpotState(
  ...
  const statusRun = gitRawReport(
    [
      ...
      'status',
      '--porcelain=v2',
      // `=all`, not `=none`: under `none` git runs a status INSIDE every
      // submodule checkout to fill the `S.M.`/`S..U` flags, and that child
      // executes the checkout's own repo-local filters, fsmonitor command
      // and hooks — surfaces the root's screen never enumerates. Nothing
      // is lost: every gitlink the index holds is probed by the index
      // sweep below regardless of what status says, and the probe screens
      // the checkout's own config BEFORE it spawns inside.
      '--ignore-submodules=all',
      '--ignored=matching',
      '-z',
      '--',
    ...
    // identically, and refuses on identically (see `blankedFilterEnv`).
    blankedFilterEnv(root, 'the discovery status'),
  );
  if (!statusRun.completed || statusRun.status !== 0) {
    // Escaped at this render boundary like every other: git echoes a
    // planted config VALUE back in its `fatal:` line, and a value holding
    // a newline would otherwise put a standalone `fix-delta:` line of the
    // planter's choosing — the all-clear wording included — on stderr.
    throw new Error(
      `fix-delta: the discovery status could not run (exit ${statusRun.status})` +
        (statusRun.stderr.length > 0
          ? `: ${escapeNoteToken(statusRun.stderr.toString('utf8').trim())}`
          : ''),
    );
  }
  ...
  const state: ProbeState = {
    ...
    identities: Object.create(null) as Record<string, string>,
    notes: [],
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const inner = probeNestedRepoState(innerAbs, state, decodePath(relBytes));
    ...
    if (innerIdentity !== null) {
      state.probed.add(innerIdentity);
      state.identities[key] = innerIdentity;
    }
  ...
  // a second probe.
  const indexEntries = splitNul(
    gitRaw('-C', root, ...probePins(root), 'ls-files', '-s', '-z'),
  );
  // Gitlinks first, then symlinks: a repository reached under two names is
  // one probe and one baseline entry under the FIRST name that reached it
  // (`probeNestedRepo`), and its registered path is the name to answer
  // under — a tracked link pointing at a submodule sorts wherever its
  // spelling does, and the status no longer reaches the submodule ahead
  // of this sweep (`--ignore-submodules=all`).
  const isGitlink = (entry: Buffer): boolean =>
    entry.subarray(0, 7).equals(GITLINK_MODE_PREFIX);
  const isSymlink = (entry: Buffer): boolean =>
    entry.subarray(0, 7).equals(SYMLINK_MODE_PREFIX);
  for (const entry of [
    ...indexEntries.filter(isGitlink),
    ...indexEntries.filter(isSymlink),
  ]) {
    const gitlink = isGitlink(entry);
    ...
    // A gitlink whose checkout does not exist, or exists EMPTY (a fresh
    // clone that never ran `submodule update --init` — git creates the
    // directory and leaves it bare, and `update --init` without
    // `--recursive` leaves every level-2 checkout in that state), holds
    // no content an edit could hide in; probing it answered 'failed' and
    // over-warned on every run. A checkout that holds content without its
    // git dir is a dead gitlink: `add -A` still records only the gitlink
    // and no status entry names it, so an edit inside would leave no
    // record in this model — disclose it as unresolved rather than skip
    // it. The inner probe alone cannot answer the state: with the git dir
    // gone, `git -C <checkout> status` walks UP into this repository and
    // reports the superproject, not the checkout. One the probe cannot
    // READ is unanswerable, never "nothing there".
    const checkout = gitlinkCheckoutState(abs);
    if (checkout === 'absent' || checkout === 'empty') continue;
    if (checkout !== 'repo') {
...
/**
 * Whether two root-relative keys (latin1-encoded bytes) resolve to the
 * same physical path now — the test for a second route to one repository
 * (a link beside its target, a link into a submodule). Either name failing
 * to resolve is not a route.
 */
function sameTargetNow(rootBuf: Buffer, a: string, b: string): boolean {
  try {
    const ra = realpathSync.native(
      joinBytes(rootBuf, Buffer.from(a, 'latin1')),
      {
        encoding: 'buffer',
      },
    );
    const rb = realpathSync.native(
      joinBytes(rootBuf, Buffer.from(b, 'latin1')),
      {
        encoding: 'buffer',
      },
    );
...
/** The tail every commit-side note ends on, phrased for the hunks file. */
function commitSideTail(hunksEmpty: boolean): string {
  return hunksEmpty
    ? 'The hunks file stays empty; the audit cannot see the change.'
    : 'The hunks file does not show it; the audit cannot see that change.';
}

function headMovedNote(
  before: string | null,
  now: string | null,
  hunksEmpty: boolean,
): string {
  const name = (sha: string | null): string =>
    sha === null ? 'an unborn HEAD' : sha.slice(0, 12);
  return (
    `fix-delta: HEAD moved between the two moments (${name(before)} → ` +
    `${name(now)}) — a change that landed by commit alone (an ignored ` +
    'path force-added, a tracked path untracked, a rebase) is one this ' +
    'command cannot see: the capture records the working tree against ' +
    "the snapshot's own tree, never the commits, so what those commits " +
    `carry leaves no hunk. ${commitSideTail(hunksEmpty)}`
  );
}

function headUnrecordedNote(hunksEmpty: boolean): string {
  return (
    'fix-delta: the snapshot records no HEAD, so a commit between the two ' +
    'moments cannot be ruled out, and a path HEAD tracked that was absent ' +
    'at the snapshot was not re-captured — either is a change this ' +
    `command cannot see. ${commitSideTail(hunksEmpty)}`
  );
}

function headGoneNote(head: string, hunksEmpty: boolean): string {
  return (
    `fix-delta: the commit the snapshot recorded as HEAD (${head.slice(0, 12)}) ` +
    'is no longer in this repository, so a path it tracked that was absent ' +
    'at the snapshot and is on disk again was not re-captured — an edit ' +
    `there is one this command cannot see. ${commitSideTail(hunksEmpty)}`
  );
}
...
function assertRootHoldsCwd(root: string): void {
  ...
  if (rel === '..' || rel.startsWith(`..${sep}`) || isAbsolute(rel)) {
    throw new Error(
      `fix-delta: git reports ${escapeNoteToken(root)} as the working tree, ` +
        `but this command runs in ${escapeNoteToken(cwd)}, which is not ` +
        `inside it — a repo-local ` +
  ...
  if (honest === null || !samePath(honest, top)) {
    throw new Error(
      `fix-delta: git reports ${escapeNoteToken(root)} as the working tree, ` +
        `but the repository this command runs in is ` +
        `${honest === null ? `none — no \`.git\` above ${escapeNoteToken(cwd)}` : escapeNoteToken(honest)}` +
  ...
  if (hereGitDir !== null) {
    for (const enclosing of enclosingGitDirs(top)) {
      ...
      if (outerGitDir !== null && samePath(outerGitDir, hereGitDir)) {
        throw new Error(
          `fix-delta: ${escapeNoteToken(root)} answers for the repository ` +
            `at ${escapeNoteToken(hereGitDir)}, but so does ` +
            `${escapeNoteToken(enclosing)}, which CONTAINS it — a ` +
            `\`.git\` entry ` +
  ...
  if (rootIsGitfile) {
    ...
    if (
      ...
      throw new Error(
        `fix-delta: ${escapeNoteToken(root)} narrows every reading to ` +
          `itself through a ` +
  ...
  if (!samePath(fromCwd, fromRoot)) {
    throw new Error(
      `fix-delta: the repository at ${escapeNoteToken(fromCwd)} names ` +
        `${escapeNoteToken(root)} as its working tree, but that tree ` +
        `belongs to ${escapeNoteToken(fromRoot)} — a repo-local ` +
...
function registersNestedCheckout(enclosing: string, nested: string): boolean {
  ...
  if (
    ...
    // The gitlink must be AT the nested path, never merely under it: a
    // pathspec matches a directory's whole subtree, so `ls-files -- <rel>`
    // lists a gitlink registered DEEPER (`tools/vendor/lib`, from a cwd of
    // `tools`) as well, and reading the first line's mode accepted that as
    // the reciprocal registration — the planted-gitfile gate stood down
    // over the very subtree narrowing it exists to refuse. Decided per
    // entry, on the recorded path's own bytes: `-z` for the name git does
    // not C-quote, and `:(literal)` so a checkout directory whose name
    // holds `*`, `?` or `[` is not read as a glob — which missed its
    // gitlink entirely and refused a genuine submodule.
    const relPosix = rel.split(sep).join('/');
    const relBytes = Buffer.from(relPosix, 'utf8');
    ...
    try {
      ...
      // Unanswerable is not "registered": the worktree registry below still
      // gets its turn, and a gate that cannot confirm refuses.
    }
    if (listing !== null) {
      for (const entry of splitNul(listing)) {
        if (!entry.subarray(0, 7).equals(GITLINK_MODE_PREFIX)) continue;
        const tab = entry.indexOf(0x09);
        if (tab !== -1 && entry.subarray(tab + 1).equals(relBytes)) {
          return true;
        }
      }
    }
...
export interface FixDeltaArgs {
  ...
  fingerprint?: string;
  /**
   * The plan report this review's Step 1 wrote — the one path that makes
   * the flow's own bookkeeping derivable instead of enumerable: the plan
   * file and everything under `promptRecordDir(plan)` are the flow's, and
   * the family re-inclusion treats them as such at both moments. Omitted by
   * a caller that has no plan (a hand run, or a target whose plan a cleanup
   * already swept), and then nothing is excluded by derivation.
   */
  plan?: string;
  /**
...
/**
 * The fingerprint of a side file this command wrote — the snapshot record
 * and the hunks alike: the SHA-256 of its exact bytes. Both live in a
 * directory the reviewed tree can write to, so neither is self-certifying;
 * the hex rides stderr out to the orchestrator and comes back as an
 * argument to whatever reads the file next.
 */
...
export function runFixDelta(args: FixDeltaArgs): void {
  ...
  // The plan-derived bookkeeping set both captures rule family paths
  // against (see `planBookkeeping`).
  const bookkeeping = planBookkeeping(root, args.plan);
  ...
  if (args.snapshot) {
    ...
    const { tree, reincluded } = snapshotWorkingTree(
      root,
      sidePaths,
      bookkeeping,
    );
    const probe = probeBlindSpotState(root, sidePaths, reviewWorktrees);
    for (const note of probe.notes) writeStderrLine(note);
    const snapshot: FixSnapshot = {
      ...
      ...(() => {
        const hidden = hiddenIgnoredPaths(root);
        return {
          hiddenPaths: hidden.files,
          hiddenLinks: hidden.links,
          hiddenRepos: hidden.repos,
        };
      })(),
      // …and what THIS moment's capture re-included as user content: the
      // ambiguous (staged) half at `--since` is gated on it, because a
      // family path that entered the index between the moments is the
      // flow's own bookkeeping until proven otherwise.
      reincluded,
      // The commit this moment sits on, for the `--since` seed and the
      // moved-HEAD disclosure (see `FixSnapshot.head`); null when unborn.
      head: gitOpt('-C', root, 'rev-parse', '--verify', '--quiet', 'HEAD'),
      // …and the physical identity behind every digest, so a name that
      // arrives or leaves between the moments can be told from a
      // repository that did (see `FixSnapshot.identities`).
  ...
  // The re-inclusion gate the record carries: null when it carries none (a
  // record the pre-fix command wrote), which leaves the second capture
  // re-including what it finds — the pre-fix behaviour, and the permissive
  // side of a missing field, like `hiddenPaths` above.
  let reincludable: ReadonlySet<string> | null = null;
  // The HEAD the record names: a sha, null for an unborn HEAD, or undefined
  // when the record carries no such field at all. The last is NOT the
  // permissive shape the fields above take: a record with no HEAD cannot
  // say whether a commit landed in the window, and the second capture
  // cannot re-admit what HEAD tracked, so both are disclosed and the bare
  // all-clear is withheld — over-warning, the direction this module fails.
  let recordedHead: string | null | undefined;
  try {
    ...
    recordedHead =
      raw.head === null
        ? null
        : typeof raw.head === 'string' && /^[0-9a-f]{40,64}$/.test(raw.head)
          ? raw.head
          : undefined;
    if (Array.isArray(raw.reincluded)) {
      reincludable = new Set(
        raw.reincluded.filter((p): p is string => typeof p === 'string'),
      );
    }
    snapshot = {
      root: raw.root,
      ...
      hiddenPaths: Array.isArray(raw.hiddenPaths)
        ...
        : [],
      hiddenLinks: Array.isArray(raw.hiddenLinks)
        ? raw.hiddenLinks.filter((p): p is string => typeof p === 'string')
        : [],
      hiddenRepos: Array.isArray(raw.hiddenRepos)
        ? raw.hiddenRepos.filter((p): p is string => typeof p === 'string')
        : [],
      // Read for the re-inclusion gate above; nothing else compares it.
      reincluded: Array.isArray(raw.reincluded)
        ? raw.reincluded.filter((p): p is string => typeof p === 'string')
        : [],
      // Carried for the type; `recordedHead` above keeps the three-way
      // answer (sha / unborn / not recorded) this field cannot.
      head: recordedHead ?? null,
      // Null-prototyped like `digests`, for the same `__proto__` reason. A
      // record without the field collapses no name: over-warning.
      identities: Object.assign(
        Object.create(null) as Record<string, string>,
        typeof raw.identities === 'object' && raw.identities !== null
          ? Object.fromEntries(
              Object.entries(raw.identities as Record<string, unknown>)
  ...
  const steering = captureSteeringSurfaces(root);
  if (steering.length > 0) writeStderrLine(captureSteeringNote(steering));
  // Seeded from the snapshot's own tree, never from HEAD now (see
  // `snapshotWorkingTree`): the two captures then differ only by what is on
  // disk, and a commit in the window moves nothing here — it is disclosed
  // below instead.
  const { tree: now, seedHeadMissing } = snapshotWorkingTree(
    root,
    sidePaths,
    bookkeeping,
    reincludable,
    { tree: snapshot.tree, head: recordedHead },
  );
  // A commit between the moments is not a working-tree edit: a path
  // force-added and committed under an ignore rule, a tracked path
  // untracked, a rebase — none of it moves a tree seeded from the first
  // tree, so none of it is in the hunks, and the auditor must be told the
  // hunks are not the whole change. Compared against the record, never
  // against a re-derivation: HEAD now is the only side this run can see.
  const headNow = gitOpt(
    '-C',
    root,
    'rev-parse',
    '--verify',
  ...
  const headMoved = recordedHead !== undefined && recordedHead !== headNow;
  // An addition the capture invented is not an edit: dropped from the
  // hunks and from the count, and disclosed on its own line.
  const ghostAdds =
    now === snapshot.tree
      ? []
      : ghostAdditions(
          ...
          new Set(snapshot.hiddenLinks),
          new Set(snapshot.hiddenRepos),
          sidePaths,
        );
  const diffPaths = [
    ...sidePaths,
  ...
  // The hunks are the fix auditor's WHOLE input, and they ride the same
  // tree-writable directory the snapshot record does: a watcher the
  // reviewed code left running can rewrite the file between this write and
  // the separate process that reads it back (`agent-prompt --role
  // fix-audit`), keeping every `diff --git` header byte-identical while
  // substituting the bodies — a forgery no content check can see. Printed
  // on BOTH paths: the empty diff returns below, ahead of the summary
  // line, and an empty hunks file is exactly the one the skill's skip
  // decision keys on.
  writeStderrLine(
    `fix-delta: hunks fingerprint ${snapshotFingerprint(diff)}; pass it back ` +
      'as --hunks-fingerprint on agent-prompt --role fix-audit',
  );
  ...
  // The baseline, read by IDENTITY as well as by name. A repository the
  // snapshot recorded under one name and the probe reaches under another
  // now — a workspace link the fix window's install step created beside a
  // submodule, a link and its target reached in the other order — is one
  // repository: its baseline digest is read under the new name too, and
  // the old name is not "vanished" while the identity behind it answers.
  // A record without identities collapses nothing (over-warning).
  const rootBuf = Buffer.from(root);
  const baseDigests: Record<string, string> = Object.assign(
    Object.create(null) as Record<string, string>,
    snapshot.digests,
  );
  const baseDirty = new Set(snapshot.dirtySubmodules);
  // Identity -> the snapshot key that HOLDS the answer for it (the probed
  // name, which carries the digest); a name recorded against the identity
  // without a digest of its own is a later route to the same place.
  const snapshotKeyOf = new Map<string, string>();
  for (const [key, identity] of Object.entries(snapshot.identities)) {
    if (snapshot.digests[key] !== undefined) snapshotKeyOf.set(identity, key);
  }
  for (const [key, identity] of Object.entries(snapshot.identities)) {
    if (!snapshotKeyOf.has(identity)) snapshotKeyOf.set(identity, key);
  }
  // A baseline recorded through a link OUT of the tree is a baseline for
  // the repository behind that link, not for whatever answers under the
  // name now. When the probe no longer reaches the name as scope — the
  // link removed (an `npm ci` in the fix window), or replaced by an
  // in-tree repository — that entry is withdrawn, so the name routes like
  // any other: gone, it is `vanished`; answered for by an in-tree
  // repository now, it is `appeared`. Left in, a DIRTY external baseline
  // was exempted from every transition list on scope and named nowhere.
  const outOfRootNow = probe.outOfRoot;
  for (const p of snapshot.outOfRoot) {
    if (outOfRootNow.includes(p)) continue;
    delete baseDigests[p];
    baseDirty.delete(p);
  }
  const aliased = new Set<string>();
  for (const [key, identity] of Object.entries(probe.identities)) {
    if (
      snapshot.digests[key] !== undefined ||
      snapshot.unresolved.includes(key)
    ) {
      continue;
    }
    const first = snapshotKeyOf.get(identity);
    if (first === undefined || first === key) continue;
    // A second ROUTE, never a move: both names must resolve to the same
    // place now. A directory renamed keeps its inode, and a filesystem
    // that reuses inode numbers can hand a repository cloned afresh the
    // one a deleted repository just freed — neither is the snapshot's
    // repository under a new name, and both stay disclosed as vanished
    // and appeared.
    if (!sameTargetNow(rootBuf, first, key)) continue;
    // Read off the baseline as it stands AFTER the withdrawal above, never
    // off the snapshot: a baseline withdrawn because its name was a link out
    // of the tree must not come back under a new name (a repository moved
    // INTO the tree in the window keeps its inode, and the retargeted link
    // still resolves to it — that is `appeared`, not the all-clear).
    const inherited = baseDigests[first];
    if (inherited === undefined) continue;
    baseDigests[key] = inherited;
    if (baseDirty.has(first)) baseDirty.add(key);
    aliased.add(first);
  }
  const sameDirtAsBaseline = (p: string): boolean => {
    const before = baseDigests[p];
  ...
  const freshDirt = dirtyNow.filter(
    (p) => !baseDirty.has(p) || !sameDirtAsBaseline(p),
  );
  const preExisting = dirtyNow.filter(
    (p) => baseDirty.has(p) && sameDirtAsBaseline(p) && !isOutOfRoot(p),
  );
  ...
  const cleaned = [...baseDirty].filter(
    (p) =>
      !dirtyNow.includes(p) &&
  ...
  const movedInside = Object.keys(baseDigests).filter(
    (p) =>
      !baseDirty.has(p) &&
      !dirtyNow.includes(p) &&
      !unresolvedNow.includes(p) &&
  ...
  if (ghostAdds.length > 0 && diff.length === 0) {
    writeStderrLine(
      'fix-delta: nothing else differs between the two states — every ' +
        'difference the trees carry is one of the invented additions ' +
        'above, so this run makes no claim about what the fix applied.',
    );
  }
  // The commit-side disclosures, on both paths: a moved HEAD, a record
  // that names none, and a recorded HEAD the repository no longer holds
  // (the re-admission of what it tracked could not run). Each is a change
  // the hunks cannot carry, so each withholds the bare all-clear.
  if (recordedHead === undefined) {
    writeStderrLine(headUnrecordedNote(diff.length === 0));
  } else if (headMoved) {
    writeStderrLine(headMovedNote(recordedHead, headNow, diff.length === 0));
  }
  if (seedHeadMissing && recordedHead) {
    writeStderrLine(headGoneNote(recordedHead, diff.length === 0));
  }
  if (diff.length === 0) {
  ...
  builder: (yargs) =>
    yargs
      ...
      .option('plan', {
        type: 'string',
        describe:
          'The plan report this review wrote (Step 1). The plan file and ' +
          'everything under its prompt-record directory are bookkeeping the ' +
          'flow itself wrote, and are never re-included as user content. ' +
          'Pass the same plan to --snapshot and --since. Omit it and ' +
          'nothing is excluded by derivation.',
      })
