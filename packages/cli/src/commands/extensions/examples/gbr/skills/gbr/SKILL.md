...
name: gbr
description: >
  Pair a phone running Build Remote Agent to this Qwen Code session.
...
This is **not** Qwen Channels, `qwen serve`, or Computer Use. Phone spectates this desktop session only.
...
| MCP | `gbr-mcp` stdio (this extension, or `qwen mcp add`) |

Phone is spectator + veto, not orchestrator. Qwen Code stays in the terminal.
