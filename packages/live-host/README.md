...
初始化框和主卡首次分别按当前可见尺寸放在启动屏幕右下角，保留 20px 边距。
拖动初始化框或主卡标题栏可移动，
位置保存到 Host 用户数据目录中的 `overlay-position.json`，下次启动恢复；显示器移除后
会调整到可见区域。主卡按卡片、摘要和字幕的紧凑区域限位，不再被初始化框的透明空白
挡住。打开 Settings 或预览时会临时调整到完整可见的位置，关闭后恢复记忆位置；临时调整
不会覆盖拖动记录。状态刷新不会重新定位窗口。

Pebble 主卡为 234 × 194 px，常驻麦克风、播报、`Start call`／`End call`、
`Settings` 和 `Quit Host` 五个按钮。拖动主卡标题栏移动窗口；`Command+E` 启动／结束通话。
Host 启动后会等待连接、当前来源权限和自检就绪，再自动开始一次交互；已有通话时不会
重复启动。手动启停／新对话／退出、自动启动失败、重连或 renderer 重载均不会再次触发
自动启动。重新启动 Host 才产生下一次自动启动意图。
结束后主卡留在原位，声音圆盘回到静止状态。圆盘、摄像头预览和设置控件保持挂载，
状态／字幕变化不会重建它们。51 px 圆盘内的波形随真实输入电平变化，思考与播报使用
各自的动效，空闲／麦克风静音时静止，支持系统减少动态效果。状态位于圆盘右侧；
第二行显示来源／获取方式或静音提示，同时保留第一行通话状态或授权入口。
麦克风静音会释放输入设备，取消静音后重新收音。

初始化页只处理连接、Source 和对应权限，不要求 Camera 用户先授权 Screen。
日常设置位于主卡旁边的 306 × 532 px 面板，分为声音、视觉、个性化三组；Memory 默认折叠，
展开后编辑记忆库和模型。面板打开时主卡控制按钮仍可操作。
模式说明随已确认的选项更新。设置支持 Esc、外部点击关闭，编辑草稿保留。
页脚的 `Open configuration`／`打开配置文件` 会使用系统为 JSON 文件关联的
...
设置标题栏可以拖动，与主卡共享位置记忆；打开时先等待原生窗口完成屏内定位再显示，
避免边缘处先露出被裁切的面板。用户说话的小音量视觉响应已增强，保留有界动画和缓慢
回落，不会提高发送给模型的音频音量。

个性化分组的 `Language`／`语言`（其后是外观），支持 `简体中文` 和 `English`。
...
主卡下方常驻 `Subagents`／`子智能体` 摘要（仅在有快照时显示），展示进行中和已完成计数，
任务或未分配授权需要用户输入时显示提醒。辅助功能标签保留完整计数。
...
点击摘要打开 280 px 内容宽度的任务窗口，直到点击关闭或 Esc 才收起。
展开的列表／详情不会因失焦、打开 Settings、拖动主卡或连接中断而消失。
任务面板不会挤压、移动或调整主卡大小，状态更新不翻边、不重排已打开列表的点击目标，也不会把用户正在看的输出滚到底部。
自动展开的位置会避开主卡、预览、字幕和摘要的完整可见区域；原侧面不足以容纳
展开面板时选择有空间的另一侧。
...
个性化分组的 `Appearance`／`外观` 位于语言后，支持跟随系统（默认）、浅色和深色。
该偏好保存在 Host 本地，主卡、设置和子智能体面板同时更新，切换不会重建媒体或中断通话。

默认主题色为雾紫（Iris）。在当前独立 Live 的 `config.json` 顶层添加或修改：

```json
{ "themeColor": "iris" }
```

可选 `iris`（雾紫）、`clay`（陶土）、`sage`（松绿）、`tide`（湖蓝）、
`graphite`（墨灰）、`rose`（玫瑰）、`berry`（莓红），各自包含浅色和深色配色。
`qwen-live init` 为新配置写入 `iris`。保存后重启 Host 或重新连接生效；
缺省、非法值或未提供配置路径的旧连接均使用雾紫。此项由 Host 在连接时只读解析，
不会改写模型、后端或 Memory 配置，也不会把配置文件中的其他内容传给 renderer。
...
球旁的小眼睛按钮可以隐藏／显示预览，独立于通话控制。隐藏只影响本地小窗，不会停止
