...
| `coreTools`              | `string[]`                                                           | -                | Uses the legacy `coreTools` / CLI `--core-tools` allowlist semantics. If specified, only matching core tools are registered for the session. This is the only option that restricts built-in tool registration; `permissions.allow` in settings.json is pure auto-approval and never removes, demotes, or hides a tool (#10075). To keep a tool's schema out of the initial model request, use `tools.eager` in settings.json (requires restart, #9827); to remove it entirely, use `excludeTools` / `permissions.deny`. Example: `['read_file', 'edit', 'run_shell_command']`. |
