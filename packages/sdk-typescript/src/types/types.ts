...
export interface QueryOptions {
   ...
   * model request. Unlisted non-exempt tools are demoted to deferred and
   * stay reachable through `tool_search` + `tool_call` while both bridge
   * tools are registered; when either bridge tool is not registered
   * (`tools.toolSearch.enabled: false`, a `tool_search`/`tool_call` deny
   * rule, or the automatic DeepSeek opt-out) the demoted tools are out of
   * reach for that session and a warning is logged. Tools already deferred
   * by default remain on demand even when listed; `tools.visible` surfaces
   * one at startup. The allowlist does not affect MCP tools, the
   * `--json-schema` `structured_output` contract, plan-mode lifecycle
   * tools, `task_stop`, `tool_search`, `tool_call`, `exec` (the CodeModeOnly
   * control tool), or the `computer_use__*` family (#9827, #10075).
   ...
   *   there are demoted to deferred — still registered and reachable via
   *   `tool_search` + `tool_call` while both bridge tools are registered;
   *   when either bridge tool is not registered (`tools.toolSearch.enabled:
   *   false`, a `tool_search`/`tool_call` deny rule, or the automatic
   *   DeepSeek opt-out) the demoted tools are out of reach for that session
   *   and a warning is logged (#9827)
   *
   * **Pattern matching:**
   * - Tool name: `'write_file'`
