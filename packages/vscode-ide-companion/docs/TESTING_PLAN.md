...
| **会话管理**                | 会话无法创建/切换     | 单元+集成 | `useSessionManagement.test.ts`, _(集成待补)_                 |
| **会话恢复**                | VSCode 重启后状态丢失 | 集成      | _(待补: serialization)_                                      |
  ...
  describe('acceptDiff', () => {
    it('should fire diffAccepted notification', async () => {
      ...
      const rightUri = vi
        ...
        .mock.results.find((r) => r.value.query?.includes('new'))?.value;
...
test('open chat via command palette', async ({ page, webview }) => {
  ...
  await expect(
    webview.getByRole('textbox', { name: 'Message input' }),
  ).toBeVisible();
});
```
