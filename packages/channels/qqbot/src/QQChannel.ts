...
export class QQChannel extends ChannelBase {
  ...
  /** Monotonic counter incremented on each connect/disconnect cycle.
   *  Captured in async closures (token refresh retry) to detect stale
   *  callbacks after reconnection. */
      ...
      if (!resp.ok) {
        ...
        if (msgId) {
          ...
          const activeBody: Record<string, unknown> = {
            ...
            msg_type: 0,
            msg_id: msgId,
            msg_seq: nextSeq + 1, // increment for retry (initial markdown already used nextSeq)
          };
  ...
  protected override onResponseChunk(
    ...
    state.timer = setTimeout(() => {
      ...
      if (!toFlush) return;
      // Clear buffer BEFORE async send so new chunks arriving during the
      // send don't get lost (same pattern as onToolCall already uses).
      s.buffer = '';
  ...
  override onToolCall(_chatId: string, event: ToolCallEvent): void {
    ...
    // again — the idle-flush's send will deliver the accumulated text.
          ...
          const retry = () => {
            ...
            this.tokenRefreshTimer = setTimeout(() => {
              this.fetchToken().catch((e2) => {
                if (
                  this.disposed ||
                  this._reconnectId !== tokenReconnectId
                )
                  return;
                process.stderr.write(
  ...
  private handleC2C(event: QQMessageEvent): void {
    ...
    this.handleInbound({
      ...
      process.stderr.write(
        `[QQ:${this.name}] C2C handler error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}\n`,
      ),
    );
  }
  ...
  private handleGroup(event: QQGroupMessageEvent): void {
    ...
    // Drop messages from other bots to prevent bot-loop amplification.
    ...
    this.handleInbound({
      channelName: this.name,
      // Use event.author.id (global OPENID) for session routing so the
      // same user has a consistent session across groups. member_openid
      // is group-scoped and kept only for display/mention purposes.
      senderId:
        event.author.id ||
      ...
      process.stderr.write(
        `[QQ:${this.name}] Group handler error: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}\n`,
      ),
    );
  }
  ...
  private handleGroupAll(event: QQGroupMessageEvent): void {
    ...
    this.handleInbound({
      ...
      chatId,
      text,
      // Use event.author.id (global OPENID) for session routing.
      senderId:
        event.author.id ||
        event.author.user_openid ||
        event.author.member_openid ||
