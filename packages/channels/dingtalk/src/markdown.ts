 ...
 *
 * DingTalk's markdown payload has size and code-fence chunking quirks:
 * - Max message length ~3800 chars — split into chunks
 * - Code fences must be closed/reopened across chunk boundaries
 *
 * Markdown tables are passed through so DingTalk can render them when supported.
 */
...
/** Full normalization pipeline: chunks only. */
