...
function createHarness(
  options: { compactResults?: boolean; language?: string } = {},
) {
  ...
  const presenter = new DingtalkInteractionPresenter({
    compactResults: options.compactResults,
    statusCards,
    questionCards,
...
describe('DingtalkInteractionPresenter', () => {
  ...
  it('keeps the current card running until the next output has text', async () => {
    ...
    expect(created.cardParamMap.statusLine).toMatch(/^\d+s$/);
    expect(created.cardParamMap.content).toBe(
