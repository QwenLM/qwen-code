...
vi.mock('@qwen-code/channel-base', async () => {
  ...
  return {
    ChannelBase: class {
      ...
      protected backgroundTaskOrigins = new Map();
      ...
      protected async deliverBackgroundResult(
        sessionId: string,
        text: string,
      ): Promise<void> {
        await (
        ...
        ).deliverBackgroundResult.call(this, sessionId, text);
      }
...
it('keeps cards and main output enabled for simplified results', async () => {
  ...
  const close = vi
    .spyOn(state.interactionPresenter, 'closeOutput')
  ...
  await getCompleteHook(channel)('chat-1', 'Main result', 's-1', segment);
...
it('replaces request progress without exposing partial file markers or delivering files', () => {
  const channel = createChannel();
  const state = channel as unknown as {
    interactionPresenter: { replaceOutput: (...args: unknown[]) => void };
    onResponseProgress: (
      chatId: string,
      text: string,
      sessionId: string,
  ...
  const replace = vi
    .spyOn(state.interactionPresenter, 'replaceOutput')
    .mockImplementation(() => {});
  ...
  state.onResponseProgress(
    'cid123',
    'Working [FILE: /workspace/private',
    'session-1',
    segment,
  );
  expect(replace).toHaveBeenCalledWith(segment, 'Working ');
...
describe('DingtalkChannel status cards', () => {
  it('routes background lifecycle updates to the existing task card', () => {
    const channel = createChannel();
    const markBackgroundRun = vi.fn();
    Object.assign(channel, { interactionPresenter: { markBackgroundRun } });
    const base = {
      channelName: 'dingtalk',
      chatId: 'cid-1',
      sessionId: 'session-1',
    ...
    expect(markBackgroundRun).toHaveBeenCalledExactlyOnceWith('run-1');
  });
