...
export interface DaemonConnectionState {
   ...
   * (`/rename` or sidebar); `auto` = generated by the session-title service.
   * `/clear` carries a manual name over to the successor session but never
   * an auto one (#8977). Absent when no title event has landed yet.
   */
