...
describe('DaemonSessionProvider', () => {
  ...
  it('retains distinct skill toggle mutations that arrive together', async () => {
    const mutationA = {
      id: 'skill-toggle-A',
      kind: 'skill_toggle' as const,
      skills: [{ name: 'web-search', enabled: false }],
    ...
    const session = createMockSession({
      events: async function* skillToggleBatchEvents() {
        yield {
          id: 27,
          v: 1,
          type: 'settings_changed',
    ...
    expect(signals).toMatchObject({
      skillsVersion: 2,
      lastSkillMutation: mutationB,
      lastSkillMutationsByCwd: {
        '/mock-workspace': [mutationA, mutationB],
      },
    });
  });
