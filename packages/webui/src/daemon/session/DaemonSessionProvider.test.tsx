...
describe('DaemonSessionProvider', () => {
  ...
  it('keeps replay tail lifecycle events when truncating large snapshots', async () => {
    const ringEvicted = createDeferred<void>();
    const reloaded = createDeferred<void>();
    const staleReplayEvents: DaemonEvent[] = Array.from(
      { length: 50_000 },
      (_, index) => ({
        id: 12 + index,
        v: 1,
    ...
    const firstSession = createMockSession({
      sessionId: 'session-ring-truncated-tail',
      lastEventId: 10,
      prompt: vi.fn(async () => ({
        promptId: 'prompt-1',
        lastEventId: 10,
    ...
    const reloadedSession = createMockSession({
      sessionId: 'session-ring-truncated-tail',
      replaySnapshot: {
        compactedReplay: staleReplayEvents,
        liveJournal: [
          {
            id: 50_012,
            v: 1,
            ...
            data: {
              update: {
                sessionUpdate: 'agent_message_chunk',
                content: { type: 'text', text: 'tail error replay' },
              },
            },
          },
          {
