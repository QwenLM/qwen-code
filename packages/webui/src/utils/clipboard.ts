 ...
 * contexts (HTTPS or loopback). Pages served over plain HTTP from a
 * non-loopback host (e.g. the Web Shell daemon on a remote machine) leave
 * `navigator.clipboard` undefined and every copy entry point used to fail.
 * Fall back to the legacy `document.execCommand('copy')` path so copying
 * keeps working there.
