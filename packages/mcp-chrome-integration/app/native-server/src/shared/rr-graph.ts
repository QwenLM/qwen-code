...
function ensureTarget(t: unknown) {
...
export function mapNodeToStep(node: RRNode): unknown {
  const c: Record<string, unknown> = node.config || {};
  const base = { id: node.id } as Record<string, unknown>;
  ...
  try {
    ...
    if (type === 'delay') {
      const sleep = Number(
        (c as Record<string, unknown>).sleep ??
          (c as Record<string, unknown>).ms ??
          1000,
      );
      return {
        ...base,
        type: 'wait',
        condition: { sleep: Math.max(0, sleep) },
      };
    }
    const step: Record<string, unknown> = { ...base, type, ...c };
...
export function nodesToSteps(nodes: RRNode[], edges: RREdge[]): unknown[] {
