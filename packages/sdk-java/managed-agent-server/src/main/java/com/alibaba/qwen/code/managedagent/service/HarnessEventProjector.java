...
public class HarnessEventProjector {
    ...
    public ProjectedEvent project(SourceEvent event) {
        ...
        if ("agent_message_chunk".equals(kind)) {
            return textEvent("item.output_text.delta", update);
        }
        if ("agent_thought_chunk".equals(kind)) {
            return textEvent("item.reasoning.delta", update);
        }
        if ("tool_call".equals(kind)
                || "tool_call_update".equals(kind)) {
            return new ProjectedEvent("item.tool_call.updated",
                    safeToolData(update), false, null, null, null);
        }
        return null;
    }

    private static ProjectedEvent textEvent(String type,
        ...
        return new ProjectedEvent(type, Map.of("text", text), false, null,
                null, null);
    }
