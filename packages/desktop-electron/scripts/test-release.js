...
testFocusedDesktopSurfaces();
testReleaseWorkflow();
testVersionSynchronization();
console.log(
  'Electron desktop browser isolation and Web Shell parity checks passed.',
);
...
function testSecurityBoundary() {
  ...
  assert.match(browserPanel, /BROWSER_PANEL_PARTITION/);
  assert.doesNotMatch(browserPanel, /persist:/);
  assert.match(browserPanel, /setPermissionRequestHandler/);
  assert.match(browserPanel, /will-download/);
  assert.match(browserPanel, /shell\.openExternal/);
  assert.match(browserPanel, /mailto:/);
  ...
  assert.match(browserPanel, /webSecurity: true/);
  assert.match(preload, /contextBridge\.exposeInMainWorld/);
  assert.match(preload, /DESKTOP_LINK_CHANNELS/);
...
function testFocusedDesktopSurfaces() {
