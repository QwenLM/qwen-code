...
describe('WebShellSidebar — version footer', () => {
  it('keeps the settings label inline at full width and moves version into More', () => {
    // 420 is the sidebar's MAX width, and the footer now carries eight items
    // (Goals joined Scheduled Tasks). They need 452px to all sit inline, so at
    // every real width the overflow system yields the version badge — which it
    // is designed to do first, being display-only metadata. Settings keeps its
    // label; the version is still reachable, in the More menu.
    ...
    expect(menu?.textContent).toContain('v1.2.3');
  });
  ...
  it('focuses and navigates the actionable items in More', async () => {
    ...
    // ArrowDown is `(i + 1) % len`, so it advances and wraps. Asserting
    // `menuItems[0]` outright only held while exactly one actionable item
    // overflowed; adding the Goals button made it two.
    expect(document.activeElement).toBe(menuItems[1 % menuItems.length]);
