...
const ZH: Messages = {
  ...
  'settings.description.tools.toolSearch.enabled':
    '启用后，deferred 工具会先通过 ToolSearch 检查 schema，再通过 ToolCall 调用；桥接的查看与调用保持工具声明列表稳定，以减少提示词大小并提高缓存命中率。会话仍可能在中途刷新声明列表：恢复会话时，或当工具集刷新（MCP 发现、会话中首次进入计划模式、子代理定义变更）在实时历史中发现对某个仍隐藏的 deferred 工具的直接调用时。',
