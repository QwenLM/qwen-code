...
describe('useQueuedPrompts mid-turn reconciliation (session_mid_turn_message_query)', () => {
  ...
  it.each([false, true])(
    ...
    async (withMedia) => {
      ...
      try {
        ...
        expect(sdkMock.actions.submitPrompt).toHaveBeenCalledOnce();
        // A media fallback's confirmation runs while its row is still
        // unbound, which suppresses materializing other queued prompts from
        // that snapshot, so the body re-syncs once the row is bound.
  ...
  it('completes a cleared fallback whose suppressed start was cancelled mid-confirmation', async () => {
    ...
    try {
      ...
      // settles when its superseded snapshot returns.
      expect(harness.store.appendLocalUserMessage).toHaveBeenCalledOnce();
      expect(onComplete).not.toHaveBeenCalled();
      await act(async () => {
        resolvePending?.({
  ...
  it('echoes a settled fallback whose bound row wins over a foreign in-flight submission', async () => {
    ...
    try {
      ...
      // guard cannot tell it apart from the first prompt's own.
      await act(async () => {
        harness
          .result()
          .enqueuePrompt('', [{ data: 'c2Vjb25k', media_type: 'image/png' }]);
  ...
  it('echoes the real payload when a cleared first-time image submission still runs', async () => {
    let resolveFirst: ((value: { promptId: string }) => void) | undefined;
    sdkMock.actions.submitPrompt
      .mockImplementationOnce(
        () =>
    ...
    try {
      ...
      expect(harness.result().queuedPrompts).toEqual([]);
      // A turn is active by the time the admission resolves — the daemon
      // has already started the prompt, so the removal is refused.
      await harness.render({
        streamingState: 'responding',
        sessionHasActivePrompt: true,
      });
