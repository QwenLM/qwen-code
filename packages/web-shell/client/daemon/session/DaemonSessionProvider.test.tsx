...
describe('DaemonSessionProvider', () => {
  ...
  it('publishes an incomplete settlement before clearing an in-flight repair', async () => {
    const terminalGate = createDeferred<void>();
    const repairLoad = createDeferred<MockSession>();
    const initialSession = createMockSession({
      sessionId: 'session-live-repair-clear',
      hasActivePrompt: true,
      lastEventId: 9,
      replaySnapshot: {
        compactedReplay: [],
    ...
    await act(async () => {
      clearPromise = requireActions(actions).clearSession();
      await flushPromises();
    });
    expect(settlements).toEqual([
      expect.objectContaining({
    ...
    await act(async () => {
      repairLoad.resolve(repairedSession);
      await clearPromise;
      await flushPromises();
    });
  });

  it('consults the replacing replay before settling a replaced repair episode', async () => {
    const terminalGate = createDeferred<void>();
    const repairLoad = createDeferred<MockSession>();
    const terminalEvent: DaemonEvent = {
      id: 10,
  ...
  it('resolves a parked repair settlement as complete when the replacing replay proves the turn', async () => {
    const terminalGate = createDeferred<void>();
    ...
    const firstToolEvent: DaemonEvent = {
      id: 8,
      v: 1,
      type: 'session_update',
      promptId: 'prompt-first',
    ...
    const initialSession = createMockSession({
      sessionId: 'session-live-repair-complete-replace',
      hasActivePrompt: true,
      lastEventId: 7,
      replaySnapshot: {
        compactedReplay: [],
      ...
      events: async function* evictMarkerThenTerminal(
        options: { signal?: AbortSignal } = {},
      ) {
        await toolGate.promise;
        if (options.signal?.aborted) return;
        yield firstToolEvent;
  ...
  it('does not let a journaled prompt_cancelled wedge the restored prompt settlement', async () => {
    const turnCompleted = createDeferred<void>();
    const session = createMockSession({
      hasActivePrompt: true,
      lastEventId: 2,
      ...
      events: async function* restoredPromptAfterCancelledReplay() {
        yield {
          id: 3,
          v: 1,
          type: 'turn_complete',
  ...
  it('clears settled live-journal marker ids on an epoch reset reload', async () => {
    const firstSettled = createDeferred<void>();
    const startResync = createDeferred<void>();
    const reloaded = createDeferred<void>();
    const sessionId = 'session-epoch-marker-ledger';
    const markerData = {
      reason: 'replay_window_exceeded',
      scope: 'live_journal',
      truncatedEvents: 8,
    ...
    const firstSession = createMockSession({
      ...
      events: async function* firstEpochEvents(
        opts: { signal?: AbortSignal } = {},
      ) {
        yield {
          id: 9,
    ...
    const reloadedSession = createMockSession({
      ...
      events: async function* secondEpochEvents() {
        reloaded.resolve();
        yield {
          id: 9,
          v: 1,
  ...
  it('drops unowned live-journal marker claims on an epoch reset reload', async () => {
    const startResync = createDeferred<void>();
    const reloadedSettled = createDeferred<void>();
    const sessionId = 'session-epoch-ghost-claim';
    const firstSession = createMockSession({
      sessionId,
      hasActivePrompt: false,
      lastEventId: 4,
        ...
        compactedReplay: [
          {
            ...
            data: {
              update: {
                sessionUpdate: 'user_message_chunk',
                content: { type: 'text', text: 'question c' },
              },
            },
          } satisfies DaemonEvent,
          {
          ...
          } satisfies DaemonEvent,
          {
            ...
            type: 'turn_complete',
            promptId: 'prompt-c',
            data: { promptId: 'prompt-c', stopReason: 'end_turn' },
          } satisfies DaemonEvent,
        ],
        liveJournal: [],
      },
  ...
  it.each([
    {
      label: 'degraded',
      replayDegraded: true,
      liveJournal: [] as DaemonEvent[],
    },
    {
      label: 'marked unrecoverable',
    ...
    async ({ replayDegraded, liveJournal }) => {
      const ringEvicted = createDeferred<void>();
      const reloaded = createDeferred<void>();
      const firstSession = createMockSession({
        sessionId: 'session-ring-degraded',
        lastEventId: 10,
        submitPrompt: vi.fn(async () => ({
          promptId: 'prompt-1',
          lastEventId: 10,
      ...
      const promptResult = requireActions(actions).sendPrompt('ring prompt');
      await act(async () => flushPromises());
      await act(async () => {
        ringEvicted.resolve();
        await reloaded.promise;
  ...
  it('repairs and publishes a live terminal after a reconnect restores an active prompt', async () => {
    const ringEvicted = createDeferred<void>();
    const reloadedStreamStarted = createDeferred<void>();
    const terminalGate = createDeferred<void>();
    const firstSession = createMockSession({
      sessionId: 'session-ring-live-repair',
      lastEventId: 10,
      submitPrompt: vi.fn(async () => ({
        promptId: 'prompt-1',
        lastEventId: 10,
    ...
    sdkMocks.sessions.push(firstSession, reloadedSession, repairedSession);
    ...
    let promptResult: Promise<unknown> | undefined;
    await act(async () => {
      promptResult = providerActions.sendPrompt('long prompt');
      await flushPromises();
    });
    await act(async () => {
      ringEvicted.resolve();
  ...
  it('claims a production-shaped id-less live-journal marker only for its owning prompt', async () => {
    const terminalDelivered = createDeferred<void>();
  ...
  it('attributes an unrecoverable live-journal marker after retention evicts its block', async () => {
  ...
  it.each([
    { label: 'id-bearing', markerId: 10 as number | undefined },
    { label: 'id-less', markerId: undefined },
  ])(
    'does not reclaim an unowned $label live-journal marker after a same-session rebuild',
    async ({ markerId }) => {
      const firstTerminalDelivered = createDeferred<void>();
      const ringEvicted = createDeferred<void>();
      const reloadedStreamStarted = createDeferred<void>();
      ...
      const firstSession = createMockSession({
        ...
        events: async function* firstEvents() {
          yield {
            ...(markerId === undefined ? {} : { id: markerId }),
            v: 1,
            type: 'history_truncated',
            data: {
              reason: 'replay_window_exceeded',
  ...
  it('defers catch-up settlement until a truncated replay is repaired', async () => {
    ...
    const repairLoad = createDeferred<MockSession>();
    const firstSession = createMockSession({
      sessionId: 'session-ring-catchup-repair',
      lastEventId: 10,
      submitPrompt: vi.fn(async () => ({
        promptId: 'prompt-1',
        lastEventId: 10,
        ...
        liveJournal: [
          ...
          },
          {
            ...
            data: {
              update: {
                sessionUpdate: 'agent_message_chunk',
                content: { type: 'text', text: 'partial catch-up tail' },
              },
            },
          },
        ],
        ...
        compactedReplay: [
          ...
          },
          {
            ...
            data: {
              update: {
                sessionUpdate: 'agent_message_chunk',
                content: { type: 'text', text: 'complete catch-up answer' },
              },
            },
          },
          {
    ...
    sdkMocks.MockDaemonSessionClient.load.mockImplementation(
      async (client: unknown): Promise<MockSession> => {
        reloadNumber += 1;
        if (reloadNumber === 1) {
          reloadedSession.client = client as MockClient;
    ...
    await vi.waitFor(() => expect(reloadNumber).toBe(2));
    expect(settlements).toEqual([]);
