...
| `brand`                    | `WebShellBrand`                                                                                                                       | 产品品牌（名称与 Logo）；提供时整体取代 daemon 解析出的品牌，见下方「品牌（白标）」      |
| `onBrandResolved`          | `(brand: WebShellBrand) => void`                                                                                                      | 品牌解析完成后触发，供宿主应用到自己的文档；shell 自身从不写 `document.title` 或 favicon |
