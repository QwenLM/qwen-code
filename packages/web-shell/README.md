...
`allowAdd: false` 隐藏新增按钮和 `/auth` 建议，并在主窗口、欢迎页、分屏和侧任务中拦截手动输入的 `/auth` 及其别名 `/connect`、`/login`。拦截位于宿主 `onSlashCommand` 回调之后、隐藏命令转发之前——宿主回调返回 `true` 即可接管这些命令。拦截以会话命令快照的解析结果为准：同名的项目/用户命令顶替内建命令后不再受影响。添加模型弹框无法打开或保存。
