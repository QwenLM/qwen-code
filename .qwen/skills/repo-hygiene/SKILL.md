...
1. **Scan phase** (when invoked as scan): Dispatch the nine partition subagents. Each subagent applies the six angles inside its partition and reports candidates. Collect, deduplicate across partitions, and write every confirmed finding to `<workdir>/findings.json`:
   ...
   After writing findings.json and report-only.md, STOP. Do NOT create a branch, edit code, or run verification.

2. **Fix phase** (when invoked as fix): Read `<workdir>/findings.json` from the previous scan. If missing or empty, write `<workdir>/failure.md` and stop. Otherwise select `fixes` entries — the most certain, lowest-risk, easiest to explain. Selecting none is valid. No cap on count.
