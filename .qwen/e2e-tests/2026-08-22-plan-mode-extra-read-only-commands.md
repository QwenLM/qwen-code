...
Only cases 1, 5b, 6c, 6d, 7, 8, and 9 change behavior with the setting
present;
every other case behaves identically with and without it. (Case 6b-2 is
deliberately not in that list even though it configures the setting: those
roots are on the refusal floor, so the prompt is identical either way — its
point is that the entries load and still fail closed, not that anything
changes.) (Case 6c's startup
...
unless it is persisted with `tools.approvalMode`, which this plan's scratch
home never sets — so re-enter it with `/plan` immediately after each restart
below. (If you adapt this plan to a home that *does* set
`"tools": {"approvalMode": "plan"}`, every restart comes back up in Plan mode
and this instruction is redundant rather than wrong.)

A post-restart case run without that step comes up in **auto** mode — not
default: with no `tools.approvalMode` set, the startup chain takes the DEFAULT
branch only for bare or safe mode, and the forced-default override needs folder
trust, which is off by default. Auto mode has its own shell classifier and the
vouch is deliberately inert there, so such a run reports the wrong result
against the wrong mode's semantics.
...
`/tmp/ib-e2e/hostile`** (the gate probes the invocation's working directory —
the shell tool's `directory` parameter when the model sets one, otherwise the
directory the session was started from, which itself cannot be changed from
inside the session) and enter Plan mode with `/plan`:
