...
1. In one workspace, create one session in VS Code and one from the terminal or browser Web Shell.
2. Open the VS Code session-history dialog and confirm it initially shows the VS Code session.
...
- Opening a Terminal / Web session never rewrites its creator attribution, including after extension re-bootstrap.
